...
%%% Scatter versus Theta, subplot alpha (via alpha_c)
...
if (L_theta_m == 1) && (L_theta_c == 1) && ...
    ...
    for idx = 1:L_alpha_0
        ...
        p_e = plot(N,squeeze(R_emp(1,1,:,idx,1,1)),'-o','MarkerSize',mrk_size,'Color',colors(1+mod(idx-1,N_clr),:));  
    ...
    for idx = 1:L_N
        hold on;
        p_a = plot(alpha_0,squeeze(R_an(1,1,idx,:,1,1))','-','Color',colors(1+mod(idx-1,N_clr),:));
        p_e = plot(alpha_0,squeeze(R_emp(1,1,idx,:,1,1))','--','Color',colors(1+mod(idx-1,N_clr),:)); 
