...
\todomid{R, Rtheta to Rbayes, R?? For f, too.}

\todolo{Use convergence in prob symbol}
...
\todohigh{Generalize to semisupervised joint decisions??? training/test!}
...
This report details a Bayesian perspective on statistical learning theory for cases in which both the observations and unobserved quantities are jointly distributed according to an unknown probability distribution function. While the validity of Bayesian methods for statistical signal processing and machine learning has long been contended, the author believes it to be a justified approach that does not necessarily imply the distribution model is `random'; rather, it simply reflects the desire of the user to formulate risk as a weighted sum of learner performance across the space of distributions. 

The success or failure of Bayesian learning methods hinges on how well the prior knowledge imparted by the designer matches reality. The chosen prior distribution over the set of data-generating probability distributions reflects the user's confidence that different distributions are responsible for generating the observed/unobserved random elements. If a highly informative prior \cite{box} that is concentrated around the actual data probability distribution is chosen, low-risk learning functions are possible even with limited training data; however, if the informative prior is poorly designed, a good solution may not be achieved. Conversely, a noninformative prior that weights the different distributions without preference provides a more robust solution for all models, but may underperform relative to learners based on well-selected informative priors.
...
This section details the mathematical notation and typesetting conventions used throughout. Note that many variable scalars and functions including $x$, $y$, $g$, etc. are repeatedly redefined and reused to avoid introducing an excess of symbols. Unless explicitly stated, none of these variable definitions will hold in subsequent sections.


\subsection*{Sets and Function Arguments}

Sets are typically typeset with a calligraphic font, such as $\Xcal$ with the exception of common number sets, such as the real numbers, which are typeset using blackboard bold $\Rbb$. Function spaces, such as the set of functions $\Xcal \mapsto \Ycal$, are compactly represented as $\Ycal^{\Xcal}$.

\todolo{noncalligraphic for risk, loss, etc?}

Various mappings are defined for which the domain and/or the range \cite{rudin} are function spaces. The set of functions $\Xcal \mapsto \Ycal$ is denoted $\Ycal^{\Xcal}$. For a mapping $g : \Zcal \mapsto \Ycal^{\Xcal}$, the argument notation $g(z) \in \Ycal^{\Xcal}$ denotes a function, while $g(x;z) \in \Ycal$ is a specific value of that function. Semicolons are used to distinguish between the arguments referring to the domain and arguments that access the resulting function. The mapping $\{1,\ldots,N\} \mapsto \Ycal$ is represented as $\Ycal^N$ for brevity. Spaces of indexed tuples are notated as $g \in \Ycal^N$ and items of a tuple are accessed with subscripts rather than parentheses, such that $g_i \in \Ycal$.

The Cartesian product of sets is frequently used, such that for $x \in \Xcal$ and $y \in \Ycal$, the pair $(x,y) \in \Xcal \times \Ycal$. For a general product of sets $\Scal_i$, the notation $\prod_i \Scal_i = \Scal_1 \times \Scal_2 \ldots$ is used.

The convention adopted for natural numbers is $\Nbb = \{1,2,\ldots\}$; the set of nonnegative integers is denoted $\Zbbgeq = \Nbb \cup \{0\}$. The set of positive real numbers $\Rbb^+$ excludes zero, while nonnegative real numbers are represented as $\Rbbgeq = \Rbb^+ \cup \{0\}$. The cardinality of countably infinite sets, including the set of natural numbers, is denoted $\aleph_0 = |\Nbb|$; the cardinality of uncountable sets, such as $\Rbb$, is at least $\aleph_1$.

Numerous probability distribution functions are defined over different domains. As such, for a given set $\Xcal$, a set function $\Pcal$ is defined such that $\Pcal(\Xcal)$ is the set of distributions over $\Xcal$. If $\Xcal$ is countable, the set is defined as $\Pcal(\Xcal) = \left\{ p \in {\Rbbgeq}^{\Xcal}: \sum_{x \in \Xcal} p(x) = 1 \right\}$; if $\Xcal$ is a Euclidean space, the set is defined as $\Pcal(\Xcal) = \left\{ p \in {\Rbbgeq}^{\Xcal}: \int_{\Xcal} p(x) {\drm}x = 1 \right\}$.
...
The multinomial coefficient and multivariate beta function, which typically operate on sequences, are defined more generally for function inputs. The multinomial operator $\Mcal$ is used for functions $g : \Xcal \mapsto \Zbbgeq$ that map to nonnegative integers from an arbitrary countable domain $\Xcal$. The output of the operator is
\begin{equation}
...
\todolo{Introduce convergence in probability, relate to delta PDF convergence?}

Random elements are denoted with roman font (e.g., $\xrm$), while specific values are denoted with italics (e.g., $x$). Random elements that assume numerical scalars or functions are referred to as random variables or processes, respectively.

Consider a random element $\xrm \in \Xcal$. If $\Xcal$ is countable, either finite with $|\Xcal| \in \Nbb$ or countably infinite with $|\Xcal| = \aleph_0$, then $\xrm$ is a discrete random element and is characterized by a probability mass function (PMF) \cite {papoulis} denoted $\Prm_{\xrm} \in \Pcal(\Xcal)$. If $\Xcal$ is a Euclidean space and is thus uncountable with $|\Xcal| \geq \aleph_1$, then $\xrm$ is a continuous random variable or process characterized by a probability density function (PDF) denoted $\prm_{\xrm} \in \Pcal(\Xcal)$.
...
\todomid{Ever need the full functional?}
...
\Crm_{\xrm}\big[g(\xrm)\big](y,y') & = & \Erm_{\xrm} \bigg[ \Big( g(y;\xrm) - \Erm_{\xrm}\big[g(y;\xrm)\big] \Big) \Big( g(y';\xrm) - \Erm_{\xrm}\big[g(y';\xrm)\big] \Big) \bigg] \;. \IEEEeqnarraynumspace
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{rCl}
...
If the expectations are evaluated with respect to a conditional distribution $\Prm_{\xrm | \zrm}$, the additional argument for the observed random element is added and the notation for the above operators is extended to $\Erm_{\xrm|\zrm}\big[ g(\xrm) \big](y|z)$ and $\Crm_{\xrm|\zrm}\big[g(\xrm)\big](y,y'|z)$ for nonscalar outputs. When $g$ is the identity operator, the notation $\mu_{\xrm|\zrm}(y|z)$ and $\Sigma_{\xrm|\zrm}(y,y'|z)$ is used. As for probability distributions, it is common for the conditional random element $\zrm$ to be left as a random quantity instead of being explicitly defined; in these cases, the dependency on $\zrm$ is implied.
...
Also of interest is the conditional distribution of $\uppsic \in \Uppsic$, where $\uppsic(x) \equiv \uppsi(\cdot,x) / \uppsim(x)$. Using the Empirical process properties proven in \cref{app:emp}, it can be shown that when conditioned on $\uppsi$ and on the model $\uptheta$, the PMF of $\uppsic$ is
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsic | \uppsim, \uptheta}(\psic | \psim , \theta) & \equiv & \Prm_{\uppsic | \uppsim, \upthetac}(\psic | \psim , \thetac) \nonumber \\
& = & \prod_{x \in \Xcals(\psim)} \Bigg[ \Mcal\big( N \psim(x) \psic(x) \big) \left( \prod_{y \in \Ycal} \thetac(y;x)^{\psic(y;x)} \right)^{N \psim(x)} \Bigg] \nonumber \\
& = & \prod_{x \in \Xcals(\psim)} \Emp\Big( \psic(x) ; N \psim(x) , \thetac(x) \Big) \;,
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsic | \uppsim, \uptheta} & \equiv & \bigotimes_{x \in \Xcals(\uppsim)} \Prm_{\uppsic(x) | \uppsim(x), \upthetac(x)} \nonumber \\
& = & \bigotimes_{x \in \Xcals(\uppsim)} \Emp\Big(N \uppsim(x), \upthetac(x) \Big) \;
\end{IEEEeqnarray}
over the domain $\prod_{x \in \Xcals(\psim)} \left\{ \frac{n}{N \psim(x)}: n \in {\Zbbgeq}^{\Ycal}, \ \sum_{y \in \Ycal} n(y) = N \psim(x) \right\}$. Observe that conditioning on the marginal empirical process renders the conditional processes $\uppsic(x)$ independent of one another and that they are also Empirically distributed, such that $\uppsic(x) | \uppsim(x),\upthetac(x) \sim \Emp\big( N \uppsim(x),\upthetac(x) \big)$ for every $x \in \Xcals(\uppsim)$. 
...
& = & \Erm_{\Drm | \uptheta} \bigg[ \Erm_{\yrm, \xrm| \uptheta} \Big[ \Lcal\big( f(\xrm;\Drm),\yrm \big) \Big] \bigg] \nonumber \\
...
\end{equation}
%For a given set of observations $\xrm$ and $\Drm$, the function $f_{\Theta}(\uptheta)$ selects the decision $h = \argmin_{h \in \Hcal} \Erm_{\yrm | \xrm,\uptheta}\big[ \Lcal(h,\yrm) \big]$. Note the conditional independence of $\yrm$ from $\Drm$ in \eqref{eq:risk_cond} -- the knowledge of $\uptheta$ renders the training data $\Drm$ valueless. As such, the range of the clairvoyant function is recast as $f_{\Theta} : \Theta \mapsto \Hcal^{\Xcal}$ and the decisions are
...
& = & \min_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \uptheta}\Big[ \Lcal\big(g(\xrm),\yrm\big) \Big] \nonumber \\
...
\end{equation}
such that
\begin{equation} \label{eq:f_opt_D}
f^*(\Drm) = \argmin_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \Drm} \Big[ \Lcal\big(g(\xrm),\yrm\big) \Big] \;.
\end{equation}
The decision expressed by the nonparametric learning function $f^*(\Drm)$ for a given novel observation $\xrm$ is
...
& = & \Erm_{\Drm} \left[ \min_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \Drm}\Big[ \Lcal\big(g(\xrm),\yrm\big) \Big] \right] \nonumber \\
...
\frac{\Prm_{\Xrm | \upthetam}(X | \thetam)}{\Prm_{\Xrm}(X)} \prm_{\upthetam}(\thetam) \IEEEeqnarraynumspace
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl}
...
\begin{IEEEeqnarray}{L}
\prm_{\upthetam | \uppsic,\uppsim}(\thetam | \psic,\psim) \nonumber \\ 
\qquad = \frac{\Erm_{\upthetac | \upthetam}\left[ \Prm_{\uppsic | \uppsim,\upthetac}\big( \psic | \psim,\upthetac \big) \right](\thetam)}{\Prm_{\uppsic | \uppsim}\big( \psic | \psim \big)} 
...
\qquad = \frac{\prm_{\upthetam | \upthetac}\left[ \Prm_{\uppsim,\xrm | \upthetam}\big( \psim,x | \upthetam \big) \right](\thetac)}{\Prm_{\uppsim,\xrm}\big( \psim,x \big)} \frac{\Prm_{\uppsic | \uppsim,\upthetac}\big( \psic | \psim,\thetac \big)}{\Prm_{\uppsic | \uppsim,\xrm}\big( \psic | \psim,x \big)} 
\prm_{\upthetac}(\thetac)
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl}
...
%&& \qquad \frac{\Prm_{\uppsic | \uppsim}\big( \psic | \psim \big)}{\Prm_{\uppsic | \uppsim,\xrm}\big( \psic | \psim,x \big)} \frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \uppsim}\big( \thetac | \psim \big)} 
%\end{IEEEeqnarray}
...
\todohi{Location???}
\todohi{Prior support? Must be bounded??}
\todolo{"Universal consistency"}
\todohigh{FIXXXXXX lim}

The trends in risk as $N \to \infty$ are of specific interest. Recall that $\Prm_{\uppsi | \uptheta}(\psi | \theta) \to \delta[\psi, \theta]$. For priors with full support, observe that as the number of training samples increases, the statistic PMF tends toward $\Prm_{\uppsi}(\psi) \approx N^{1-|\Ycal||\Xcal|}\prm_{\uptheta}(\psi)$; this can be proven using Gautschi's inequality \cite{wendel}. Thus,
\begin{IEEEeqnarray}{rCl}
\prm_{\uptheta | \uppsi}(\theta | \psi) & = & \frac{\Prm_{\uppsi | \uptheta}(\psi | \theta)}{\Prm_{\uppsi}(\psi)} \prm_{\uptheta}(\theta) \nonumber \\
& \to & \delta(\theta - \psi)
\end{IEEEeqnarray}
and $\Prm_{\yrm, \xrm| \uppsi} = \mu_{\uptheta| \uppsi} \to \uppsi$.

Representing $\uppsi | \uptheta \inprob \uptheta$ as $\Prm_{\yrm, \xrm| \uppsi} | \uptheta \inprob \Prm_{\yrm, \xrm| \uptheta}$, it is clear that $f^*(\Drm) | \uptheta \inprob f_{\Theta}(\uptheta)$, such that the clairvoyant learner is precisely identified, achieving
\begin{IEEEeqnarray}{rCl} 
\Rcal_{\Theta}(f ; \uptheta) & = & \Erm_{\yrm, \xrm | \uptheta} \bigg[ \Erm_{\Drm | \uptheta} \Big[ \Lcal\big( f^*(\xrm; \Drm),\yrm \big) \Big] \bigg] \nonumber \\
& = & \Erm_{\yrm, \xrm | \uptheta} \Big[ \Lcal\big( f_{\Theta}(\xrm; \uptheta),\yrm \big) \Big] \nonumber \\
& = & \Rcal_{\Theta}^*(\uptheta) \;.
\end{IEEEeqnarray}
...
\qquad = \mathrm{Bias}(\xrm; \upthetam,\upthetac) \otimes \mathrm{Bias}(\xrm; \upthetam,\upthetac) + \mathrm{Cov}(\xrm; \upthetam,\upthetac)
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl} 
%\IEEEeqnarraymulticol{3}{l}{\Erm_{\Drm | \upthetam,\upthetac} \Big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \Big]} \nonumber \\
%\qquad & = & \mathrm{Bias}(\xrm; \upthetam,\upthetac) \otimes \mathrm{Bias}(\xrm; \upthetam,\upthetac) + \mathrm{Cov}(\xrm; \upthetam,\upthetac)
%\label{eq:predictive_del_sq}
%\end{IEEEeqnarray}
...
Note that the risk can also be represented as $\Rcal_{\Theta}(f ; \uptheta) = \Rcal_{\Theta}^*(\uptheta) + \Rcal_{\Theta, \mathrm{ex}}(f ; \uptheta)$, where the first term is the irreducible squared error (as demonstrated in the next subsection) and the second term is the excess squared error,
...
& = &  \mu_{\yrm | \xrm,\uptheta} \;. %= \sum_{y \in \Ycal} y \upthetac(y;\xrm) 
\end{IEEEeqnarray}
...
\cref{fig:Risk_clv_SE_tilde} displays the irreducible risk for predictive models $\thetac(x)$ independent of $x$. 
\begin{figure}
\centering
...
\begin{IEEEeqnarray}{rCl} 
\IEEEeqnarraymulticol{3}{l}{\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \Erm_{\xrm,\Drm | \uptheta} \Big[ \big( \mu_{\yrm | \xrm,\Drm} - \mu_{\yrm | \xrm,\uptheta} \big)^2 \Big]} \nonumber \\
\qquad & \equiv & \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\Drm | \upthetam,\upthetac} \Big[ \Delta(y; \xrm; \Drm,\upthetac) \Delta(y'; \xrm; \Drm,\upthetac) \Big] \nonumber \\
...
\label{eq:risk_cond_ex_SE_bayes}
\end{IEEEeqnarray}
...
\cref{fig:Risk_clv_01_tilde} displays the irreducible risk for predictive models $\thetac(x)$ independent of $x$. Intuitively, the models that are more concentrated lead to lower probability of error.
\begin{figure}
\centering
...
\todohigh{Uncoupled m/c alphas = nonDir Ptheta??}
...
\cref{fig:P_nbar_a0} displays example distributions of $\uppsi$ for $N=10$ and different model concentrations $\alpha_0$. Observe that for large $\alpha_0$, the distribution approaches an Empirical distribution $\uppsi \sim \Emp(N,\alpha)$. 
...
& = & \left(\frac{|\Ycal|}{|\Ycal| + N \Psim(\xrm;\Xrm)}\right) \frac{1}{|\Ycal|} + \left(\frac{N \Psim(\xrm;\Xrm)}{|\Ycal| + N \Psim(\xrm;\Xrm)}\right) \Psic(\xrm;\Drm) \;. \IEEEeqnarraynumspace
\end{IEEEeqnarray}
...
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha_0 \alpham(x') + N \Psim(x';D), \mu_{\upthetac | \Drm}(x'; D) \big) \;, \IEEEeqnarraynumspace
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{rCl}
...
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{\prm_{\upthetac | \xrm,\uppsi}( \thetac | x,\psi ) = \prm_{\upthetac | \uppsi}( \thetac | \psi ) \equiv \prm_{\upthetac | \uppsim, \uppsic}( \thetac | \psim, \psic )} \nonumber \\
\qquad & = & \prod_{x' \in \Xcal} \prm_{\upthetac(x') | \uppsim(x'), \uppsic(x')}\big(\thetac(x') | \psim(x'), \psic(x') \big) \nonumber \\
...
\todolo{Use lambda weight notation from conf. papers? And table!}
...
\begin{IEEEeqnarray}{L} \label{eq:predictive_dist_avg_dir}
\Erm_{\uppsim,\uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \nonumber \\
\qquad = \Erm_{\uppsim | \upthetam} \big[\gammam(\xrm; \uppsim)\big] \alphac(\xrm) + \Erm_{\uppsim | \upthetam} \big[1 - \gammam(\xrm; \uppsim)\big] \upthetac(\xrm) \;,
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl} \label{eq:predictive_dist_avg_dir}
%\Erm_{\uppsim,\uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] 
%& = & \Erm_{\uppsim | \upthetam} \big[\gammam(\xrm; \uppsim)\big] \alphac(\xrm) \nonumber \\
%&& + \Erm_{\uppsim | \upthetam} \big[1 - \gammam(\xrm; \uppsim)\big] \upthetac(\xrm) \;,
%\end{IEEEeqnarray}
...
and, noting that
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{\Prm_{\yrm | \xrm,\uppsim,\uppsic} - \Erm_{\uppsim,\uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big]} \nonumber \\
\qquad & = & \Big( \gammam(\xrm; \uppsim) - \Erm_{\uppsim | \upthetam}\big[ \gammam(\xrm; \uppsim) \big] \Big) \big(\alphac(\xrm) - \upthetac(\xrm)\big) \nonumber \\
&& + \big(1 - \gammam(\xrm; \uppsim)\big) \big(\uppsic(\xrm) - \upthetac(\xrm)\big) \;,
\end{IEEEeqnarray}
the covariance \eqref{eq:predictive_cov} of the estimate can be represented as
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_cov_dir}
\mathrm{Cov}(\xrm;\upthetam,\upthetac) & = & \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \nonumber \\
& = & \Crm_{\uppsim | \upthetam}\big[ \gammam(\xrm; \uppsim) \big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
&& + \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \; . \nonumber \\
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{L} \label{eq:predictive_cov_dir}
%\mathrm{Cov}(\xrm;\upthetam,\upthetac) = \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \nonumber \\
%\qquad = \Crm_{\uppsim | \upthetam}\big[ \gammam(\xrm; \uppsim) \big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
%\qquad \quad+ \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \;.
%\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl} 
\IEEEeqnarraymulticol{3}{l}{\Erm_{\Drm | \upthetam,\upthetac} \big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \big]} \nonumber \\
\qquad & \equiv & \Erm_{\uppsim | \upthetam}\left[ \gammam(\xrm; \uppsim)^2 \right] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
&& + \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \;. \IEEEeqnarraynumspace
\label{eq:predictive_del_sq_dir}
\end{IEEEeqnarray}
...
It is informative to substitute the Bayes distribution using the Dirichlet prior \eqref{eq:P_yx_D_dir} into \eqref{eq:f_opt_D}, expressing the decision function for a given training set $\Drm$ as
\begin{IEEEeqnarray}{rCl} \label{eq:E_yx|D L}
f^*(\Drm) & = & \argmin_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \Drm} \Big[ \Lcal\big(g(\xrm),\yrm\big) \Big] \nonumber \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \gamma \sum_{y \in \Ycal} \sum_{x \in \Xcal} \alpha(y,x) \Lcal\big(g(x),y\big) + (1-\gamma) \sum_{y \in \Ycal} \sum_{x \in \Xcal} \Psi(y,x;\Drm) \Lcal\big(g(x),y\big) \nonumber \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \gamma \Erm_{\yrm,\xrm}\Big[ \Lcal\big(g(\xrm), \yrm\big) \Big] + (1-\gamma) \frac{1}{N} \sum_{n=1}^N \Lcal\big(g(\Xrm_n), \Yrm_n \big) \nonumber \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \alpha_0 \Erm_{\yrm,\xrm}\Big[ \Lcal\big(g(\xrm), \yrm\big) \Big] + \sum_{n=1}^N \Lcal\big(g(\Xrm_n), \Yrm_n \big) \;.
\end{IEEEeqnarray}
Observe that the objective function can be represented as a convex combination of two expected losses, one with respect to the Dirichlet prior mean $\alpha$, and one with respect to the empirical distribution $\Psi(D)$. As $N$ increases, $\gamma$ decreases and more emphasis is placed on the empirical loss. The final representation casts the learning optimization task as regularized empirical risk minimization. Note that the regularizing term is scaled using the Dirichlet localization $\alpha_0$ -- the stronger the prior knowledge, the more extreme the regularization. Importantly, the selection of the probability distribution $\alpha$ allows the designer to effect very different types of regularization from those typically used in the literature, such as the $\ell^2$ norm for parametric learners.

\todohi{Regularized loss REFERENCE? See TheoML p.72}


It is useful to substitute the Bayes predictive distribution using the Dirichlet prior \eqref{eq:P_y_xD_dir} into \eqref{eq:f_opt_xD}, expressing the decision for a given input $\xrm$ and training set $\Drm$ as
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{f^*(\xrm;\Drm) = \argmin_{h \in \Hcal} \Erm_{\yrm | \xrm,\Drm} \big[ \Lcal(h,\yrm) \big]} \nonumber \\
\qquad & = & \argmin_{h \in \Hcal} \frac{\alpha_0 \sum_{y \in \Ycal} \alpha(y,\xrm) \Lcal(h,y) + N \sum_{y \in \Ycal} \Psi(y,\xrm;\Drm) \Lcal(h,y)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)} \nonumber \\
& = & \argmin_{h \in \Hcal} \left(\frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)}\right) \sum_{y \in \Ycal} \alphac(y;\xrm) \Lcal(h,y) \nonumber \\
&& + \left(\frac{N \Psim(\xrm;\Xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)}\right) \sum_{y \in \Ycal} \Psic(y;\xrm;\Drm) \Lcal(h,y) \nonumber \\
& = & \argmin_{h \in \Hcal} \gammam(\xrm; \Xrm) \sum_{y \in \Ycal} \alphac(y;\xrm) \Lcal(h,y) + \big(1-\gammam(\xrm; \Xrm)\big) \sum_{y \in \Ycal} \Psic(y;\xrm;\Drm) \Lcal(h,y) \nonumber \\
& = & \argmin_{h \in \Hcal} \gammam(\xrm; \Xrm) \Erm_{\yrm | \xrm}\big[ \Lcal(h,\yrm) \big] + \big(1-\gammam(\xrm; \Xrm)\big) \frac{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big] \Lcal\big( h,\Yrm_n \big)}{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big]} \nonumber \\
& = & \argmin_{h \in \Hcal} \alpha_0 \sum_{y \in \Ycal} \alpha(y,\xrm) \Lcal(h,y) + \sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big] \Lcal\big( h,\Yrm_n \big) \;.
\label{eq:E_y|xD L}
\end{IEEEeqnarray}
...
& = & \left( \frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)} \right) \sum_{y \in \Ycal} y \alphac(y;\xrm) \nonumber \\
&& + \left( \frac{N \Psim(\xrm;\Xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)} \right) \sum_{y \in \Ycal} y \Psic(y;\xrm;\Drm) \nonumber \\
& \equiv & \gammam(\xrm; \Xrm) \mu_{\yrm | \xrm} + \big(1-\gammam(\xrm; \Xrm)\big) \frac{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big] \Yrm_n}{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big]} \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{f^*(\xrm;\Drm) = \left( \frac{|\Ycal|}{N \Psim(\xrm;\Xrm) + |\Ycal|} \right) \frac{1}{|\Ycal|} \sum_{y \in \Ycal} y + \left( \frac{N \Psim(\xrm;\Xrm)}{N \Psim(\xrm;\Xrm) + |\Ycal|} \right) \sum_{y \in \Ycal} y \Psic(y;\xrm;\Drm)} \nonumber \\
\qquad & = & \left( \frac{|\Ycal|}{N \Psim(\xrm;\Xrm) + |\Ycal|} \right) \frac{1}{|\Ycal|} \sum_{y \in \Ycal} y + \left( \frac{N \Psim(\xrm;\Xrm)}{N \Psim(\xrm;\Xrm) + |\Ycal|} \right) \frac{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big] \Yrm_n}{\sum_{n=1}^N \delta\big[ \xrm,\Xrm_n \big]} \;. \nonumber \\
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \equiv \Erm_{\xrm} \left[ \Erm_{\uppsic,\uppsim | \xrm} \left[ \frac{\big( \alpha_0 \alpham(\xrm) \mu_{\yrm|\xrm} + N \uppsim(\xrm) \sum_{y \in \Ycal} y \uppsic(y;\xrm) \big)^2}{\alpha_0 \alpham(\xrm) \big(\alpha_0 \alpham(\xrm) + N \uppsim(\xrm) \big)^2} \right] \right]} \nonumber \\
\qquad & = & \Erm_{\xrm} \left[ \Erm_{\uppsic,\uppsim} \left[ \frac{\big( \alpha_0 \alpham(\xrm) \mu_{\yrm|\xrm} + N \uppsim(\xrm) \sum_{y \in \Ycal} y \uppsic(y;\xrm) \big)^2}{\alpham(\xrm) \big(\alpha_0 \alpham(\xrm) + N \uppsim(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
...
\begin{IEEEeqnarray}{rCl} \label{eq:Risk_min_SE_dir}
...
%\cref{fig:Risk_SE_IO_N} displays how the risk increases with $M_x$; \cref{fig:Risk_SE_IO_N-Mx} makes the dependency on $N/M_x$ explicit.
%
%\begin{figure}
%\centering
...
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\Drm | \upthetam,\upthetac} \Big[ \Delta(y; \xrm; \Drm,\upthetac) \Delta(y'; \xrm; \Drm,\upthetac) \Big]} \nonumber \\
\qquad & = & \Erm_{\xrm | \upthetam}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^2 \Erm_{\uppsim | \upthetam}\left[ \left(\frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \uppsim(\xrm)}\right)^2 \right] \right] \nonumber \\
&& +  \Erm_{\xrm | \upthetam}\left[ \Sigma_{\yrm | \xrm,\upthetac} \Erm_{\uppsim | \upthetam}\left[ \frac{N \uppsim(\xrm)}{\big( \alpha_0 \alpham(\xrm) + N \uppsim(\xrm) \big)^2} \right] \right] \nonumber \\
& = & \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \gammam(\xrm; \uppsim)^2 \right] \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^2 \right] \nonumber \\
&& + \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim\big)^2}{N \uppsim(\xrm)} \right]  \Sigma_{\yrm | \xrm,\upthetac} \right] \;.
\label{eq:risk_cond_SE_dir_ex}
\end{IEEEeqnarray}
...
%\qquad = \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \gammam(\xrm; \uppsim)^2 \right] \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^2 \right] \nonumber \\
...
The excess risk at this potentially nonintegral value would be 
\begin{equation}
...
&&+ \Erm_{\xrm | \upthetam}\left[ \left( \sum_{n=1}^N \binom{N}{n} \upthetam(\xrm)^n \big( 1 - \upthetam(\xrm) \big)^{N-n} \frac{1}{n} \right) \Sigma_{\yrm | \xrm,\upthetac} \right] \;.
...
\todohi{Use harder nonlinearity that fails even for higher-order poly??}
\todolo{use markers for predict plots?!?}
...
\qquad = \begin{cases} 1 & \mathrm{if} \ n < 0, \\ \sum_{m=1}^{|\Ycal|} \binom{|\Ycal|}{m} (-1)^{m-1} \prod_{l=1}^{|\Ycal||\Xcal|-1} \Big( 1-\frac{mn}{N+l} \Big) H\Big( \big\lfloor\frac{N}{m}\big\rfloor - n \Big) & \mathrm{if} \ 0 \leq n \leq N, \\ 0 & \mathrm{if} \ n > N. \end{cases} \nonumber \\
\end{IEEEeqnarray}
...
\todohigh{CITE for geometric integral!! Just remove if unused?}
...
As shown in \cref{app:EP}, given $\uptheta$, the empirical model is a continuous-domain empirical process $\uppsi | \uptheta \sim \EP(N,\theta)$. The mean and covariance functions take the same form as those of the discrete Empirical process (using the continuous set form of the $\diag$ operator.)
...
The model is characterized by a Dirichlet process $\uptheta \sim \DP(\alpha_0, \alpha)$ with concentration $\alpha_0$ and mean function $\alpha \in \Pcal(\Ycal \times \Xcal)$. In \cref{app:DP}, it is shown that the mean and covariance functions of the continuous process have the same form as those of the discrete process (using the continuous variant of the $\diag$ operator).

%As shown in \cref{app:DP}, the expected value of a Dirichlet process is $\DP(\alpha_0, \alpha)$
%\begin{equation}
%\mu_{\uptheta} = \alpha
%\end{equation}
...
Using the Dirichlet process properties shown in \cref{app:DP},
\begin{IEEEeqnarray}{rCl}
...
%&& \prod_{n=2}^N \frac{\alpha(Y_n,X_n) + \sum_{i=1}^{n-1} \delta(Y_n-Y(i)) \delta(X_n-X(i))}{\alpha_0+n-1} \\
%& = & \int_{Y(1)} {\drm}Y(1) \ldots \int_{Y(N-1)} {\drm}Y(N-1) \frac{\alpha(Y(1),X(1))}{\alpha_0} \\
%&& \prod_{n=2}^{N-1} \frac{\alpha(Y_n,X_n) + \sum_{i=1}^{n-1} \delta(Y_n-Y(i)) \delta(X_n-X(i))}{\alpha_0+n-1} \\
...
\qquad = \frac{\diag\big(\alphac(\Xrm_n)\big) + \delta(0)^{-1} \alpha_0 \alpham(\Xrm_n) \alphac(\Xrm_n) \otimes \alpha(\Xrm_n)}{\delta(0)^{-1} \alpha_0 \alpham(\Xrm_n) + 1} \nonumber \\
\qquad = \frac{\delta(0)}{\alpha_0 \alpham(\Xrm_n) + \delta(0)} \diag\big(\alphac(\Xrm_n)\big) + \frac{\alpha_0 \alpham(\Xrm_n)}{\alpha_0 \alpham(\Xrm_n) + \delta(0)} \alphac(\Xrm_n) \otimes  \alpha(\Xrm_n) \;.
\IEEEeqnarraynumspace
\end{IEEEeqnarray}
...
&& + \frac{\alpha_0 \alpham(\Xrm_n)}{\alpha_0 \alpham(\Xrm_n) + \delta(\Xrm_n-\Xrm_{n'})} \alphac(\Xrm_n) \otimes \alpha(\Xrm_{n'}). 
\end{IEEEeqnarray}
...
%&& + \frac{1 - N^{-1}}{1 + \alpha_0^{-1}} \alpha(y,x) \alpha(y',x')
%\end{IEEEeqnarray}
...
Observe that by the aggregation principle, $\uppsim \sim \DEP(N,\alpha_0,\alpham)$ is a DEP over the set $\Xcal$. Additionally, the one-dimensional subsets conditioned on the marginalized DEP are characterized as
\begin{equation}
...
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_cov_cont}
\mathrm{Cov}(\xrm;\upthetam,\upthetac) & = & \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \nonumber \\
& = & \Crm_{\uppsim | \upthetam}\big[\gammam(\xrm; \uppsim)\big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
&& + \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(\xrm)} \right] \big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \big) \nonumber \\
\end{IEEEeqnarray}
...
\Erm_{\Drm | \upthetam,\upthetac} \Big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \Big] \nonumber \\
\qquad \equiv \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\big[ \gammam(\xrm; \uppsim)^2 \big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big)  \right] \nonumber \\
\qquad \quad+ \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \right] \;. \nonumber \\
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{f^*(\Drm) = \argmin_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \Drm} \Big[ \Lcal\big(g(\xrm),\yrm\big) \Big]} \nonumber \\
\quad & = & \argmin_{g \in \Hcal^{\Xcal}} \gamma \iint_{\Ycal \times \Xcal} \alpha(y,x) \Lcal\big(g(x),y\big) {\drm}y {\drm}x + (1-\gamma) \iint_{\Ycal \times \Xcal} \Psi(y,x;\Drm) \Lcal\big(g(x),y\big) {\drm}y {\drm}x \nonumber \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \gamma \Erm_{\yrm,\xrm}\Big[ \Lcal\big(g(\xrm), \yrm\big) \Big] + (1-\gamma) \frac{1}{N} \sum_{n=1}^N \Lcal\big(g(\Xrm_n), \Yrm_n \big) \nonumber \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \alpha_0 \Erm_{\yrm,\xrm}\Big[ \Lcal\big(g(\xrm), \yrm\big) \Big] + \sum_{n=1}^N \Lcal\big(g(\Xrm_n), \Yrm_n \big) \;.
\label{eq:E_yx|D L_cont}
\end{IEEEeqnarray}
...
&& + \left(\frac{N \Psim(\xrm;\Xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)}\right) \int_{\Ycal} \Psic(y;\xrm;\Drm) \Lcal(h,y) {\drm}y \nonumber \\
...
&& + \left( \frac{N \Psim(\xrm;\Xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)} \right) \int_{\Ycal} y \Psic(y;\xrm;\Drm) {\drm}y \nonumber \\
...
To evaluate the expectation directly using training samples $\Yrm$ and $\Xrm$, first note that  $\mu_{\Yrm_n | \Xrm} = \mu_{\yrm|\xrm}\big( \Xrm_n \big)$ and $\Erm_{\Yrm_n | \Xrm}\big[ \Yrm_n^2 \big] = \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \big( \Xrm_n \big)$ and
\begin{IEEEeqnarray}{L}
\Erm_{\Yrm_n,\Yrm_{n'} | \Xrm}\big[ \Yrm_n \Yrm_{n'} \big] \nonumber \\
\qquad = \frac{\alpha_0 \alpham\big( \Xrm_n \big) \mu_{\yrm|\xrm}\big( \Xrm_n ) \mu_{\yrm|\xrm}\big (\Xrm_{n'} \big) + \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \big( \Xrm_n \big) \delta\big( \Xrm_n - \Xrm_{n'} \big)}{\alpha_0 \alpham\big( \Xrm_n \big) + \delta\big( \Xrm_n - \Xrm_{n'} \big)} \;.
\IEEEeqnarraynumspace
\end{IEEEeqnarray}
...
\quad = \alpha_0^2 \alpham(\xrm)^2 \mu_{\yrm | \xrm}^2 + 2\alpha_0 \alpham(\xrm) \mu_{\yrm | \xrm} \sum_{n=1}^N \mu_{\yrm | \xrm}\big( \Xrm_n \big) \delta\big( \xrm - \Xrm_n \big) \nonumber \\
\qquad + \sum_{n=1}^N \Erm_{\yrm|\xrm}\big[ \yrm^2 \big]\big( \Xrm_n \big) \delta\big( \xrm - \Xrm_n \big)^2 \nonumber \\
...
\quad = \ldots \nonumber \\
\quad = \alpha_0^2 \alpham(\xrm)^2 \mu_{\yrm | \xrm}^2 + 2\alpha_0 \alpham(\xrm) \mu_{\yrm | \xrm}^2 \sum_{n=1}^N \delta\big( \xrm - \Xrm_n \big) + \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \sum_{n=1}^N \delta\big( \xrm - \Xrm_n \big)^2 \nonumber \\
\qquad + \frac{\alpha_0 \alpham(\xrm) \mu_{\yrm | \xrm}^2 + \Erm_{\yrm|\xrm}\big[ \yrm^2  \big] \delta(0)}{\alpha_0 \alpham(\xrm) + \delta(0)} \sum_{n \neq n'} \delta\big( \xrm - \Xrm_n \big) \delta\big( \xrm - \Xrm_{n'} \big) \nonumber \\
\quad = \ldots \nonumber \\
\quad= \frac{\alpha_0 \alpham(\xrm) + \sum_{n=1}^N \delta\big( \xrm-\Xrm_n \big)}{\alpha_0 \alpham(\xrm) + \delta(0)} \nonumber \\
...
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\Drm} \Big[ \mu_{\yrm | \xrm,\Drm}^2 \Big] \nonumber \\
\qquad = \Erm_{\xrm} \left[ \frac{\Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \delta(0) N + \mu_{\yrm | \xrm}^2 \alpha_0 \big( \alpha_0 \alpham(\xrm) + \delta(0) + N \alpham(\xrm) \big)}{ \big( \alpha_0 \alpham(\xrm) + \delta(0) \big) (\alpha_0+N)} \right] \;.
\end{IEEEeqnarray}
...
\qquad + \left( \frac{\delta(0)}{\alpha_0 \alpham(\xrm)} + \frac{\delta(0)}{N \uppsim(\xrm)} \right) \alphac(y;\xrm) \delta(y-y') \Bigg] {\drm}y {\drm}y' \nonumber \\
...
\todomid{Change form to have conditional prior concentration?}
...
\begin{IEEEeqnarray}{rCl} \label{eq:risk_cond_SE_dir_ex_cont}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) & = & \Erm_{\xrm,\Drm | \uptheta} \Big[ \big( \mu_{\yrm | \xrm,\Drm} - \mu_{\yrm | \xrm,\uptheta} \big)^2 \Big] \nonumber \\
\qquad & \equiv & \int_{\Ycal} y \int_{\Ycal} y' \Erm_{\xrm,\Drm | \upthetam,\upthetac} \Big[ \Delta(y; \xrm; \Drm,\upthetac) \Delta(y'; \xrm; \Drm,\upthetac) \Big] {\drm}y {\drm}y' \nonumber \\
& = & \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\big[ \gammam(\xrm; \uppsim)^2 \big] \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^2 \right] \nonumber \\
&& + \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(\xrm)} \right] \Sigma_{\yrm | \xrm,\upthetac} \right] \;.
\end{IEEEeqnarray}
...
If instead $\upthetam$ includes Dirac delta functions, there are values $x \in \Xcal$ with nonzero probabilities of being observed. At these values, if $\alpham(x)$ is bounded, the weights tend to
\begin{IEEEeqnarray}{L} 
...
%\end{figure}
%
%Next, consider the risk trends when the true model is fixed. The marginal distribution $\thetam = 1$ is uniform and $\thetac(\xrm) = \Beta\big(126 \mu_{\yrm | \xrm,\uptheta}, 126 (1- \mu_{\yrm | \xrm,\uptheta})\big)$, where the clairvoyant regressor is $\mu_{\yrm | \xrm,\uptheta} = 1 / \big(2 + \sin(2\pi \xrm)\big)$. Comparing with the discrete-space models used for demonstration in \cref{sec:SE_dir_example}, observe that the optimal regressor $\mu_{\yrm | \xrm, \uptheta}$ and true predictive variance $\Sigma_{\yrm | \xrm, \uptheta} = \frac{1}{127} \mu_{\yrm | \xrm, \uptheta} (1 - \mu_{\yrm | \xrm, \uptheta})$ are the same. However, since $\upthetam$ is bounded, no effective learning is possible, the Bayesian estimator tends to $\mu_{\yrm | \xrm, \Drm} \to \mu_{\yrm | \xrm}$, and the excess risk tends to the fixed, biased value $\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) \approx \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right] \approx 0.059$.
...
\todohi{Reconsider full thesis structure. Orig stats sec after problem statement!?}

\todomid{Separate general feature theory from discretization. If Tcal not subset of Xcal!?}

\todohi{Develop from low-dim prior! More SUBJECTIVE priors!!}
...
As seen in \cref{ch:dir_cont}, the flexibility of using a full-support prior, while effective for countable data spaces, has limited practical value when operating on observations drawn from continuous spaces. This section develops a new Bayesian predictive distribution using discretization that may be used to realize the benefits of Dirichlet process priors while avoiding the aforementioned limitation. 

Define a discretizing data transformation $T: \Xcal \mapsto \Tcal$, where the range is a countable subset $\Tcal \subset \Xcal$, such that $|\Tcal| \leq \aleph_0$. This transform is applied to each of the training values $\Xrm_n$, as well as to the novel observation $\xrm$. Note that $T$ reduces the cardinality of the observation set; this is conceptually similar to the dimensionality reduction induced by feature transforms commonly used in machine learning and yields similar benefits. Use $T$ to define a partitioning $\{\ldots, \Xcal'(t), \ldots\}$ of the observation space $\Xcal$, where $\Xcal'(t) = \{x \in \Xcal: T(x) = t\}$. It is assumed that these subsets are all connected spaces; furthermore, assume that $t \in \Xcal'(t), \ \forall \ t \in \Tcal$, such that the transform maps $T(t) = t$.

Having effectively changed the domain of the observations, the Dirichlet prior marginal mean is redefined as $\alpham = \sum_{t \in \Tcal} \alphamd(t) \delta(\cdot - t)$, a mixture of delta functions at the discretized values, where $\alphamd \in \Pcal(\Tcal)$ is the discrete marginal function. 
...
&& + \Big(1 - \Erm_{\uppsimd | \upthetamd} \big[\gammamd(\trm; \uppsimd)\big]\Big) \upthetacd(\trm) \;.
\end{IEEEeqnarray}
...
&& + \Big(\upthetacd\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \;.
\end{IEEEeqnarray}
...
\qquad = \Crm_{\uppsimd | \upthetamd}\big[ \gammamd(\trm; \uppsimd) \big] \big( \alphac(\trm) - \upthetacd(\trm) \big) \otimes \big( \alphac(\trm) - \upthetacd(\trm) \big) \nonumber \\
\qquad \quad + \Erm_{\uppsimd | \upthetamd}\left[ \frac{\big(1 - \gammamd(\trm; \uppsimd)\big)^2}{N \uppsimd(\trm)} \right] \Big( \diag\big(\upthetacd(\trm)\big) - \upthetacd(\trm) \otimes \upthetacd(\trm) \Big) 
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl} 
\IEEEeqnarraymulticol{3}{l}{\Erm_{\Drm | \upthetam,\upthetac} \big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \big]} \nonumber \\
\qquad & \equiv & \Erm_{\uppsimd | \upthetamd}\Big[ \gammamd\big(T(\xrm); \uppsimd\big)^2 \Big] \Big( \alphac\big(T(\xrm)\big) - \upthetacd\big(T(\xrm)\big) \Big) \otimes \Big( \alphac\big(T(\xrm)\big) - \upthetacd\big(T(\xrm)\big) \Big) \nonumber \\
&& + \Erm_{\uppsimd | \upthetamd}\left[ \frac{\Big(1 - \gammamd\big(T(\xrm); \uppsimd\big)\Big)^2}{N \uppsimd\big(T(\xrm)\big)} \right] \bigg( \diag\Big( \upthetacd\big(T(\xrm)\big) \Big) - \upthetacd\big(T(\xrm)\big) \otimes \upthetacd\big(T(\xrm)\big) \bigg) \nonumber \\
&& + \Erm_{\uppsimd | \upthetamd}\Big[\gammamd\big(T(\xrm); \uppsimd\big)\Big] \Big( \alphac\big(T(\xrm)\big) - \upthetacd\big(T(\xrm)\big) \Big) \otimes \Big(\upthetacd\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \nonumber \\
&& + \Erm_{\uppsimd | \upthetamd}\Big[\gammamd\big(T(\xrm); \uppsimd\big)\Big] \Big(\upthetacd\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \otimes \Big( \alphac\big(T(\xrm)\big) - \upthetacd\big(T(\xrm)\big) \Big) \nonumber \\
&& + \Big(\upthetacd\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \otimes \Big(\upthetacd\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \;.
\label{eq:predictive_del_sq_dir_disc}
\end{IEEEeqnarray}
...
\todohigh{New regularized risk form?? Empirical loss now in feature space?!??}

\begin{IEEEeqnarray}{rCl}
\Erm_{\yrm | \xrm,\Drm} \big[ \Lcal(h,\yrm) \big] & = & \int_{\Ycal} \Lcal(h,y) \prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) {\drm}y \nonumber \\
& = & \gammamd\big(T(\xrm); \Xrm\big) \int_{\Ycal} \alphac\big(y; T(\xrm)\big) \Lcal(h,y) {\drm}y \nonumber \\
&& + \Big(1 - \gammamd\big(T(\xrm); \Xrm\big)\Big) \int_{\Ycal} \Psicd\big(y; T(\xrm); \Drm\big) \Lcal(h,y) {\drm}y \nonumber \\
& = & \gammamd\big(T(\xrm); \Xrm\big) \Erm_{\yrm | \xrm}\big[ \Lcal(h, \yrm) \big] \nonumber \\
&& + \Big(1 - \gammamd\big(T(\xrm); \Xrm\big)\Big) \frac{\sum_{n=1}^N \delta\big[T(\xrm), T(\Xrm_n)\big] \Lcal\big(h, \Yrm_n \big)}{\sum_{n=1}^N \delta\big[T(\xrm), T(\Xrm_n)\big]} \; . \IEEEeqnarraynumspace
\end{IEEEeqnarray}
...
\IEEEeqnarraymulticol{3}{l}{\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \Erm_{\xrm,\Drm | \uptheta} \Big[ \big( \mu_{\yrm | \xrm,\Drm} - \mu_{\yrm | \xrm,\uptheta} \big)^2 \Big]} \nonumber \\
\qquad & \equiv & \int_{\Ycal} y \int_{\Ycal} y' \Erm_{\xrm,\Drm | \upthetam,\upthetac} \Big[ \Delta(y; \xrm; \Drm,\upthetac) \Delta(y'; \xrm; \Drm,\upthetac) \Big] {\drm}y {\drm}y' \nonumber \\
& \equiv & \Erm_{\trm | \upthetam}\Big[ \Erm_{\uppsimd | \upthetamd}\big[ \gammamd(\trm; \uppsimd)^2 \big] \big( \mu_{\yrm | \xrm}(\trm) - \Erm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \big)^2 \Big] \nonumber \\
&& + \Erm_{\trm | \upthetam}\left[ \Erm_{\uppsimd | \upthetamd}\left[ \frac{\big(1 - \gammamd(\trm; \uppsimd)\big)^2}{N \uppsimd(\trm)} \right] \big( \Erm_{\xrm | \trm, \upthetam}[\Sigma_{\yrm | \xrm,\upthetac}] + \Crm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \big) \right] \nonumber \\
&& + \Erm_{\trm | \upthetam}\big[ \Crm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \big] \nonumber \\
& \equiv & \Erm_{\trm | \upthetam}\Big[ \Erm_{\uppsimd | \upthetamd}\big[ \gammamd(\trm; \uppsimd)^2 \big] \big( \mu_{\yrm | \xrm}(\trm) - \Erm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \big)^2 \Big] \nonumber \\
&& + \Erm_{\trm | \upthetam}\left[ \Erm_{\uppsimd | \upthetamd}\left[ \frac{\big(1 - \gammamd(\trm; \uppsimd)\big)^2}{N \uppsimd(\trm)} \right] \Erm_{\xrm | \trm, \upthetam}[\Sigma_{\yrm | \xrm,\upthetac}] \right] \nonumber \\
&& + \Erm_{\trm | \upthetam}\left[ \left( 1 + \Erm_{\uppsimd | \upthetamd}\left[ \frac{\big(1 - \gammamd(\trm; \uppsimd)\big)^2}{N \uppsimd(\trm)} \right] \right) \Crm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \right] \;,
\label{eq:risk_cond_SE_dir_ex_disc}
\end{IEEEeqnarray}
...
To demonstrate the efficacy of the discretized Dirichlet regressor, the scenario detailed in \cref{sec:SE_dir_cont_example} will be again used; $\Xcal$ and $\Ycal$ are the closed unit interval and the true model dictates a nonlinear clairvoyant regressor. The Bayesian linear regressor detailed in \cref{sec:SE_dir_example} will be used for comparison; note that it can operate on the continuous data without any modification.
...
\end{IEEEeqnarray}
where the cardinality $|\Tcal| \equiv M$ is selected by the designer. Note that when $M = 128$, this set is equivalent to the set used for the discrete-domain Dirichlet example in \cref{sec:SE_dir_example}. The discretization transform used is $T(x) = \argmin_{t \in \Tcal} \| x - t \|$, rounding each observation to the nearest discretized value.
...
%%&& \prod_{z \in \Zcal} \delta\left[ \uppsim(z),\sum_{y \in \Scal_z} \uppsi(y) \right] \nonumber \\
...
and, finally,
\begin{IEEEeqnarray}{rCl}
\IEEEeqnarraymulticol{3}{l}{\Erm_{\uppsi}\big[ \uppsi(y_1)\uppsi(y_2) \big] = \frac{{\drm}^2}{{\drm}t_1 {\drm}t_2} g(t_1,t_2)} \nonumber \\
\qquad & = & \frac{{\drm}}{{\drm}t_2} \left[ \frac{1}{N} u(t_2-t_1) \theta\big( \min(t_1,t_2) \big) + \left(1-\frac{1}{N}\right) \theta(y_1) \left( \int_{-\infty}^{y_2} \theta(t_2) {\drm}t_2 \right) \right] \nonumber \\
...
\qquad = \Erm_{\uptheta}\left[ \left( \int_{-\infty}^{y_1} \uptheta(t_1) {\drm}t_1 \right)^2 + \left( \int_{-\infty}^{y_1} \uptheta(t_1) {\drm}t_1 \right) \left( \int_{y_1}^{y_2} \uptheta(t_2) {\drm}t_2 \right) \right] \nonumber \\
\qquad = \frac{\left( \int_{-\infty}^{y_1} \alpha(t_1) {\drm}t_1 \right) \left( 1 + \alpha_0\int_{-\infty}^{y_1} \alpha(t_1) {\drm}t_1 \right) + \alpha_0 \left( \int_{-\infty}^{y_1} \alpha(t_1) {\drm}t_1 \right) \left( \int_{y_1}^{y_2} \alpha(t_2) {\drm}t_2 \right)}{\alpha_0+1} \nonumber \\
\qquad = \frac{\left( \int_{-\infty}^{y_1} \alpha(t_1) {\drm}t_1 \right) + \alpha_0 \left( \int_{-\infty}^{y_1} \alpha(t_1) {\drm}t_1 \right) \left( \int_{-\infty}^{y_2} \alpha(t_2) {\drm}t_2 \right)}{\alpha_0+1} \quad \forall \ y_2 \geq y_1 \;.
\end{IEEEeqnarray}
...
\qquad = \frac{\frac{{\drm}}{{\drm}t_2} \left[ u(t_2-t_1) \alpha\big( \min(t_1,t_2) \big) + \alpha_0 \alpha(y_1) \left( \int_{-\infty}^{y_2} \alpha(t_2) {\drm}t_2 \right) \right]}{\alpha_0+1} \nonumber \\
\qquad = \frac{\alpha(y_1)\delta(y_1-y_2) + \alpha_0 \alpha(y_1)\alpha(y_2)}{\alpha_0+1} \;.
\end{IEEEeqnarray}
...
\qquad = \frac{\frac{{\drm}}{{\drm}t_2} \left[ (\alpha_0^{-1} + N^{-1}) \Big( u(t_2-t_1) \alpha\big( \min(t_1,t_2) \big) \Big) + \left(1-N^{-1}\right) \alpha(y_1) \left( \int_{-\infty}^{y_2} \alpha(t_2) {\drm}t_2 \right) \right]}{1 + \alpha_0^{-1}} \nonumber \\
\qquad = \frac{(\alpha_0^{-1} + N^{-1}) \alpha(y_1) \delta(y_1-y_2) + (1 - N^{-1}) \alpha(y_1) \alpha(y_2)}{1 + \alpha_0^{-1}} \;.
\end{IEEEeqnarray}
...
%&& \binom{m(n+1)-N-1}{M-1} U\left(n+1-\frac{N+M}{m}\right) \;,
%\end{IEEEeqnarray}
%
...
%&& \prod_{k=1}^M \frac{m(n+1)-N-k}{N+M-k} \\
...
%
%Next, we assess the second term in equation ???. We seek to reuse the previous identity. To this effect, we expand the final term,
%
%\begin{IEEEeqnarray}{rCl}
