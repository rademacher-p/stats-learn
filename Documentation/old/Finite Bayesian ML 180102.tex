...
\mathcal{R}(f^*) = \text{E}_{\bar{\bm{\mathrm{n}}}} \left[ \Sigma_{\mathrm{y} | \bar{\bm{\mathrm{n}}}} \right] = \frac{M(M-1)(N+M+1)}{12(N+M)} \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\text{P}(y_{m_y} | x_{m_x},D) & = & \frac{\text{P}(y_{m_y},x_{m_x} | D)}{\sum_{m_y=1}^{M_y} \text{P}(y_{m_y},x_{m_x} | D)} \\
...
\begin{IEEEeqnarray}{C}
f^*(\mathrm{x},\mathrm{D}) = \argmax_{h \in \mathcal{Y}} \text{P}_{\mathrm{y} | \mathrm{x},\mathrm{D}}(h | \mathrm{D}) \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
f^*(x_{m_x},D) & = & \sum_{m_y=1}^{M_y} m_y \text{P}_{\mathrm{y} | \mathrm{x},\mathrm{D}}(m_y | m_x, D) \equiv \mu_{\mathrm{y} | \mathrm{x},\mathrm{D}}(m_x, D) \\
...
%& = & \left( \frac{M_y}{N'(m_x,D) + M_y} \right) \frac{M_y+1}{2} + \\
%&& \quad \left( \frac{N'(m_x,D)}{N'(m_x,D) + M_y} \right) \frac{1}{N} \sum_{n=1}^N D(n) \;.
\end{IEEEeqnarray}
...
& = & \sum_{m=0}^{M-1} \binom{M}{m} (-1)^m (m+1)^{-2} \;.
\end{IEEEeqnarray}
