...
PGR: use todonotes package instead of my initials?
...
PGR: generalize to joint decisions!!! training/test!
...
It is informative to treat the distribution $\Prm_{\yrm | \xrm,\Drm}$ as an estimate of the unknown conditional PMF $\Prm_{\yrm | \xrm,\uptheta} \equiv \tilde{\uptheta}(\xrm)$ and investigate the effects of subjective prior knowledge. For a given $\xrm$ and corresponding number of training samples $\nrm'(\xrm)$, the expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
...
\begin{IEEEeqnarray}{rCl}
f^*(\xrm;\Drm) & = & \left( \frac{|\Ycal|}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{1}{|\Ycal|} \sum_{y \in \Ycal} y +  \\
...
The excess conditional risk can thus be represented as the conditional expectation (with respect to $\Prm_{\xrm | \uptheta}$) of a sum of two random functions of $\xrm$. The first function depends on $\Sigma_{\yrm | \xrm,\uptheta}$, the conditional variance of the clairvoyant estimate for a given observation of $\xrm$; this is similar to the clairvoyant squared-error. The second function is dependent on the squared bias between the clairvoyant estimate $\mu_{\yrm | \xrm,\uptheta}$ and the prior estimate $\mu_{\yrm | \xrm}$. These two terms fully contain the dependencies of the risk on the true predictive distribution $\Prm_{\yrm | \xrm,\uptheta} = \tilde{\uptheta}(\xrm)$ and on the data-independent Bayes predictive distribution $\Prm_{\yrm | \xrm} = \alpha(\cdot,\xrm) / \alpha'(\xrm)$.

The two second-order terms are scaled by factors dependent on $\uptheta'(x)$, $\alpha'(x)$, and $N$ via conditional expectations with respect to $\nrm'(x)$. Note that by the aggregation property of multinomial distributions, the random variable $\nrm'(x) | \uptheta'(x) \sim \Bi \big(N,\uptheta'(x)\big)$. Closed-forms have not been found for the function expectations of binomial random variables above.

PGR: binomial citations?

It is informative to consider the trends in the conditional squared-error risk \eqref{eq:risk_cond_SE_dir_ex} for different selections of $\alpha$ and different true models $\uptheta$. The analysis will express the Dirichlet parameters as the conditional prior distributions $\alpha(\cdot,x)/\alpha'(x)$ and their concentrations $\alpha'(x)$. First consider the conditional prior PMF's; as shown, they manifest themselves in the risk through the squared estimator bias. It is clear that regardless of how the values $\alpha'(x)$ are chosen, the best selections for the conditional priors have first moments matching those of the corresponding clarivoyant predictive distrbutions $\Prm_{\yrm | \xrm,\uptheta}$. Thus if the estimates are unbiased, the excess conditional risk is equivalent to the first term in \eqref{eq:risk_cond_SE_dir_ex}; this term measures additional estimator variance proportionate to the clairvoyant estimator variance $\Sigma_{\yrm | \xrm,\uptheta}$.

The other user-selected Dirichlet parameters $\alpha'(x)$ are the concentration parameters for the corresponding conditional distributions; they control important bias/variance trade-offs via the two scaling factors in \eqref{eq:risk_cond_SE_dir_ex}. First, consider how the 


PGR

The trends of the excess conditional risk with the prior concentration parameter $\alpha_0$ and the volume of training data $N$ are again of interest. If the value of $\alpha_0$ chosen to characterize the prior is large relative to the training volume $N$, the estimate will trend toward the first moment of the distribution $\Prm_{\yrm | \xrm} = \alpha(\cdot,\xrm)/\alpha'(\xrm)$ and the influence of the training data is minimal. Recalling the discussion of section \ref{sec:predictive_est}, the estimator may be biased, but will have low variance. In the limit of increasing concentration, the excess conditional risk is
\begin{IEEEeqnarray}{rCl}
\lim_{\alpha_0 \to \infty} \Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) & = & \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right] \;,
\end{IEEEeqnarray}
the expected conditional squared-error between the means of the Bayesian predicitve PMF and the clairvoyant predictive PMF. If a low prior concentration is chosen, the Bayesian estimate trends to the empirical mean and the excess risk to
\begin{IEEEeqnarray}{rCl}
\lim_{\alpha_0 \to 0} \Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) & = & \Erm_{\xrm | \uptheta}\left[ \Sigma_{\yrm | \xrm,\uptheta} \sum_{n=1}^N \binom{N}{n} \uptheta'(\xrm)^n \big( 1 - \uptheta'(\xrm) \big)^{N-n} n^{-1} \right] \nonumber \\
&& \qquad + \Erm_{\xrm | \uptheta}\left[ \big( 1 - \uptheta'(\xrm) \big)^N \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right] \nonumber \;.
\end{IEEEeqnarray}
Note that the expectation over $\nrm'(x)$ is proportionate to the first inverse moment of a positive binomial random variable \cite{stephan}. Also, as $N$ increases the contribution to the risk from the squared bias between the Bayesian and clairvoyant means decreases and the excess risk becomes a function of $\uptheta$ only.

The other asymptotic result of importance is how the excess risk changes with increasing training volume $N$. In determining the excess risk \eqref{eq:risk_cond_SE_dir_ex}, the dependency on the value of $N$ is through the expectations of functions of the binomial variables $\nrm'(x)$. As $N$ trends to infinity, the binomial distribution concentrates at $\nrm'(x) \approx N \theta'(x)$; as such, the expectations of interest and thus the excess risk trend to zero. This desirable property of the estimator is a consequence of the full support of the Dirichlet prior, ensuring that the model posterior concentrates and the empirical PMF.
