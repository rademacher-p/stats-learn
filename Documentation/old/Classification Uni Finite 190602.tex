...
%\begin{figure}
%\centering
%\includegraphics[width=1.0\linewidth]{P_theta_post_uni.pdf}
...
%\begin{figure}
%\centering
%\includegraphics[width=1.0\linewidth]{Risk_01_uni_N-Mx.pdf}
%\caption{Optimal 0-1 Risk vs training set size for various input set sizes}
...
\quad = \binom{N+|\Ycal||\Xcal|-1}{|\Ycal||\Xcal|-1}^{-1} \big| \cup_{y \in \Ycal} \{ \bar{n} : \bar{n}(y,x) \geq n \} \big| \nonumber \\
\quad = \begin{cases} 1 & \mathrm{if} \ n < 0, \\ \sum_{m=1}^{|\Ycal|} \binom{|\Ycal|}{m} (-1)^{m-1} \\ \quad \prod_{l=1}^{|\Ycal||\Xcal|-1} \Big( 1-\frac{mn}{N+l} \Big) H\Big( \big\lfloor\frac{N}{m}\big\rfloor - n \Big) & \mathrm{if} \ 0 \leq n \leq N, \\ 0 & \mathrm{if} \ n > N. \end{cases} \nonumber
\end{IEEEeqnarray}
