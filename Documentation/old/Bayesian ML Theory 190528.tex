...
\qquad = \frac{1}{m|\Ycal|} \nonumber \;.
\end{IEEEeqnarray}
The irreducible 0-1 risk for the uniform prior is thus
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - |\Ycal|^{-1} \sum_{m=1}^{|\Ycal|} \binom{|\Ycal|}{m} (-1)^{m-1} m^{-1} \\
...
\Ycal|^\mathrm{th}$ harmonic number. Observe that the irreducible risk does not depend on the cardinality $|\Xcal|$.
