...
& = & \prod_{n=1}^N \frac{\alpha\big( \Yrm(n),\Xrm(n) \big)}{\alpha_0} \nonumber \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
where ``marginalized'' functions $\alpha'(x) \equiv \sum_{y \in \Ycal} \alpha(y,x)$ and $N'(x;\Drm) \equiv \sum_{y \in \Ycal} \bar{N}(y,x;\Drm) = \sum_{n=1}^N \delta\big[ x,\Xrm(n) \big]$ are introduced. Note that $\text{P}(\xrm) = \alpha'(\xrm)/\alpha_0$. Additionally, as $\nbarrm \equiv \bar{\bm{N}}(\Drm) \sim \text{DM}(N,\alpha)$, the aggregation property for Dirichlet-Multinomial random functions ensures $\bm{\mathrm{n}}' \equiv \bm{N}'(\Drm) \sim \text{DM}(N,\alpha')$ is Dirichlet-Multinomial over the set $\Xcal$.
...
& = & \left( \frac{|\Ycal|}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{1}{|\Ycal|} \sum_{y \in \Ycal} y + \nonumber \\
&& \quad \left( \frac{N'(\xrm;\Drm)}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{\sum_{n=1}^N \delta\big[ \xrm,\Xrm(n) \big] \Yrm(n)}{N'(\xrm;\Drm)} \nonumber \;.
\end{IEEEeqnarray}
