...
Certain specialized functions are detailed next. Specificially, the multinomial coefficient and multivariate beta function, which are typically defined for sequences, are provided generally for function inputs. The multinomial operator $\Mcal$ is used for functions with codomain $\Nbb$, such that 
\begin{equation}
\Mcal(\bar{n}) = \frac{\left( \sum_{y \in \Ycal} \bar{n}(y) \right)!}{\prod_{y \in \Ycal} \bar{n}(y)!} \;.
\end{equation}
Similarly, the beta function $\beta$ operates on functions with codomain $\Rbb^+$, such that
\begin{equation}
...
\end{IEEEeqnarray}
which counts the number of occurences of the pair $(y,x)$ in the training set $D$. The codomain of the transform is the function space 
\begin{IEEEeqnarray}{rCl}
\bar{\Ncal} & = & \left\{ \bar{n} \in \Nbb^{\Ycal \times \Xcal}: \sum_{y \in \Ycal} \sum_{x \in \Xcal} \bar{n}(y,x) = N \right\} \;.
\end{IEEEeqnarray}
...
where 
\begin{IEEEeqnarray}{rCl}
\Ncal' & = & \left\{ n' \in \Nbb^{\Xcal}: \sum_{x \in \Xcal} n'(x) = N \right\} \;.
\end{IEEEeqnarray}
...
It may not seem intutitve for the risk to decrease when $\alpha_0$ is smaller -- the variance of the model $\theta$ increases and the prior knowledge is less definitive. This is a result of the Dirichlet PDF weight shifting towards the $|\Ycal||\Xcal|$ models which have $\ell_0$ norms satisfying $\| \theta \|_0 = 1$. Although these PMF's are maximally separated (and uncorrelated), they all have zero variance and thus zero risk. The optimal learner will simply use the empirical distribution supplied via the training data - this allows exact identification of $\theta$ with a single training pair and leads to predictions that match the single class represented in $\Yrm$.
...
This chapter extends previous results for applications where the space $\Ycal$ can have an infinite number of elements. Specifically, the model distribution will be determined by using a Dirichlet random process prior. First, we assume that the set is countably infinite, that is $|\Ycal| = |\Nbb|$; the model distribution is thus a discrete random process. 
...
We extend the notion of the multinomial coefficient for general functions $\Ycal \mapsto \Nbb$, such that 
\begin{equation}
\Mcal(\bar{n}) = \frac{\left( \sum_{y \in \Ycal} \bar{n}(y) \right)!}{\prod_{y \in \Ycal} \bar{n}(y)!} \;.
\end{equation}
...
This chapter extends further to the case where $\Ycal$ is countably infinite, that is $|\Ycal| > |\Nbb|$; the model distribution is thus a continuous random process. 
...
This section adds the input space $\Xcal$, now considered to be countably infinite; that is $|\Xcal| > |\Nbb|$. The model distribution is a random process over the space $\Ycal \times \Xcal$.
