...
In the preceeding formula, the expectation over the novel pair $(\yrm,\xrm)$ is split into two separate expectations as only $\yrm$ is unobservable. It will be useful to decompose the model distribution in a similar fashion as $\uptheta \equiv (\uptheta',\tilde{\uptheta})$. First, introduce the marginal distribution $\uptheta' \in \Pcal(\Xcal)$, defined as $\uptheta' \equiv \sum_{y \in \Ycal} \uptheta(y,\cdot) \in \Pcal(\Xcal)$; note that the summation is replaced by an integral when $\yrm$ is a continuous random variable. Next, introduce the conditional distributions $\tilde{\uptheta} \in \Pcal(\Ycal)^{\Xcal}$ defined as $\tilde{\uptheta}(x) \equiv \uptheta(\cdot,x) / \uptheta'(x)$. 

This decomposition can be used to show that $\Prm_{\xrm | \uptheta} = \Prm_{\xrm | \uptheta'} = \uptheta'$ and $\Prm_{\yrm | \xrm,\uptheta} = \Prm_{\yrm | \xrm,\tilde{\uptheta}(\xrm)} = \tilde{\uptheta}(\xrm)$; these distributions will be of recurring importance.
...
PGR: conditional theta condition on x necessary above?
...
PGR: use posterior for tilde(x) ONLY!?

PGR: MIXTURE IS OVERKILL. Can be simplified to single Dir.
