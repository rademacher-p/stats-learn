...
Random variables and functions are denoted with roman font (e.g. $\xrm$), while specific values are denoted with italics (e.g. $x$). Discrete and continuous random functions are statistically characterized by probability mass functions (PMF) and probability density functions (PDF), respectively. The formal notations for PMF's/PDF's are $\Prm_{\xrm}$ and $\prm_{\xrm}$, where the subscript indicates the random variable/function. For notational simplicity, probability notations are occasionally represented as $\Prm(\xrm)$; in such instances, the formal notation can be recovered by replacing the distribution with $\Prm_{\xrm}(x)$ and all instances of the roman font $\xrm$ with the italic value $x$. Conditional distributions such as $\Prm(\xrm|\zrm)$ are functions of both $x$ and $z$ - the arguments are notated as $\Prm(\xrm|\zrm)(x|z)$ to separate the unobserved random values from the conditional ones.
...
\end{equation}
where the argument $g$ is an arbitrary scalar function of $\xrm$. Additionally, define a   variance operator $\Crm_{\xrm}$ 
\begin{equation}
\Crm_{\xrm}\big[g(\xrm)\big] = \Erm_{\xrm} \bigg[ \Big( g(\xrm) - \Erm_{\xrm}\big[g(\xrm)\big] \Big)^2 \bigg] \;.
\end{equation}

If the argument $g$ is a function of other variables besides $\xrm$, such that $g(\cdot,\xrm)$, or the codomain of $g$ is a function space, such that $g(\cdot;\xrm)$, the notation for these operators is expanded. Let the additional free variable or the domain of the output function space be $y \in \Ycal$. Then, the expectation operator is itself a function over $\Ycal$ and is represented as
\begin{equation}
...
\end{equation}
Similarly, the beta function $\beta$ operates on functions with codomain $\mathbb{R}_{>0}$, such that
\begin{equation}
...
\end{equation}
As the model $\theta$ is also unobserved, $\Rcal_{\Theta}: \Fcal \times \Theta \mapsto \mathbb{R}^+$ is not yet a valid objective function for optimization. An operator must be chosen to remove the dependency on $\theta$ and form an objective function $\Fcal \mapsto \mathbb{R}^+$.
...
Define the PDF $\prm(\theta): \Theta \mapsto \mathbb{R}^+$. Now using Bayes rule, the risk can be formulated as
\begin{IEEEeqnarray}{rCl} \label{eq:risk}
\Rcal(f) & = & \Erm_{\theta}\big[ \Rcal_{\theta}(f | \theta) \big] \\
...
\end{IEEEeqnarray}
where the user-selected PDF parameters $\alpha : \Ycal \times \Xcal \mapsto \mathbb{R}_{>0}$ are introduced and $\beta$ is the generalized beta function.
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\theta}(y,x,y',x') & = & \Erm_{\theta}\Big[ \big(\theta(y,x)-\mu_{\theta}(y,x)\big) \big(\theta(y',x')-\mu_{\theta}(y',x')\big) \Big] \\
...
\begin{IEEEeqnarray}{rCl}
\prm(\theta) & = & \sum_{y \in \Ycal} \sum_{x \in \Xcal} \frac{\alpha(y,x)}{\alpha_0} \delta\big( \theta - \bm{e}(y,x) \big) \\
...
PGR: functional argument notation conflict??? need bold???

For a given data set $\Drm$, the resulting function $\bar{N}(\Drm)$ over $\Ycal \times \Xcal$ effectively counts the number of occurences of each possible pair $(y,x)$ and thus satisfies
\begin{IEEEeqnarray}{rCl}
\bar{N}(\Drm) & \in & \bar{\Ncal}
= \left\{ \bar{n} \in \mathbb{N}^{\Ycal \times \Xcal}: \sum_{y \in \Ycal} \sum_{x \in \Xcal} \bar{n}(y,x) = N \right\} \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
This perspective shows values of the PMF $\Prm(\Drm)$ to be joint moments of the model $\theta$. 
...
or
\begin{IEEEeqnarray}{rCl}
\Prm(\Drm) & = & \prod_{y \in \Ycal} \prod_{x \in \Xcal} \left(\frac{\alpha(y,x)}{\alpha_0}\right)^{\bar{N}(y,x;\Drm)} \\
...
PGR: prove zero limit using data conditional?

\begin{IEEEeqnarray}{rCl}
\Prm(\Drm) & = & \sum_{y \in \Ycal} \sum_{x \in \Xcal} \frac{\alpha(y,x)}{\alpha_0} \prod_{n=1}^N \delta\big[ (y,x),\Drm_n \big] \;.
\end{IEEEeqnarray}


Figure \ref{fig:P_nbar} displays the distribution of $\nbarrm$ for $N=10$ and different model concentrations $\alpha_0$. Observe that for large $\alpha_0$, the distribution approaches a multinomial distribution $\nbarrm \sim \mathrm{Mult}(\alpha/\alpha_0)$. Figure \ref{fig:P_nbar_N} shows how a specific model prior influences the data PMF differently for different $N$. Observe that as the number of training samples increases, the PMF $\Prm(\nbarrm)$ visually approximates the corresponding model prior $\prm(\theta)$, such that $\Prm_{\nbarrm}(\nbarrm) \approx N^{1-|\Ycal||\Xcal|}\prm_{\theta}(\nbarrm/N)$.
...
\Prm(\Drm) & = & \binom{N+|\Ycal||\Xcal|-1}{N,|\Ycal||\Xcal|-1}^{-1} \Mcal\big( \bar{N}(\Drm) \big)^{-1} \;.
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{rCl} \label{P_D_io}
...
As shown in Equation \eqref{eq:f_opt_xD}, the decision selected by the optimally designed function depends on $\Prm(\yrm | \xrm,\Drm)$, the distribution of the unobserved $\yrm$ conditioned on all observable random variables.
...
\begin{IEEEeqnarray}{rCl}
\Prm(\yrm,\xrm | \Drm) & = & \frac{\Erm_{\theta}\big[ \theta(\yrm,\xrm) \Prm(\Drm | \theta) \big]}{\Prm(\Drm)} = \Erm_{\theta | \Drm}\big[ \theta(\yrm,\xrm) | \Drm \big] \\
...
\end{IEEEeqnarray}
This is a mixture distribution between the prior expectation $\Erm[\theta] = \alpha/\alpha_0$ and the empirical distribution $\bar{N}(\yrm,\xrm;\Drm)/N$. The more subjective the model prior (i.e. larger $\alpha_0$), the more the prior mean is favored; the more data, the more the empirical PMF is favored.
...
\begin{IEEEeqnarray}{rCl}
\Erm_{\Drm|\theta}\big[ \Prm_{\yrm,\xrm}(y,x | \Drm) \big] & = & \left(\frac{\alpha_0}{\alpha_0 + N}\right) \frac{\alpha(y,x)}{\alpha_0} + \left(\frac{N}{\alpha_0 + N}\right) \theta(y,x) \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\mathrm{Bias}(y,x ; \alpha,\theta) & = & \Erm_{\Drm | \theta} \big[ \Prm_{\yrm,\xrm}(y,x | \Drm) - \theta \big] \\
& = & \frac{\alpha(y,x) + \Erm\big[ \bar{\nrm}(y,x) \big]}{\alpha_0+N} - \theta(y,x) \nonumber \\
...
\begin{IEEEeqnarray}{L}
\Crm_{\nbarrm | \theta} \big[ y,x,y',x' ; \Prm_{\yrm,\xrm | \nbarrm}(\cdot,\cdot | \nbarrm) \big] \\
...
\caption{Model Estimates, $\alpha_0 = 0.1$}
...
\caption{Model Estimates, $\alpha_0 = 10$}
...
\end{IEEEeqnarray}
where ``marginalized'' functions $\alpha'(x) \equiv \sum_{y \in \Ycal} \alpha(y,x)$ and $N'(x;\Drm) \equiv \sum_{y \in \Ycal} \bar{N}(y,x;\Drm) = \sum_{n=1}^N \delta\big[ x,\Xrm_n \big]$ are introduced. Note that $\Prm(\xrm) = \alpha'(\xrm)/\alpha_0$. Additionally, as $\nbarrm \equiv \bar{N}(\Drm) \sim \mathrm{DM}(N,\alpha)$, the aggregation property for Dirichlet-Multinomial random functions ensures $\nrm' \equiv N'(\Drm) \sim \mathrm{DM}(N,\alpha')$ is Dirichlet-Multinomial over the set $\Xcal$.
...
\begin{IEEEeqnarray}{rCl}
\Prm(\yrm | \xrm,\Drm) & = & \frac{\Prm(\yrm,\xrm | \Drm)}{\Prm(\xrm | \Drm)} \\
...
\end{IEEEeqnarray}
where the properties of a multinomial distribution and its aggregation, detailed in Appendix \ref{app:mult}, have been used.

Similarly, the weighting factors are only influenced by these values. As the number of training examples increases or as $\alpha_0 \to 0$, $\Prm(\yrm | \xrm,\Drm)$ trends towards the empirical conditional distribution. For $N'(\xrm;\Drm) = 0$ or as $\alpha_0 \to \infty$, the PMF trends toward the conditional distribution $\Prm(\yrm|\xrm)$, which only depends on the model parameter $\alpha$.
...
\end{IEEEeqnarray}
where $\theta'(\xrm) \equiv \sum_{y' \in \Ycal} \theta(y',\xrm)$; this aggregation is a Dirichlet random function parameterized by $\alpha'$. Just as $\Prm(\yrm,\xrm | \Drm)$ is an expectation of the model $\theta$ posterior conditioned on $\Drm$, $\Prm(\yrm | \xrm,\Drm)$ is an expectation based on $\theta$ conditioned on both observed random variables. The expectation operates on a function of $\theta$ where each portion of the function $\theta(\cdot,\xrm)$ is normalized by its aggregation $\theta'(\xrm)$. 
...
& = & \sum_{y' \in \Ycal} \frac{\alpha(y',\xrm) + \bar{N}(y',\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \mathrm{Dir}\bigg( \theta ; \alpha + \bar{N}\Big( \big( (y',\xrm),\Drm \big) \Big) \bigg) \nonumber \;.
\end{IEEEeqnarray}
This is a mixture distribution. The weighting factors are, interestingly, the values of $\Prm(\yrm | \xrm,\Drm)$ given the observations. The PDF's being combined are Dirichlet distributions with different parameterizing functions $\alpha(y,x) + \bar{N}\Big( y,x;\big( (y',\xrm),\Drm \big) \Big) = \alpha(y,x) + \bar{N}(y,x;\Drm) + \delta[y,y'] \delta[x,\xrm]$; additional emphasis is put on the $|\Ycal|$ different possible pairs $(y,x)$ including the observed value $\xrm$.

This posterior PDF shares the same asymptotic distributions with $\prm(\theta|\Drm)$. As $\alpha_0 \to \infty$ or $N \to \infty$, the $|\Ycal|$ Dirichlet mixture PDF's converge to the same impulsive distribution and thus $\prm(\theta|\xrm,\Drm)$ again trends toward $\alpha / \alpha_0$ and $\bar{N}/N$, respectively.

To confirm that the expectation produces the same distribution $\Prm(\yrm | \xrm,\Drm)$ displayed previously, use the distribution of a Dirichlet random function conditioned on its aggregation, proven in Appendix \ref{app:Dir_agg}, to show $\Erm_{\theta}\big[ \theta(y,x) / \theta'(x) \big] = \alpha(y,x) / \alpha'(x)$. Thus, the PMF is evaulated as
\begin{IEEEeqnarray}{rCl}
...
& = & \left( \frac{M_y}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{1}{|\Ycal|} + \nonumber \\
...
The expectation can be represented as a convex combination of two expected losses. The first expected loss is evaluated with respect to the conditional distribution $\Prm(\yrm|\xrm)$, which reflects the prior knowledge encapsulated in model parameter $\alpha$. The second term is a conditional emprical loss, or the average loss among samples $\Yrm_n$ whose corresponding values $\Xrm_n$ match the observed value $\xrm$. The weighting factors are inherited from the conditional PMF $\Prm(\yrm|\xrm,\Drm)$; thus, for a given observation $\xrm$, the model prior concentration $\alpha'$ and the number of matching training samples $N'(\cdot;\Drm)$ dictate which expectations are emphasized.
...
f^*(\xrm;\Drm) & = & \mu_{\yrm | \xrm,\Drm} = \sum_{y \in \Ycal} y \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \\
...
As done for the posterior distribution $\Prm(\yrm | \xrm,\Drm)$, the estimator is interpreted as a convex combination of two separate estimates - the expected value of $\yrm$ conditioned on the observed $\xrm$ and the mean of the training values $\Yrm_n$ which have a value $\Xrm_n$ matching the observed value $\xrm$. The weighting factors are the same; thus, stronger prior information (larger $\alpha'(\xrm)$) provides more weight to the estimate $\mu_{\yrm|\xrm}$ and more voluminous training data puts emphasis on the empirical conditional mean.
...
\begin{IEEEeqnarray}{rCl}
f^*(\xrm;\Drm) & = & \argmin_{h \in \mathbb{R}} \Erm_{\yrm | \xrm,\Drm} \left[ (h-\yrm)^2 \right] \\
& = & \sum_{y \in \Ycal} y \Prm_{\yrm | \xrm,\Drm}(y | \xrm, \Drm) \equiv \mu_{\yrm | \xrm,\Drm}(\xrm, \Drm) \nonumber \\
...
= \Erm_{\xrm} \left[ \frac{N \alpha'(\xrm) + \alpha_0 \alpha'(\xrm) + \alpha_0}{(\alpha_0+N) \big( \alpha'(\xrm)+1 \big)} \Sigma_{\yrm | \xrm} \right] \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Prm(\yrm | \Drm) & = & \Erm_{\theta | \Drm}\big[ \theta(\yrm) | \Drm \big] \\
...
As detailed in Appendix \ref{app:E_DP}, the first and second moments of a Dirichlet process $\mathrm{DP}(\alpha)$ are
\begin{equation}
\mu_{\theta}(y) = \Erm\big[\theta(y)\big] = \frac{\alpha(y)}{\alpha_0}
\end{equation}
and
...
In Appendix \ref{app:DP_post}, it was shown that if the model $\theta \sim \mathrm{DP}(\alpha)$ is a Dirichlet process, then the model conditioned on the training data $\Drm$ is also a Dirichlet process with parameterizing function $\alpha(\cdot) + \bar{N}(\cdot;\Drm)$, where $\bar{N}(y;\Drm) = \sum_{n=1}^N \delta\left( y - \Drm_n \right)$ generalizes from the discrete case. Thus, the conditional PDF of interest can be formulated as
\begin{IEEEeqnarray}{rCl}
\prm(\yrm | \Drm) & = & \Erm_{\theta | \Drm}\big[ \theta(\yrm) | \Drm \big] \\
...
\begin{IEEEeqnarray}{rCl}
\prm\big( \Drm_{n+1} | \Drm_n,\ldots,\Drm_1 \big) & = & \frac{\alpha\big( \Drm_{n+1} \big) + \sum_{n'=1}^N \delta\big( \Drm_{n+1} - \Drm(n') \big)}{\alpha_0 + N} \;
\end{IEEEeqnarray}
and thus the complete PDF is
\begin{IEEEeqnarray}{rCl}
\prm(\Drm) & = & \prm\big( \Drm_1 \big) \prod_{n=2}^N \prm\big( \Drm_n | \Drm(n-1),\ldots,\Drm_1 \big) \\
& = & \frac{\alpha\big( \Drm_1 \big)}{\alpha_0} \prod_{n=2}^N \frac{\alpha\big( \Drm_n \big) + \sum_{i=1}^{n-1} \delta\big( \Drm_n-\Drm(i) \big)}{\alpha_0+n-1} \nonumber \;.
\end{IEEEeqnarray}

Additionally, note that since $\prm_{\Drm_n | \theta}(y|\theta) = \theta(y)$ is independent of sample index $n$, the PDF does not vary when the input arguments are permuted. Furthermore, all marginal distributions of $\Drm$ will have the same form, regardless of which training samples $\Drm_n$ are used.
...
\begin{IEEEeqnarray}{rCl}
\Erm\big[ \Drm_n\Drm(n') \big] & = & \int_{\Ycal} \int_{\Ycal} y y' \prm_{\Drm_n,\Drm(n')}(y,y') \mathrm{d} y \mathrm{d}y' \\
& = & \int_{\Ycal} \int_{\Ycal} y y' \Erm_{\theta} \big[ \prm_{\Drm_n | \theta}(y) \prm_{\Drm(n') | \theta}(y') \big] \mathrm{d}y \mathrm{d}y' \nonumber \\
...
Combining,
\begin{equation}
\Erm\big[ \Drm_n\Drm(n') \big] = \Erm[\yrm^2] - \big(1 - \delta[n,n']\big) \frac{\alpha_0}{\alpha_0+1} \Sigma_{\yrm} \;.
\end{equation}
...
\begin{IEEEeqnarray}{rCl}
f^*(\Drm) & = & \argmin_{h \in \mathbb{R}} \Erm_{\yrm | \Drm} \big[ (h-\yrm)^2 \big]  \\
...
& = & \left( \frac{\alpha_0}{\alpha_0+N} \right) \Erm[\yrm] +  \left( \frac{N}{\alpha_0+N} \right) \frac{1}{N} \sum_{n=1}^N \Drm_n \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\yrm | \Drm} & = & \Erm_{\yrm | \Drm}[\yrm^2]
- \big( \Erm_{\yrm | \Drm}[\yrm] \big)^2 
\end{IEEEeqnarray}
...
\quad = \frac{\alpha_0^2 \mu_{\yrm}^2 + 2\alpha_0 \mu_{\yrm} \sum_{n=1}^N \Erm\big[ \Drm_n \big] + \sum_{n=1}^N \sum_{n'=1}^N \Erm\big[ \Drm_n\Drm(n') \big]}{(\alpha_0+N)^2} \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\prm(\yrm,\xrm | \Drm) & = & \Erm_{\theta | \Drm}\big[ \theta(\yrm,\xrm) | \Drm \big] \\
& = & \frac{\alpha(\yrm,\xrm) + \bar{N}(\yrm,\xrm;\Drm)}{\alpha_0 + N} \nonumber \\
...
& = & \frac{\alpha(\yrm,\xrm) + \sum_{n=1}^N \delta\big( \yrm - \Yrm_n \big) \delta\big( \xrm - \Xrm_n \big)}{\alpha'(\xrm) + \sum_{n=1}^N \delta\big( \xrm - X_n \big)} \nonumber \\
& = & \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm)+N'(\xrm;\Drm)}\right) \frac{\alpha(\yrm,\xrm)}{\alpha'(\xrm)} + \left(\frac{N'(\xrm;\Drm)}{\alpha'(\xrm)+N'(\xrm;\Drm)}\right) \frac{\bar{N}(\yrm,\xrm;\Drm)}{N'(\xrm;\Drm)} \nonumber \;,
\end{IEEEeqnarray}
...
\quad \frac{\alpha\big( \Yrm_{n+1},\Xrm_{n+1} \big) + \sum_{n'=1}^N \delta\big( \Yrm_{n+1} - \Yrm_{n'} \big) \delta\big( \Xrm_{n+1} - \Xrm_{n'} \big)}{\alpha_0 + N} \nonumber
\end{IEEEeqnarray}
and thus
\begin{IEEEeqnarray}{rCl}
\prm(\Drm) & = & \prm\big (\Drm_1 \big) \prod_{n=2}^N \prm\big( \Drm_n | \Drm(n-1),\ldots,\Drm_1 \big) \\
...
\begin{IEEEeqnarray}{L}
\prm\big( \Yrm_1,\ldots,\Yrm(N-1) | \Xrm \big) \\
= \int_{\Ycal} \frac{\alpha\big( \Yrm_1,\Xrm_1 \big)}{\alpha'\big( \Xrm_1 \big)} \nonumber \\
\quad \prod_{n=2}^N \frac{\alpha
\big( \Yrm_n,\Xrm_n \big) + \sum_{i=1}^{n-1} \delta\big( \Yrm_n-\Yrm_i \big) \delta\big( \Xrm_n-\Xrm_i \big)}{\alpha'\big( \Xrm_n \big) + \sum_{i=1}^{n-1} \delta\big( \Xrm_n-\Xrm_i \big)} \mathrm{d}\Yrm(N) \nonumber \\
...
\end{IEEEeqnarray}
which is independent of $\Xrm_n$. Repeated integrations and an application of the permutation invariance principle produce the first and second order conditional distributions
\begin{IEEEeqnarray}{rCl}
\Prm_{\Yrm_n | \Xrm_n} (y | x) & = & \frac{\alpha(y,x)}{\alpha'(x)}
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{L}
...
\begin{equation}
\Erm\big[ \Yrm_n | \Xrm \big] = \mu_{\yrm|\xrm}\big( \Xrm_n \big) \;,
\end{equation}
\begin{equation}
\Erm\big[ \Yrm_n^2 | \Xrm \big] = \Erm\big[ \yrm^2 | \xrm \big] \big( \Xrm_n \big) \;,
\end{equation}
and 
\begin{IEEEeqnarray}{rCl}
\Erm\big[ \Yrm_n \Yrm_{n'} | \Xrm \big] & = & \frac{\alpha'\big( \Xrm_n \big) \mu_{\yrm|\xrm}\big( \Xrm_n ) \mu_{\yrm|\xrm}\big (\Xrm_{n'} \big) + \Erm\big[ \yrm^2 | \xrm \big] \big( \Xrm_n \big) \delta\big( \Xrm_n - \Xrm_{n'} \big)}{\alpha'\big( \Xrm_n \big) + \delta\big( \Xrm_n - \Xrm_{n'} \big)} \;.
\end{IEEEeqnarray}
...
Observe that by the aggregation principle, $\nrm'(x) = \int_{\Ycal} \bar{\nrm}(y,x) \mathrm{d}y \equiv \sum_{n=1}^N \delta\big( x-\Xrm_n \big)$ is a DMP over the set $\Xcal$ with parametrizing function $\alpha' : \Xcal \mapsto \mathbb{R}_{>0}$.
...
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\Drm} \left[ \big( \Erm_{\yrm | \xrm,\Drm}[\yrm] \big)^2 \right] \\
...
\quad = \int_{\Xcal} \Erm_{\Xrm} \left[ \frac{ \Erm_{\Yrm | \Xrm} \left[ \left( \alpha'(x) \mu_{\yrm | \yrm}(x) + \sum_{n=1}^N \Yrm_n \delta\big( x - \Xrm_n \big) \right)^2 \right] }{(\alpha_0+N) \left(\alpha'(x) + \sum_{n=1}^N \delta\big( x - \Xrm_n \big) \right)} \right] \mathrm{d}x \nonumber 
\end{IEEEeqnarray}
...
\quad + \sum_{n=1}^N \Erm\big[ \yrm^2 | \xrm \big]\big( \Xrm_n \big) \delta\big( x - \Xrm_n \big)^2 \nonumber \\
\quad + \sum_{n \neq n'} \frac{\alpha'\big( \Xrm_n \big) \mu_{\yrm | \xrm}\big(\Xrm_n \big) \alpha'\big( \Xrm_{n'} \big) \mu_{\yrm | \xrm}\big( \Xrm_{n'} \big) + \alpha'\big( \Xrm_n \big) \Erm\big[ \yrm^2 | \xrm \big]\big( \Xrm_n \big) \delta\big( \Xrm_n-\Xrm_{n'} \big)}{\alpha'\big( \Xrm_n \big) \alpha'\big( \Xrm_{n'} \big) + \alpha'\big( \Xrm_n \big) \delta\big( \Xrm_n-\Xrm_{n'} \big)} \nonumber \\
\qquad \delta\big( x - \Xrm_n \big) \delta\big( x - \Xrm_{n'} \big) \nonumber \\
\ldots \nonumber \\
= \alpha'(x)^2 \mu_{\yrm | \xrm}(x)^2 + 2\alpha'(x) \mu_{\yrm | \xrm}(x)^2 \sum_{n=1}^N \delta\big( x - \Xrm_n \big) + \Erm\big[ \yrm^2 | \xrm \big](x) \sum_{n=1}^N \delta\big( x - \Xrm_n \big)^2 \nonumber \\
\quad + \frac{\alpha'(x) \mu_{\yrm | \xrm}(x)^2 + \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0)}{\alpha'(x) + \delta(0)} \nonumber \\
\qquad \sum_{n \neq n'} \delta\big( x - \Xrm_n \big) \delta\big( x - \Xrm_{n'} \big) \nonumber \\
\ldots \nonumber \\
= \frac{\alpha'(x) + \sum_{n=1}^N \delta\big( x-\Xrm_n \big)}{\alpha'(x) + \delta(0)} \nonumber \\
\quad \left( \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) \sum_{n=1}^N \delta\big( x-\Xrm_n \big) + \alpha'(x) \mu_{\yrm | \xrm}(x)^2 \left( \alpha'(x) + \delta(0) + \sum_{n=1}^N \delta\big( x-\Xrm_n \big) \right) \right) \nonumber
\end{IEEEeqnarray}
...
Plugging,
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\Drm} \Big[ \big( \Erm_{\yrm | \xrm,\Drm}[\yrm] \big)^2 \Big] \\
...
\quad = \int_{\Xcal} \frac{ \Erm_{\Xrm} \left[ \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) \sum_{n=1}^N \delta\big( x-\Xrm_n \big) + \alpha'(x) \mu_{\yrm | \xrm}(x)^2 \left( \alpha'(x) + \delta(0) + \sum_{n=1}^N \delta\big( x-\Xrm_n \right) \big) \right] }{(\alpha_0+N) \big( \alpha'(x) + \delta(0) \big)} \mathrm{d}x \nonumber 
\end{IEEEeqnarray}

Evaluating the expectation over $\Xrm$,
\begin{IEEEeqnarray}{L}
\Erm_{\Xrm} \left[ \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) \sum_{n=1}^N \delta\big( x-\Xrm_n \big) + \alpha'(x) \mu_{\yrm | \xrm}(x)^2 \left( \alpha'(x) + \delta(0) + \sum_{n=1}^N \delta\big( x-\Xrm_n \big) \right) \right] \nonumber \\
\quad = \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) N \frac{\alpha'(x)}{\alpha_0} + \alpha'(x) \mu_{\yrm | \xrm}(x)^2 \left( \alpha'(x) + \delta(0) + N \frac{\alpha'(x)}{\alpha_0} \right) \nonumber \\
\quad = \frac{\alpha'(x)}{\alpha_0} \Big( \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) N + \mu_{\yrm | \xrm}(x)^2 \big( \alpha_0 \alpha'(x) + \alpha_0 \delta(0) + N \alpha'(x) \big) \Big) \nonumber
\end{IEEEeqnarray}

Plugging,
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\Drm} \Big[ \big( \Erm_{\yrm | \xrm,\Drm}[\yrm] \big)^2 \Big] \\
\quad = \Erm_{\xrm} \frac{\Erm\big[ \yrm^2 | \xrm \big] \delta(0) N + \mu_{\yrm | \xrm}^2 \big( \alpha_0 \alpha'(\xrm) + \alpha_0 \delta(0) + N \alpha'(\xrm) \big)}{(\alpha_0+N) \big( \alpha'(\xrm) + \delta(0) \big)} \nonumber
\end{IEEEeqnarray}

Combining with the second moment produces the risk,
\begin{IEEEeqnarray}{L}
\Rcal(f^*) = \Erm_{\xrm,\Drm} \left[ \Erm_{\yrm | \xrm,\Drm}\big[ \yrm^2 \big] - \big( \Erm_{\yrm | \xrm,\Drm}[\yrm] \big)^2 \right] \\
...
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\bar{\nrm}} \Big[ \big( \Erm_{\yrm | \xrm,\bar{\nrm}}[\yrm] \big)^2 \Big] \\
\quad = \int_{\Xcal} \int_{\bar{\Ncal}} \prm_{\xrm,\bar{\nrm}}(x,\bar{n}) \left( \int_{\Ycal} y \prm_{\yrm | \xrm,\bar{\nrm}}(y | x,\bar{n}) \mathrm{d}y \right)^2 \mathrm{d}\bar{n} \mathrm{d}x \nonumber \\
...
\quad = \int_{\Xcal} \frac{\mu_{\yrm | \xrm}(x)^2 \alpha'(x) \big( \alpha'(x) + N \alpha_0^{-1} \alpha'(x) + \delta(0) \big) + \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) N \alpha_0^{-1} \alpha'(x)}{(\alpha_0+N) \big( \alpha'(x)+\delta(0) \big)} \mathrm{d}x \nonumber \\ 
\quad = \int_{\Xcal} \frac{\alpha'(x)}{\alpha_0} \frac{\mu_{\yrm | \xrm}(x)^2 \big( \alpha_0 \alpha'(x) + N \alpha'(x) + \delta(0) \alpha_0 \big) + \Erm\big[ \yrm^2 | \xrm \big](x) \delta(0) N}{(\alpha_0+N) \big( \alpha'(x)+\delta(0) \big)} \mathrm{d}x \nonumber \\
\quad = \Erm_{\xrm} \left[ \frac{\mu_{\yrm | \xrm}^2 \big( \alpha_0 \alpha'(\xrm) + N \alpha'(\xrm) + \delta(0) \alpha_0 \big) + \Erm\big[ \yrm^2 | \xrm \big] \delta(0) N}{(\alpha_0+N) \big( \alpha'(\xrm)+\delta(0) \big)} \right] \nonumber
\end{IEEEeqnarray}
...
A defining characteristic of a Multinomial random function is that its aggregations are also Multinomial. Consider a random function $\bar{\nrm} \sim \mathrm{Mult}(N,\theta)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,S(z),\ldots \right\}$, $z \in \Zcal$; the transformed random process $\nrm'(z) \equiv \sum_{y \in S(z)} \bar{\nrm}(y)$ is neccessarily Multinomial with sum $N$ and parameterizing function $\theta'(z) = \sum_{y \in S(z)} \theta(y)$.

To demonstrate this, observe
\begin{IEEEeqnarray}{rCl}
\Prm_{\bm{\mathrm{n}}'}(\bm{n}') & = & \ldots \sum_{\sum_{y \in S(z)} \bar{n}(y) = n'(z)} \ldots \Prm_{\nbarrm}(\bm{\bar{n}}) \\
& = & \Mcal(\bm{n}') \prod_{z \in \Zcal} \sum_{\sum_{y \in S(z)} \bar{n}(y) = n'(z)} \Mcal\big( \{ \bar{n}(y): y \in S(z) \} \big) \prod_{y \in S(z)} \theta(y)^{\bar{n}(y)} \nonumber \\
& = & \Mcal(\bm{n}') \prod_{z \in \Zcal} \theta'(z)^{n'(z)} \nonumber \;,
\end{IEEEeqnarray}
where the multinomial theorem has been used.

PGR: multinomial theorem reference?
...
If the multinomial random function $\bar{\nrm}$ is conditioned on its aggregation over the partition $\left\{ \ldots,S(z),\ldots \right\}$, $z \in \Zcal$, the disjoint segements $\bar{\nrm}(y)$, $y \in S(z)$ become independent multinomial random functions,
\begin{IEEEeqnarray}{rCl}
\Prm(\bar{\nrm} | \nrm') & = & \frac{\Prm(\bar{\nrm}) }{\Prm(\nrm')} \Prm(\nrm' | \bar{\nrm}) \\ 
& = & \frac{\Mcal(\bar{\nrm}) \prod_{y \in \Ycal} \theta(y)^{\bar{\nrm}(y)}}{\Mcal(\nrm') \prod_{z \in \Zcal} \theta'(z)^{n'(z)}} \nonumber \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal\Big( \big\{ \bar{\nrm}(y) : y \in S(z) \big\} \Big) \prod_{y \in S(z)} \frac{\theta(y)}{\theta'(z)}^{\bar{\nrm}(y)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \mathrm{Mult}\Big( \big\{ \bar{\nrm}(y) : y \in S(z) \big\} ; \nrm'(z) , \big\{ \theta(y)/\theta'(z) : y \in S(z) \big\} \Big) \nonumber \\
&& \quad \forall \bar{\nrm} \in \left\{ \mathbb{N}^{\Ycal} : \sum_{y \in S(z)} \bar{n}(y) = n'(z), \quad \forall z \in \Zcal \right\} \;.
\end{IEEEeqnarray}
Observe that the segment over the set $S(z)$ sums to $\nrm'(z)$ and is parameterized by the normalized segment of $\theta$.
...
Observe that the partitioned segements are conditionally independent; introduce the subscript notation $\theta_z$ to refer to the function segment $\theta_z = \big\{ \theta(y): y \in S(z) \big\}$. The PDF $\prm(\theta | \theta')$ can now be decomposed as $\prm(\ldots,\theta_z,\ldots | \theta') = \prod_{z \in \Zcal} \prm\big( \theta_z | \theta'(z) \big)$
...
\begin{IEEEeqnarray}{rCl}
\prm\left( \tilde{\theta} | \phi \right) & = & \prod_{z \in \Zcal} \Bigg[ \frac{\prod_{y \in S(z)} \tilde{\theta}_z(y)^{\alpha(y)-1}}{\beta\Big( \big\{ \alpha(y) : y \in S(z) \big\} \Big)} \Bigg] \\
&& \qquad \forall \tilde{\theta} \in \left\{ {\mathbb{R}^+}^{\Ycal} : \sum_{y \in S(z)} \tilde{\theta}_z(y) = 1, \quad \forall z \in \Zcal \right\} \nonumber \\
& = & \prod_{z \in \Zcal} \mathrm{Dir}\Big( \tilde{\theta}_z ; \big\{ \alpha(y) : y \in S(z) \big\} \Big) \nonumber \;.
\end{IEEEeqnarray}
...
A defining characteristic of a Dirichlet-Multinomial random function is that its aggregations are also Dirichlet-Multinomial. Consider a DM random function $\bar{\nrm} \sim \mathrm{DM}(N,\alpha)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,S(z),\ldots \right\}$, $z \in \Zcal$; the transformed random process $\nrm'(z) \equiv \sum_{y \in S(z)} \bar{\nrm}(y)$ is neccessarily Dirichlet-Multinomial with parameterizing function $\alpha'(z) = \sum_{y \in S(z)} \alpha(y)$.
...
& = & \prod_{z \in \Zcal} \mathrm{DirMult}\Big( \big\{ \bar{\nrm}(y) : y \in S(z) \big\} ; \nrm'(z), \big\{ \alpha(y) : y \in S(z) \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
where $\Mcal$ is the multinomial coefficient operator defined earlier. 
...
\begin{IEEEeqnarray}{rCl}
\theta' | \Drm & \sim & \mathrm{Dir}\big( \alpha' + \bar{N}(\cdot;\Drm) \big) \;,
\end{IEEEeqnarray}
...
= \prod_{z \in \Zcal} \Erm_{\theta_z | \theta'(z)}\left[ \prod_{n=1}^N \theta_z\big( \Drm_n \big)^{\chi\big( \Drm_n;S(z) \big)} \right] \nonumber \\
= \left( \prod_{z \in \Zcal} \prod_{n=1}^N \theta'(z)^{\chi\big( \Drm_n;S(z) \big)} \right) \prod_{z \in \Zcal} \Erm_{\tilde{\theta}_z}\left[ \prod_{n=1}^N \tilde{\theta}_z\big( \Drm_n \big)^{\chi\big( \Drm_n;S(z) \big)} \right] \nonumber \\
= \left( \prod_{z \in \Zcal} \theta'(z)^{\bar{N}(z;\Drm)} \right) \prod_{z \in \Zcal} \Erm_{\tilde{\theta}_z}\left[ \prod_{n=1}^N \tilde{\theta}_z\big( \Drm_n \big)^{\chi\big( \Drm_n;S(z) \big)} \right] \nonumber \;,
\end{IEEEeqnarray}
Observe that the dependency on $\theta'$ is polynomial. The training data marginal distribution is
\begin{IEEEeqnarray}{rCl}
\prm(\Drm) & = & \Erm_{\theta'} \left[ \prod_{z \in \Zcal} \theta'(z)^{\bar{N}(z;\Drm)} \right] \prod_{z \in \Zcal} \Erm_{\tilde{\theta}_z}\left[ \prod_{n=1}^N \tilde{\theta}_z\big( \Drm_n \big)^{\chi\big( \Drm_n;S(z) \big)} \right] \\
& = & \frac{\beta\big( \alpha' + \bar{N}(\cdot;\Drm) \big)}{\beta(\alpha')} \prod_{z \in \Zcal} \Erm_{\tilde{\theta}_z}\left[ \prod_{n=1}^N \tilde{\theta}_z\big( \Drm_n \big)^{\chi\big( \Drm_n;S(z) \big)} \right] \nonumber
\end{IEEEeqnarray}
...
& = & \mathrm{Dir}\left( \theta' ; \alpha' + \bar{N}(\cdot;\Drm) \right) \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Erm\big[ \bar{\nrm}(y) \bar{\nrm}(y') \big] & = & \sum_{n=1}^N \Erm_{\Drm_n}\Big[\delta\big( y-\Drm_n \big) \Big] \\
& = & \sum_{n=1}^N \sum_{n'=1}^N \Erm_{\Drm_n,\Drm(n')}\Big[ \delta\big( y-\Drm_n \big) \delta\big( y-\Drm(n') \big) \Big] \nonumber \\
& = & \sum_n \Erm_{\Drm_n}\Big[ \delta\big( y-\Drm_n \big) \delta\big( y'-\Drm_n \big) \Big] + \nonumber \\
&& \quad \sum_{n \neq n'} \Erm_{\Drm_n,\Drm(n')}\Big[ \delta\big( y-\Drm_n \big) \delta\big( y'-\Drm(n') \big) \Big] \nonumber \\
...
%To simplify the proof, we will simply find the maximum over a different set $\Theta' = \left\{ \theta \in \mathbb{R}^M: \sum_{m=1}^{M} \theta_m = 1 \right\} \supset \Theta$ and show that the maximimizing value over $\Theta'$ is a member of $\Theta$.
%
...
%\end{equation}
%
%The single stationary point $\theta^* \in {\mathbb{R}^+}^M$ and is thus a member of $\Theta$ as well. To complete the proof, we must examine the second-order derivatives to confirm that the stationary point is the global maximum. To this effect, we use a bordered Hessian matrix,
%
%\begin{equation}
