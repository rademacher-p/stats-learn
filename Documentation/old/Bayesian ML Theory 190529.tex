...
\end{IEEEeqnarray}
where $H$ is the discrete Heaviside step function. For $n < 0$, the cardinality is equivalent to that of the entire set $\bar{\Ncal}$. 
...
\end{equation}
where $g$ is a polynomial function of degree less than $M$ \cite{graham}, it can be shown that for $N = 0$, the minimum risk is $\Rcal^*  = 1 - |\Ycal|^{-1}$. 

PGR: use ruiz citation for identity?

PGR: find irreducible risk explicitly?
...
& = & 1 - |\Ycal|^{-1} H_{|\Ycal|} \nonumber \;.
\end{IEEEeqnarray}
The above formulation has made use of the alternating summation identity from \cite{roman} to display the risk with a form including the $|\
\Ycal|^\mathrm{th}$ harmonic number $H_{|\Ycal|}$. Observe that the irreducible risk does not depend on the cardinality $|\Xcal|$.
...
Figure \label{fig:Risk_01_uni_N_leg_Mx} displays the minimum 0-1 risk as a function of the training data volume $N$; the cardinality $|\Ycal| = 2$ is fixed and multiple series are provided for different cardinalities $|\Xcal|$. Note that risk increases for larger $|\Xcal|$ - this is intuitive, as more training data is required to approximate the additional conditional empirical distributions $\bar{N}(\cdot,x;D) / N'(x;D)$ with the same level of accuracy. 
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_01_IO_N.pdf}
%\caption{Optimal 0-1 Risk vs training set size for various input set sizes}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_01_IO_N-Mx.pdf}
%\caption{Optimal 0-1 Risk vs training set size for various input set sizes}
...
Before refining this expression, observe how the minimum risk changes as $M$ increases for fixed training set size $N$. Clearly, $\lim_{M \to \infty} \Rcal^* = 1 - M^{-1}$. This should be intuitively satisfying -- the posterior PMF tends toward a uniform distribution, the same as if no data had been observed at all (e.g. $\Prm(\yrm) = M^{-1}$).
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_01_vsN.pdf}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_01_LB.pdf}
