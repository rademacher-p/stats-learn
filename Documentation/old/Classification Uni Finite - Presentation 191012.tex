...
\end{itemize}
%\item $|\bar{\Ncal}| = \binom{N+|\Ycal||\Xcal|-1}{|\Ycal||\Xcal|-1} \leq |\Dcal|$  \\$\quad \Rightarrow \quad$ \underline{Efficient Transform} 
\end{itemize}
...
\large
\framebox[\framewidth]{
%\fcolorbox{NRL_yellow}{white}{
%\colorbox{NRL_navy}{\color{NRL_yellow}
...
%\begin{IEEEeqnarray*}{rCl}
%\prm_{\uptheta}(\theta) & = & \Dir\big( \theta ; \alpha \big) \Big|_{\alpha(\cdot,\cdot) = 1} \\
%& = & \bigg[ \beta(\alpha)^{-1} \prod_{y \in \Ycal} \prod_{x \in \Xcal} \theta(y,x)^{\alpha(y,x) - 1} \bigg]_{\alpha(\cdot,\cdot) = 1} \\
%& = & \big( |\Ycal||\Xcal|-1 \big)!  
%\end{IEEEeqnarray*}
%
%\end{column}
...
\begin{itemize}
\item[\Large$*$] Posterior for model $\tilde{\uptheta}(x)$ dependent solely on the sufficient statistic elements $\nbarrm(\cdot,x)$
\end{itemize}
...
\centering
\large
\textbf{Asymptotically consistent estimation of $\theta$ due to full support of prior}
