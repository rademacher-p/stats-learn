...
Note that $\prm_{\tilde{\uptheta} | \Xrm}(X) = \prm_{\tilde{\uptheta} | \nrm'}\big( N'(X) \big) = \Erm_{\uptheta' | \Xrm}\big[ \prm_{\tilde{\uptheta} | \uptheta'} \big](X)$. Also, note that $\Prm_{\Yrm | \Xrm} = \Erm_{\uptheta' | \Xrm} \Big[ \Erm_{\tilde{\uptheta} | \uptheta'} \big[ \Prm_{\Yrm | \Xrm,\tilde{\uptheta}} \big] \Big]$ and $\Prm_{\nbarrm | \nrm'} = \Erm_{\uptheta' | \nrm'} \Big[ \Erm_{\tilde{\uptheta} | \uptheta'} \big[ \Prm_{\nbarrm | \nrm',\tilde{\uptheta}} \big] \Big]$.
...
\end{IEEEeqnarray}
In terms of the empirical PMF sufficient statistic, the distribution is
\begin{IEEEeqnarray}{rCl}
\Prm_{\xrm | \nbarrm}(\bar{n}) & = & \mu_{\uptheta' | \nbarrm}(\bar{n}) = \mu_{\uptheta' | \nrm'}\left( \sum_y \bar{n}(y,\cdot) \right) \\
& = & \frac{\alpha' + \sum_y \bar{n}(y,\cdot)}{\alpha_0 + N} \nonumber \;.
\end{IEEEeqnarray}

The posterior $\prm_{\tilde{\uptheta} | \xrm,\Drm}$ can be compactly represented by noting that the independence of $\uptheta'$ and $\tilde{\uptheta}$ implies that $\Xrm$ and $\tilde{\uptheta}$ are independent and that $\Prm_{\Yrm | \Xrm,\xrm} = \Erm_{\tilde{\uptheta}}\big[ \Prm_{\Yrm | \Xrm,\tilde{\uptheta}} \big] = \Prm_{\Yrm | \Xrm}$. As a result, $\tilde{\uptheta}$ is conditionally independent of $\xrm$ given $\Drm$ and the distinct model conditional PMF's are independent from one another, such that
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
Observe that when the conditioning is performed using the sufficient statistic, the independent conditional models $\tilde{\uptheta}(x)$ are only dependent on the empirical PMF subset $\nbarrm(\cdot,x)$.
