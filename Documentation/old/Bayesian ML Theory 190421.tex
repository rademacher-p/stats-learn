...
PGR: check theta font

Consider an observable random element $\xrm \in \Xcal$ and and unobservable random element $\yrm \in \Ycal$ which are jointly distributed according to an unknown probability distribution $\theta \in \Theta = \Pcal(\Ycal \times \Xcal)$, such that $\Prm_{\yrm,\xrm | \uptheta}(y,x | \theta) = \uptheta(y,x)$. Note that the uppercase PMF notation implies that the random elements are discrete; PDF's are used when $\xrm$ and/or $\yrm$ are continuous random variables/processes.

Also observed is a random sequence of $N$ samples from $\uptheta$, denoted $\Drm = ( \Yrm,\Xrm ) \in \Dcal = \{\Ycal \times \Xcal\}^N$. The $N$ data pairs are conditionally independent from one another and are identically distributed as $\Prm_{\Drm_n | \uptheta}(y,x | \theta) = \uptheta(y,x)$. The samples are also conditionally independent from $(\yrm,\xrm)$. Thus,
\begin{equation}
...
\Rcal_{\Theta}(f ; \theta) = \Erm_{\Drm | \uptheta} \bigg[ \Erm_{\yrm,\xrm | \uptheta} \Big[ \mathcal{L}\big( f(\xrm;\Drm),\yrm \big) \Big] \bigg] \;.
\end{equation}
...
PGR: roman theta in risk argument?
...
Define the PDF $\prm(\uptheta): \Theta \mapsto \Rbb_{\geq 0}$. Now using Bayes rule, the risk can be formulated as
...
\end{IEEEeqnarray}
which is defined for $\theta \in \left\{ {\Rbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \theta(y,x) = \theta'(x), \quad \forall x \in \Xcal \right\}$. More interestingly, define $\tilde{\uptheta}(y;x) \equiv \uptheta(y,x) / \uptheta'(x)$ - it can be shown that the transformed random process conditioned on the aggregation is characterized by
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
a product of Dirichlet distributions defined for $\tilde{\theta} \in \left\{ \tilde{\theta}(\cdot,x) \in \Pcal(\Ycal), \quad \forall x \in \Xcal \right\}$. As shown, the scaled functions $\tilde{\uptheta}(\cdot;x)$ are Dirichlet with parameterizing functions $\alpha(\cdot,x)$, independent of one another, and independent of the aggregation $\uptheta'$.
...
Next, the conditional distribution $\Prm(\Drm | \uptheta)$ will be used top determine the marginal PMF, $\Prm(\Drm)$; properties of both distributions will be discussed. 
...
Note that both $\Prm(\Drm | \uptheta)$ and $\Prm(\Drm)$ depend on the training data $\Drm$ only through the transform $\bar{N}$; consequently, $\bar{N}(\Drm)$ will be a sufficient statistic for any distributions conditioned on $\Drm$. As such, it is useful to define a new random process $\nbarrm \equiv \bar{N}(\Drm) \in \bar{\Ncal}$. 
...
Next the joint PMF of $\yrm$ and $\xrm$ conditioned on the training data is expressed as \cite{murphy}
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
This is a mixture distribution between the prior expectation $\Erm[\uptheta] = \alpha/\alpha_0$ and the empirical distribution $\bar{N}(\Drm)/N$. The more subjective the model prior (i.e. larger $\alpha_0$), the more the prior mean is favored; the more data, the more the empirical PMF is favored.

It is informative to compare the PMF $\Prm(\yrm,\xrm | \uptheta)$ with its ``estimate'' $\Prm(\yrm,\xrm | \Drm)$ and investigate the fit provided by different model priors. Consider a fixed model $\uptheta$ and user-selected prior PDF parameters $\alpha$. The expected value of the data-conditioned PMF is
\begin{IEEEeqnarray}{rCl}
\Erm_{\Drm | \uptheta}\big[ \Prm_{\yrm,\xrm | \Drm}(y,x | \Drm) \big] & = & \frac{\alpha(y,x) + \Erm_{\bar{\nrm} | \uptheta}\big[ \bar{\nrm}(y,x) \big]}{\alpha_0+N} \\
& = & \left(\frac{\alpha_0}{\alpha_0 + N}\right) \frac{\alpha(y,x)}{\alpha_0} + \left(\frac{N}{\alpha_0 + N}\right) \uptheta(y,x) \nonumber \;,
\end{IEEEeqnarray}
a mixture of the prior expectation $\Erm[\uptheta] = \alpha/\alpha_0$ and the model $\uptheta$ itself; observe how as the volume of training data increases relative to the model prior concentration, the expectation trends away from the data-independent estimate and toward the true model. The expected bias of the model estimate $\Prm(\yrm,\xrm | \Drm)$ is
\begin{IEEEeqnarray}{rCl}
\mathrm{Bias}(y,x ; \alpha,\uptheta) & = & \Erm_{\Drm | \uptheta} \big[ \Prm_{\yrm,\xrm | \Drm}(y,x | \Drm) - \Prm_{\yrm,\xrm | \uptheta}(y,x | \uptheta) \big] \\
& = & \frac{\alpha_0}{\alpha_0+N} \left( \frac{\alpha(y,x)}{\alpha_0} - \uptheta(y,x) \right) \nonumber \;,
\end{IEEEeqnarray}
proportionate to the difference between $\uptheta$ and the data-independent expectation $\Erm[\uptheta]$. The scaling factor depends on the prior concentration and the number of training samples. For a highly subjective prior ($\alpha_0/N \to \infty$), the estimate becomes nearly independent of $\Drm$ and the scaling factor is unity. For an ``uncertain'' prior with low $\alpha_0$, the empirical distribution is used to estimate $\uptheta$ and the expected bias trends to zero.

The conditional variance of the estimate $\Prm(\yrm,\xrm | \Drm)$ is
\begin{IEEEeqnarray}{L}
\Crm_{\Drm | \uptheta} \big[\Prm_{\yrm,\xrm | \Drm}(\cdot,\cdot | \Drm) \big](y,x,y',x') \\
\quad = (\alpha_0+N)^{-2} \Sigma_{\nbarrm | \uptheta}(y,x,y',x') \nonumber \\
\quad = \frac{N}{(\alpha_0+N)^2} \big( \uptheta(y,x) \delta[y,y'] \delta[x,x'] - \uptheta(y,x) \uptheta(y',x') \big) \nonumber \;,
\end{IEEEeqnarray}
proportionate to the conditional variance of $\nbarrm$ given $\uptheta$. The scaling factor trends to zero with both increasing $\alpha_0$ and $N$.

Finally, consider the expected squared difference between the estimate and the true model,
\begin{IEEEeqnarray}{L}
\mathcal{E}(y,x,y',x';\alpha,\uptheta) \\
\quad = \Erm_{\Drm | \uptheta} \Big[ \big( \Prm_{\yrm,\xrm | \Drm}(y,x | \Drm) - \Prm_{\yrm,\xrm | \uptheta}(y,x | \uptheta) \big) \big( \Prm_{\yrm,\xrm | \Drm}(y',x ' | \Drm) - \Prm_{\yrm,\xrm | \uptheta}(y',x' | \uptheta) \big) \Big] \nonumber \\
\quad = \mathrm{Bias}(y,x;\alpha,\uptheta) \mathrm{Bias}(y',x';\alpha,\uptheta) + \Crm_{\Drm | \uptheta} \big[\Prm_{\yrm,\xrm | \Drm}(\cdot,\cdot | \Drm) \big](y,x,y',x') \nonumber \;,
\end{IEEEeqnarray}
which combines both the bias of the model estimate $\Prm(\yrm,\xrm | \Drm)$ and its variance. As $N \to \infty$, this function trends to zero and thus the underlying model $\uptheta$ is determined precisely. A more realistic and interesting case is estimation with a finite volume of training data. Specification of the Dirichlet model prior can be interpreted as providing a distribution estimate $\alpha/\alpha_0$ and a confidence level $\alpha_0$. Higher confidence reduces error due to the variance of the estimator, but increases the error due to bias between $\alpha/\alpha_0$ and $\uptheta$. Low confidence renders the estimate unbiased, but maximizes the estimator variance.

To exemplify how the model estimate $\Prm(\yrm,\xrm | \Drm)$ approximates $\uptheta$, consider a scenario with $|\Ycal| = 10$ and $|\Xcal| = 1$; as in previous sections, $\xrm$ is essentially treated as deterministic to allow simpler graphics. The data-independent PMF $\alpha / \alpha_0$ and true model $\uptheta$ are shown in Figure \ref{fig:P_yx_error_N_0} - note the significant mismatch. 

Figures \ref{fig:P_yx_error_a0_0_1} and \ref{fig:P_yx_error_a0_10} show how the expected value and variance of the estimate (represented by the blue markers and error bars, respectively) change for different values of $N$ and $\alpha_0$ . Each individual plot heading provides the error $\sqrt{\sum_{y \in \Ycal} \sum_{x \in \Xcal} \mathcal{E}(y,x,y,x;\alpha,\uptheta)}$ to assess the quality of the PMF estimate. Observe that for $N=1$, the high variance of the $\alpha_0 = 0.1$ estimate (favoring the empirical PMF) renders it worse than the $\alpha_0 = 10$ estimate; in fact, the variance is so high that the error exceeds that of the data-independent estimate $\alpha / \alpha_0$ (Figure \ref{fig:P_yx_error_N_0}). Conversely, for $N=10$, the confidence of the $\alpha_0 = 10$ estimate leads to high bias and the $\alpha_0 = 0.1$ estimate is superior.
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{P_yx_error_a0_0_1.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{P_yx_error_a0_10.pdf}
...
It is also informative to compare the unknown conditional PMF $\Prm(\yrm | \xrm,\uptheta)$ with the estimated distribution $\Prm(\yrm | \xrm,\Drm)$. The expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
\Erm_{\Drm | \uptheta}\big[ \Prm_{\yrm | \xrm,\Drm}(y | x,\Drm) \big] \\ 
\quad = \Erm_{\nrm' | \uptheta}\left[\frac{\alpha'(x)}{\alpha'(x) + \nrm'(x)}\right] \frac{\alpha(y,x)}{\alpha'(x)} + \Erm_{\nrm' | \uptheta}\left[\frac{\nrm'(x)}{\alpha'(x) + \nrm'(x)}\right] \frac{\uptheta(y,x)}{\uptheta'(x)} \nonumber \;,
\end{IEEEeqnarray}
where the properties of a multinomial distribution conditioned on its aggregation have been used. The result is a convex combination of the conditional data-independent distribution $\alpha(\cdot,x) / \alpha'(x)$ and the true conditional distribution $\uptheta(\cdot,x) / \uptheta'(x)$. The convex coefficients are expectations dependent on the ``marginal'' values $\alpha'$ and $\nrm'$; note that as $\alpha'(x)$ trends toward zero and infinity, the arguments of the expectations trend toward zero or one. The bias between the true and estimated conditional PMF's is thus
\begin{IEEEeqnarray}{rCl}
\mathrm{Bias}(y;x,\alpha,\theta) & = & \Erm_{\Drm | \uptheta}\big[ \Prm_{\yrm | \xrm,\Drm}(y | x,\Drm) - \Prm_{\yrm | \xrm,\uptheta}(y | x,\uptheta) \big]\\
& = & \Erm_{\nrm' | \uptheta}\left[\frac{\alpha'(x)}{\alpha'(x) + \nrm'(x)}\right] \left( \frac{\alpha(y,x)}{\alpha'(x)} - \frac{\uptheta(y,x)}{\uptheta'(x)} \right) \nonumber \;.
\end{IEEEeqnarray}
The conditional variance of the estimate is 
\begin{IEEEeqnarray}{L}
\Crm_{\Drm | \uptheta} \big[\Prm_{\yrm | \xrm,\Drm}(\cdot | x,\Drm) \big](y,y') \\
\quad = \Erm_{\nrm' | \uptheta}\left[ \frac{\Sigma_{\nbarrm(\cdot,x) | \nrm'(x),\uptheta}(y,y')}{\big( \alpha'(x) + \nrm'(x) \big)^2} \right] \nonumber \\
\quad = \Erm_{\nrm' | \uptheta}\left[ \frac{\nrm'(x)}{\big( \alpha'(x) + \nrm'(x) \big)^2} \right] \left( \frac{\uptheta(y,x)}{\uptheta'(x)} \delta[y,y'] - \frac{\uptheta(y,x)}{\uptheta'(x)} \frac{\uptheta(y',x)}{\uptheta'(x)} \right) \nonumber \;.
\end{IEEEeqnarray}

The expected squared difference between the estimate and the true conditional model is
\begin{IEEEeqnarray}{L}
\mathcal{E}(y,y';x,\alpha,\uptheta) \\
\quad = \Erm_{\Drm | \uptheta} \Big[ \big( \Prm_{\yrm | \xrm,\Drm}(y | x,\Drm) - \Prm_{\yrm | \xrm,\uptheta}(y | x,\uptheta) \big) \big( \Prm_{\yrm | \xrm,\Drm}(y' | x,\Drm) - \Prm_{\yrm | \xrm,\uptheta}(y' | x,\uptheta) \big) \Big] \nonumber \\
\quad = \mathrm{Bias}(y;x,\alpha,\uptheta) \mathrm{Bias}(y';x,\alpha,\uptheta) + \Crm_{\Drm | \uptheta} \big[\Prm_{\yrm | \xrm,\Drm}(\cdot | x,\Drm) \big](y,y') \nonumber \;.
\end{IEEEeqnarray}
...
& = & \Erm_{\uptheta | \xrm,\Drm} \left[ \frac{\uptheta(\yrm,\xrm)}{\uptheta'(\xrm)} \right] \nonumber \;,
\end{IEEEeqnarray}
Just as $\Prm(\yrm,\xrm | \Drm)$ is an expectation of the model $\uptheta$ posterior conditioned on $\Drm$, $\Prm(\yrm | \xrm,\Drm)$ is an expectation of $\uptheta$ conditioned on both observed random elements. The expectation operates on the random process $\theta(\cdot,\xrm)$ normalized by its sum $\uptheta'(\xrm)$. 
...
To confirm that the expectation produces the same distribution $\Prm(\yrm | \xrm,\Drm)$ displayed previously, use the properties of a Dirichlet random process conditioned on its aggregation to show that $\Erm_{\uptheta}\big[ \uptheta(y,x) / \uptheta'(x) \big] = \alpha(y,x) / \alpha'(x)$. Thus, the PMF is evaulated as
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
a sum of two terms. The first term is independent of the estimator; this irreducible risk is the expected conditional variance when the model $\uptheta$ is known. The second term is the additional risk due to the model uncertainty; it is the expected squared-error between the estimator output and the clairvoyant estimate $\mu_{\yrm | \xrm,\uptheta}$.
...
\end{IEEEeqnarray}
The first term is the expected irreducible risk. The second term is the expected variance of the clairvoyant estimate $\mu_{\yrm | \xrm,\uptheta}$, where the variance is evaluated with respect to the model posterior PDF $\prm(\uptheta | \xrm,\Drm)$
...
In this section, we will apply the developed framework to a common machine learning task: classification. In classification problems, the set $\Ycal$ is countable and typically finite. Furthermore, the hypothesis space  is usually identical to the unobserved variable space, that is $\Hcal = \Ycal$. The 0-1 loss function is the most well known for these problems; it is represented as
...
\begin{IEEEeqnarray}{rCl}
\Rcal_{\Theta}(f ; \theta) & = & 1 -  \Erm_{\Drm | \uptheta} \bigg[ \Erm_{\yrm,\xrm | \uptheta} \Big[ \delta\big[ f(\xrm;\Drm),\yrm \big] \Big] \bigg] \\
& = & 1 -  \sum_{x \in \Xcal} \Erm_{\Drm | \uptheta} \Big[ \theta\big( f(x,\Drm),x \big) \Big] \nonumber \;.
\end{IEEEeqnarray}
Evaluated with a Dirichlet prior, the full risk is
\begin{IEEEeqnarray}{rCl} \label{eq:risk_01}
\Rcal(f) & = & 1 - \Erm_{\uptheta} \left[ \sum_{x \in \Xcal} \Erm_{\Drm | \uptheta} \Big[ \theta\big( f(x,\Drm),x \big) \Big] \right] \\
...
\caption{Optimal 0-1 Risk vs $\mu_{\uptheta}$}
...
Now, however, the argument of expectation is the maximum of a subset of $\bar{N}(\Drm)$, $M_y$ out of $M_xM_y$, rather than the entire random process. As shown in Appendix \ref{app:E_N_bar}, all marginal distributions of $\nbarrm$ have the same form; as such, the expectations will be the same regardless of which $M_y$ variables the maximum operator is applied to. 
...
%\begin{IEEEeqnarray}{rCl}
%\prm(\uptheta | \uptheta') & = & \frac{\prm(\uptheta' | \theta)\prm(\uptheta)}{\prm(\uptheta')} \\
%& = & \frac{\beta(\alpha') \prod_{y \in \Ycal} \uptheta(y)^{\alpha(y)-1}}{\beta(\alpha) \prod_{z \in \Zcal} \uptheta'(z)^{\alpha'(z)-1}} \prod_{z \in \Zcal} \delta\left( \uptheta'(z) - \sum_{y \in \Scal(z)} \uptheta(y) \right) \nonumber \\
%& = & \prod_{z \in \Zcal} \Bigg[ \beta\Big( \big\{ \alpha(y) : y \in \Scal(z) \big\} \Big)^{-1} \frac{\prod_{y \in \Scal(z)} \uptheta(y)^{\alpha(y)-1}}{\uptheta'(z)^{\alpha'(z)-1}} \nonumber \\ 
%&& \qquad \delta\left( \uptheta'(z) - \sum_{y \in \Scal(z)} \uptheta(y) \right) \Bigg] \nonumber \\
%& = & \prod_{z \in \Zcal} \Bigg[ \frac{\uptheta'(z)^{1-|\Scal(z)|}}{\beta\Big( \big\{ \alpha(y) : y \in \Scal(z) \big\} \Big)} \prod_{y \in \Scal(z)} \left(\frac{\uptheta(y)}{\uptheta'(z)}\right)^{\alpha(y)-1} \nonumber \\
%&& \qquad \delta\left( \uptheta'(z) - \sum_{y \in \Scal(z)} \uptheta(y) \right) \Bigg] \nonumber \;.
%\end{IEEEeqnarray}
...
\section{Proof: Euclidean Domain Model Posterior Distribution is Dirichlet Process} \label{app:DP_post}
