...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{P_nbar_uni.pdf}
...
\end{equation}
Calculation of the second derivative confirms that this value is a local minimum. Furthermore, the excess risk for this value is 
\begin{equation}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \Erm_{\nrm'(\xrm) | \uptheta'(\xrm)}\left[ \frac{1}{\nrm'(\xrm)\Sigma_{\yrm | \xrm,\uptheta}^{-1} + \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^{-2}} \right] \;,
\end{equation}
...
PGR: SOLVE for local maxima over N???
