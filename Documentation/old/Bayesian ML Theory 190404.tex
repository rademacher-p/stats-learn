...
\end{IEEEeqnarray}
The cardinality of this set is $|\bar{\Ncal}| = \Mcal\big( \{N,|\Ycal||\Xcal|-1\} \big)$; this can be shown using the stars-and-bars method from \cite{feller}. Note that $|\bar{\Ncal}| \leq |\Dcal|$. Also, observe that the set $\{ \bar{n}/N : \bar{n} \in \bar{\Ncal} \} \subset \Theta$, that is, $\bar{N}(\Drm)/N$ assumes one of a finite number of PDF values. 
...
The first and second joint moments of the multinomial distribution are \cite{theodoridis-ML}
\begin{IEEEeqnarray}{rCl}
...
Again, the data PMF's for minimal and maximal concentration $\alpha_0$ are important. For $\alpha_0 \to \infty$, the model PDF $\prm(\theta)$ concentrates at its mean and thus $\bar{\nrm}$ is characterized by a multinomial distribution,
\begin{IEEEeqnarray}{rCl}
\Prm(\nbarrm) & = & \Mcal(\nbarrm) \prod_{y \in \Ycal} \prod_{x \in \Xcal} \left(\frac{\alpha(y,x)}{\alpha_0}\right)^{\bar{\nrm}(y,x)}
\end{IEEEeqnarray}
and the training data $\Drm$ distribution is
\begin{IEEEeqnarray}{rCl}
\Prm(\Drm) & = & \Erm_{\theta}\left[ \prod_{n=1}^N \theta(\Yrm_n,\Xrm_n) \right] \\
& = & \prod_{n=1}^N \frac{\alpha\big( \Yrm_n,\Xrm_n \big)}{\alpha_0} \nonumber \;.
\end{IEEEeqnarray}
Conversely, for $\alpha_0 \to 0$, the PMF's trend toward
...
\begin{IEEEeqnarray}{rCl}
\Prm(\yrm,\xrm | \Drm) & = & \frac{\Erm_{\theta}\big[ \theta(\yrm,\xrm) \Prm(\Drm | \theta) \big]}{\Prm(\Drm)} = \Erm_{\theta | \Drm}\big[ \theta(\yrm,\xrm)  \big] \\
...
\end{IEEEeqnarray}
where ``marginalized'' functions $\alpha'(x) \equiv \sum_{y \in \Ycal} \alpha(y,x)$ and $N'(x;\Drm) \equiv \sum_{y \in \Ycal} \bar{N}(y,x;\Drm) = \sum_{n=1}^N \delta\big[ x,\Xrm_n \big]$ are introduced. Note that $\Prm(\xrm) = \alpha'(\xrm)/\alpha_0$. 

Additionally, since $\nbarrm \equiv \bar{N}(\Drm) \sim \mathrm{DM}(N,\alpha)$, the aggregation property for Dirichlet-Multinomial random functions dictates that $\nrm' \equiv N'(\Drm) \sim \mathrm{DM}(N,\alpha')$ is Dirichlet-Multinomial over the set $\Xcal$.

PGR: add mult agg? add mult cond of empirical?

PGR: DM aggregation reference
...
Note that for a given model $\theta$, the expected value of the conditional empirical distribution is 
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
where the properties of a multinomial distribution and its aggregation, detailed in Appendix \ref{app:mult}, have been used. In this formula, $\theta'(\xrm) \equiv \sum_{y' \in \Ycal} \theta(y',\xrm)$ is introduced; by the aggregation property, $\theta'$ is a Dirichlet random function over $\Xcal$ parameterized by $\alpha'$.
...
\end{IEEEeqnarray}
where $\bar{N}\Big( \big( (y,x),D \big) \Big) = \bar{N}(D) + \delta[\cdot,y] \delta[\cdot,x]$. This is a mixture distribution whose weighting factors are the values of $\Prm(\yrm | \xrm,\Drm)$ given the observations. The PDF's being combined are Dirichlet distributions with different parameterizing functions $\alpha + \bar{N}\Big( \big( (y',\xrm),\Drm \big) \Big)$; additional emphasis is put on the $|\Ycal|$ different possible pairs $(y,x)$ including the observed value $\xrm$.

This posterior PDF shares the same asymptotic distributions with $\prm(\theta|\Drm)$. As $\alpha_0 \to \infty$ or $N \to \infty$, the $|\Ycal|$ Dirichlet mixture PDF's converge to the same impulsive distribution and thus $\prm(\theta|\xrm,\Drm)$ again trends toward $\alpha / \alpha_0$ and $\bar{N}(\Drm)/N$, respectively.
...
The expectation can be represented as a convex combination of two expected losses. The first expected loss is evaluated with respect to the conditional distribution $\Prm(\yrm|\xrm)$, which reflects the prior knowledge encapsulated in model parameter $\alpha$. The second term is a conditional emprical loss, or the average loss among samples $\Yrm_n$ whose corresponding values $\Xrm_n$ match the observed value $\xrm$. The weighting factors are inherited from the conditional distribution $\Prm(\yrm|\xrm,\Drm)$; thus, for a given observation $\xrm$, the model prior concentration $\alpha'(\xrm)$ and the number of matching training samples $N'(\xrm;\Drm)$ dictate which expectations are emphasized.
...
The estimator is interpreted as a convex combination of two separate estimates - the expected value of $\yrm$ conditioned on the observed $\xrm$ and the mean of the training values $\Yrm_n$ which have a value $\Xrm_n$ matching the observed value $\xrm$. The weighting factors are the same as those of $\Prm(\yrm | \xrm,\Drm)$; thus, stronger prior information (larger $\alpha'(\xrm)$) provides more weight to the estimate $\mu_{\yrm|\xrm}$ and more voluminous training data puts emphasis on the empirical conditional mean.
...
Decompose the conditional variance as
\begin{IEEEeqnarray}{rCl}
...
To illustrate these trends, explicitly define the sets $\Ycal = \{ i/M_{\yrm} : i = 0,\ldots,M_{\yrm}-1 \}$ and $\Xcal = \{ i/M_{\xrm} : i = 0,\ldots,M_{\xrm}-1 \}$; Figures \ref{fig:Risk_SE_Dir_IO_N_leg_a0} and \ref{fig:Risk_SE_Dir_IO_a0_leg_N} display risk for fixed $\Prm(\yrm|\xrm)$ and $\Prm(\xrm)$.
...
\end{IEEEeqnarray}
since, by the aggregation principle, $\theta'(x) = \int_{\Ycal} \theta(y,x) \mathrm{d} y$ is a Dirichlet process with parameter function $\alpha': \Xcal \mapsto \mathbb{R}^+$.
