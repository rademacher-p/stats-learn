...
Next, consider the conditional risk for a learning function without model knowledge. Substituting the squared-error loss into \eqref{eq:risk_cond},
\begin{IEEEeqnarray}{rCl}
...
& = & \Rcal_{\Theta}^*(\uptheta) + \Erm_{\xrm,\Drm | \uptheta} \Big[ \big( f(\xrm;\Drm) - f_{\Theta}(\xrm;\uptheta) \big)^2 \Big] \nonumber \;,
\end{IEEEeqnarray}
a sum of two terms. The first term is the clairvoyant squared-error risk. The second term is the additional risk from the unknown $\uptheta$; it is the expected squared difference between the Bayes estimate and the clairvoyant estimate $\mu_{\yrm | \xrm,\uptheta}$.
...
Consider the conditional risk for a function without knowledge of the model. Applying the 0-1 loss, the conditional risk \eqref{eq:risk_cond} is
\begin{IEEEeqnarray}{rCl}
...
Using a Dirichlet prior, the Bayes risk is
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & \Erm_{\xrm,\Drm} \bigg[ \Erm_{\yrm | \xrm,\Drm} \Big[ \Lcal\big( f^*(\xrm;\Drm),\yrm \big) \Big] \bigg] \\
