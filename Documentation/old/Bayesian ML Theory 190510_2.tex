...
It is informative to formulate the optimal decision function assuming the model $\uptheta$ was in fact observed. This ``clairvoyant'' decision function $f_{\Theta}: \Theta \mapsto \Fcal$ is represented by
\begin{equation}
...
\begin{IEEEeqnarray}{rCl} \label{eq:risk_min_clv}
...
The clairvoyant risk \eqref{eq:risk_min_clv} for a given model satisfies $\Rcal_{\Theta}^*(\theta) \leq \Rcal_{\Theta}(f;\theta) \quad \forall f \in \Fcal, \ \theta \in \Theta$. Consequently, the Bayes risk satisfies $\Erm_{\uptheta} \big[ \Rcal_{\Theta}^*(\uptheta) \big] \leq \Rcal(f)$; the expected value of the clairvoyant risk will thus be referred to as the ``irreducible'' risk. 
