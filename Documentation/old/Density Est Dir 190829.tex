...
In this section, the Bayesian predicitive PMF $\Prm_{\yrm | \xrm,\Drm}$ is provided. As $\bar{N}(\Drm)$ is a sufficient statistic for the training data, $\Prm_{\yrm | \xrm,\nbarrm}$ will be derived first and any distributions dependent on $\Drm$ will be replaced by their corresponding distributions of $\nbarrm$, simplifying the analysis.
 ...
 As mentioned in Section \ref{sec:objective}, the Bayesian predictive PMF can be interpreted as the expectation of the true predictive PMF with respect to the model posterior distribution $\prm_{\uptheta | \xrm,\Drm}$. Using the notation detailed in Section \ref{sec:theta_agg}, the Bayesian predictive PMF can be expressed as $\Prm_{\yrm | \xrm,\Drm} = \Erm_{\uptheta | \xrm,\Drm}\big[ \Prm_{\yrm | \xrm,\uptheta} \big] = \Erm_{\uptheta | \xrm,\Drm}\big[ \tilde{\uptheta}(\xrm) \big] = \mu_{\tilde{\uptheta}(\xrm) | \xrm,\Drm}$, the conditional mean of the predictive model $\tilde{\uptheta}(\xrm)$.

\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\Drm}(\cdot | x,D) & = & \Erm_{\uptheta | \xrm,\Drm}\big[ \Prm_{\yrm | \xrm,\uptheta} \big](x,D) \nonumber \\
& = & \Erm_{\uptheta | \Drm}\big[ \tilde{\uptheta}(x) \big] \nonumber \\
& = & \mu_{\tilde{\uptheta}(x) | \xrm,\Drm}(x,D) = \mu_{\tilde{\uptheta}(x) | \Drm}(D) \;.
\end{IEEEeqnarray}



\begin{IEEEeqnarray}{rCl}
\prm_{\tilde{\uptheta} | \xrm,\Drm}\big( \tilde{\theta} | x,D \big) & = & \prm_{\tilde{\uptheta} | \Drm}\big( \tilde{\theta} | D \big) = \prod_{x \in \Xcal} \prm_{\tilde{\uptheta}(x) | \Drm}\big(\tilde{\theta}(x) | D \big) \\
& = & \prod_{x \in \Xcal} \Dir\big( \tilde{\theta}(x) ; \alpha(\cdot,x) + \bar{N}(\cdot,x;D) \big) \nonumber
\end{IEEEeqnarray}

\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\Drm}(\cdot | x,D) & = & \Erm_{\uptheta | \Drm}\big[ \Prm_{\yrm | \xrm,\uptheta} \big](x,D) \nonumber \\
& = & \Erm_{\uptheta | \Drm}\big[ \tilde{\uptheta}(x) \big] \nonumber \\
& = & \mu_{\tilde{\uptheta}(x) | \xrm,\Drm}(x,D) = \mu_{\tilde{\uptheta}(x) | \Drm}(D) \;.
\end{IEEEeqnarray}
