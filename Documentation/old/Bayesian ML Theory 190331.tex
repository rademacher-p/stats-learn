...
For notational simplicity, probability notations are occasionally represented as $\Prm(\xrm)$; in such instances, the formal notation can be recovered by replacing the distribution with $\Prm_{\xrm}(x)$ and all instances of the roman font $\xrm$ with the italic value $x$. Conditional distributions such as $\Prm(\xrm|\zrm)$ are functions of both $x$ and $z$ - the arguments are notated as $\Prm(\xrm|\zrm)(x|z)$ to separate the unobserved random values from the conditional ones.
...
Define the PDF $\prm(\theta): \Theta \mapsto \mathbb{R}_{\geq 0}$. Now using Bayes rule, the risk can be formulated as
...
and
\begin{IEEEeqnarray}{rCl}
\Erm_{\theta}\big[ \theta(y,x) \theta(y',x') \big] & = & \frac{\alpha(y,x) \alpha(y',x') + \alpha(y,x) \delta[y,y'] \delta[x,x']}{\alpha_0 (\alpha_0+1)} \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\nbarrm | \theta}(y,x,y',x' | \theta)  & = & N \big( \theta(y,x) \delta[y,y'] \delta[x,x'] - \theta(y,x) \theta(y',x') \big) \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
where, by the aggregation principle, $\theta'(x) = \int_{\Ycal} \theta(y,x) \mathrm{d} y$ is a Dirichlet process with parameter function $\alpha': \Xcal \mapsto \mathbb{R}^+$.
...
and 
\begin{IEEEeqnarray}{L}
\Erm\big[ \Yrm_n \Yrm_{n'} | \Xrm \big] \\
\quad = \frac{\alpha'\big( \Xrm_n \big) \mu_{\yrm|\xrm}\big( \Xrm_n ) \mu_{\yrm|\xrm}\big (\Xrm_{n'} \big) + \Erm\big[ \yrm^2 | \xrm \big] \big( \Xrm_n \big) \delta\big( \Xrm_n - \Xrm_{n'} \big)}{\alpha'\big( \Xrm_n \big) + \delta\big( \Xrm_n - \Xrm_{n'} \big)} \nonumber \;.
\end{IEEEeqnarray}
...
Let the process $\theta \in \Theta = \left\{ {\mathbb{R}_{\geq 0}}^{\Ycal} : \sum_{y \in \Ycal} \theta(y) = 1 \right\}$ be Dirichlet with parameterizing function $\alpha \in {\mathbb{R}^+}^{\Ycal}$; its aggregation $\theta' \in \left\{ {\mathbb{R}_{\geq 0}}^{\Zcal} : \sum_{z \in \Zcal} \theta'(z) = 1 \right\}$, $\theta'(z) \equiv \sum_{y \in S(z)} \theta(y)$ is thus also Dirichlet and has a parameterizing function $\alpha' \in {\mathbb{R}^+}^{\Zcal}$, $\alpha'(z) \equiv \sum_{y \in S(z)} \alpha(y)$.
...
&& \qquad \delta\left( \theta'(z) - \sum_{y \in S(z)} \theta(y) \right) \Bigg] \nonumber \;.
\end{IEEEeqnarray}
which is defined for $\theta \in \left\{ {\mathbb{R}_{\geq 0}}^{\Ycal} : \sum_{y \in S(z)} \theta(y) = \theta'(z), \quad \forall z \in \Zcal \right\}$. Note that the beta function argument notation has been expanded to allow the interpretation of functions as sets, such that for arbitrary set $A$,
\begin{IEEEeqnarray}{rCl}
...
&& \qquad \forall \tilde{\theta} \in \left\{ {\mathbb{R}_{\geq 0}}^{\Ycal} : \sum_{y \in S(z)} \tilde{\theta}_z(y) = 1, \quad \forall z \in \Zcal \right\} \nonumber \\
...
%\end{equation}
%
%The single stationary point $\theta^* \in {\mathbb{R}_{\geq 0}}^M$ and is thus a member of $\Theta$ as well. To complete the proof, we must examine the second-order derivatives to confirm that the stationary point is the global maximum. To this effect, we use a bordered Hessian matrix,
%
%\begin{equation}
