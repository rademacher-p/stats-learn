...
PGR: figure notation: theta font + Ycal indexing. use R,f opt?
...
Often, the dependency on the conditional variable $\zrm$ will not be expressed in terms of a specific value $z$, but will be left in terms of the random element itself; in this case, the more compact notation $\Prm_{\xrm | \zrm}(x)$ is used and the distribution is implied to be a function of $\zrm$.

PGR: reconsider conditional argument suppression, etc. ALWAYS random function? Can't do both preceeding paragraph styles.
...
PGR: reconsider the above paragraph!!!!!!!
...
PGR: are they independent???

In the preceeding formula, the expectation over the novel pair $(\yrm,\xrm)$ is split into two separate expectations as only $\yrm$ is unobservable. It will be useful to decompose the model distribution $\uptheta$ in a similar fashion. First, introduce the marginal distribution $\uptheta' \in \Pcal(\Xcal)$, defined as $\uptheta' \equiv \sum_{y \in \Ycal} \uptheta(y,\cdot) \in \Pcal(\Xcal)$; note that the summation is replaced by an integral when $\yrm$ is a continuous random variable. Next, introduce the conditional distributions $\tilde{\uptheta} \in \Pcal(\Ycal)^{\Xcal}$ defined as $\tilde{\uptheta}(x) \equiv \uptheta(\cdot,x) / \uptheta'(x)$. 

Note that there is a one-to-one correspondence between the model $\uptheta$ and the pair $(\uptheta',\tilde{\uptheta})$ - as such, the former can be replaced with the latter when desirable. This decomposition can be used to show that $\Prm_{\xrm | \uptheta} = \Prm_{\xrm | \uptheta'} = \uptheta'$ and $\Prm_{\yrm | \xrm,\uptheta} = \Prm_{\yrm | \xrm,\tilde{\uptheta}} = \tilde{\uptheta}(\xrm)$; these distributions will be of recurring importance.
...
\end{IEEEeqnarray}
and $\yrm$, $\xrm$, and $\Drm$ are treated as jointly distributed random elements. Observe that the Bayesian distributions can be represented as $\Prm_{\xrm | \Drm} = \Erm_{\uptheta | \Drm}\big[ \Prm_{\xrm | \uptheta} \big] = \mu_{\uptheta' | \Drm}$ and $\Prm_{\yrm | \xrm,\Drm} = \Erm_{\uptheta | \xrm,\Drm}\big[ \Prm_{\yrm | \xrm,\uptheta} \big] = \mu_{\tilde{\uptheta}(\xrm) | \xrm,\Drm}$, the expected values of the corresponding clairvoyant distributions with respect to the model posteriors $\prm_{\uptheta | \Drm}$ and $\prm_{\uptheta | \xrm,\Drm}$, respectively.

PGR: is condition on x necessary above?

PGR: conditional on X, not full D above????
...
\section{Conditional Training Set PMF, $\Prm_{\Drm | \uptheta}$}

PGR: continuous? DMP?

PGR: change to emp PMF RP?

PGR: trends in limit of N??
...
\begin{IEEEeqnarray}{rCl}
\bar{N}(y,x;D) & = & \sum_{n=1}^N \delta \big[ (\cdot,\cdot),D_n \big] \\
& = & \sum_{n=1}^N \delta \left[ \cdot,Y_n \right] \delta \left[ \cdot,X_n \right] \nonumber \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
where the multinomial operator $\Mcal$ is used. Note that $\Mcal\big( \bar{N}(D) \big) \Prm_{\Drm | \uptheta}(D | \theta) = \Prm_{\nbarrm | \uptheta}\big( \bar{N}(D) | \theta \big)$. The first and second joint moments of this multinomial distribution are \cite{theodoridis-ML}
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
which is dependent on the model $\uptheta$ only through the conditional model $\tilde{\uptheta}$.


PGR PGR

Introduce the ``marginalized'' random process $\nrm'$ over the set $\Xcal$, defined as $\nrm' \equiv \sum_{y \in \Ycal} \nbarrm(y,\cdot) \equiv N'(\Xrm)$. By the aggregation property of Multinomial random processes \cite{johnson}, the aggregation conditioned on the model $\uptheta$ is distributed as $\nrm' | \uptheta \sim \Mult(N,\uptheta')$. 
...
& = & \prod_{x \in \Xcal} \Mult\Big( \bar{n}(\cdot,x) ; n'(x) , \tilde{\theta}(x) \Big) \nonumber \;,
\end{IEEEeqnarray}
over the domain $\bar{n} \in \left\{ {\Zbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \bar{n}(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\nbarrm(\cdot,x)$ independent of one another and that they are also Multinomial, such that $\nbarrm(\cdot,x) | \nrm'(x),\uptheta \sim \Mult\big( \nrm'(x),\tilde{\uptheta}(x) \big)$.
...
\end{IEEEeqnarray}
a product of Dirichlet distributions defined on $\tilde{\theta} \in \left\{ \tilde{\theta}(x) \in \Pcal(\Ycal), \; \forall x \in \Xcal \right\}$. As shown, the processes $\tilde{\uptheta}(x)$ are Dirichlet with parameterizing functions $\alpha(\cdot,x)$, independent of one another, and independent of the marginal distribution $\uptheta'$. Note that $\Prm_{\yrm | \xrm} = \mu_{\tilde{\uptheta}(\xrm)} = \alpha(\cdot,\xrm) / \alpha'(\xrm)$. 


PGR: Use tilde theta in later sections? In predictive PMF section?
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{P_nbar_N.pdf}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\Xrm}(X) & = & \Erm_{\uptheta} \left[ \prod_{n=1}^N \Prm_{\Xrm_n | \uptheta}\big( X_n | \uptheta \big) \right] \\
& = & \Erm_{\uptheta'} \left[ \prod_{x \in \Xcal} \uptheta'(x)^{N'(x;X)} \right] \nonumber \\
& = & \beta(\alpha')^{-1} \beta\big( \alpha' + N'(X) \big) \nonumber \;.
\end{IEEEeqnarray}

\begin{IEEEeqnarray}{rCl}
\Prm_{\Yrm | \Xrm}(Y | X) & = & \Erm_{\uptheta} \left[ \prod_{n=1}^N \frac{\Prm_{\Yrm_n,\Xrm_n | \uptheta}\big( Y_n.X_n | \uptheta \big)}{\Prm_{\Xrm_n | \uptheta}\big( X_n | \uptheta \big)} \right] \\
...
Recall that $\nrm' | \uptheta \sim \Mult(N,\uptheta')$; by the aggregation property of Dirichlet-Multinomial functions \cite{johnson}, the random process is distributed as $\nrm' \sim \DM(N,\alpha')$.
...
PGR: REWORK using theta marginal and conditionals????
...
\begin{IEEEeqnarray}{rCl} \label{eq:P_y_xD}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\Drm} & = & \Erm_{\uptheta | \xrm,\Drm} \big[ \Prm_{\yrm | \xrm,\uptheta} \big] \\
& = & \mu_{\tilde{\uptheta}(\xrm) | \xrm,\Drm} \nonumber \;,
\end{IEEEeqnarray}

PGR: repeated eq above...

Just as $\Prm_{\yrm,\xrm | \Drm}$ is an expectation of the model $\uptheta$ posterior conditioned on $\Drm$, $\Prm_{\yrm | \xrm,\Drm}$ is an expectation of the normalized process $\tilde{\uptheta}$ conditioned on all observed random elements. 

The complete model posterior PDF is represented as
\begin{IEEEeqnarray}{rCl}
\prm_{\uptheta | \xrm, \Drm} & = & \Erm_{\yrm | \xrm,\Drm} \big[ \prm_{\uptheta | \yrm,\xrm,\Drm} \big] \\
& = & \sum_{y \in \Ycal} \frac{\alpha(y,\xrm) + \bar{N}(y,\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \Dir\big( \cdot ; \alpha + \bar{N}(D) + \delta[\cdot,y] \delta[\cdot,\xrm] \big) \nonumber \;,
\end{IEEEeqnarray}
a mixture distribution whose weighting factors are, interestingly, the values of $\Prm_{\yrm | \xrm,\Drm}$ itself. The PDF's being combined are Dirichlet distributions with different parameterizing functions $\alpha + \bar{N}(D) + \delta[\cdot,y] \delta[\cdot,x]$; additional emphasis is put on the $|\Ycal|$ different possible pairs $(y,x)$ including the observed value $\xrm$.

This posterior PDF shares the same asymptotic distributions with $\prm_{\uptheta | \Drm}$. As $\alpha_0 \to \infty$ or $N \to \infty$, the $|\Ycal|$ distinct Dirichlet PDF's converge to the same impulsive distribution and thus $\prm_{\uptheta | \xrm,\Drm}$ again tends toward $\delta(\cdot - \alpha / \alpha_0)$ and $\delta\big( \cdot - \bar{N}(\Drm)/N \big)$, respectively.

To confirm that the expectation produces the same distribution $\Prm_{\yrm | \xrm,\Drm}$ displayed previously, use the properties of a Dirichlet random process conditioned on its aggregation to show that $\Erm_{\uptheta}\Big[ \tilde{\uptheta}(x) \Big] = \alpha(\cdot,x) / \alpha'(x)$. Thus, the PMF is evaulated as
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\Drm} & = & \sum_{y' \in \Ycal} \frac{\alpha(y',\xrm) + \bar{N}(y',\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \\
&& \qquad \left( \frac{\alpha(\cdot,\xrm) + \bar{N}(\cdot,\xrm;\Drm) + \delta[\cdot,y'] \delta[\xrm,\xrm]}{\alpha'(\xrm) + N'(\xrm;\Drm) + \delta[\xrm,\xrm]} \right) \nonumber \\
& = & \frac{\alpha(\cdot,\xrm) + \bar{N}(\cdot,\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \nonumber \;.
\end{IEEEeqnarray}
...
It is informative to treat the distribution $\Prm_{\yrm | \xrm,\Drm}$ as an estimate of the unknown conditional PMF $\Prm_{\yrm | \xrm,\uptheta}$ and investigate the effects of subjective prior knowledge. For a given $\xrm$ and corresponding number of training samples $\nrm'(\xrm)$, the expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
\Erm_{\nbarrm | \nrm',\uptheta}\big[ \Prm_{\yrm | \xrm,\nbarrm} \big] \\ 
\quad = \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)}\right) \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} + \left(\frac{\nrm'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)}\right) \frac{\uptheta(\cdot,\xrm)}{\uptheta'(\xrm)} \nonumber \;,
\end{IEEEeqnarray}
...
PGR: formula below appropriately located? rework to optimal function

Substituting the Bayes predictive distribution \eqref{eq:P_y_xD} into Equation \eqref{eq:f_opt_xD}, the decision expressed for a given input $\xrm$ and training set $\Drm$ minimizes the metric
...
It may not seem intutitve for the risk to decrease when $\alpha_0$ is smaller -- the variance of the model $\uptheta$ increases and the prior knowledge is less definitive. This is a result of the Dirichlet PDF weight shifting towards the $|\Ycal||\Xcal|$ models which have $\ell_0$ norms satisfying $\| \theta \|_0 = 1$. Although these PMF's are maximally separated (and uncorrelated), they all have zero variance. The optimal learner \eqref{eq:f_opt_SE} will simply use the empirical distribution supplied via the training data - this allows exact identification of $\uptheta$ with a single training pair.
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Risk_SE_Dir_IO_Pyx.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Risk_SE_Dir_IO_Px_N_10_a0_1.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Risk_SE_Dir_IO_N_leg_Px.pdf}
\caption{Minimum SE Risk for different training set sizes $N$}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Risk_SE_Dir_IO_a0_leg_Px.pdf}
\caption{Minimum SE Risk for different prior concentrations $\alpha_0$}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Risk_SE_uniform_N_lim.pdf}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_SE_IO_N.pdf}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.9\linewidth]{Risk_SE_IO_N-Mx.pdf}
...
\end{IEEEeqnarray}
The optimal classifier chooses the value $y \in \Ycal$ that maximizes the conditional PMF for the observed values of $\xrm$ and $\Drm$; using the Dirichlet prior, different classes are ``scored'' by counting the number of training samples with a matching value of $\xrm$ and combining with the prior parameters $\alpha(\cdot,\xrm)$.  
...
As for Dirichlet distributions for functions with countable domains, the optimal risk is dependent on the model only through the concentration parameter $\alpha_0$ and the variance of the expected distribution $\Prm_{\yrm} = \mu_{\uptheta}$.
...
& = & \prod_{z \in \Zcal} \Mult\Big( \big\{ \bar{n}(y) : y \in \Scal(z) \big\} ; n'(z) , \big\{ \tilde{\theta}_z(y) : y \in \Scal(z) \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
on the domain $\bar{n} \in \left\{ {\Zbb_{\geq 0}}^{\Ycal} : \sum_{y \in \Scal(z)} \bar{n}(y) = n'(z), \quad \forall z \in \Zcal \right\}$. Observe that the segment over the set $\Scal(z)$ sums to $\nrm'(z)$ and is parameterized by the normalized segment $\tilde{\theta}_z \equiv \big\{ \theta(y)/\theta'(z) : y \in \Scal(z) \big\}$.
...
%\end{IEEEeqnarray}
on the domain $\bar{n} \in \left\{ {\Zbb_{\geq 0}}^{\Ycal} : \sum_{y \in \Scal(z)} \bar{n}(y) = n'(z), \quad \forall z \in \Zcal \right\}$. 
...
\begin{equation}
\Prm_{\nrm' | \uptheta}(n' | \theta) = \Mcal(n') \prod_{z \in \Zcal} \left( \int_{\Scal(z)} \theta(y) \mathrm{d} y \right)^{n'(z)} = \Mult\left( n' ; N,\theta'(z) \right) \;,
\end{equation}
