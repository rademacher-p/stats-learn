...
The objective is to design a decision function which maps from the observation space $\Xcal$ to a decision space $\Hcal$. The metric guiding the design is a loss function $\mathcal{L}: \Hcal \times \Ycal \mapsto \Rbb_{\geq 0}$ which penalizes the decision $h \in \Hcal$ based on the value of $\yrm$. 

If the model $\uptheta$ were known, a decision function $f_{\Theta} : \Theta \to \Hcal^{\Xcal}$ could be designed to minimize the conditional expected loss, or conditional ``risk'',
\begin{equation} 
\Rcal_{\Theta}(f ; \uptheta) = \Erm_{\yrm,\xrm | \uptheta} \Big[ \mathcal{L}\big( f(\xrm;\uptheta),\yrm \big) \Big] \;.
\end{equation}
The optimal ``clairvoyant'' decision function is expressed as
\begin{equation}
f_{\Theta}(\xrm;\uptheta) = \argmin_{h \in \Hcal} \Erm_{\yrm | \xrm,\uptheta}\big[ \mathcal{L}(h,\yrm) \big] 
\end{equation}
and the corresponding irreducible risk is
...
If the model $\uptheta$ was known and the decision function domain extended to $\Dcal \times \Theta$, the optimal function $f_{\Theta}$ could be found directly. For a given set of observations, this function selects the decision $h = \argmin_{h \in \Hcal} \Erm_{\yrm | \xrm,\Drm,\uptheta}\big[ \mathcal{L}(h,\yrm) \big]$. It can be shown that given the model $\uptheta$, the unobserved element $\yrm$ is conditionally independent of the training data $\Drm$. As such, the knowledge of $\uptheta$ renders $\Drm$ uninformative - the ``clairvoyant'' decision function is recast as $f_{\Theta} : \Theta \to \Hcal^{\Xcal}$, expressed by


As the model $\theta$ is unobserved, the conditional risk $\Rcal_{\Theta}: \Theta \mapsto {\Rbb_{\geq 0}}^{\Fcal}$ is not yet a valid objective function for optimization. An operator must be chosen to remove the dependency on $\theta$ and form an objective function $\Fcal \mapsto \Rbb_{\geq 0}$.
