...
The ``clairvoyant'' \cite{kay-det} decision function $f_{\uptheta}: \Theta \mapsto \Hcal^{\Xcal}$ is
...
\begin{itemize}
\item The model probability density function (PDF) is Dirichlet \cite{bishop} with parameters $\alpha : \Ycal \times \Xcal \mapsto \Rbb^+$:
\end{itemize}
...
Statistic $\trm \equiv T(\xrm)$ must satisfy \cite{kay-est}
\begin{IEEEeqnarray*}{rCl}
...
\centering
\large
\textbf{Dirichlet Prior leads to tractable Dirichlet-Multinomial \cite{johnson} Evidence function}
}
}
