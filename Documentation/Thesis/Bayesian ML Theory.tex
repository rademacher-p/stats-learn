...
\end{IEEEeqnarray}
where the dependency on the training data $\Drm$ is expressed though a transform function $\Psi : \Dcal \mapsto \Theta_\uppsi$, where the range is 
\begin{IEEEeqnarray}{rCl}
\Theta_\uppsi & = & \left\{ \frac{\bar{n}}{N} : \bar{n} \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal}, \ \sum_{y \in \Ycal} \sum_{x \in \Xcal} \bar{n}(y,x) = N \right\} 
\end{IEEEeqnarray}
...
Frequently, the corresponding distributions $\Prm_{\uppsi}$, $\Prm_{\xrm | \uppsi}$, and $\Prm_{\yrm | \xrm,\uppsi}$ will be used to find the optimal learning functions and the minimum risk. Note that $\Mcal\big( \Psi(D) \big) \Prm_{\Drm | \uptheta}(D | \theta) = \Prm_{\uppsi | \uptheta}\big( \Psi(D) | \theta \big)$, where $\Mcal$ is the multinomial operator. Also note that $\Prm_{\xrm | \Drm}(D) = \Prm_{\xrm | \uppsi}\big( \Psi(D) \big)$ and $\Prm_{\yrm | \xrm,\Drm}(x,D) = \Prm_{\yrm | \xrm,\uppsi}\big( x,\Psi(D) \big)$.

\todomid{move above paragraph?}

The cardinality of the random process' domain is $|\Theta_\uppsi| = \Mcal\big( \{N,|\Ycal||\Xcal|-1\} \big)$; this can be shown using the stars-and-bars method \cite{feller}. The cardinality of original set is $|\Dcal| = \big( |\Ycal| |\Xcal| \big)^N$; thus $|\Theta_\uppsi| \leq |\Dcal|$ and the sufficient statistic compactly represents the valuable information in the training data. Also, observe that the set $\{ \bar{n}/N : \bar{n} \in \Theta_\uppsi \} \subset \Theta$ and thus that the empirical distribution $\Psi(\Drm)/N$ assumes one of a finite number of the elements from $\Theta$.

\todolo{sufficient statistic savings in memory bits?}

Conditioned on the model $\uptheta$, the PMF of $\uppsi$ is a multinomial distribution
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uptheta}(\bar{n} | \theta) & = & \sum_{D : \Psi(D) = \bar{n}} \Prm_{\Drm | \uptheta}(D | \theta) \\
& = & \big|\{ D : \Psi(D) = \bar{n} \}\big| \prod_{y \in \Ycal} \prod_{x \in \Xcal} \theta(y,x)^{\bar{n}(y,x)} \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\uppsi | \uptheta}(y,x,y',x')  & = & N \big( \uptheta(y,x) \delta[y,y'] \delta[x,x'] - \uptheta(y,x) \uptheta(y',x') \big) \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\theta_\mathrm{ML}\big( \bar{n} \big) & = & \argmax_{\theta \in \Theta} \Prm_{\uppsi | \uptheta}(\bar{n} | \theta) \\ \nonumber
& = & \frac{\bar{n}}{N} \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\bar{\nrm} | \nrm' , \uptheta}(\bar{n} | n' , \theta) & \equiv & \prod_{x \in \Xcal} \Bigg[ \Mcal\big( \bar{n}(\cdot,x) \big) \prod_{y \in \Ycal} \thetac(y;x)^{\bar{n}(y,x)} \Bigg] \\
& = & \prod_{x \in \Xcal} \Multi\Big( \bar{n}(\cdot,x) ; n'(x) , \thetac(x) \Big) \nonumber \;,
\end{IEEEeqnarray}
over the domain $\left\{ \bar{n} \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \bar{n}(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Multinomial, such that $\uppsi(\cdot,x) | \nrm'(x),\uptheta \sim \Multi\big( \nrm'(x),\upthetac(x) \big)$. Furthermore, the dependency on $\uptheta$ is expressed through the conditional model $\upthetac$
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi}(\thetam,\thetac | \bar{n}) & = & \frac{\Prm_{\uppsi | \nrm',\upthetac}\big( \bar{n} | \sum_y \bar{n}(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \nrm'}\big( \bar{n} | \sum_y \bar{n}(y,\cdot) \big)} 
\frac{\Prm_{\nrm' | \upthetam}\big( \sum_y \bar{n}(y,\cdot) | \thetam \big)}{\Prm_{\nrm'}\big( \sum_y \bar{n}(y,\cdot) \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\nrm'}\Big( \thetac | \bar{n},\sum_y \bar{n}(y,\cdot) \Big) \prm_{\upthetam | \nrm'}\Big( \thetam | \sum_y \bar{n}(y,\cdot) \Big) 
\frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \nrm'}\big( \thetac | \sum_y \bar{n}(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi,\xrm}(\thetam,\thetac | \bar{n},x) & = & \frac{\Prm_{\uppsi | \nrm',\upthetac}\big( \bar{n} | \sum_y \bar{n}(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \nrm',\xrm}\big( \bar{n} | \sum_y \bar{n}(y,\cdot),x \big)} 
\frac{\Prm_{\nrm',\xrm | \upthetam}\big( \sum_y \bar{n}(y,\cdot),x | \thetam \big)}{\Prm_{\nrm',\xrm}\big( \sum_y \bar{n}(y,\cdot),x \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\nrm'}\Big( \thetac | \bar{n},\sum_y \bar{n}(y,\cdot) \Big) \prm_{\upthetam | \nrm',\xrm}\Big( \thetam | \sum_y \bar{n}(y,\cdot),x \Big) \nonumber \\
&& \quad \frac{\Prm_{\uppsi | \nrm'}\big( \bar{n} | \sum_y \bar{n}(y,\cdot) \big)}{\Prm_{\uppsi | \nrm',\xrm}\big( \bar{n} | \sum_y \bar{n}(y,\cdot),x \big)} \frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \nrm'}\big( \thetac | \sum_y \bar{n}(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}
...
\todohigh{ALPHA REWORK!!!}

The marginal distribution $\upthetam$ and the conditional distribution $\upthetac$ will also be of interest. To keep notation simple, it is helpful to introduce the bijection $\alpha \Leftrightarrow \big( \alpha_0,\alpham,\alphac \big)$, where $\alpha_0$ has been defined previously, $\alpham \equiv \sum_{y \in \Ycal} \alpha(y,\cdot) / \alpha_0$, and $\alphac(x) = \alpha(\cdot,x) / \big( \alpha_0 \alpham(x) \big)$ for each $x \in \Xcal$. Observe that $\alpham \in \Pcal(\Xcal)$ and $\alphac \in \Pcal(\Ycal)^{\Xcal}$.


By the aggregation property \cite{ferguson}, $\upthetam$ is a Dirichlet random process parameterized by $\alpha_0 \alpham$; thus, the concentration parameter is $\alpha_0$ and that $\Prm_{\xrm} = \mu_{\upthetam} = \alpham$.
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \upthetam}\big( \thetac | \thetam \big) & = & \prod_{x \in \Xcal} \Bigg[ \beta\big( \alpha_0 \alpham(x) \alphac(x) \big)^{-1} \prod_{y \in \Ycal} \thetac(y;x)^{\alpha_0 \alpham(x) \alphac(y;x) - 1} \Bigg] \\
& = & \prod_{x \in \Xcal} \Dir\Big( \thetac(x) ; \alpha_0 \alpham(x) \alphac(x) \Big) \nonumber \;,
\end{IEEEeqnarray}
a product of Dirichlet distributions defined on $\thetac \in \Pcal(\Ycal)^{\Xcal}$. As shown, the processes $\upthetac(x)$ are Dirichlet with parameterizing functions $\alpha(\cdot,x)$, independent of one another, and independent of the marginal distribution $\upthetam$. Observe that the values $\alpha_0 \alpham(x)$ represent the concentration parameters for the individual Dirichlet processes; also, note that $\Prm_{\yrm | \xrm} = \mu_{\upthetac(\xrm)} = \alphac(\xrm)$. 
...
& = & \beta(\alpha)^{-1} \beta \left(  \alpha + \Psi(D) \right) \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi}(\bar{n}) & = & \Mcal(\bar{n}) \beta(\alpha)^{-1} \beta(\alpha + \bar{n}) \\
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi}(\bar{n}) & \to & \Mcal(\bar{n}) \prod_{y \in \Ycal} \prod_{x \in \Xcal} \left(\frac{\alpha(y,x)}{\alpha_0}\right)^{\bar{n}(y,x)}
\end{IEEEeqnarray}
Conversely, for $\alpha_0 \to 0$, the PMF tends toward
\begin{IEEEeqnarray}{rCl} \label{eq:P_n_lim_zero}
\Prm_{\uppsi}(\bar{n}) & \to & \sum_{y \in \Ycal} \sum_{x \in \Xcal} \frac{\alpha(y,x)}{\alpha_0} \delta\big[ \bar{n} , N \delta[\cdot,y] \delta[\cdot,x] \big] \;.
\end{IEEEeqnarray}

PGR: formal proofs for limiting PMFs? stirling/gautschi?

Figure \ref{fig:P_nbar_a0} displays the distribution of $\uppsi$ for $N=10$ and different model concentrations $\alpha_0$. Observe that for large $\alpha_0$, the distribution approaches a multinomial distribution $\uppsi \sim \Multi(N,\alpha/\alpha_0)$. Figure \ref{fig:P_nbar_N} shows how a specific model prior influences the data PMF differently for different $N$. Observe that as the number of training samples increases, the PMF $\Prm_{\uppsi}$ tends toward $\Prm_{\uppsi}(\bar{n}) \approx N^{1-|\Ycal||\Xcal|}\prm_{\uptheta}(\bar{n}/N)$; this can be proven using Gautschi's inequality \cite{wendel}.
\begin{figure}
\centering
...
\end{IEEEeqnarray}
over the domain $\left\{ \bar{n} \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \bar{n}(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Dirichlet-Multinomial, such that $\uppsi(\cdot,x) | \nrm'(x) \sim \DM\big( \nrm'(x),\alpha(\cdot,x) \big)$.
...
& = & \left(\frac{\alpha_0}{\alpha_0 + N}\right) \frac{\alpha}{\alpha_0} + \left(\frac{N}{\alpha_0 + N}\right) \frac{\Psi(\Drm)}{N} \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\xrm | \uppsi}(\bar{n}) & = & \mu_{\upthetam | \uppsi}(\bar{n}) = \mu_{\upthetam | \nrm'}\left( \sum_y \bar{n}(y,\cdot) \right) \\
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \uppsi,\xrm}\big( \thetac | \bar{n},x \big) & = & \prm_{\upthetac | \uppsi}\big( \thetac | \bar{n} \big) = \prod_{x' \in \Xcal} \prm_{\upthetac(x') | \uppsi(\cdot,x')}\big(\thetac(x') | \bar{n}(\cdot,x') \big) \\
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha(\cdot,x') + \bar{n}(\cdot,x') \big) \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\uppsi}(x,\bar{n}) & = & \mu_{\upthetac(x) | \xrm,\uppsi}(x,\bar{n}) = \mu_{\upthetac(x) | \uppsi(\cdot,x)}\big(\bar{n}(\cdot,x)\big) \\
& = & \frac{\alpha(\cdot,x) + \bar{n}(\cdot,x)}{\alpha'(x) + \sum_y \bar{n}(y,x)} \nonumber \;.
\end{IEEEeqnarray}
A consequence of the Dirichlet prior is that the predictive PMF for a given value of $\xrm$ only depends on the corresponding training data $\uppsi(\cdot,\xrm)$, such that $\Prm_{\yrm | \xrm,\uppsi}(x,\bar{n}) = \Prm_{\yrm | \xrm,\uppsi(\cdot,\xrm)}\big( x,\bar{n}(\cdot,x) \big)$. This is intuitive considering the independence of the conditional models $\thetac(x)$ from one another.
...
\begin{IEEEeqnarray}{rCl}
\Erm_{\xrm,\uppsi} \left[ \Erm_{\yrm | \xrm,\uppsi}[\yrm^2] \right] & = & \Erm_{\yrm}[\yrm^2] = \sum_{y \in \Ycal} y^2 \left( \sum_{x \in \Xcal} \frac{\alpha(y,x)}{\alpha_0} \right) \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] & = & \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \frac{\big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \bar{\nrm}(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big)^2} \right] \right] \\
& = & \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \bar{\nrm}(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
...
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha(y,\xrm)+\bar{\nrm}(y,\xrm) \big) \big(\alpha(y',\xrm) + \bar{\nrm}(y',\xrm) \big)}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
...
%\begin{IEEEeqnarray}{L}
%\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] =
%\sum_{\bar{n} \in \Theta_\uppsi} \sum_{x \in \Xcal} \Prm_{\xrm,\uppsi}(x,\bar{n}) \left( \sum_{y \in \Ycal} y \Prm_{\yrm | \xrm,\uppsi}(y | x,\bar{n}) \right)^2 \\
%= \sum_{x \in \Xcal} \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\uppsi} \left[ \frac{\big( \alpha(y,x)+\bar{\nrm}(y,x) \big) \big(\alpha(y',x)+\bar{\nrm}(y',x) \big)}{(\alpha_0+N) \big(\alpha'(x) + \nrm'(x) \big)} \right] \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - \Erm_{\xrm,\Drm} \left[ \max_{y \in \Ycal} \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \right] \\
& = & 1 - \Erm_{\xrm,\uppsi} \left[ \frac{ \max_{y \in \Ycal} \big( \alpha(y,x) + \bar{\nrm}(y,x) \big)}{\alpha'(x) + \nrm'(x)} \right] \nonumber \\
& = & 1 - \sum_{x \in \Xcal} \frac{\Erm_{\uppsi} \Big[ \max_{y \in \Ycal} \big( \alpha(y,x) + \bar{\nrm}(y,x) \big) \Big]}{\alpha_0 + N} \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - \Erm_{\xrm,\Drm} \left[ \max_{y \in \Ycal} \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \right] \\
& = & 1 - \sum_{x \in \Xcal} \frac{\Erm_{\uppsi} \big[ \max_{y \in \Ycal} \bar{\nrm}(y,x) \big] + 1}{|\Ycal||\Xcal| + N} \nonumber \\
& = & 1 - \frac{1 + |\Xcal|^{-1} \sum_{x \in \Xcal} \Erm_{\uppsi} \big[ \max_{y \in \Ycal} \bar{\nrm}(y,x) \big]}{|\Ycal| + N/|\Xcal|} \nonumber \;.
\end{IEEEeqnarray}
...
To evaluate this expectation, new random variables $\uppsi_{\max}(x) \equiv \max_{y \in \Ycal} \uppsi(y,x)$ are introduced and characterized by their identical PMF. To this end, the probability of the event $\Prm\big( \uppsi_{\max}(x) \geq n \big) = \Prm\big( \cup_{y \in \Ycal} \{ \uppsi(y,x) \geq n \} \big)$ will be determined. As the distribution of $\uppsi$ is uniform, the event probability is proportionate to the cardinality of the set $\cup_{y \in \Ycal} \{ \bar{n}: \bar{n}(y,x) \geq n \}$. Using the inclusion-exclusion principle \cite{brualdi}, the cardinality is represented as
\begin{IEEEeqnarray}{L}
...
\begin{IEEEeqnarray}{L}
\Prm\big( \uppsi_{\max}(x) \geq n \big) = \binom{N+|\Ycal||\Xcal|-1}{|\Ycal||\Xcal|-1}^{-1} \big| \cup_{y \in \Ycal} \{ \bar{n} : \bar{n}(y,x) \geq n \} \big| \\
...
\begin{equation}
\Prm(\uppsi | \uptheta) = \Mcal(\uppsi) \prod_{y \in \Ycal} \uptheta(y)^{\bar{\nrm}(y)} \;,
\end{equation}

Thus,
\begin{IEEEeqnarray}{rCl}
\Prm(\uppsi) & = & \Mcal(\uppsi) \beta(\alpha)^{-1} \beta(\alpha + \uppsi) \;.
\end{IEEEeqnarray}
...
Also,
\begin{equation}
\Prm(\Drm) = \beta(\alpha)^{-1} \beta \big( \alpha + \Psi(\Drm) \big) \;.
\end{equation}
...
Define the Dirichlet-Multinomial process $\bar{\nrm} \equiv \Psi(\Drm)$. The mean and correlation functions below are found in Appendix \ref{app:DMP}. The mean function is
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi} & = & N \frac{\alpha}{\alpha_0} 
\end{IEEEeqnarray}
...
We have the DMP $\bar{\nrm} \equiv \Psi(\Drm)$ with mean and correlation functions
\begin{IEEEeqnarray}{rCl}
...
\quad = \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \left( \frac{\alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \bar{\nrm}(y,\xrm) \mathrm{d}y}{\alpha'(\xrm) + \nrm'(\xrm)} \right)^2 \right] \right] \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\prm_{\uptheta | \upthetam}(\theta | \thetam) & = & \frac{\beta(\alpha') \prod_{y \in \Ycal} \theta(y)^{\alpha(y)-1}}{\beta(\alpha) \prod_{z \in \Zcal} \thetam(z)^{\alpha'(z)-1}} \\
...
To prove this principle, define the subset $\tilde{\Ncal} = \big\{ \bar{n} \in \Ncal : \sum_{y \in \Scal(z)} \bar{n}(y) = n'(z), \quad \forall z \in \Zcal \big\} \subseteq \Theta_\uppsi$, where the original random process $\bar{\nrm} \in \Theta_\uppsi$. Next, observe
\begin{IEEEeqnarray}{rCl}
\Prm_{\mathrm{n}'}(n') & = & \sum_{\bar{n} \in \tilde{\Ncal}} \Prm_{\uppsi}(\bar{n}) \\
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\bar{\nrm} | \nrm'}(\bar{n} | n') & = & \frac{\Mcal(\bar{n}) \prod_{y \in \Ycal} \theta(y)^{\bar{n}(y)}}{\Mcal(n') \prod_{z \in \Zcal} \thetam(z)^{n'(z)}} \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal\Big( \big\{ \bar{n}(y) : y \in \Scal(z) \big\} \Big) \prod_{y \in \Scal(z)} \left(\frac{\theta(y)}{\thetam(z)}\right)^{\bar{n}(y)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal\Big( \big\{ \bar{n}(y) : y \in \Scal(z) \big\} \Big) \prod_{y \in \Scal(z)} \thetac_z(y)^{\bar{n}(y)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Multi\Big( \big\{ \bar{n}(y) : y \in \Scal(z) \big\} ; n'(z) , \big\{ \thetac_z(y) : y \in \Scal(z) \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
...
Next, it is demonstrated that the random process $\bar{\nrm}(y) \equiv \Psi(y;\Drm) = \sum_{n=1}^N \delta\big( y-\Drm_n \big)$ is a DMP, given that $\prm_{\Drm|\uptheta}(D|\theta) = \prod_{n=1}^N \theta\big( D_n \big)$ and $\uptheta \sim \DP(\alpha)$. 
