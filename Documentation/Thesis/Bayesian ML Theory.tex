...
\todolo{High concentration makes Bayes results too similar to true model results?}
...
\todohi{make N=4000 for predict convergence}
...
\begin{IEEEeqnarray}{rCl} \label{eq:SE_alpha_bar_opt}
...
To underscore the limitation of predictors with coarse discretization, consider \cref{fig:Discretization/SE/predict_N_T4_a0_high}. Unlike the predictors visualized in Section \ref{sec:SE_dir_example}, the discretized predictors tend to $\Erm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}]\big(T(x)\big)$ in the limit of training data, not to the clairvoyant regressor $\mu_{\yrm | \xrm,\upthetac}$; this reflects the independence of the discretization bias from the data volume $N$. As such, the quality of the match is dependent on the variation of the clairvoyant regressor and on the size of the subsets $\Xcal'(t)$.
\begin{figure}
	\centering
...
To further demonstrate the importance of the selection of the discretization function, \Cref{fig:Discretization/SE/_lofi_risk_T_leg_N,fig:Discretization/SE/_lofi_risk_T_leg_a0} demonstrate the error trends as a function of $|\Tcal|$ for different data volumes and prior concentrations, respectively. In the former, note that the transform set cardinality that minimizes the squared-error is directly proportionate to the training data volume $N$. Conforming with the previous results, more data allows finer discretizers to be used, minimizing the discretization error without suffering from the lower data sensitivity.
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{Discretization/SE/_lofi_risk_T_leg_N.png}
	\caption{Squared-Error vs. discretization $|\Tcal|$, various $N$}
	\label{fig:Discretization/SE/_lofi_risk_T_leg_N}
\end{figure} 
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{Discretization/SE/_lofi_risk_T_leg_a0.png}
	\caption{Squared-Error vs. discretization $|\Tcal|$, various $\alpha_0$}
	\label{fig:Discretization/SE/_lofi_risk_T_leg_a0}
\end{figure} 
