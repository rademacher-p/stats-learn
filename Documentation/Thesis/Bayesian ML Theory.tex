...
\section{Dirichlet Process conditioned on its aggregation}
\label{app:Dir_agg}

This section details an important property of Dirichlet distributed random processes: when conditioned on its own aggregation, the partitioned segments of the process become independent. Furthermore, if normalized, they are also Dirichlet processes. 
...
\section{Dirichlet-Empirical Process Properties} 
\label{app:DE}
...
To prove the hypothesis, exploit the results of Appendix \ref{app:Dir_agg} to represent the training data distribution conditioned on the aggregation $\upthetam$. Introduce the normalized segments $\upthetac(z) \equiv \uptheta_z / \upthetam(z) \in \Pcal(\Scal_z)$, which have the Dirichlet properties of independence from one another and from the aggregation process $\upthetam$. The conditional distribution of interest is
\begin{IEEEeqnarray}{rCl}
...
\todohigh{TRUE BELOW?? Move to subsection.}
...
By the aggregation property of the Dirichlet process $\uptheta$, the parameters of this Empirical distribution are characterized as $\upthetam \sim \Dir(\alpha_0,\alpha)$, and thus $\uppsim$ is drawn from a Dirichlet-Empirical PMF $\DE(N,\alpha_0,\alpha)$. As this aggregation property holds for any partition of $\Ycal$, $\uppsi$ is a Dirichlet-Empirical Process.
