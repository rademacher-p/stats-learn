...
Having effectively changed the domain of the observations, the Dirichlet prior marginal mean is re-defined as $\alpham = \sum_{t \in \Tcal} \alphamd(t) \delta(\cdot - t)$, a mixture of delta functions at the discretized values, where $\alphamd \in \Pcal(\Tcal)$ is the discrete space marginal function. 
...
\end{IEEEeqnarray}
Observe that for a novel observation $\xrm$, the convex weight will now depend on $\sum_{n=1}^N \delta\big[T(\xrm), T(X_n)\big]$, the number of training samples that discretize to the same value. This contrasts with the pure continuous-domain Dirichlet predictive distribution, which only counts training values that match precisely. Similarly, the conditional empirical distribution being mixed in \eqref{eq:P_y_xD_dir_cont_disc} is formed using the same, larger group of training observations.
...
Defining the novel feature $\trm \equiv T(\xrm)$, it is evident that $\Prm_{\trm | \uptheta} = \Pr\big(T(\xrm) = \trm | \uptheta\big) = \Pr\big(\xrm \in \Xcal'(\trm) | \uptheta\big)$. As a result, $\Prm_{\trm | \uptheta} \equiv \upthetamd \in \Pcal(\Tcal)$, where $\upthetamd$ is the discretized marginal model defined as $\upthetamd(t) = \int_{\Xcal'(t)} \upthetam(x) {\drm}x$. Also, note that given the discretization, the raw observation PDF is
\begin{IEEEeqnarray}{rCl}
\prm_{\xrm | \trm, \uptheta} \equiv \frac{\upthetam}{\upthetamd(\trm)} \in \Pcal\big( \Xcal'(\trm) \big) \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
convexly combining the true predictive distributions $\upthetac(x)$ for observations $x \in \Xcal'(t)$. More compactly, the discretized conditional model can be represented as $\upthetacd(\trm) = \Erm_{\xrm | \trm, \upthetam}\big[\upthetac(\xrm)\big]$.



\subsection{PGR predictive dist w psi}

The Bayesian predictive distribution \eqref{eq:P_y_xD_dir_cont_disc} can be represented in terms of the discretized sufficient statistics as $\prm_{\yrm | \xrm,\Drm} = \prm_{\yrm | \trm,\uppsimd,\uppsicd}\big(T(\xrm), \Psimd(\Xrm), \Psicd(\Drm)\big)$, where
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \trm,\uppsimd,\uppsicd} & = & \gammamd(\trm; \uppsimd) \alphac(\trm) + \big(1 - \gammamd(\trm; \uppsimd)\big) \uppsicd(\trm) \;.
\end{IEEEeqnarray}
Note that the weighting function is redefined to operate on the discretized empirical distribution rather than the raw observations, such that $\gammamd: \Pcal(\Xcal) \mapsto (0,1]^{\Tcal}$ and 
\begin{IEEEeqnarray}{rCl}
...
\Erm_{\Drm | \upthetam,\upthetac} \big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \big] \nonumber \\
\quad \equiv \Erm_{\uppsimd | \upthetamd}\Big[ \gammamd\big(T(\xrm); \uppsimd\big)^2 \Big] \Big( \alphac\big(T(\xrm)\big) - \upthetac\big(T(\xrm)\big) \Big) \otimes \Big( \alphac\big(T(\xrm)\big) - \upthetac\big(T(\xrm)\big) \Big) \nonumber \\
...
\end{IEEEeqnarray}
Note that the first two terms are similar to the two terms in  \eqref{eq:predictive_del_sq_dir}, with the evaluations performed in terms of the transformed observation $\trm$; the final three terms quantify additional deviation due to the discretization bias.
...
\begin{IEEEeqnarray}{rCl}
f^*(\xrm;\Drm) & = & \mu_{\yrm | \xrm,\Drm} \\
& = & \gammamd\big(T(\xrm); \Xrm\big) \mu_{\yrm | \xrm} + \Big(1 - \gammamd\big(T(\xrm); \Xrm\big)\Big) \frac{\sum_{n=1}^N \delta\big[T(\xrm), T(\Xrm_n)\big] \Yrm_n}{\sum_{n=1}^N \delta\big[T(\xrm), T(\Xrm_n)\big]} \nonumber \;.
\end{IEEEeqnarray}

Inheriting the properties of the discretized predictive distribution \eqref{eq:P_y_xD_dir_cont_disc}, the empirical mean for a given observation $\xrm$ now averages all values $\Yrm_n$ whose corresponding observation satisfy $\Xrm_n \in \Xcal'\big(T(\xrm)\big)$.
...
\qquad + \Erm_{\trm | \upthetam}\left[ \Erm_{\uppsimd | \upthetamd}\left[ 1 + \frac{\big(1 - \gammamd(\trm; \uppsimd)\big)^2}{N \uppsimd(\trm)} \right] \Crm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}] \right] \;.
\end{IEEEeqnarray}
Comparing with the discrete-domain excess squared-error \eqref{eq:risk_cond_SE_dir_ex}, there are, as with the predictive distribution estimation, both similar and new terms. The first two terms are directly comparable; however, instead of integrating over the observation space $\Xcal$, the expectation is evaluated using $\trm$. Additionally, note that the clairvoyant regressor \eqref{eq:f_clv_SE} and the its conditional variance $[\Sigma_{\yrm | \xrm,\upthetac}$ are replaced by their expectations with respect to the conditional distribution $\prm_{\xrm | \trm, \upthetam}$. The new third term quantifies the discretization error via $\Crm_{\xrm | \trm, \upthetam}[\mu_{\yrm | \xrm,\upthetac}]$, measuring the variation of the clairvoyant regressor within each discretization subset $\Xcal'(t)$. 

Next, consider the trends of the excess squared-error \eqref{eq:risk_cond_SE_dir_ex_disc}. 
