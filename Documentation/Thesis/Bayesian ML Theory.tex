...
\todohi{Is Dir and DP redundant?? DM and DMP?}

\todolo{likelihood function terminology}

\todohi{Dirichlet localization or concentration?}

\todolow{DIM and PR operator from AMS?}
...
\todohi{CHECK for $\Theta$ vs $\Uptheta$}

\todomid{check notation throughout for expectation/variance operators, proper arguments}

\todolo{use clairvoyant/irreducible terms and symbols $\Rcal$}

\todomid{suppress arguments where sensible? eg Pd = theta}
...
\todohigh{introduce tilde alpha to match tilde theta?}

\todohigh{use x,y subs instead of prime/tilde?}

\todohigh{change from nbar to emprical RV??? PROPERTIES??}

\todohi{multinomial random process?}

\todohi{Investigate N lim for nbar given theta, risks. Model support, bounded prior?}

\todohi{SEMI-SUPERVISED - generalize to joint decisions!!! training/test!}

\todohi{priors = sparse conditionals; w/ sufficient statistics}


\todolow{NFLT investigation? try matlab examples}
...
\todohi{ABOVE NOTATION CREATES AMBIGUITY!!!!!!! Check for uses...}
...
\todolo{define subset functions, ditch set inputs? appendix?}
...
\todolo{italic theta font before Bayes?}
...
As only $\yrm$ is unobservable, it will be useful to decompose the model distribution as $\uptheta \Leftrightarrow (\upthetam,\upthetac)$. First, introduce the marginal distribution $\upthetam \equiv \sum_{y \in \Ycal} \uptheta(y,\cdot) \in \Pcal(\Xcal)$; note that the summation is replaced by an integral when $\yrm$ is a continuous random variable. Next, introduce the conditional distributions $\upthetac \in \Pcal(\Ycal)^{\Xcal}$ defined as $\upthetac(x) \equiv \uptheta(\cdot,x) / \upthetam(x)$. Note that the mapping between the two representations is a bijection.

This decomposition enables the clairvoyant distributions to be represented as $\Prm_{\xrm | \uptheta} = \Prm_{\xrm | \upthetam} = \upthetam$ and $\Prm_{\yrm | \xrm,\uptheta} = \Prm_{\yrm | \xrm,\upthetac} = \upthetac(\xrm)$; these distributions will be of recurring importance. 
...
\todohi{likelihood with D AND x???}

\todohi{N TRENDS? See Glivenko–Cantelli theorem}
...
\begin{IEEEeqnarray}{rCl}
\Psi(y,x;D) & = & \frac{1}{N} \sum_{n=1}^N \delta \big[ (y,x),D_n \big] \\
& \equiv & \frac{1}{N} \sum_{n=1}^N \delta \left[ y,Y_n \right] \delta \left[ x,X_n \right] \nonumber \;.
\end{IEEEeqnarray}
This function determines the empirical probability of the pair $(y,x)$. Note that the set $\Uppsi$ is a finite subset of $\Uptheta$ and thus that the empirical distribution $\Psi(\Drm)$ is a valid probability distribution.

\todolo{comment on greek symbol forms}


Note that $\Prm_{\Drm | \uptheta}$ depends on the training data $\Drm$ only through the transform $\Psi$; $\Psi(\Drm)$ is thus a sufficient statistic \cite{bernardo} for the model $\uptheta$. Consequently, other distributions of interest $\Prm_{\Drm}$, $\Prm_{\xrm | \Drm}$, and $\Prm_{\yrm | \xrm,\Drm}$ will also depend on $\Drm$ via $\Psi(\Drm)$. As such, it is useful to define a new random process $\uppsi \equiv \Psi(\Drm) \in \Uppsi$. 

\todomid{show SS via Kay, data likelihood??!!}

Frequently, the corresponding distributions $\Prm_{\uppsi}$, $\Prm_{\xrm | \uppsi}$, and $\Prm_{\yrm | \xrm,\uppsi}$ will be used to find the optimal learning functions and the minimum risk. Note that $\Mcal\big( N \Psi(D) \big) \Prm_{\Drm | \uptheta}(D | \theta) = \Prm_{\uppsi | \uptheta}\big( \Psi(D) | \theta \big)$, where $\Mcal$ is the multinomial operator. Also note that $\Prm_{\xrm | \Drm}(D) = \Prm_{\xrm | \uppsi}\big( \Psi(D) \big)$ and $\Prm_{\yrm | \xrm,\Drm}(x,D) = \Prm_{\yrm | \xrm,\uppsi}\big( x,\Psi(D) \big)$.

\todomid{move above paragraph?}
...
\end{IEEEeqnarray}
where the multinomial operator $\Mcal$ is used. Observe that the empirical process is equivalent to a multinomial process within a scale factor.

The first and second joint moments of the conditional empirical distribution are \cite{theodoridis-ML}
\begin{IEEEeqnarray}{rCl}
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\uppsi | \uptheta}(y,x,y',x')  & = & \frac{1}{N} \big( \uptheta(y,x) \delta[y,y'] \delta[x,x'] - \uptheta(y,x) \uptheta(y',x') \big) \;.
\end{IEEEeqnarray}
The first and second moments of the Empirical distribution are proportionate to those of the Multinomial distribution.

\todolo{trends with N? Figures??}

Also, using the maximum likelihood estimate of a Multinomial distribution \cite{rao}, the maximum likelihood estimate of $\theta$ given the training data empirical distribution is simply
\begin{IEEEeqnarray}{rCl}
\theta_\mathrm{ML}\big( \psi \big) & = & \argmax_{\theta \in \Uptheta} \Prm_{\uppsi | \uptheta}(\psi | \theta) \\ \nonumber
& = & \psi \;.
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
where the dependency on $\uptheta$ is only through the marginal model $\upthetam$. Additionally, note that the dependency on the training observations $\Xrm$ is expressed though a ``marginal'' counting function $\Psim : \Xcal^N \mapsto \Uppsim \subset \Pcal(\Xcal)$ with range
\begin{IEEEeqnarray}{rCl}
...
& \equiv & \prod_{x \in \Xcal} \left[ \prod_{y \in \Ycal} \thetac(y;x)^{N \Psi(y,x;Y,X)} \right] \nonumber \\
& \equiv & \prod_{x \in \Xcal} \left( \prod_{y \in \Ycal} \thetac(y;x)^{\Psic(y;x;Y,X)} \right)^{N \Psim(x;X)} \nonumber \;,
\end{IEEEeqnarray}
where the function $\Psic : \{\Ycal \times \Xcal\}^N \mapsto \Uppsic \subset \Pcal(\Ycal)^{\Xcal}$ with range
\begin{equation}
\Uppsic = \bigcup_{p \in \Uppsim} \left\{ \frac{n}{N p(x)} : n \in {\Zbbgeq}^{\Ycal}, \ \sum_{y \in \Ycal} n(y) = N p(x) \right\}^{\Xcal}
\end{equation}
\todohigh{FIX $\Uppsic$}
is defined as
\begin{equation}
...
Analagous to the decomposition of the model $\uptheta$ into its marginal and conditional models, the empirical process $\uppsi$ can be decomposed into marginal and conditional empirical processes. Introduce the ``marginalized'' random process $\uppsim$ over the set $\Xcal$, defined as $\uppsim \equiv \sum_{y \in \Ycal} \uppsi(y,\cdot) \equiv \Psim(\Xrm) \in \Uppsim$. Similar to the Multinomial random processes \cite{johnson}, the Empirical random process has an aggregation property (Appendix \ref{app:emp}) - using this principle, it can be shown that conditioned on the model $\uptheta$, the marginal model is distributed as $\uppsim | \upthetam,\upthetac = \uppsim | \upthetam \sim \Emp(N,\upthetam)$. 

\todohigh{EDIT HERE}

Also of interest is the distribution of $\uppsic \equiv$ conditioned on its aggregation $\uppsim$. Using the Empirical process properties proven in Appendix \ref{app:emp}, it can be shown that when conditioned on the model $\uptheta$ as well, the PMF of $\uppsi$ is
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim , \uptheta}(\psi | \psim , \theta) & \equiv & \prod_{x \in \Xcal} \Bigg[ \Mcal\big( \psi(\cdot,x) \big) \prod_{y \in \Ycal} \thetac(y;x)^{\psi(y,x)} \Bigg] \\
& = & \prod_{x \in \Xcal} \Multi\Big( \psi(\cdot,x) ; n'(x) , \thetac(x) \Big) \nonumber \;,
\end{IEEEeqnarray}
over the domain $\left\{ \psi \in {\Zbbgeq}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \psi(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Multinomial, such that $\uppsi(\cdot,x) | \uppsim(x),\uptheta \sim \Multi\big( \uppsim(x),\upthetac(x) \big)$. Furthermore, the dependency on $\uptheta$ is expressed through the conditional model $\upthetac$
...
\todomid{use marginal/conditional model? D or nbar?}
...
\todolo{BELOW, add forumla for f(D)?}
...
Using the marginal and conditional model representations, the Bayesian predicitive distributions analagous to the clairvoyant distributions can now be simplified as $\Prm_{\xrm | \Drm} = \mu_{\upthetam | \Drm}$ and $\Prm_{\yrm | \xrm,\Drm} = \mu_{\upthetac(\xrm) | \xrm,\Drm}$.

\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \Yrm,\Xrm}(\thetam,\thetac | Y,X) & = & \frac{\Prm_{\Yrm | \Xrm,\upthetac}(Y | X,\thetac)}{\Prm_{\Yrm | \Xrm}(Y | X)} 
\frac{\Prm_{\Xrm | \upthetam}(X | \thetam)}{\Prm_{\Xrm}(X)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi}(\thetam,\thetac | \psi) & = & \frac{\Prm_{\uppsi | \uppsim,\upthetac}\big( \psi | \sum_y \psi(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \uppsim}\big( \psi | \sum_y \psi(y,\cdot) \big)} 
\frac{\Prm_{\uppsim | \upthetam}\big( \sum_y \psi(y,\cdot) | \thetam \big)}{\Prm_{\uppsim}\big( \sum_y \psi(y,\cdot) \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\uppsim}\Big( \thetac | \psi,\sum_y \psi(y,\cdot) \Big) \prm_{\upthetam | \uppsim}\Big( \thetam | \sum_y \psi(y,\cdot) \Big) 
\frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \uppsim}\big( \thetac | \sum_y \psi(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}

% Note that $\prm_{\upthetac | \Xrm}(X) = \prm_{\upthetac | \uppsim}\big( \Psim(X) \big) = \Erm_{\upthetam | \Xrm}\big[ \prm_{\upthetac | \upthetam} \big](X)$.

Note that $\prm_{\upthetac | \Xrm} = \Erm_{\upthetam | \Xrm}\big[ \prm_{\upthetac | \upthetam} \big]$ and $\prm_{\upthetac | \uppsim} = \Erm_{\upthetam | \uppsim}\big[ \prm_{\upthetac | \upthetam} \big]$. Also, note that $\Prm_{\Yrm | \Xrm} = \Erm_{\upthetam | \Xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and $\Prm_{\uppsi | \uppsim} = \Erm_{\upthetam | \uppsim} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\uppsi | \uppsim,\upthetac} \big] \Big]$.


\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \Yrm,\Xrm,\xrm}(\thetam,\thetac | Y,X,x) & = & \frac{\Prm_{\Yrm | \Xrm,\upthetac}(Y | X,\thetac)}{\Prm_{\Yrm | \Xrm,\xrm}(Y | X,x)} 
\frac{\Prm_{\Xrm,\xrm | \upthetam}(X,x | \thetam)}{\Prm_{\Xrm,\xrm}(X,x)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi,\xrm}(\thetam,\thetac | \psi,x) & = & \frac{\Prm_{\uppsi | \uppsim,\upthetac}\big( \psi | \sum_y \psi(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \uppsim,\xrm}\big( \psi | \sum_y \psi(y,\cdot),x \big)} 
\frac{\Prm_{\uppsim,\xrm | \upthetam}\big( \sum_y \psi(y,\cdot),x | \thetam \big)}{\Prm_{\uppsim,\xrm}\big( \sum_y \psi(y,\cdot),x \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\uppsim}\Big( \thetac | \psi,\sum_y \psi(y,\cdot) \Big) \prm_{\upthetam | \uppsim,\xrm}\Big( \thetam | \sum_y \psi(y,\cdot),x \Big) \nonumber \\
&& \quad \frac{\Prm_{\uppsi | \uppsim}\big( \psi | \sum_y \psi(y,\cdot) \big)}{\Prm_{\uppsi | \uppsim,\xrm}\big( \psi | \sum_y \psi(y,\cdot),x \big)} \frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \uppsim}\big( \thetac | \sum_y \psi(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}

Note that $\Prm_{\Yrm | \Xrm,\xrm} = \Erm_{\upthetam | \Xrm,\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and also that $\Prm_{\uppsi | \uppsim,\xrm} = \Erm_{\upthetam | \uppsim,\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\uppsi | \uppsim,\upthetac} \big] \Big]$.
...
\todohi{marginal/condtitional??? D or nbar?}
...
\end{IEEEeqnarray}
Also, for $\alpha_0 \alpha(y,x) > 1$, the maximizing value of the distribution is
\begin{equation}
\theta_\mathrm{max} = \argmax_{\theta \in \Uptheta} \prm_{\uptheta}(\theta) = \frac{\alpha_0 \alpha - 1}{\alpha_0 - |\Ycal||\Xcal|} \;.
\end{equation}
...
\end{IEEEeqnarray}
which distributes its weight among the $|\Ycal| |\Xcal|$ models with an $\ell_0$ norm satisfying $\| \theta \|_0 = 1$. Note that the Dirac delta for these formulas is defined on the set $\Uptheta$, such that $\int_{\Uptheta} \delta(\theta) \mathrm{d}\theta = 1$.
...
The marginal distribution $\upthetam$ and the conditional distribution $\upthetac$ will also be of interest. To keep notation simple, it is helpful to introduce the bijection $\alpha \Leftrightarrow (\alpham,\alphac)$, where $\alpham \equiv \sum_{y \in \Ycal} \alpha(y,\cdot)$, and $\alphac(x) = \alpha(\cdot,x) / \alpham(x)$ for each $x \in \Xcal$. Observe that $\alpham \in \Pcal(\Xcal)$ and $\alphac \in \Pcal(\Ycal)^{\Xcal}$.


By the aggregation property \cite{ferguson}, $\upthetam \sim \Dir(\alpha_0,\alpham)$ is a Dirichlet random process parameterized by concentation $\alpha_0$ and distribution $\alpham$; observe that $\Prm_{\xrm} = \mu_{\upthetam} = \alpham$.

Also of interest is the distribution of the predictive model $\upthetac$ conditioned on the marginal $\upthetam$. As demonstrated in Appendix \ref{app:Dir_agg}, these random processes are jointly distributed as
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \upthetam}(\thetac | \thetam) & = & \prod_{x \in \Xcal} \Bigg[ \beta\big( \alpha_0 \alpham(x) \alphac(x) \big)^{-1} \prod_{y \in \Ycal} \thetac(y;x)^{\alpha_0 \alpham(x) \alphac(y;x) - 1} \Bigg] \\
...
\todohi{Keep normal alpha in exponents above??}
...
& = & \Erm_{\uptheta} \left[ \prod_{y \in \Ycal} \prod_{x \in \Xcal} \uptheta(y,x)^{\Psi(y,x;D)} \right] \nonumber \\
& = & \beta(\alpha_0 \alpha)^{-1} \beta \left(  \alpha + \Psi(D) \right) \nonumber \;.
\end{IEEEeqnarray}
...
Next, the distribution of the sufficient statistic $\uppsi$ will be represented. As a Dirichlet distribution characterizes the parameters of the multinomial distribution $\Prm_{\Drm | \uptheta}$, the marginal PMF of $\uppsi$ is a Dirichlet-Multinomial distribution \cite{johnson} parameterized by $\alpha$,
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi}(\psi) & = & \Mcal(N \psi) \beta(\alpha_0 \alpha)^{-1} \beta(\alpha + \psi) \\
& = & \DM\big( \psi; N,\alpha \big) \nonumber \;.
\end{IEEEeqnarray}

The first and second joint moments of $\uppsi$ are
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi} & = & N \alpha = N \mu_\uptheta 
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{L}
...
\begin{IEEEeqnarray}{rCl}
\Sigma_{\uppsi}(y,x,y',x') & = & \frac{N (\alpha_0+N)}{\alpha_0+1} \big( \mu_\uptheta(y,x) \delta[y,y'] \delta[x,x'] - \mu_\uptheta(y,x) \mu_\uptheta(y',x') \big) \\
& = & N (\alpha_0+N) \Sigma_{\uptheta}(y,x,y',x') \nonumber \;.
\end{IEEEeqnarray}


Again, the data PMF's for minimal and maximal concentration $\alpha_0$ are relevant. For $\alpha_0 \to \infty$, the model PDF $\prm_{\uptheta}$ concentrates at its mean and thus $\uppsi$ is characterized by a multinomial distribution,
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi}(\psi) & \to & \Mcal(N \psi) \prod_{y \in \Ycal} \prod_{x \in \Xcal} \left(\alpha(y,x)\right)^{N \psi(y,x)}
\end{IEEEeqnarray}
Conversely, for $\alpha_0 \to 0$, the PMF tends toward
\begin{IEEEeqnarray}{rCl} \label{eq:P_n_lim_zero}
\Prm_{\uppsi}(\psi) & \to & \sum_{y \in \Ycal} \sum_{x \in \Xcal} \alpha(y,x) \delta\big[ \psi , N \delta[\cdot,y] \delta[\cdot,x] \big] \;.
\end{IEEEeqnarray}

PGR: formal proofs for limiting PMFs? stirling/gautschi?

Figure \ref{fig:P_nbar_a0} displays the distribution of $\uppsi$ for $N=10$ and different model concentrations $\alpha_0$. Observe that for large $\alpha_0$, the distribution approaches a multinomial distribution $\uppsi \sim \Multi(N,\alpha/\alpha_0)$. Figure \ref{fig:P_nbar_N} shows how a specific model prior influences the data PMF differently for different $N$. Observe that as the number of training samples increases, the PMF $\Prm_{\uppsi}$ tends toward $\Prm_{\uppsi}(\psi) \approx N^{1-|\Ycal||\Xcal|}\prm_{\uptheta}(\psi/N)$; this can be proven using Gautschi's inequality \cite{wendel}.
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{P_nbar_a0.pdf}
\caption{$\Prm(\uppsi)$ for different prior concentrations $\alpha_0$}
\label{fig:P_nbar_a0}
\end{figure}
\begin{figure}
\centering
...
\begin{IEEEeqnarray}{rCl} 
\Prm_{\Drm}(D) & = & \Mcal\big( \{N,|\Ycal||\Xcal|-1\} \big)^{-1} \Mcal\big( \Psi(D) \big)^{-1}
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{rCl}
...
& = & \Erm_{\upthetam} \left[ \prod_{x \in \Xcal} \upthetam(x)^{N \Psim(x;X)} \right] \nonumber \\
& = & \beta(\alpha_0 \alpham)^{-1} \beta\big( \alpha_0 \alpham + \Psim(X) \big) \nonumber \;.
\end{IEEEeqnarray}
...
& = & \prod_{x \in \Xcal} \Erm_{\upthetac(x)}\left[ \prod_{y \in \Ycal} \upthetac(y;x)^{\Psi(y,x;Y,X)} \right] \nonumber \\
& = & \prod_{x \in \Xcal} \beta\big(\alpha(\cdot,x)\big)^{-1} \beta\big( \alpha(\cdot,x) + \Psi(\cdot,x;Y,X) \big) \nonumber \;.
\end{IEEEeqnarray}

The corresponding distributions for the sufficient statistics will be expressed as well. Recall that $\uppsim | \uptheta \sim \Multi(N,\upthetam)$; by the aggregation property of Dirichlet-Multinomial functions \cite{johnson}, the random process is distributed as $\uppsim \sim \DM(N,\alpha')$.

Also of interest is the distribution of $\uppsi$ conditioned on its aggregation $\uppsim$. Using the Dirichlet-Multinomial properties presented in Appendix \ref{app:DM_agg}, it can be shown that
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim}(\psi | \psim) & = & \prod_{x \in \Xcal} \left[ \Mcal\big( \psi(\cdot,x) \big) \beta\big( \alpha(\cdot,x) \big)^{-1} \beta\big( \alpha(\cdot,x) + \psi(\cdot,x) \big) \right] \\
& = & \prod_{x \in \Xcal} \DM\Big( \psi(\cdot,x) ; n'(x) , \alpha(\cdot,x) \Big) \nonumber
\end{IEEEeqnarray}
over the domain $\left\{ \psi \in {\Zbbgeq}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \psi(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Dirichlet-Multinomial, such that $\uppsi(\cdot,x) | \uppsim(x) \sim \DM\big( \uppsim(x),\alpha(\cdot,x) \big)$.
...
\begin{IEEEeqnarray}{rCl}
\prm_{\uptheta | \Drm}(\theta | D) & = & \beta \left( \alpha + \Psi(D) \right)^{-1} \prod_{y \in \Ycal} \prod_{x \in \Xcal} 
\theta(y,x)^{\alpha(y,x) + \Psi(y,x;D) - 1} \;, 
\end{IEEEeqnarray}
a Dirichlet distribution with parameter function $\alpha + \Psi(D)$.
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm,\xrm | \Drm} & = & \mu_{\uptheta | \Drm} = \frac{\alpha + \Psi(\Drm)}{\alpha_0 + N} \\
& = & \left(\frac{\alpha_0}{\alpha_0 + N}\right) \alpha + \left(\frac{N}{\alpha_0 + N}\right) \frac{\Psi(\Drm)}{N} \nonumber \\
& = & \left(\frac{\alpha_0}{\alpha_0 + N}\right) \Prm_{\yrm,\xrm} + \left(\frac{N}{\alpha_0 + N}\right) \frac{\Psi(\Drm)}{N} \nonumber \;.
\end{IEEEeqnarray}
This is a mixture distribution of the prior expectation $\mu_{\uptheta} = \alpha/\alpha_0$ and the empirical distribution $\Psi(\Drm)/N$. The more informative the model prior (i.e. larger $\alpha_0$), the more the prior mean is favored; the more data, the more the empirical PMF is favored. The marginal distribution for $\xrm$ given $\Drm$ is
\begin{IEEEeqnarray}{rCl}
...
\Prm_{\yrm | \xrm,\Drm} & = & \frac{\alpha(\cdot,\xrm) + \Psi(\cdot,\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \\
& = & \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} + \left(\frac{N'(\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \frac{\Psi(\cdot,\xrm;\Drm)}{N'(\xrm;\Drm)} \nonumber \\
& = & \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \Prm_{\yrm | \xrm} + \left(\frac{N'(\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \frac{\Psi(\cdot,\xrm;\Drm)}{N'(\xrm;\Drm)} \nonumber \;.
\end{IEEEeqnarray}
...
\Prm_{\yrm | \xrm,\Drm} & = & \frac{\Psi(\cdot,\xrm;\Drm)+1}{N'(\xrm;\Drm) + |\Ycal|} \\
& = & \left( \frac{|\Ycal|}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{1}{|\Ycal|} + \nonumber \\
&& \quad \left( \frac{N'(\xrm;\Drm)}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{\Psi(\cdot,\xrm;\Drm)}{N'(\xrm;\Drm)} \nonumber \;.
\end{IEEEeqnarray}
...
The Bayesian distributions $\Prm_{\xrm | \Drm}$ and $\Prm_{\yrm | \xrm,\Drm}$ can also be found from the posterior distributions $\prm_{\upthetam | \Drm}$ and $\prm_{\upthetac | \xrm,\Drm}$, respectively. As the Dirichlet assumption renders $\upthetam$ and $\upthetac$ independent, it can be shown that $\Prm_{\Yrm | \Xrm} = \Erm_{\upthetac}\big[ \Prm_{\Yrm | \Xrm,\upthetac} \big]$ and thus that $\upthetam$ is conditionally independent of $\Yrm$ given $\Xrm$. Furthermore, the Dirichlet distribution $\prm_{\upthetam}$ is the conjugate prior for $\Prm_{\Xrm | \upthetam}$. As a result, $\upthetam | \Drm \sim \Dir(\alpha' + \Psim(\Xrm))$ and
\begin{IEEEeqnarray}{rCl}
\Prm_{\xrm | \Drm}(D) & = & \mu_{\upthetam | \Drm}(D) = \mu_{\upthetam | \Xrm}(X) \\
& = & \frac{\alpha' + \Psim(X)}{\alpha_0 + N} \nonumber \;.
\end{IEEEeqnarray}
Similarly, the distribution can be expressed in terms of the empirical PMF sufficient statistic as
\begin{IEEEeqnarray}{rCl}
\Prm_{\xrm | \uppsi}(\psi) & = & \mu_{\upthetam | \uppsi}(\psi) = \mu_{\upthetam | \uppsim}\left( \sum_y \psi(y,\cdot) \right) \\
& = & \frac{\alpha' + \sum_y \psi(y,\cdot)}{\alpha_0 + N} \nonumber \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \Drm,\xrm}\big( \thetac | D,x \big) & = & \prm_{\upthetac | \Drm}\big( \thetac | D \big) = \prod_{x' \in \Xcal} \prm_{\upthetac(x') | \Drm}\big(\thetac(x') | D \big) \\
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha(\cdot,x') + \Psi(\cdot,x';D) \big) \nonumber
\end{IEEEeqnarray}
and the distinct model conditional PMF's are independent from one another. A similar treatment demonstrates that
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \uppsi,\xrm}\big( \thetac | \psi,x \big) & = & \prm_{\upthetac | \uppsi}\big( \thetac | \psi \big) = \prod_{x' \in \Xcal} \prm_{\upthetac(x') | \uppsi(\cdot,x')}\big(\thetac(x') | \psi(\cdot,x') \big) \\
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha(\cdot,x') + \psi(\cdot,x') \big) \nonumber \;.
\end{IEEEeqnarray}
Observe that when the conditioning is performed using the sufficient statistic, the independent conditional models $\upthetac(x)$ are only dependent on their corresponding subset of the  empirical PMF, $\uppsi(\cdot,x)$.

The Bayes predictive PMF can thus be expressed as
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\Drm}(x,D) & = & \mu_{\upthetac(x) | \xrm,\Drm}(x,D) = \mu_{\upthetac(x) | \Drm}(D) \\
& = & \frac{\alpha(\cdot,x) + \Psi(\cdot,x;D)}{\alpha'(x) + N'(x;D)} \nonumber
\end{IEEEeqnarray}
or, via the sufficient statistic,
\begin{IEEEeqnarray}{rCl}
\Prm_{\yrm | \xrm,\uppsi}(x,\psi) & = & \mu_{\upthetac(x) | \xrm,\uppsi}(x,\psi) = \mu_{\upthetac(x) | \uppsi(\cdot,x)}\big(\psi(\cdot,x)\big) \\
& = & \frac{\alpha(\cdot,x) + \psi(\cdot,x)}{\alpha'(x) + \sum_y \psi(y,x)} \nonumber \;.
\end{IEEEeqnarray}
...
It is instructive to treat the distribution $\Prm_{\yrm | \xrm,\uppsi}$ as an estimate of the unknown conditional PMF $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and investigate the effects of informative prior knowledge. For a given $\xrm$ and corresponding number of training samples $\uppsim(\xrm)$, the expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
\Erm_{\uppsi | \uppsim,\uptheta}\big[ \Prm_{\yrm | \xrm,\uppsi} \big] 
= \Erm_{\uppsi(\cdot,\xrm) | \uppsim(\xrm),\upthetac(\xrm)}\big[ \Prm_{\yrm | \xrm,\uppsi(\cdot,\xrm)} \big] \\ 
\quad = \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)}\right) \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} + \left(\frac{\uppsim(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)}\right) \upthetac(\xrm) \nonumber \;,
\end{IEEEeqnarray}
where the properties of a multinomial distribution conditioned on its aggregation have been used. The result is a convex combination of the conditional data-independent distribution $\alpha(\cdot,x) / \alpha'(x)$ and the true conditional distribution $\upthetac(\xrm)$. The convex coefficients are dependent on the ``marginal'' values $\alpha'$ and $\uppsim$; note that as the number of matching training samples $\uppsim(x)$ increases relative to $\alpha'(x)$, the estimate tends towards the true conditional PMF. 

PGR: suppress delta dependency on nbar and theta?

To aid characterization of the estimator, define the random process $\Delta(\xrm,\uppsi,\uptheta) \equiv \Prm_{\yrm | \xrm,\uppsi} - \Prm_{\yrm | \xrm,\uptheta} \in \Rbb^{\Ycal}$. For a given $\xrm$ and corresponding number of training samples $\uppsim(\xrm)$, the bias of the conditional PMF estimate is
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_bias}
\mathrm{Bias}(\xrm,\uppsim,\uptheta) & = & \Erm_{\uppsi | \uppsim,\uptheta}\big[ \Delta(\xrm,\uppsi,\uptheta) \big] \\
& = & \frac{\alpha'(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)} \left( \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} - \upthetac(\xrm) \right) \nonumber 
\end{IEEEeqnarray}
and its covariance function is 
\begin{IEEEeqnarray}{L} \label{eq:predictive_cov}
\mathrm{Cov}(y,y';\xrm,\uppsim,\uptheta) = \Crm_{\uppsi | \uppsim,\uptheta} \big[\Prm_{\yrm | \xrm,\uppsi}(\cdot | \xrm,\uppsi) \big](y,y') \\
\quad = \frac{\Sigma_{\uppsi(\cdot,\xrm) | \uppsim(\xrm),\upthetac(\xrm)}(y,y')}{\big( \alpha'(\xrm) + \uppsim(\xrm) \big)^2} \nonumber \\
\quad = \frac{\uppsim(\xrm)}{\big( \alpha'(\xrm) + \uppsim(\xrm) \big)^2} \left( \upthetac(y;\xrm) \delta[y,y'] - \upthetac(y;\xrm) \upthetac(y';\xrm) \right) \nonumber \;,
\end{IEEEeqnarray}
where the properties of multinomial random processes have been used. Note that the bias is proportionate to the difference between the true conditional model and the data-independent estimate. The scaling factor tends from one to zero as $\uppsim(x)/\alpha'(x)$ tends from zero to infinity; as such, more informative priors (large $\alpha'(x)$) will lead to PMF estimates that are prone to bias. Conversely, the variance of the PMF estimate tends to zero as $\alpha'(x) \to \infty$. 


Combining the estimator bias and variance, the conditional second moments of $\Delta(\xrm,\uppsi,\uptheta)$ are
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_del_sq}
\mathcal{E}(y,y' ; \xrm,\uppsim,\uptheta) & = & \Erm_{\uppsi | \uppsim,\uptheta} \Big[ \Delta(y;\xrm,\uppsi,\uptheta) \Delta(y';\xrm,\uppsi,\uptheta) \Big] \\
& = & \mathrm{Bias}(y;\xrm,\uppsim,\uptheta) \mathrm{Bias}(y';\xrm,\uppsim,\uptheta) + \mathrm{Cov}(y,y';\xrm,\uppsim,\uptheta) \nonumber \;.
\end{IEEEeqnarray}
As $\uppsim(x) \to \infty$, this function tends to zero and thus the underlying model $\upthetac(x)$ is determined precisely. A more practical case is estimation with a finite volume of training data. Specification of the Dirichlet model prior can be interpreted as providing a distribution estimate $\alpha(\cdot,x)/\alpha'(x)$ and a confidence level $\alpha'(x)$. Higher confidence reduces error due to the variance of the estimator, but increases the error due to bias between the true model and its estimate; low confidence renders the estimate unbiased, but maximizes the estimator variance. 



Also of interest, the conditional expectation of $\mathcal{E}(\cdot,\cdot;\xrm,\uppsim,\uptheta)$ is
\begin{IEEEeqnarray}{L} \label{eq:predictive_E_del_sq}
\Erm_{\xrm,\uppsim | \uptheta}\Big[ \mathcal{E}(y,y' ; \xrm,\uppsim,\uptheta) \Big] \\
\quad = \Erm_{\xrm | \uptheta}\left[ \Erm_{\uppsim(\xrm) | \uptheta}\left[ \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)}\right)^2 \right] \left( \frac{\alpha(y,\xrm)}{\alpha'(\xrm)} - \upthetac(y;\xrm) \right) \left( \frac{\alpha(y',\xrm)}{\alpha'(\xrm)} - \upthetac(y';\xrm) \right) \right] \nonumber \\
\qquad + \Erm_{\xrm | \uptheta}\left[ \Erm_{\uppsim(\xrm) | \uptheta}\left[ \frac{\uppsim(\xrm)}{\big( \alpha'(\xrm) + \uppsim(\xrm) \big)^2} \right] \left( \upthetac(y;\xrm) \delta[y,y'] - \upthetac(y;\xrm) \upthetac(y';\xrm) \right) \right] \nonumber \;.
\end{IEEEeqnarray}



To exemplify how the model estimate $\Prm_{\yrm | \xrm,\uppsi}$ approximates $\Prm_{\yrm | \xrm,\uptheta}$, consider a scenario with $|\Ycal| = 10$. The data-independent PMF $\alpha(\cdot,x) / \alpha'(x)$ and true model $\upthetac(x)$ are shown in Figure \ref{fig:P_yx_error_N_0} - note the significant mismatch. 

PGR: ABOVE - STATE XCAL cardinality = 1

Figures \ref{fig:P_yx_error_a0_0_1} and \ref{fig:P_yx_error_a0_10} show how the bias and variance of the estimate change for different values of $\uppsim(x)$ and $\alpha'(x)$. The plot markers represent the conditional mean of the estimator, $\Erm_{\uppsi | \uppsim,\uptheta}\big[ \Prm_{\yrm | \xrm,\uppsi}(y | \xrm,\uppsi) \big]$; the upper and lower error bars represent the square-root of the expected squared deviation above and below the conditional mean, respectively. Each individual plot heading provides the error $\sqrt{\sum_{y \in \Ycal} \mathcal{E}(y,y ; \xrm,\uppsim,\uptheta)}$ to assess the quality of the PMF estimate. 

Observe that for $\uppsim(x) = 1$, the high variance of the $\alpha'(x) = 0.1$ estimate (favoring the empirical PMF) renders it worse than the $\alpha_0 = 10$ estimate; in fact, the variance is so high that the error exceeds that of the data-independent estimate $\alpha(\cdot,x) / \alpha'(x)$ (Figure \ref{fig:P_yx_error_N_0}). Conversely, for $\uppsim(x) = 10$, the confidence of the $\alpha'(x) = 10$ estimate leads to high bias and the $\alpha'(x) = 0.1$ estimate is superior. For $\uppsim(x) = 100$, both the $\alpha'(x) = 0.1$ and $\alpha'(x) = 10$ estimates begin converging to the true distribution - this is guaranteed due to the full support of the Dirichlet prior.
...
f^*(\xrm;\Drm) = \argmin_{h \in \Hcal} \Erm_{\yrm | \xrm,\Drm} \big[ \Lcal(h,\yrm) \big] \\
= \argmin_{h \in \Hcal} \frac{\sum_{y \in \Ycal} \alpha(y,\xrm) \Lcal(h,y) + \sum_{y \in \Ycal} \Psi(y,\xrm;\Drm) \Lcal(h,y)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \nonumber \\
= \argmin_{h \in \Hcal} \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \sum_{y \in \Ycal} \frac{\alpha(y,\xrm)}{\alpha'(\xrm)} \Lcal(h,y) + \left(\frac{N'(\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)}\right) \sum_{y \in \Ycal} \frac{\Psi(y,\xrm;\Drm)}{N'(\xrm;\Drm)} \Lcal(h,y) \nonumber \\
...
& = & \left( \frac{\alpha'(\xrm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \right) \sum_{y \in \Ycal} y \frac{\alpha(y,\xrm)}{\alpha'(\xrm)} + \left( \frac{N'(\xrm;\Drm)}{\alpha'(\xrm) + N'(\xrm;\Drm)} \right) \sum_{y \in \Ycal} y \frac{\Psi(y,\xrm;\Drm)}{N'(\xrm;\Drm)} \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
f^*(\xrm;\Drm) & = & \left( \frac{|\Ycal|}{N'(\xrm;\Drm) + |\Ycal|} \right) \frac{1}{|\Ycal|} \sum_{y \in \Ycal} y + \left( \frac{N'(\xrm;\Drm)}{N'(\xrm;\Drm) + |\Ycal|} \right) \sum_{y \in \Ycal} y \frac{\Psi(y,\xrm;\Drm)}{N'(\xrm;\Drm)} \\
...
\begin{IEEEeqnarray}{rCl}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] & = & \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \frac{\big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \uppsim(\xrm) \big)^2} \right] \right] \\
& = & \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \uppsim(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
& = & \Erm_{\xrm} \left[ \Erm_{\uppsim} \left[ \frac{\alpha_0 \Erm_{\uppsi | \uppsim} \left[ \big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2 \right]}{\alpha'(\xrm) \big(\alpha'(\xrm) + \uppsim(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
& = & \ldots \nonumber \\
& = & \Erm_{\xrm} \left[ \frac{\alpha_0 \Erm_{\uppsim} \Big[ \uppsim(\xrm) \Erm_{\yrm|\xrm}[\yrm^2] + \big( \alpha'(\xrm) + \uppsim(\xrm) + 1 \big) \alpha'(\xrm) \mu_{\yrm|\xrm}^2 \Big]}{\alpha'(\xrm) \big(\alpha'(\xrm) + 1 \big) (\alpha_0+N)} \right] \nonumber \\
...
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha(y,\xrm)+\uppsi(y,\xrm) \big) \big(\alpha(y',\xrm) + \uppsi(y',\xrm) \big)}{\alpha'(\xrm) \big(\alpha'(\xrm) + \uppsim(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \Erm_{\uppsim} \left[ \frac{\alpha_0 \Erm_{\uppsi | \uppsim} \left[ \big( \alpha(y,\xrm)+\uppsi(y,\xrm) \big) \big(\alpha(y',\xrm)+\uppsi(y',\xrm) \big) \right]}{\alpha'(\xrm) \big(\alpha'(\xrm) + \uppsim(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
%= \ldots \nonumber \\
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \frac{\alpha_0 \Erm_{\uppsim} \Big[ \uppsim(\xrm) \alpha(y,\xrm) \delta[y,y'] + \big( \alpha'(\xrm) + \uppsim(\xrm) + 1 \big) \alpha(y,\xrm) \alpha(y',\xrm) \Big]}{\alpha'(\xrm)^2 \big(\alpha'(\xrm) + 1 \big) (\alpha_0+N)} \right] \nonumber \\
...
%= \sum_{x \in \Xcal} \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\uppsim} \left[ \frac{\Erm_{\uppsi | \uppsim} \left[ \big( \alpha(y,x)+\uppsi(y,x) \big) \big(\alpha(y',x)+\uppsi(y',x) \big) \right]}{(\alpha_0+N) \big(\alpha'(x) + \uppsim(x) \big)} \right] \nonumber \\
...
%\end{IEEEeqnarray}
The above formulation exploits the statistical characterization of the aggregation, $\uppsim \sim \DM(N,\alpha')$; also used is the property that the Dirichlet-Multinomial random process $\uppsi$ conditioned on its aggregation $\uppsim$ yields independent  conditional DM functions $\uppsi(\cdot,x) | \uppsim(x) \sim \DM\big( \uppsim(x),\alpha(\cdot,x) \big)$.
...
Evaluation of the excess risk for an estimator based on the Dirichlet prior will be performed using the sufficient statistic $\uppsi$ in place of the training set $\Drm$. Using the random process $\Delta(\xrm,\uppsi,\uptheta) \equiv \Prm_{\yrm | \xrm,\uppsi} - \Prm_{\yrm | \xrm,\uptheta} \in \Rbb^{\Ycal}$ introduced in \ref{sec:predictive_est}, the term is expressed as
...
\quad = \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\uppsi | \uptheta} \Big[ \Delta(y;\xrm,\uppsi,\uptheta) \Delta(y';\xrm,\uppsi,\uptheta) \Big] \nonumber \\
\quad = \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\uppsim | \uptheta}\Big[ \mathcal{E}(y,y' ; \xrm,\uppsim,\uptheta) \Big] \nonumber \\
\quad = \Erm_{\xrm | \uptheta}\left[ \Sigma_{\yrm | \xrm,\uptheta} \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\uppsim(\xrm)}{\big( \alpha'(\xrm) + \uppsim(\xrm) \big)^2} \right] \right] \nonumber \\
\qquad \qquad + \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)}\right)^2 \right] \right] \nonumber \;,
...
First consider how the excess risk changes with the training volume $N$. For $N=0$, it is evident that the excess risk is $\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) \to \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right]$,  the expected squared bias between the clairvoyant and data-independent estimators. As $N$ tends to infinity, the binomial distribution controlling the scaling factors  concentrates at $\uppsim(x) \approx N \thetam(x)$; as such, the two expectations of interest tend to zero and thus $\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) \to 0$. This desirable property of the estimator is a consequence of the full support of the Dirichlet prior, ensuring that the model posterior concentrates at the empirical PMF.
...
\begin{IEEEeqnarray}{rCl}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) & \to & \Erm_{\xrm | \uptheta}\left[ \Sigma_{\yrm | \xrm,\uptheta} \sum_{n=1}^N \binom{N}{n} \upthetam(\xrm)^n \big( 1 - \upthetam(\xrm) \big)^{N-n} \frac{1}{n} \right] \nonumber \\
&& \qquad + \Erm_{\xrm | \uptheta}\left[ \big( 1 - \upthetam(\xrm) \big)^N \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right] \nonumber \;.
\end{IEEEeqnarray}
Note that the first term's scaling factor is proportionate to the first inverse moment of a positive binomial random variable \cite{stephan}. The second term's scaling factor tends to $\Prm_{\uppsim(\xrm) | \upthetam(\xrm)}\big( 0 | \thetam(x) \big)$, the probability that no training samples are observed matching the value $\xrm$. As $N$ increases, this term tends to zero, the risk due to the prior estimate bias decreases, and the excess risk becomes a function of $\uptheta$ only.
...
\begin{equation}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \Erm_{\xrm | \uptheta}\left[ \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{1}{\uppsim(\xrm)\Sigma_{\yrm | \xrm,\uptheta}^{-1} + \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^{-2}} \right] \right] \;,
\end{equation}
...
%\Erm_{\xrm | \uptheta}\left[ \Sigma_{\yrm | \xrm,\uptheta} \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\uppsim(\xrm)}{\big( \alpha'(\xrm) + \uppsim(\xrm) \big)^2} \right] \right] \nonumber \\
%\qquad \qquad + \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \uppsim(\xrm)}\right)^2 \right] \right] \nonumber
...
& = & \argmax_{y \in \Ycal} \frac{\alpha(y,x) + \Psi(y,x;D)}{\alpha'(x) + N'(x;D)} \nonumber \\
& = & \argmax_{y \in \Ycal} \left( \alpha(y,x) + \Psi(y,x;D) \right) \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
f^*(x;D) & = & \argmax_{y \in \Ycal} \Psi(y,x;D) \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - \Erm_{\xrm,\Drm} \left[ \max_{y \in \Ycal} \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \right] \\
& = & 1 - \Erm_{\xrm,\uppsi} \left[ \frac{ \max_{y \in \Ycal} \big( \alpha(y,x) + \uppsi(y,x) \big)}{\alpha'(x) + \uppsim(x)} \right] \nonumber \\
& = & 1 - \sum_{x \in \Xcal} \frac{\Erm_{\uppsi} \Big[ \max_{y \in \Ycal} \big( \alpha(y,x) + \uppsi(y,x) \big) \Big]}{\alpha_0 + N} \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - \Erm_{\xrm,\Drm} \left[ \max_{y \in \Ycal} \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \right] \\
& = & 1 - \sum_{x \in \Xcal} \frac{\Erm_{\uppsi} \big[ \max_{y \in \Ycal} \uppsi(y,x) \big] + 1}{|\Ycal||\Xcal| + N} \nonumber \\
& = & 1 - \frac{1 + |\Xcal|^{-1} \sum_{x \in \Xcal} \Erm_{\uppsi} \big[ \max_{y \in \Ycal} \uppsi(y,x) \big]}{|\Ycal| + N/|\Xcal|} \nonumber \;.
\end{IEEEeqnarray}
...
To evaluate this expectation, new random variables $\uppsi_{\max}(x) \equiv \max_{y \in \Ycal} \uppsi(y,x)$ are introduced and characterized by their identical PMF. To this end, the probability of the event $\Prm\big( \uppsi_{\max}(x) \geq n \big) = \Prm\big( \cup_{y \in \Ycal} \{ \uppsi(y,x) \geq n \} \big)$ will be determined. As the distribution of $\uppsi$ is uniform, the event probability is proportionate to the cardinality of the set $\cup_{y \in \Ycal} \{ \psi: \psi(y,x) \geq n \}$. Using the inclusion-exclusion principle \cite{brualdi}, the cardinality is represented as
\begin{IEEEeqnarray}{L}
\big| \cup_{y \in \Ycal} \{ \psi : \psi(y,x) \geq n \} \big| \\
...
As the PMF of $\uppsi_{\max}(x)$ has support on $n \in [0,\ldots,N]$, the expectation over $\uppsi$ is evaluated as
\begin{IEEEeqnarray}{rCl}
\Erm_{\uppsi}\big[ \uppsi_{\max}(x) \big] & = & \sum_{n=0}^N n \Big( \Prm\big( \uppsi_{\max}(x) \geq n \big) - \Prm\big( \uppsi_{\max}(x) \geq n+1 \big) \Big) \\
& = & -1 + \sum_{n=0}^N \Prm\big( \uppsi_{\max}(x) \geq n \big) \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
\Rcal_{\Theta}(f ; \uptheta) & = & 1 - \sum_{x \in \Xcal} \upthetam(x) \Erm_{\uppsi | \uptheta} \bigg[ \upthetac\Big( \argmax_{y \in \Ycal} \big( \uppsi(y,x) + \alpha(y,x) \big) ;x \Big) \bigg] \;.
\end{IEEEeqnarray}
Figures \ref{fig:Risk_cond_01_Dir_N_leg_a0__subj_good} and \ref{fig:Risk_cond_01_Dir_N_leg_a0__subj_bad} show how the conditional risk trends for classifiers based on well-matched and poorly-matched informative Dirichlet priors, respectively. Note that the well-matched prior does better with higher prior concentrations $\alpha_0$; this is reflective of the fact that the maximizing arguments $y \in \Ycal$ of both the true model $\thetac(x)$ and the prior mean $\alpha(\cdot,x) / \alpha'(x)$ are the same.
\begin{figure}
\centering
...
\begin{IEEEeqnarray}{rCl}
\prm(\uptheta) & = & \beta(\alpha_0 \alpha)^{-1} \prod_{y \in \Ycal} \uptheta(y)^{\alpha(y) - 1} \;,
\end{IEEEeqnarray}

\begin{equation}
\beta(\alpha_0 \alpha) = \frac{\prod_{y \in \Ycal} \Gamma\big( \alpha(y) \big)}{\Gamma \left( \sum_{y \in \Ycal} \alpha(y) \right)} \;.
\end{equation}
...
The model is now a continuous-domain Dirichlet process $\uptheta \sim \DP(\alpha_0 \alpha)$. The concentration parameter is $\alpha_0 \equiv \int_{y \in \Ycal} \alpha(y) \mathrm{d} y$. By definition, for any partition of the set $\Ycal$, $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, we can generate a Dirichlet random process $\upphi(z) \equiv \int_{\Scal_z} \uptheta(y) \mathrm{d} y$ with parameterizing function $\lambda(z) \equiv \int_{\Scal_z} \alpha(y) \mathrm{d} y$. This is commonly referred to as the aggregation property. The PDF for the aggregation is thus
\begin{IEEEeqnarray}{rCl}
...
In Appendix \ref{app:DP_post}, it was shown that if the model $\uptheta \sim \DP(\alpha_0 \alpha)$ is a Dirichlet process, then the model conditioned on the training data $\Drm$ is also a Dirichlet process with parameterizing function $\alpha + \Psi(\Drm)$, where $\Psi(y;\Drm) = \sum_{n=1}^N \delta\left( y - \Drm_n \right)$ generalizes for a continuous domain. Thus, the conditional PDF of interest can be formulated as
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \Drm} & = & \mu_{\uptheta | \Drm} \\
& = & \frac{\alpha + \Psi(\Drm)}{\alpha_0 + N} = \frac{\alpha + \sum_{n=1}^N \delta\big( \cdot - \Drm_n \big)}{\alpha_0 + N} \nonumber \\
& = & \left(\frac{\alpha_0}{\alpha_0+N}\right) \alpha + \left(\frac{N}{\alpha_0+N}\right) \frac{\sum_{n=1}^N \delta\big( \cdot - \Drm_n \big)}{N} \nonumber \\
...
Define the Dirichlet-Multinomial process $\uppsi \equiv \Psi(\Drm)$. The mean and correlation functions below are found in Appendix \ref{app:DMP}. The mean function is
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi} & = & N \alpha 
\end{IEEEeqnarray}
and the correlation function is
\begin{IEEEeqnarray}{rCl}
\Erm_{\uppsi}\big[ \uppsi(y) \uppsi(y') \big] & = & \frac{N}{\alpha_0 (\alpha_0+1)} \big[ (N-1)\alpha(y) \alpha(y') + (\alpha_0+N) \alpha(y) \delta(y-y') \big] \;.
\end{IEEEeqnarray}
...
& = & \left( \frac{\alpha_0}{\alpha_0+N} \right) \mu_{\yrm} +  \left( \frac{N}{\alpha_0+N} \right) \int_{\Ycal} y \frac{\Psi(y;\Drm)}{N} \mathrm{d}y \nonumber \;.
\end{IEEEeqnarray}
...
& = & \Erm_{\uppsi} \left[ \Sigma_{\yrm | \uppsi} \right] \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{L}
\Erm_{\Drm}\big[ \mu_{\yrm | \Drm}^2 \big] = \Erm_{\uppsi}\big[ \mu_{\yrm | \uppsi}^2 \big] \\
\quad = \frac{\alpha_0^2 \mu_{\yrm}^2 + 2\alpha_0 \mu_{\yrm} \int_{\Ycal} y \mu_{\uppsi}(y) \big] \mathrm{d}y + \int_{\Ycal} \int_{\Ycal} y y'\Erm_{\uppsi}\big[ \uppsi(y)\uppsi(y') \big] \mathrm{d}y \mathrm{d}y'}{(\alpha_0+N)^2} \nonumber \\
...
The model is characterized by a Dirichlet process $\uptheta \sim \DP(\alpha_0 \alpha)$ with parameter function $\alpha : \Ycal \times \Xcal \mapsto \Rbb^+$. The concentration parameter generalizes to $\alpha_0 = \int_{\Ycal} \int_{\Xcal} \alpha(y,x) \mathrm{d} x \mathrm{d} y$. Using the aggregation property, any partition of the set $\Ycal \times \Xcal$, $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$ is a Dirichlet random process $\upphi(z) = \iint_{\Scal_z} \uptheta(y,x) \mathrm{d} x \mathrm{d} y$ with parameterizing function $\lambda(z) = \iint_{\Scal_z} \alpha(y,x) \mathrm{d} x \mathrm{d} y$. The PDF for the aggregation is
\begin{IEEEeqnarray}{rCl}
\prm_{\upphi}(\phi) & = & \beta(\lambda)^{-1} \prod_{z \in \Zcal} \phi(z)^{\lambda(z) - 1} \;.
\end{IEEEeqnarray}

The expected value of a Dirichlet process generalizes to $\DP(\alpha_0 \alpha)$
\begin{equation}
\mu_{\uptheta} = \alpha
\end{equation}
and the correlation function is
...
The properties of the Dirichlet distribution proven in Appendix \ref{app:DP_post} generalize, such that the model conditioned on the training data $\Drm$ is Dirichlet with parameterizing function $\alpha + \Psi(\Drm)$, where $\Psi(y,x;\Drm) = \sum_{n=1}^N \delta\left( y - \Yrm_n \right)\left( x - \Xrm_n \right)$. Recall that $\Drm_n = \big( \Yrm_n,\Xrm_n \big)$ with $Y \in \Ycal^N$ and $X \in \Xcal^N$.

The PDF $\prm_{\yrm,\xrm | \Drm}$ is thus
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm,\xrm | \Drm} & = & \mu_{\uptheta | \Drm} \\
& = & \frac{\alpha + \Psi(\Drm)}{\alpha_0 + N} \nonumber \\
& = & \frac{\alpha + \sum_{n=1}^N \delta\big( \cdot - \Yrm_n \big)\delta\big( \cdot - \Xrm_n \big)}{\alpha_0 + N} \nonumber \\
& = & \left(\frac{\alpha_0}{\alpha_0+N}\right) \alpha + \left(\frac{N}{\alpha_0+N}\right) \frac{\sum_{n=1}^N \delta\big( \cdot - \Yrm_n \big) \delta\big( \cdot - \Xrm_n \big)}{N} \nonumber
\end{IEEEeqnarray}
...
It is instructional to find the PDF's for the training output values $\Yrm$ given the input values $\Xrm$, as well as the marginal PDF for the input values alone. Observe that the PDF for $\Xrm$ can be represented as
\begin{IEEEeqnarray}{rCl}
\prm_{\Xrm}(X) & = & \Erm_{\uptheta}\big[ \prm_{\Xrm | \uptheta}(X | \uptheta) \big] = \Erm_{\uptheta}\left[ \prod_{n=1}^N \upthetam\big( X_n \big) \right] \\
& = & \frac{\alpha'\big( X_1 \big)}{\alpha_0} \prod_{n=2}^N \frac{\alpha'\big( X_n \big) + \sum_{i=1}^{n-1} \delta\big( X_n-X_i \big)}{\alpha_0+n-1} \nonumber
\end{IEEEeqnarray}
since, by the aggregation principle, $\upthetam = \int_{\Ycal} \uptheta(y,\cdot) \mathrm{d} y$ is a Dirichlet process with parameter function $\alpha': \Xcal \mapsto \Rbb^+$.
...
We have the DMP $\uppsi \equiv \Psi(\Drm)$ with mean and correlation functions
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi} & = & N \alpha 
\end{IEEEeqnarray}
and
...
Observe that by the aggregation principle, $\uppsim = \int_{\Ycal} \uppsi(y,\cdot) \mathrm{d}y \equiv \sum_{n=1}^N \delta\big( \cdot - \Xrm_n \big)$ is a DMP over the set $\Xcal$ with parametrizing function $\alpha' : \Xcal \mapsto \Rbb^+$.

Additionally, the 1-dimensional subsets conditioned on the marginalized DMP are characterized as

\begin{equation}
\frac{\uppsi(\cdot,x)}{\delta(0)} \Big| \uppsim(x) \sim \DMP\left( \frac{\uppsim(x)}{\delta(0)},\frac{\alpha(\cdot,x)}{\delta(0)} \right)
\end{equation}
...
= \frac{\int_{\Ycal} \alpha(y,\xrm) \Lcal(h,y) \mathrm{d}y + \int_{\Ycal} \Psi(y,\xrm;\Drm) \Lcal(h,y) \mathrm{d}y}{\alpha'(\xrm)+N'(\xrm;\Drm)} \nonumber \\
...
& = & \frac{\int_{\Ycal} y \big( \alpha(y,\xrm) + \Psi(y,\xrm;\Drm) \big) \mathrm{d}y}{\alpha'(\xrm) + N'(\xrm;\Drm)} \nonumber \\
...
\quad = \Erm_{\yrm}[\yrm^2] = \int_{\Ycal} y^2 \int_{\Xcal} \alpha(y,x) \mathrm{d} x \mathrm{d}y \nonumber \\
...
To perform the expectation over the Dirichlet-Multinomial process $\uppsi \sim \DMP(\alpha_0 \alpha)$, split the expectation into an expectation over the marginal DMP $\uppsim \sim \DMP(\alpha')$ and a conditional expectation over $\uppsi$ given $\uppsim$. The characterization of the conditional DMP is found in Appendix \ref{app:DM_agg}.

\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \\
\quad = \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \left( \frac{\alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y}{\alpha'(\xrm) + \uppsim(\xrm)} \right)^2 \right] \right] \nonumber \\
\quad = \Erm_{\xrm} \Erm_{\uppsim} \left[ \frac{\alpha_0 \Erm_{\uppsi | \uppsim} \left[ \left( \alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y \right)^2 \right] }{\alpha'(\xrm)\big(\alpha'(x) + \uppsim(x) \big) (\alpha_0+N)} \right] \nonumber
\end{IEEEeqnarray}


Evaluating the conditional expectation,
\begin{IEEEeqnarray}{L}
\Erm_{\uppsi | \uppsim} \left[ \left( \alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y \right)^2 \right] \\
\quad = \alpha'(\xrm)^2 \mu_{\yrm | \xrm}^2 + 2 \alpha'(\xrm) \mu_{\yrm | \xrm} \int_{\Ycal} y \delta(0) \frac{\uppsim(\xrm)}{\delta(0)} \frac{\delta(0)^{-1} \alpha(y,\xrm)}{\delta(0)^{-1} \alpha'(\xrm)} \mathrm{d}y \nonumber \\
\qquad + \delta(0)^2 \frac{\delta(0)^{-1} \uppsim(\xrm)}{\big( \delta(0)^{-1}\alpha'(\xrm) \big) \big( \delta(0)^{-1}\alpha'(\xrm) + 1 \big)} \int_{\Ycal} \int_{\Ycal} y y' \Bigg[ \left( \frac{\uppsim(\xrm)}{\delta(0)} - 1 \right) \frac{\alpha(y,\xrm)}{\delta(0)} \frac{\alpha(y',\xrm)}{\delta(0)} \nonumber \\
\qquad + \left( \frac{\alpha'(\xrm)}{\delta(0)} + \frac{\uppsim(\xrm)}{\delta(0)} \right) \frac{\alpha(y,\xrm)}{\delta(0)} \delta(y-y') \Bigg] \mathrm{d}y \mathrm{d}y' \nonumber \\
\quad = \alpha'(\xrm)^2 \mu_{\yrm | \xrm}^2 + 2 \alpha'(\xrm) \uppsim(\xrm) \mu_{\yrm | \xrm}^2 \nonumber \\
\qquad + \frac{\uppsim(\xrm)}{\alpha'(\xrm)\big( \alpha'(\xrm)+\delta(0) \big)} \Big[ \big( \uppsim(\xrm) - \delta(0) \big) \alpha'(\xrm)^2 \mu_{\yrm | \xrm}^2 + \delta(0) \big( \alpha'(\xrm)+\uppsim(\xrm) \big) \alpha'(\xrm) \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \Big] \nonumber \\
\quad = \frac{\alpha'(\xrm)+\uppsim(\xrm)}{\alpha'(\xrm)+\delta(0)} \Big[ \mu_{\yrm | \xrm}^2 \alpha'(\xrm) \big( \alpha'(\xrm) + \uppsim(\xrm) + \delta(0) \big) + \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \delta(0) \uppsim(\xrm) \Big] \nonumber
\end{IEEEeqnarray}

Plugging,
\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \\
\quad = \Erm_{\xrm} \left[ \frac{\alpha_0 \Erm_{\uppsim} \Big[ \mu_{\yrm | \xrm}^2 \alpha'(\xrm) \big( \alpha'(\xrm) + \uppsim(\xrm) + \delta(0) \big) + \Erm_{\yrm|\xrm}\big[ \yrm^2 \big] \delta(0) \uppsim(\xrm) \Big] }{\alpha'(\xrm) \big( \alpha'(\xrm)+\delta(0) \big) (\alpha_0+N)} \right] \nonumber \\ 
...
%\begin{IEEEeqnarray}{L}
%\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \\
%\quad = \Erm_{\xrm,\uppsi} \Bigg[ \left( \int_{\Ycal} y \prm_{\yrm | \xrm,\uppsi}(y | \xrm,\uppsi) \mathrm{d}y \right)^2 \Bigg] \nonumber \\
%\quad = \int_{\Xcal} \Erm_{\uppsi} \left[ \int_{\Ycal} y \prm_{\yrm,\xrm | \uppsi}(y,x | \uppsi) \mathrm{d}y \int_{\Ycal} y' \prm_{\yrm | \xrm,\uppsi}(y' | x,\uppsi) \mathrm{d}y' \right] \mathrm{d}x \nonumber \\ 
%\quad = \int_{\Xcal} \Erm_{\uppsim} \left[ \frac{ \Erm_{\uppsi | \uppsim} \left[ \left( \alpha'(x) \mu_{\yrm | \xrm}(x) + \int_{\Ycal} y \uppsi(y,x) \mathrm{d}y \right)^2 \right] }{(\alpha_0+N) \big(\alpha'(x) + \uppsim(x) \big)} \right] \mathrm{d}x \nonumber
%\end{IEEEeqnarray}
%
%
%Evaluating the conditional expectation,
%\begin{IEEEeqnarray}{L}
%\Erm_{\uppsi | \uppsim} \left[ \left( \alpha'(x) \mu_{\yrm | \xrm}(x) + \int_{\Ycal} y \uppsi(y,x) \mathrm{d}y \right)^2 \right] \\
%\quad = \alpha'(x)^2 \mu_{\yrm | \xrm}^2(x) + 2 \alpha'(x) \mu_{\yrm | \xrm}(x) \int_{\Ycal} y \delta(0) \frac{\uppsim(x)}{\delta(0)} \frac{\delta(0)^{-1} \alpha(y,x)}{\delta(0)^{-1} \alpha'(x)} \mathrm{d}y \nonumber \\
...
%\qquad + \left( \frac{\alpha'(x)}{\delta(0)} + \frac{\uppsim(x)}{\delta(0)} \right) \frac{\alpha(y,x)}{\delta(0)} \delta(y-y') \Bigg] \mathrm{d}y \mathrm{d}y' \nonumber \\
...
%\begin{IEEEeqnarray}{L}
%\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \\
%\quad = \int_{\Xcal} \frac{ \Erm_{\uppsim} \Big[ \mu_{\yrm | \xrm}^2(x) \alpha'(x) \big( \alpha'(x) + \uppsim(x) + \delta(0) \big) + \Erm_{\yrm|\xrm}\big[ \yrm^2 \big](x) \delta(0) \uppsim(x) \Big] }{(\alpha_0+N) \big( \alpha'(x)+\delta(0) \big)} \mathrm{d}x \nonumber \\ 
...
\todohi{remove beta set arguments by introducing alpha sub z?}

\todohigh{FIX. Notation broken... use $\vartheta$??}
...
A characteristic of an Empirical random process is that its aggregations are also Empirical processes. Consider a random process $\uppsi \sim \Emp(N,\theta) \in \Uppsi \subset \Pcal(\Ycal)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$ and the corresponding function partitions $\uppsi_z \in \Rbbgeq^{\Scal_z}$, such that $\uppsi = \left\{ \ldots,\uppsi_z,\ldots \right\}$ and $\theta_z \in \Rbbgeq^{\Scal_z}$, such that $\theta = \left\{ \ldots,\theta_z,\ldots \right\}$. The transformed random process $\uppsim(z) \equiv \sum_{y \in \Scal_z} \uppsi_z(y)$ is distributed as $\uppsim \sim \Emp(N,\thetam)$ with a parameterizing distribution defined as $\thetam(z) = \sum_{y \in \Scal_z} \theta_z(y)$.

To prove this principle, define the subset $\Uppsim(\psim) = \big\{ \psi \in \Uppsi : \sum_{y \in \Scal_z} \psi_z(y) = \psim(z), \ \forall z \in \Zcal \big\} \subseteq \Uppsi$. Next, observe that
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsim}(\psim) & = & \sum_{\psi \in \Uppsim(\psim)} \Prm_{\uppsi}(\psi) 
= \sum_{\psi \in \Uppsim(\psim)} \Mcal(N \psi) \left( \prod_{y \in \Ycal} \theta(y)^{\psi(y)} \right)^N \\
& = & \Mcal(N \psim) \prod_{z \in \Zcal} \sum_{\substack{\sum_{y \in \Scal_z} \psi_z(y) \\ = \psim(z)}} \Mcal\big( N \psi_z \big) \left( \prod_{y \in \Scal_z} \theta_z(y)^{\psi_z(y)} \right)^N \nonumber \\
& = & \Mcal(N \psim) \left( \prod_{z \in \Zcal} \thetam(z)^{\psim(z)} \right)^N = \Emp(\psim ; N,\thetam) \nonumber \;,
\end{IEEEeqnarray}
...
If the empirical random process $\uppsi$ is conditioned on its aggregation $\uppsim$ over the partition $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, the distinct segements $\uppsi_z$ become independent random processes, such that for $\psi \in \Uppsim(\psim)$,
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim}(\psi | \psim) & = & \frac{\Prm_{\uppsi,\uppsim}(\psi,\psim)}{\Prm_{\uppsim}(\psim)} = \frac{\Mcal(N \psi)}{\Mcal(N \psim)} \left( \frac{\prod_{y \in \Ycal} \theta(y)^{\psi(y)}}{\prod_{z \in \Zcal} \thetam(z)^{\psim(z)}} \right)^N \\
& \equiv & \prod_{z \in \Zcal} \Bigg[ \Mcal(N \psi_z) \left(\frac{\prod_{y \in \Scal_z} \theta(y)^{\psi_z(y)}}{\thetam(z)^{\psim(z)}} \right)^N \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal(N \psi_z) \left( \prod_{y \in \Scal_z} \left(\frac{\theta(y)}{\thetam(z)}\right)^{\psi_z(y)} \right)^N \Bigg] \nonumber \;.
\end{IEEEeqnarray}

While these function segments are independent, they are not Empirical processes. Introducing the conditional distributions $\thetac(z) \equiv \theta / \thetam(z) \in \Pcal(\Scal_z)$ and the normalized segments $\uppsic(z) \equiv \uppsi / \uppsim(z)$, it can be shown that
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsic | \uppsim}(\psic | \psim) & = & \prod_{z \in \Zcal} \Bigg[ \Mcal(N \psim(z) \psic(z)) \left( \prod_{y \in \Scal_z} \thetac(y;z)^{\psic(y;z)} \right)^{N \psim(z)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Emp\Big( \psic(z) ; N \psim(z), \thetac(z) \Big) \;.
\end{IEEEeqnarray}
Thus, when conditioned on the aggregation $\uppsim$, the individual segments of the random process $\uppsic \in \Pcal(\Scal_z)^{\Zcal}$ are independent empirical processes of $N \uppsim(z)$ samples, parameterized by the individual segments of $\upthetac \in \Pcal(\Scal_z)^{\Zcal}$.
...
\todohigh{Even needed in addition to EMP proofs?}

\subsection{Aggregation}

\todomid{Redundant given citation??}

A characteristic of a Multinomial random process is that its aggregations are also Multinomial \cite{johnson}. Consider a random process $\uppsi \sim \Multi(N,\theta)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$; the transformed random process $\uppsim(z) \equiv \sum_{y \in \Scal_z} \uppsi(y)$ is distributed as $\uppsim \sim \Multi(N,\thetam)$ with parameterizing function $\thetam(z) = \sum_{y \in \Scal_z} \theta(y)$.

To prove this principle, define the subset $\tilde{\Ncal} = \big\{ \psi \in \Ncal : \sum_{y \in \Scal_z} \psi(y) = \psim(z), \quad \forall z \in \Zcal \big\} \subseteq \Uppsi$, where the original random process $\uppsi \in \Uppsi$. Next, observe
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsim}(n') & = & \sum_{\psi \in \tilde{\Ncal}} \Prm_{\uppsi}(\psi) \\
& = & \Mcal(N \psim) \prod_{z \in \Zcal} \sum_{\substack{\psim(z) = \\ \sum_{y \in \Scal_z} \psi(y)}} \Mcal\big( \{ \psi(y): y \in \Scal_z \} \big) \prod_{y \in \Scal_z} \theta(y)^{N \psi(y)} \nonumber \\
& = & \Mcal(N \psim) \prod_{z \in \Zcal} \thetam(z)^{N \psim(z)} = \Multi(n' ; N,\thetam) \nonumber \;,
\end{IEEEeqnarray}
where the multinomial theorem \cite{graham} has been used.
...
If the multinomial random process $\uppsi$ is conditioned on its aggregation over the partition $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, the distinct segements $\uppsi(y)$, $y \in \Scal_z$ become independent multinomial random processes,
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim}(\psi | \psim) & = & \frac{\Mcal(N \psi) \prod_{y \in \Ycal} \theta(y)^{N \psi(y)}}{\Mcal(N \psim) \prod_{z \in \Zcal} \thetam(z)^{N \psim(z)}} \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal\Big( \big\{ \psi(y) : y \in \Scal_z \big\} \Big) \prod_{y \in \Scal_z} \left(\frac{\theta(y)}{\thetam(z)}\right)^{\psi(y)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal\Big( \big\{ \psi(y) : y \in \Scal_z \big\} \Big) \prod_{y \in \Scal_z} \thetac(y;z)^{\psi(y)} \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Multi\Big( \big\{ \psi(y) : y \in \Scal_z \big\} ; \psim(z) , \big\{ \thetac(y;z) : y \in \Scal_z \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
on the domain $\left\{ \psi \in {\Zbbgeq}^{\Ycal} : \sum_{y \in \Scal_z} \psi(y) = \psim(z), \quad \forall z \in \Zcal \right\}$. Observe that the segment over the set $\Scal_z$ sums to $\uppsim(z)$ and is parameterized by the normalized segment $\thetac_z \equiv \big\{ \theta(y)/\thetam(z) : y \in \Scal_z \big\}$.
...
First, define the PDF of a Dirichlet aggregation \cite{ferguson}. Let the random process $\uptheta \in \Uptheta = \Pcal(\Ycal)$ be Dirichlet over the countable set $\Ycal$ with parameterizing function $\alpha \in {\Rbb^+}^{\Ycal}$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$; the aggregation $\upthetam \in \Pcal(\Zcal)$, $\upthetam(z) \equiv \sum_{y \in \Scal_z} \uptheta(y)$ is thus also Dirichlet and has a parameterizing function $\alpha' \in {\Rbb^+}^{\Zcal}$, $\alpha'(z) \equiv \sum_{y \in \Scal_z} \alpha(y)$.

The PDF of the original random process $\uptheta$ conditioned on its aggregation $\upthetam$ can be formulated as
\begin{IEEEeqnarray}{rCl}
\prm_{\uptheta | \upthetam}(\theta | \thetam) & = & \frac{\beta(\alpha') \prod_{y \in \Ycal} \theta(y)^{\alpha(y)-1}}{\beta(\alpha_0 \alpha) \prod_{z \in \Zcal} \thetam(z)^{\alpha'(z)-1}} \\
& = & \prod_{z \in \Zcal} \Bigg[ \beta\Big( \big\{ \alpha(y) : y \in \Scal_z \big\} \Big)^{-1} \frac{\prod_{y \in \Scal_z} \theta(y)^{\alpha(y)-1}}{\thetam(z)^{\alpha'(z)-1}} \Bigg] \nonumber \\ 
& = & \prod_{z \in \Zcal} \Bigg[ \frac{\thetam(z)^{1-|\Scal_z|}}{\beta\Big( \big\{ \alpha(y) : y \in \Scal_z \big\} \Big)} \prod_{y \in \Scal_z} \left(\frac{\theta(y)}{\thetam(z)}\right)^{\alpha(y)-1} \Bigg] \nonumber \;,
\end{IEEEeqnarray}
which is defined for $\left\{ \theta \in {\Rbbgeq}^{\Ycal} : \sum_{y \in \Scal_z} \theta(y) = \thetam(z), \quad \forall z \in \Zcal \right\}$.

Observe that the partitioned segements are conditionally independent; introduce subscript notation to refer to the function segment $\uptheta_z = \big\{ \uptheta(y): y \in \Scal_z \big\}$. The PDF $\prm_{\uptheta | \upthetam}$ can now be decomposed as $\prm_{\uptheta | \upthetam}(\ldots,\theta_z,\ldots | \thetam) = \prod_{z \in \Zcal} \prm_{\uptheta_z | \upthetam(z)}\big( \theta_z | \thetam(z) \big)$

Next, normalize the segments of $\uptheta$ to form $\upthetac = \big( \ldots,\upthetac_z,\ldots \big)$, where $\upthetac_z \equiv \uptheta_z / \upthetam(z)$, and formulate the conditional PDF
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \upthetam}\left( \thetac | \thetam \right) & = & \prod_{z \in \Zcal} \Bigg[ \frac{\prod_{y \in \Scal_z} \thetac(y;z)^{\alpha(y)-1}}{\beta\Big( \big\{ \alpha(y) : y \in \Scal_z \big\} \Big)} \Bigg] \\
& = & \prod_{z \in \Zcal} \Dir\Big( \thetac_z ; \big\{ \alpha(y) : y \in \Scal_z \big\} \Big) \nonumber \;.
\end{IEEEeqnarray}
which is defined for $\thetac \in \prod_{z \in \Zcal} \left\{ \thetac_z \in \Pcal\big(\Scal_z\big) \right\}$. Thus after conditioning, the normalized segments $\upthetac_z$ are Dirichlet distributed, independent of one another, and independent of the aggregation $\upthetam$. 

PGR: discuss transform Jacobian and dimensionality? 

This principle holds for continuous-domain Dirichlet processes $\uptheta$ as well - the segments $\upthetac_z$ are now continuous-domain Dirichlet processes.
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim}(\psi | \psim) & = & \frac{\Mcal(N \psi) \beta(\alpha_0 \alpha)^{-1} \beta(\alpha+\psi)}{\Mcal(N \psim) \beta(\alpha')^{-1} \beta(\alpha'+n')} \\
& = & \left( \prod_{z \in \Zcal} \frac{\Gamma\big( \alpha'(z)+\psim(z) \big)}{\psim(z)! \Gamma\big( \alpha'(z) \big)} \right)^{-1} \left( \prod_{y \in \Ycal} \frac{\Gamma\big( \alpha(y)+\psi(y) \big)}{\psi(y)! \Gamma\big( \alpha(y) \big)} \right) \nonumber \\
& = & \prod_{z \in \Zcal} \left[ \frac{\psim(z)! \Gamma\big( \alpha'(z) \big)}{\Gamma\big( \alpha'(z)+\psim(z) \big)} \prod_{y \in \Scal_z} \frac{\Gamma\big( \alpha(y)+\psi(y) \big)}{\psi(y)! \Gamma\big( \alpha(y) \big)} \right] \nonumber \\
...
%& = & \frac{\Mcal(\uppsi) \beta(\alpha_0 \alpha)^{-1} \beta(\alpha+\uppsi)}{\Mcal(\uppsim) \beta(\alpha')^{-1} \beta(\alpha'+\uppsim)} \prod_{z \in \Zcal} \delta\left[ \uppsim(z),\sum_{y \in \Scal_z} \uppsi(y) \right] \nonumber \\
%& = & \left( \prod_{z \in \Zcal} \frac{\Gamma\big( \alpha'(z)+\uppsim(z) \big)}{\uppsim(z)! \Gamma\big( \alpha'(z) \big)} \right)^{-1} \left( \prod_{y \in \Ycal} \frac{\Gamma\big( \alpha(y)+\uppsi(y) \big)}{\uppsi(y)! \Gamma\big( \alpha(y) \big)} \right) \nonumber \\
%&& \quad \prod_{z \in \Zcal} \delta\left[ \uppsim(z),\sum_{y \in \Scal_z} \uppsi(y) \right] \nonumber \\
%& = & \prod_{z \in \Zcal} \left[ \delta\left[ \uppsim(z),\sum_{y \in \Scal_z} \uppsi(y) \right] \frac{\uppsim(z)! \Gamma\big( \alpha'(z) \big)}{\Gamma\big( \alpha'(z)+\uppsim(z) \big)} \prod_{y \in \Scal_z} \frac{\Gamma\big( \alpha(y)+\uppsi(y) \big)}{\uppsi(y)! \Gamma\big( \alpha(y) \big)} \right] \nonumber \\
...
In this section, it is shown that if the model $\uptheta \sim \DP(\alpha_0 \alpha)$ is a Dirichlet process, then the model conditioned on the training data $\Drm$ is also a Dirichlet process with parameterizing function $\alpha + \sum_{n=1}^N \delta\big( \cdot - \Drm_n \big)$. 

The defining characteristic of Dirichlet processes is that their aggregations are also Dirichlet. Consider a DP over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$; the transformed random process $\upthetam(z) \equiv \int_{\Scal_z} \uptheta(y) \mathrm{d} y$ is neccessarily Dirichlet with parameterizing function $\alpha'(z) \equiv \int_{\Scal_z} \alpha(y) \mathrm{d} y$.

To prove the hypothesis, it is required that
\begin{IEEEeqnarray}{rCl}
\upthetam | \Drm & \sim & \Dir\big( \alpha' + \Psi(\Drm) \big) \;,
\end{IEEEeqnarray}
where $\Psi(z;\Drm) = \int_{\Scal_z} \sum_{n=1}^N \delta\big( y - \Drm_n \big) \mathrm{d} y = \sum_{n=1}^N \chi\big( \Drm_n;\Scal_z \big)$. $\chi$ is the indicator function
\begin{equation}
...
To prove the hypothesis, exploit the results of Appendix \ref{app:Dir_agg} to represent the training data distribution conditioned on the aggregation $\upthetam$. The conditional distribution of interest is
\begin{IEEEeqnarray}{L}
\prm_{\Drm | \upthetam}(D | \upthetam) = \Erm_{\uptheta | \upthetam}\big[ \prm_{\Drm | \uptheta}(D | \uptheta) \big] = \Erm_{\uptheta | \upthetam}\left[ \prod_{n=1}^N \uptheta\big( D_n \big) \right] \\
= \prod_{z \in \Zcal} \Erm_{\uptheta_z | \upthetam(z)}\left[ \prod_{n=1}^N \uptheta_z\big( D_n \big)^{\chi\big( D_n;\Scal_z \big)} \right] \nonumber \\
= \left( \prod_{z \in \Zcal} \prod_{n=1}^N \upthetam(z)^{\chi\big( D_n;\Scal_z \big)} \right) \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal_z \big)} \right] \nonumber \\
= \left( \prod_{z \in \Zcal} \upthetam(z)^{\Psi(z;D)} \right) \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal_z \big)} \right] \nonumber \;,
\end{IEEEeqnarray}
Observe that the dependency on $\upthetam$ is polynomial. The training data marginal distribution is
\begin{IEEEeqnarray}{rCl}
\prm_{\Drm}(D) & = & \Erm_{\upthetam} \left[ \prod_{z \in \Zcal} \upthetam(z)^{\Psi(z;D)} \right] \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal_z \big)} \right] \\
& = & \frac{\beta\big( \alpha' + \Psi(D) \big)}{\beta(\alpha')} \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal_z \big)} \right] \nonumber
\end{IEEEeqnarray}
and thus the distribution of interest is
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam | \Drm}(\thetam | D) & = & \frac{\prod_{z \in \Zcal} \thetam(z)^{\alpha'(z)+\Psi(z;D)-1}}{\beta\big( \alpha' + \Psi(D) \big)} \\
& = & \Dir\left( \upthetam ; \alpha' + \Psi(\cdot;\Drm) \right) \nonumber \;.
\end{IEEEeqnarray}
...
This section introduces a new random process, referred to as the Dirichlet-Multinomial process (DMP). It is the generalization of the Dirichlet-Multinomial distribution for i.i.d. samples drawn from a PDF; the underlying distribution is characterized by a Dirchlet process with parameter $\alpha$. The Dirichlet-Multinomial process assumes functions from the set $\left\{ \psi \in {\Rbbgeq}^{\Ycal} : \int_{\Ycal} \psi(y) \mathrm{d} y = N \right\}$ and is parameterized by a function $\alpha : \Ycal \mapsto \Rbb^+$.

Analagous to the Dirichlet and Dirichlet-Multinomial distributions for countable spaces, the Dirichlet-Multinomial process inherits the aggregation property from the Dirichlet process prior. That is, for a Dirichlet-Multinomial process $\uppsi \in \left\{ \psi \in {\Rbbgeq}^{\Ycal} : \int_{\Ycal} \psi(y) \mathrm{d} y = N \right\}$ and a partition of $\Ycal$, $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, the transformed random process $\uppsim(z) \equiv \int_{\Scal_z} \uppsi(y) \mathrm{d} y$ is neccessarily Dirichlet-Multinomial with parameterizing function $\alpha'(z) \equiv \int_{\Scal_z} \alpha(y) \mathrm{d} y$.

PGR: tilde not prime?

\subsection{Proof that $\sum_{n=1}^N \delta\big( y-\Drm_n \big)$ is a DMP}

Next, it is demonstrated that the random process $\uppsi(y) \equiv \Psi(y;\Drm) = \sum_{n=1}^N \delta\big( y-\Drm_n \big)$ is a DMP, given that $\prm_{\Drm|\uptheta}(D|\theta) = \prod_{n=1}^N \theta\big( D_n \big)$ and $\uptheta \sim \DP(\alpha_0 \alpha)$. 

Observe that $\uppsim(z) \equiv \sum_{n=1}^N \chi\big( \Drm_n;\Scal_z \big)$, where $\chi$ is the indicator function
\begin{equation}
\chi(x;S) = \begin{cases} 1 & \mathrm{if} \ x \in S, \\ 0 & \mathrm{if} \ x \notin S.  \end{cases}
\end{equation}
and note that $\Prm\Big( \chi\big( \Drm_n;\Scal_z \big) = 1 \big| \uptheta \Big) = \int_{\Scal_z} \uptheta(y) \mathrm{d} y$. As such, $\uppsim$ conditioned on the model $\uptheta$ is characterized by a multinomial distribution 
\begin{equation}
\Prm_{\uppsim | \uptheta}(n' | \theta) = \Mcal(N \psim) \prod_{z \in \Zcal} \left( \int_{\Scal_z} \theta(y) \mathrm{d} y \right)^{N \psim(z)} = \Multi\left( n' ; N,\thetam(z) \right) \;,
\end{equation}
where $\upthetam(z) \equiv \int_{\Scal_z} \uptheta(y) \mathrm{d} y$, $z \in \Zcal$.

By the aggregation property of the Dirichlet process $\uptheta$, the parameters of this multinomial distribution are characterized as $\upthetam \sim \Dir\left( \alpha' \right)$, and thus $\tilde{\nrm}$ is drawn from a Dirichlet-Multinomial PMF with the same parameters $\alpha'$. As this holds for any partition of $\Ycal$, $\uppsi$ is a Dirichlet-Multinomial Process.
...
If $\uppsi$ is a Dirichlet-Multinomial process over a Euclidean space $\Ycal$, then conditioning on its discrete aggregation $\uppsim$ produces independent Dirichlet-Multinomial processes $\big\{ \uppsi(y) : y \in \Scal_z \big\} \sim \DMP\Big( \uppsim(z),\big\{ \alpha(y) : y \in \Scal_z \big\} \Big)$ over the partition spaces $\Scal_z$.

The previous result can be extended to conditioning on a continuous aggregation. Define $\uppsi \sim \DMP(N,\alpha)$ over the set $\Ycal \times \Xcal$ and the aggregation DMP $\uppsim = \int_{\Ycal} \uppsi(y,\cdot) \mathrm{d}y$ over set $\Xcal$ with parameterizing function $\alpha' = \int_{\Ycal} \alpha(y,\cdot) \mathrm{d}y$.

Use the aggregation propery to introduce a Dirichlet-Multinomial process $\tilde{\nrm}(y;k) = \int_{\Delta k}^{\Delta (k+1)} \uppsi(y,x) \mathrm{d}x$ with parameter $\tilde{\alpha}(y;k) = \int_{\Delta k}^{\Delta (k+1)} \alpha(y,x) \mathrm{d}x$. Additionally, introduce its own aggregation, a Dirichlet-Multinomial random process $\dot{n}(k) = \int_{\Ycal} \tilde{n}(y,k) \mathrm{d}y$ with parameter $\dot{\alpha}(k) = \int_{\Ycal} \tilde{\alpha}(y,k) \mathrm{d}y$. By the conditioning property for discrete aggregations demonstrated previously, $\tilde{\nrm}(\cdot,k) | \dot{\nrm}(k) \sim \DMP\big( \dot{\nrm}(k),\tilde{\alpha}(\cdot,k) \big)$ are independent DMP's.
...
\end{equation}
we perform contrained optimization. Note that the set $\Uptheta = \left\{ \theta \in {\Rbbgeq}^{M}: \sum_{m=1}^{M} \theta_m = 1 \right\}$ implies both equality and inequality constraints...
...
%
%\begin{IEEEeqnarray}{rCl}
%\lim_{N \to \infty} \frac{\Erm_{\psi} \left[ \uppsi_{\mathrm{max}} \right]}{N} & = & p(1) - \int_0^1 p(t) \mathrm{d} t \\
