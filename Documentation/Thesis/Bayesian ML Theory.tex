...
f^*(\Drm) & = & \argmin_{g \in \Hcal^{\Xcal}} \Erm_{\yrm,\xrm | \Drm} \Big[ \Lcal\big(g(\xrm),\yrm\big) \Big] \\
& = & \argmin_{g \in \Hcal^{\Xcal}} \gamma \sum_{y,x} \alpha(y,x) \Lcal\big(g(x),y\big) + (1-\gamma) \sum_{y,x} \Psi(y,x;\Drm) \Lcal\big(g(x),y\big) \nonumber \\
...
\todomid{Separate general feature theory from discretization}
...
\todohigh{New regularized risk form??}
