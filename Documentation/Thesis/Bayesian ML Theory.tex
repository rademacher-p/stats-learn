...
\todolo{use clairvoyant terms and symbols $\Rcal$}
...
For this approach, the distributions $\Prm_{\uppsi}$, $\Prm_{\xrm | \uppsi}$, and $\Prm_{\yrm | \xrm,\uppsi}$ are required. Note that $\Prm_{\Drm | \uptheta}(D | \theta) = \Mcal\big( N \Psi(D) \big)^{-1} \Prm_{\uppsi | \uptheta}\big( \Psi(D) | \theta \big)$. Also, observe that the relevant posterior distributions satisfy $\prm_{\upthetam | \Drm}(D) = \prm_{\upthetam | \uppsi}\big( \Psi(D) \big)$ and $\prm_{\upthetac(x) | \xrm,\Drm}(x,D) = \prm_{\upthetac(x) | \xrm,\uppsi}\big( x,\Psi(D) \big)$, due to the sufficiency of the statistic $\Psi(\Drm)$ \cite{bernardo}. Consequently, $\Prm_{\xrm | \Drm}(D) = \Prm_{\xrm | \uppsi}\big( \Psi(D) \big)$, and $\Prm_{\yrm | \xrm,\Drm}(x,D) = \Prm_{\yrm | \xrm,\uppsi}\big( x,\Psi(D) \big)$.
...
It is instructive to treat the Bayesian predictive distribution $\Prm_{\yrm | \xrm,\Drm}$ as an estimator of the clairvoyant predictive distribution $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and investigate the effects of prior knowledge. Of specific interest are the mean $\Erm_{\Drm | \uptheta}\big[\Prm_{\yrm | \xrm, \Drm}\big]$ and covariance $\Crm_{\Drm | \uptheta}\big[\Prm_{\yrm | \xrm, \Drm}\big]$ with respect to the true model $\uptheta$.  To simplify the analysis, the training data $\Drm$ will be represented using the marginal and conditional sufficient statistics $(\uppsim, \uppsic)$. 

To aid characterization of the estimator, define the random process $\Delta(\xrm;\uppsim,\uppsic,\upthetac) \equiv \Prm_{\yrm | \xrm,\uppsim,\uppsic} - \Prm_{\yrm | \xrm,\upthetac} \in \Rbb^{\Ycal}$. The expected bias of the estimator is thus
...
& = & \Erm_{\uppsim, \uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] - \Prm_{\yrm | \xrm,\upthetac} \nonumber \;,
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
the conditional second moments of $\Delta(\xrm;\uppsim,\uppsic,\upthetac)$ can be readily shown to be
\begin{IEEEeqnarray}{L} \label{eq:predictive_del_sq}
\Erm_{\uppsim,\uppsic | \upthetam,\upthetac} \Big[ \Delta(\xrm;\uppsim,\uppsic,\upthetac) \otimes \Delta(\xrm;\uppsim,\uppsic,\upthetac) \Big] \nonumber \\
...
& \equiv & \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\uppsim,\uppsic | \upthetam,\upthetac} \Big[ \Delta(y; \xrm;\uppsim,\uppsic,\upthetac) \Delta(y'; \xrm;\uppsim,\uppsic,\upthetac) \Big] \nonumber \\
& = & \Erm_{\xrm | \uptheta} \left[ \left( \sum_{y \in \Ycal} y \ \mathrm{Bias}(y; \xrm; \upthetam,\upthetac) \right)^2 + \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \ \mathrm{Cov}(y,y'; \xrm; \upthetam,\upthetac) \right] \nonumber \;,
\end{IEEEeqnarray}
...
This section analyzes the bias and variance of the Dirichlet-based $\Prm_{\yrm | \xrm,\Drm}$ when used to estimate the clairvoyant predictive distribution $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$.
...
= \Erm_{\uppsim(\xrm),\uppsic(\xrm) | \upthetam(\xrm),\upthetac(\xrm)}\big[ \Prm_{\yrm | \xrm,\uppsim(\xrm),\uppsic(\xrm)} \big] \\ 
...
\mathrm{Bias}(\xrm;\upthetam,\upthetac) & = & \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\big[\gammam(\xrm; \uppsim)\big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \;.
\end{IEEEeqnarray}
and noting that
\begin{IEEEeqnarray}{L}
...
\end{IEEEeqnarray}
the covariance of the estimate can be represented as
\begin{IEEEeqnarray}{L} \label{eq:predictive_cov_dir}
\mathrm{Cov}(\xrm;\upthetam,\upthetac) = \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \\
\quad = \Crm_{\uppsim(\xrm) | \upthetam(\xrm)}\big[ \gammam(\xrm; \uppsim) \big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
\qquad + \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \nonumber \;.
\end{IEEEeqnarray}


Substituting the estimator bias and variance into \eqref{eq:predictive_del_sq}, the conditional second moments of $\Delta(\xrm;\uppsim,\uppsic,\upthetac)$ are
\begin{IEEEeqnarray}{L} \label{eq:predictive_del_sq_dir}
\Erm_{\uppsim,\uppsic | \upthetam,\upthetac} \Big[ \Delta(\xrm;\uppsim,\uppsic,\upthetac) \otimes \Delta(\xrm;\uppsim,\uppsic,\upthetac) \Big] \\
\quad = \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \gammam(\xrm; \uppsim)^2 \right] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
\qquad + \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \Big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \Big) \nonumber \;.
\end{IEEEeqnarray}
...
First consider the effects of the training data volume. Clearly, if $N=0$, $\gammam(\uppsim)$ is equal to one. Consequently, the prior mean $\alphac(\xrm)$ is used and the bias weight is maximal at unity; as this estimator is data-independent, the covariance is zero. As $N \to \infty$, the data PMF tends to $\Prm_{\uppsim(x) | \upthetam(x)} \approx \delta[\cdot, \upthetam(x)]$ and $\gammam(\uppsim) \approx \left(1 + \frac{N \upthetam(x)}{\alpha_0 \alpham(x)}\right)^{-1} \to 0$. As a result, the bias scaling factor tends to zero for all values $x \in \Xcal$ satisfying $\upthetam(x) > 0$ and to one otherwise. Thus for observations falling in the support of the marginal model $\upthetam$, the empirical data distribution $\uppsic(\xrm)$ is used and there is no bias. Similarly, the expectations affecting the covariance tend to zero. This demonstrates that the Dirichlet-based Bayesian estimate $\Prm_{\yrm | \xrm,\uppsim,\uppsic}$ converges to the true predictive distribution $\Prm_{\yrm | \xrm,\upthetac}$ in the limit of training data volume; this is guaranteed due to the full-support of the prior.


Next consider the effects of the Dirichlet parameterization. As $\alpha_0 \alpham(x) \to \infty$, $\gammam(\uppsim)$ is again equal to one, the bias is maximized and the covariance is zero. Conversely, as $\alpha_0 \alpham(x) \to 0$, the empirical data distribution is emphasized and the bias scaling factor tends to its minimal value $\big(1 - \upthetam(x)\big)^{N}$; this value is equivalent to $\Prm_{\uppsim(x) | \upthetam(x)}\big(0 | \thetam(x) \big)$, the probability that no training samples satisfy $\Xrm_n = x$ and thus that $\uppsic(x)$ is undefined. Additionally, the expectations affecting the covariance tend to
\begin{IEEEeqnarray}{L} 
\Crm_{\uppsim(\xrm) | \upthetam(\xrm)}\big[ \gammam(\xrm; \uppsim) \big] \to \big(1 - \upthetam(x)\big)^{N} \left( 1 - \big(1 - \upthetam(x)\big)^{N} \right) \nonumber
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{L} 
\Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{N \uppsim(\xrm)} \right] \to \sum_{n=1}^N \binom{N}{n} \upthetam(\xrm)^n \big( 1 - \upthetam(\xrm) \big)^{N-n} \frac{1}{n} \;.
\end{IEEEeqnarray}
...
Substituting the second moments of $\Delta(\xrm;\uppsim,\uppsic,\upthetac)$ \eqref{eq:predictive_del_sq_dir} into \eqref{eq:risk_cond_ex_SE_bayes}, the excess squared-error can be represented as
\begin{IEEEeqnarray}{L} \label{eq:risk_cond_SE_dir_ex}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm,\uppsim,\uppsic | \upthetam,\upthetac} \Big[ \Delta(y; \xrm;\uppsim,\uppsic,\upthetac) \Delta(y'; \xrm;\uppsim,\uppsic,\upthetac) \Big] \nonumber \\
...
\subsubsection{Example} \label{sec:SE_ex}

To demonstrate the efficacy of Dirichlet-based regressors, consider a data model where $\Xcal$ and $\Ycal$ are 128-point discretizations of the unit interval $[0, 1]$, inclusive, such that $\Xcal = \Ycal = \{n/127: n = 0, \ldots, 127\}$. The Dirichlet estimator is parameterized by $\alpham = 1 / 128$ and $\alphac(y;x) = \Bi(127y; 127, 0.5)$, such that the prior estimator $\mu_{\yrm | \xrm} = 0.5$ is constant; various localizations $\alpha_0$ will be used. Results were generated by averaging 50,000 iterations of a novel Python learning simulation.
...
\end{figure} 
\Cref{fig:SSP_2021/Risk_cond_SE_a0_biased_hard_zoom} highlights the localization $\alpha_0$ that optimizes the bias/variance trade-off; this value is independent of the training volume $N$. Due to the severity of the bias, the corresponding optimal Dirichlet estimator is only marginally better than the empirical estimator with $\alpha_0 \to 0$.
\begin{figure}
\centering
...
For continuous data spaces, the analysis of the Bayesian predictive distribution as an estimate of the true predictive distribution has some unique differences from the discrete space analysis put forth in Section \ref{sec:predictive_est_dir}. Redefine the difference function $\Delta(\xrm;\uppsim,\uppsic,\upthetac) \equiv \prm_{\yrm | \xrm,\uppsim,\uppsic} - \prm_{\yrm | \xrm,\upthetac} \in \Rbb^{\Ycal}$. Using the properties of the continuous-domain empirical process conditioned on its aggregation, the covariance function takes on the new form
\begin{IEEEeqnarray}{L} \label{eq:predictive_cov_cont}
\mathrm{Cov}(\xrm;\upthetam,\upthetac) = \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \\
\quad = \Crm_{\uppsim(\xrm) | \upthetam(\xrm)}\big[\gammam(\xrm; \uppsim)\big] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber \\
\qquad + \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(\xrm)} \right] \big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \big) \nonumber
\end{IEEEeqnarray}
...
&& \qquad + \left(\frac{N \Psim(\xrm;\Xrm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Xrm)}\right) \int_{\Ycal} \Psic(y;\xrm;\Drm) \Lcal(h,y) {\drm}y \nonumber \\
...
Using the second moments of the random process $\Delta(\xrm;\uppsim,\uppsic,\upthetac)$ formulated in \eqref{eq:predictive_del_sq_dir_cont}, the excess squared-error is redefined as
...
\quad = \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\big[ \gammam(\xrm; \uppsim)^2 \big] \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^2 \right] \nonumber \\
\qquad \quad + \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\big(1 - \gammam(\xrm; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(\xrm)} \right] \Sigma_{\yrm | \xrm,\upthetac} \right] \nonumber \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{L} 
\Erm_{\uppsim(x) | \upthetam(x)}\big[ \gammam(x; \uppsim)^2 \big] \to \left(1 - \frac{\upthetam(x)}{\delta(0)}\right)^{N} 
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{L} 
\Erm_{\uppsim(x) | \upthetam(x)}\left[ \frac{\big(1 - \gammam(x; \uppsim)\big)^2}{\delta(0)^{-1} N \uppsim(x)} \right] \to \sum_{n=1}^N \binom{N}{n} \left(\frac{\upthetam(x)}{\delta(0)}\right)^n \left( 1 - \frac{\upthetam(x)}{\delta(0)} \right)^{N-n} \frac{1}{n} \;,
\end{IEEEeqnarray}
...
Consider a data model where $\Xcal = \Ycal = [0, 1]$, the unit interval. The Dirichlet estimator is parameterized by $\alpham = 1$ and $\alphac(x) = \Beta(63, 63)$, such that the prior estimator $\mu_{\yrm | \xrm} = 0.5$ is constant; various localizations $\alpha_0$ will be used. 

\Cref{fig:SSP_2021/Risk_SE_N_cont} displays the Bayesian squared-error realized by the Dirichlet-based regressor. The model $\uptheta$ is randomly selected from a Dirichlet process with the same $\alpham$ and $\alphac$ used for the regressor design and with the localization fixed at $\alpha_0 = 100$. Since $\alpham$ is bounded, the Bayes risk is $\Rcal^* \approx (1 + N / \alpha_0)^{-1} \Erm_{\xrm}\big[ \Sigma_{\yrm | \xrm} \big]$. Observe that the risk is independent of the value $\alpha_0$ chosen for the estimator, since $\delta(0)^{-1} \alpha_0 \alpham(x) \to 0$. Additionally, note that $\Rcal^* \to 0$ as $N \to \infty$.
\begin{figure}
\centering
...
Next, consider the risk trends when the true model is fixed. The marginal distribution $\thetam = 1$ is uniform and $\thetac(\xrm) = \Beta\big(126 \mu_{\yrm | \xrm,\uptheta}, 126 (1- \mu_{\yrm | \xrm,\uptheta})\big)$, where the clairvoyant regressor is $\mu_{\yrm | \xrm,\uptheta} = 1 / \big(2 + \sin(2\pi \xrm)\big)$. Comparing with the discrete-space models used for demonstration in Section \ref{sec:SE_ex}, observe that the optimal regressor $\mu_{\yrm | \xrm, \uptheta}$ and true predictive variance $\Sigma_{\yrm | \xrm, \uptheta} = \frac{1}{127} \mu_{\yrm | \xrm, \uptheta} (1 - \mu_{\yrm | \xrm, \uptheta})$ are the same. However, since $\upthetam$ is bounded, no effective learning is possible, the Bayesian estimator tends to $\mu_{\yrm | \xrm, \Drm} \to \mu_{\yrm | \xrm}$, and the excess risk tends to the fixed, biased value $\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) \approx \Erm_{\xrm | \uptheta}\left[ \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\uptheta} \right)^2 \right] \approx 0.059$.
...
\todomid{prime accent formatting}
...
\todohi{CITE for discretization work}

\todohigh{USE DQE feature transform NOTATION!!}

Define a discretizing data transformation $T: \Xcal \mapsto \Xcal'$, where the range is a countable subset $\Xcal' \subset \Xcal$, such that $|\Xcal'| \leq \aleph_0$. This transform is applied to each of the training values $\Xrm_n$, as well as to the novel observation $\xrm$. Note that $T$ is effectively a dimensionality-reducing feature transformation.

Having effectively changed the domain of the observations, the Dirichlet prior marginal mean is re-defined as $\alpham = \sum_{x' \in \Xcal'} \alpham'(x') \delta(\cdot - x')$, a mixture of delta functions at the discretized values, where $\alpham' \in \Pcal(\Xcal')$ is the discrete space marginal function.
...
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \xrm,\Drm} & = & \gammam'\big(T(\xrm); \Xrm \big) \alphac\big(T(\xrm)\big) + \Big(1 - \gammam'\big(T(\xrm); \Xrm \big)\Big) \Psic'\big(T(\xrm); \Drm\big) \;,
\end{IEEEeqnarray}
where the discretized weighting function $\gammam'(X) \in (0,1]^{\Xcal'}$ is defined as 
\begin{IEEEeqnarray}{rCl}
\gammam'(x'; X) & = & \left(1 + \frac{\sum_{n=1}^N \delta\big(x' - T(X_n)\big)}{\alpha_0 \sum_{x'' \in \Xcal'} \alpham'(x'') \delta(x' - x'')}\right)^{-1} \nonumber \\
& = & \left(1 + \frac{\sum_{n=1}^N \delta\big[x', T(X_n)\big]}{\alpha_0 \sum_{x'' \in \Xcal'} \alpham'(x'') \delta[x', x'']}\right)^{-1} \nonumber \\
& = & \left(1 + \frac{N \Psim'(x'; X)}{\alpha_0 \alpham'(x') }\right)^{-1} \;.
\end{IEEEeqnarray}
The discretized marginal empirical function is 
\begin{IEEEeqnarray}{rCl}
\Psim'(X) & = & \frac{1}{N} \sum_{n=1}^N \delta\big[\cdot, T(X_n)\big] \in \Pcal(\Xcal')
\end{IEEEeqnarray}
and the discretized conditional empirical transform $\Psic'(D) \in \Pcal(\Ycal)^{\Xcal'}$ is defined as 
\begin{IEEEeqnarray}{rCl}
\Psic'(x'; D) & = & \frac{\sum_{n=1}^N \delta\big( \cdot - Y_n \big) \delta\big[x', T(X_n) \big]}{\sum_{n=1}^N \delta\big[x', T(X_n) \big]} \;.
\end{IEEEeqnarray}
...
\todohigh{Formulate from new low-dim likelihood func}

As with the original empirical transform, it is useful to define and characterize new random processes. Introduce a partitioning of the observation space $\Xcal$ as $\{\ldots, \Scal(x'), \ldots\}$, where $\Scal(x') = \{x \in \Xcal: T(x) = x'\}$.

Define the discretized empirical process $\uppsi' \equiv \Psi'(D) \in \Pcal(\Ycal \times \Xcal')$, where
\begin{IEEEeqnarray}{rCl}
\Psi'(y, x'; D) & \equiv & \frac{1}{N} \sum_{n=1}^N \delta(y - Y_n) \delta\big[x', T(X_n) \big] \nonumber \\
& = & \frac{1}{N} \sum_{n=1}^{N} \delta(y - Y_n) \chi\big(X_n; \Scal(x')\big) \nonumber \\
& = & \frac{1}{N} \sum_{n=1}^{N} \delta(y - Y_n) \int_{\Scal(x')} \delta(x - X_n) {\drm}x \nonumber \\
& = & \int_{\Scal(x')} \Psi(y,x; D) {\drm}x \;.
\end{IEEEeqnarray}
By the aggregation property of Empirical processes (Appendix \ref{app:EP}), $\uppsi' | \uptheta \sim \EP(N, \uptheta')$, where the dependency on the model is expressed through the discretized $\uptheta' \in \Pcal(\Ycal \times \Xcal')$, defined as $\uptheta'(y,x') = \int_{\Scal(x')} \uptheta(y,x) {\drm}x$.

Consequently, when conditioned on the model, the marginal process $\uppsim' \equiv \Psim'(X) \equiv \int_{\Ycal} \uppsi'(y,x') {\drm}y$ is also Empirical with $N$ samples and mean $\upthetam' \in \Pcal(\Xcal')$, defined as $\upthetam'(x') = \int_{\Scal(x')} \upthetam(x) {\drm}x$. Additionally, the conditional processes $\uppsic'(x')$ are conditionally independent and distributed as $\uppsic'(x') | \uppsim'(x'), \upthetac'(x') \sim \EP\big(N \uppsim'(x'), \upthetac'(x')\big)$, where
\begin{IEEEeqnarray}{rCl}
\upthetac'(x') & \equiv & \frac{\uptheta'(\cdot,x')}{\upthetam'(x')} = \int_{\Scal(x')} \upthetac(x) \frac{\upthetam(x)}{\upthetam'(x')} {\drm}x \;,
\end{IEEEeqnarray}
convexly combining the true predictive distributions $\upthetac(x)$ for observations $x \in \Scal(x')$. 

\todomid{Discuss: no longer independent of thetam!?}
...
\todohigh{USE TRANSFORMED OBS t in place of x, use for expectations!??!}

The Bayesian predictive distribution in terms of the discretized statistics is
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \xrm,\uppsim',\uppsic'} & = & \gammam'\big(T(\xrm); \uppsim' \big) \alphac\big(T(\xrm)\big) + \Big(1 - \gammam'\big(T(\xrm); \uppsim' \big)\Big) \uppsic'\big(T(\xrm)\big) \;.
\end{IEEEeqnarray}
Note that the domain of the function $\gammam'$ will be changed from $\Xcal^N$ to $\Pcal(\Xcal)$, operating on the discretized empirical distribution rather than the raw observations, such that
\begin{IEEEeqnarray}{rCl}
\gammam'(\psim') & = & \left(1 + \frac{N \psim'}{\alpha_0 \alpham'}\right)^{-1} \;.
\end{IEEEeqnarray}



\section{Predictive Model Estimation}

To evaluate the bias and covariance functions of the Bayesian predictive distribution $\prm_{\yrm | \xrm,\Drm}$, the training data will be represented by the discretized sufficient statistics $(\uppsim', \uppsic')$. 

\todohigh{RECONSIDER use of subscript arguments EVERYWHERE?!?!!?}
\todohigh{RECONSIDER use of subscript arguments EVERYWHERE?!?!!?}
\todohigh{FIX SSP PAPER!}

The expected value of the estimate conditioned on the true model is
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_dist_avg_dir_disc}
\Erm_{\uppsim,\uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] 
& = & \Erm_{\uppsim' | \upthetam'}\big[\gammam'(\uppsim')\big]\big(T(\xrm)\big) \alphac\big(T(\xrm)\big) \\
&& \quad + \Big(1 - \Erm_{\uppsim' | \upthetam'} \big[\gammam'(\uppsim')\big]\big(T(\xrm)\big)\Big) \upthetac'\big(T(\xrm)\big) \nonumber \;.
\end{IEEEeqnarray}
Note that the distributions being mixed no longer includes the true predictive distribution $\upthetac(\xrm)$, but rather an approximation $\upthetac'\big(T(\xrm)\big)$. Substituting into \eqref{eq:predictive_bias}, the expected bias is
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_bias_dir_disc}	
\mathrm{Bias}(\xrm;\upthetam,\upthetac) & = & \Erm_{\uppsim' | \upthetam'}\big[\gammam'(\uppsim')\big]\big(T(\xrm)\big) \big( \alphac - \upthetac' \big)\big(T(\xrm)\big) \nonumber \\
&& \quad + \left(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \right) \;.
\end{IEEEeqnarray}

\todomid{in terms of original model? excess space above?}

Observe that while similar in form to \eqref{eq:predictive_bias_dir}, the bias includes an additional term that captures the approximation error due to discretization. Since the discretized conditional empirical function $\uppsic(x')$ integrates all training samples falling in the subset $\Scal(x') \subset \Xcal$, its expected value integrates the true predictive distributions $\upthetac$ in the same region.
