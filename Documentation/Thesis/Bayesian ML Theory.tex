...
Defining the novel feature $\trm \equiv T(\xrm)$, it is evident that $\Prm_{\trm | \uptheta} = \Pr\big(T(\xrm) = \trm | \uptheta\big) = \Pr\big(\xrm \in \Xcal'(\trm) | \uptheta\big)$. As a result, $\Prm_{\trm | \uptheta} \equiv \upthetamd \in \Pcal(\Tcal)$, where $\upthetamd$ is the discretized marginal model defined as $\upthetamd(t) = \int_{\Xcal'(t)} \upthetam(x) {\drm}x$.
...
\end{IEEEeqnarray}
convexly combining the true predictive distributions $\upthetac(x)$ for observations $x \in \Xcal'(t)$. 
