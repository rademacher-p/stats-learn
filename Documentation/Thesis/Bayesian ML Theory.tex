...
This chapter extends further to the case where $\Ycal$ and $\Xcal$ are Euclidean spaces and the model $\uptheta$ is a continuous-domain Dirichlet process. Note that $|\Ycal| \geq \aleph_1$ and $|\Xcal| \geq \aleph_1$. 
...
The model is characterized by a Dirichlet process $\uptheta \sim \DP(\alpha_0, \alpha)$ with concentration $\alpha_0$ and mean function $\alpha : \Ycal \times \Xcal \mapsto \Rbb^+$. As shown in Appendix \ref{app:DP}, the expected value of a Dirichlet process is $\DP(\alpha_0, \alpha)$
\begin{equation}
\mu_{\uptheta} = \alpha
\end{equation}
and the correlation function is
\begin{IEEEeqnarray}{rCl}
\Erm_{\uptheta}\big[ \uptheta(y,x) \uptheta(y',x') \big] & = & \frac{\alpha_0 \alpha(y,x) \alpha(y',x') + \alpha(y,x) \delta(y-y')\delta(x-x')}{\alpha_0+1} \;.
\end{IEEEeqnarray}
...
The Dirichlet process $\uptheta \sim \Dir(\alpha_0,\alpha) \in \Uptheta \equiv \Pcal(\Ycal)$ over the Euclidean set $\Ycal$ is parameterized by the concentration $\alpha_0 \in \Rbb^+$ and mean function $\alpha \in \left\{ {\Rbb^+}^{\Ycal} : \int_{\Ycal} \alpha(y) {\drm}y = 1 \right\}$. The continuous-domain Dirichlet process is characterized by the same aggregation property as its discrete-domain variant. Define a countable partition of $\Ycal$, $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, and the corresponding function partitions $\uptheta_z \in \Rbbgeq^{\Scal_z}$, such that $\uptheta = \left\{ \ldots,\uptheta_z,\ldots \right\}$ and $\alpha_z \in {\Rbb^+}^{\Scal_z}$, such that $\alpha = \left\{ \ldots,\alpha_z,\ldots \right\}$. Thus, the transformed random process $\upthetam(z) \equiv \int_{\Scal_z} \uptheta_z(y) {\drm}y$ is distributed as $\upthetam \sim \Dir(\alpha_0,\alpham) \in \Pcal(\Zcal)$ with a parameterizing distribution defined as $\alpham(z) = \int_{\Scal_z} \alpha_z(y) {\drm}y$.

Additionally, the normalized random processes $\upthetac(z) \equiv \uptheta_z / \upthetam(z)$ are independent continuous-domain Dirichlet processes, $\upthetac(z) \sim \Dir\big(\alphac(z)\big)$, where $\alphac(z) \equiv \alpha_z / \alpham(z) \in \Pcal(\Scal_z)$, and are independent of the aggregation process $\upthetam$.
...
The aggregation property has been stated for countable partitions of the process domain - it also holds for continuous aggregations. Define an Dirichlet process $\uptheta \in \Pcal(\Ycal \times \Xcal)$ parameterized by concentration $\alpha_0$ and mean function $\alpha$, where $\Ycal = \Xcal = \Rbb$ for simplicity. The aggregation $\upthetam \equiv \int_{\Ycal} \uptheta(y,\cdot) {\drm}y \in \Pcal(\Xcal)$ is a Dirichlet process with concentration $\alpha_0$ and parameterizing function $\alpham \equiv \int_{\Ycal} \alpha(y,\cdot) {\drm}y$. 

Additionally, the normalized functinons $\upthetac(x) \equiv \uptheta(\cdot,x) / \upthetam(x)$ are independent continuous-domain Dirichlet processes, $\upthetac(x) \sim \DP\big(\alpha_0 \alpham(x), \alphac(x)\big)$, where $\alphac(x) \equiv \alpha(\cdot,x) / \alpham(x) \in \Pcal(\Ycal)$, and are independent of the aggregation process $\upthetam$.
