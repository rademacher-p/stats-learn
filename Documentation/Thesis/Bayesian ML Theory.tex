...
This chapter extends further to the case where $\Ycal$ and $\Xcal$ are Euclidean spaces and the model $\uptheta$ is a continuous-domain random process. Note that $|\Ycal| \geq \aleph_1$ and $|\Xcal| \geq \aleph_1$. 
...
It can be shown that the likelihood $\prm_{\Drm | \uptheta}$ depends on the training data $\Drm$ only via $\Psi : \Dcal \mapsto \Uppsi \subset \Uptheta$, defined as
\begin{IEEEeqnarray}{rCl}
\Psi(y,x;D) & = & \frac{1}{N} \sum_{n=1}^N \delta \big( (y,x) - D_n \big) \\
& \equiv & \frac{1}{N} \sum_{n=1}^N \delta(y - Y_n) \delta(x - X_n) \nonumber \;.
\end{IEEEeqnarray}
As such, the distribution $\prm_{\Drm | \uppsi,\uptheta}$ is uniform on the discrete set $\{D \in \Dcal : \Uppsi(D) = \uppsi\}$ and $\uppsi$ is a sufficient statistic.
