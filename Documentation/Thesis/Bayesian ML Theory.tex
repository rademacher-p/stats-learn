...
\mathrm{Cov}(\xrm; \upthetam,\upthetac) = \Crm_{\Drm | \upthetam,\upthetac} \big[\Prm_{\yrm | \xrm,\Drm} \big] \;,
\end{IEEEeqnarray}
...
& = & \Erm_{\xrm | \upthetam} \left[ \left( \sum_{y \in \Ycal} y \ \mathrm{Bias}(y; \xrm; \upthetam,\upthetac) \right)^2 + \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \ \mathrm{Cov}(y,y'; \xrm; \upthetam,\upthetac) \right] \nonumber \;,
\end{IEEEeqnarray}
...
\begin{equation}
\Rcal_{\Theta, \mathrm{ex}}(f^* ; \uptheta) = \Erm_{\xrm | \upthetam}\left[ \Erm_{\uppsim | \upthetam}\left[ \frac{1}{N \uppsim(\xrm) \Sigma_{\yrm | \xrm,\upthetac}^{-1} + \left( \mu_{\yrm | \xrm} - \mu_{\yrm | \xrm,\upthetac} \right)^{-2}} \right] \right] \;,
\end{equation}
...
Define a discretizing data transformation $T: \Xcal \mapsto \Tcal$, where the range is a countable subset $\Tcal \subset \Xcal$, such that $|\Tcal| \leq \aleph_0$. This transform is applied to each of the training values $\Xrm_n$, as well as to the novel observation $\xrm$. Note that $T$ is effectively a dimensionality-reducing feature transformation. Use $T$ to define a partitioning $\{\ldots, \Scal(t), \ldots\}$ of the observation space $\Xcal$, where $\Scal(t) = \{x \in \Xcal: T(x) = t\}$; it is assumed that these subsets are contiguous. Furthermore, assume that $t \in S(t), \ \forall \ t \in \Tcal$, such that the transform maps $T(t) = t$.

Having effectively changed the domain of the observations, the Dirichlet prior marginal mean is re-defined as $\alpham = \sum_{t \in \Tcal} \alpham'(t) \delta(\cdot - t)$, a mixture of delta functions at the discretized values, where $\alpham' \in \Pcal(\Tcal)$ is the discrete space marginal function. $\uppsim' \uppsi_{\mathrm{m}}'$
...
\begin{IEEEeqnarray}{rCl} \label{eq:P_y_xD_dir_cont_disc}
\prm_{\yrm | \xrm,\Drm} & = & \gammam'\big(T(\xrm); \Xrm \big) \alphac\big(T(\xrm)\big) + \Big(1 - \gammam'\big(T(\xrm); \Xrm \big)\Big) \Psic'\big(T(\xrm); \Drm\big) \;,
\end{IEEEeqnarray}
where the discretized weighting function $\gammam'(X): \Xcal^N \mapsto (0,1]^{\Tcal}$ is defined as 
\begin{IEEEeqnarray}{rCl}
\gammam'(t; X) & = & \left(1 + \frac{\sum_{n=1}^N \delta\big(t - T(X_n)\big)}{\alpha_0 \sum_{t' \in \Tcal} \alpham'(t') \delta(t - t')}\right)^{-1} \nonumber \\
& = & \left(1 + \frac{\sum_{n=1}^N \delta\big[t, T(X_n)\big]}{\alpha_0 \sum_{t' \in \Tcal} \alpham'(t') \delta[t, t']}\right)^{-1} \nonumber \\
& = & \left(1 + \frac{N \Psim'(t; X)}{\alpha_0 \alpham'(t) }\right)^{-1} \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Psim'(X) & = & \frac{1}{N} \sum_{n=1}^N \delta\big[\cdot, T(X_n)\big] \in \Pcal(\Tcal)
\end{IEEEeqnarray}
and the discretized conditional empirical transform $\Psic'(D) \in \Pcal(\Ycal)^{\Tcal}$ is defined as 
\begin{IEEEeqnarray}{rCl}
\Psic'(t; D) & = & \frac{\sum_{n=1}^N \delta\big( \cdot - Y_n \big) \delta\big[t, T(X_n) \big]}{\sum_{n=1}^N \delta\big[t, T(X_n) \big]} \;.
\end{IEEEeqnarray}
...
Inspecting the new predictive distribution $\prm_{\yrm | \xrm,\Drm}$, it is clear that the dependency on the joint observations $(\xrm, \Drm)$ can be represented using the sufficient statistics $\big(T(\xrm), \Psim'(\Xrm), \Psic'(\Drm)\big)$. Again, it is useful to define and characterize new random processes. 

Defining the novel feature $\trm \equiv T(\xrm)$, it is evident that $\Prm_{\trm | \uptheta} = \Pr\big(T(\xrm) = \trm | \uptheta\big) = \Pr\big(\xrm \in \Scal(\trm) | \uptheta\big)$. As a result, $\Prm_{\trm | \uptheta} \equiv \upthetam' \in \Pcal(\Tcal)$, where $\upthetam'$ is the discretized marginal model defined as $\upthetam'(t) = \int_{\Scal(t)} \upthetam(x) {\drm}x$.

Next, characterize the new empirical processes. Define the discretized empirical process $\uppsi' \equiv \Psi'(D) \in \Pcal(\Ycal \times \Tcal)$, where
\begin{IEEEeqnarray}{rCl}
\Psi'(y, t; D) & \equiv & \frac{1}{N} \sum_{n=1}^N \delta(y - Y_n) \delta\big[t, T(X_n) \big] \nonumber \\
& = & \frac{1}{N} \sum_{n=1}^{N} \delta(y - Y_n) \chi\big(X_n; \Scal(t)\big) \nonumber \\
& = & \frac{1}{N} \sum_{n=1}^{N} \delta(y - Y_n) \int_{\Scal(t)} \delta(x - X_n) {\drm}x \nonumber \\
& = & \int_{\Scal(t)} \Psi(y,x; D) {\drm}x \;.
\end{IEEEeqnarray}
Observe that $\uppsi'$ is a function of the original empirical process $\uppsi$. By the aggregation property of Empirical processes (Appendix \ref{app:EP}), it is shown that $\uppsi' | \uptheta \sim \EP(N, \uptheta')$, where the dependency on the model is expressed through the discretized $\uptheta' \in \Pcal(\Ycal \times \Tcal)$, defined as $\uptheta'(y,t) = \int_{\Scal(t)} \uptheta(y,x) {\drm}x$.

Consequently, when conditioned on the model, the marginal process $\uppsim' \equiv \Psim'(\Xrm) \equiv \int_{\Ycal} \uppsi'(y,\cdot) {\drm}y$ is also Empirical with $N$ samples and mean $\upthetam'$. Additionally, the conditional processes $\uppsic'(t) \equiv \Psic'(t; \Drm)$ are conditionally independent and distributed as $\uppsic'(t) | \uppsim'(t), \upthetac'(t) \sim \EP\big(N \uppsim'(t), \upthetac'(t)\big)$, where
\begin{IEEEeqnarray}{rCl}
\upthetac'(t) & \equiv & \frac{\uptheta'(\cdot,t)}{\upthetam'(t)} = \int_{\Scal(t)} \upthetac(x) \frac{\upthetam(x)}{\upthetam'(t)} {\drm}x \;,
\end{IEEEeqnarray}
convexly combining the true predictive distributions $\upthetac(x)$ for observations $x \in \Scal(t)$. 



\subsection{PGR predictive dist w psi}

The Bayesian predictive distribution \eqref{eq:P_y_xD_dir_cont_disc} can be represented in terms of the discretized sufficient statistics as $\prm_{\yrm | \xrm,\Drm} = \prm_{\yrm | \trm,\uppsim',\uppsic'}\big(T(\xrm), \Psim'(\Xrm), \Psic'(\Drm)\big)$, where
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \trm,\uppsim',\uppsic'} & = & \gammam'(\trm; \uppsim') \alphac(\trm) + \big(1 - \gammam'(\trm; \uppsim')\big) \uppsic'(\trm) \;.
\end{IEEEeqnarray}
Note that the weighting function is redefined to operate on the discretized empirical distribution rather than the raw observations, such that $\gammam': \Pcal(\Xcal) \mapsto (0,1]^{\Tcal}$ and 
\begin{IEEEeqnarray}{rCl}
...
To evaluate the bias and covariance functions of the Bayesian predictive distribution $\prm_{\yrm | \xrm,\Drm}$, the training data will be represented by the discretized sufficient statistics $(\uppsim', \uppsic')$. Note that $\Erm_{\Drm | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\Drm} \big] = \Erm_{\uppsim',\uppsic' | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim',\uppsic'} \big]$ and $\Crm_{\Drm | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\Drm} \big] = \Crm_{\uppsim',\uppsic' | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim',\uppsic'} \big]$. 
...
\Erm_{\uppsim',\uppsic' | \upthetam,\upthetac}\big[ \prm_{\yrm | \trm,\uppsim',\uppsic'} \big] 
& = & \Erm_{\uppsim' | \upthetam'}\big[\gammam'(\trm; \uppsim')\big] \alphac(\trm) \\
&& \quad + \Big(1 - \Erm_{\uppsim' | \upthetam'} \big[\gammam'(\trm; \uppsim')\big]\Big) \upthetac'(\trm) \nonumber \;.
\end{IEEEeqnarray}
Comparing to \ref{eq:predictive_dist_avg_dir}, observe that the mixture distribution no longer includes the true predictive distribution $\upthetac$, but rather the discretized $\upthetac'$. 

Using the equivalence $\prm_{\yrm | \xrm,\uppsim',\uppsic'} = \prm_{\yrm | \trm, \uppsim', \uppsic'}\big(T(\xrm), \uppsim',\uppsic' \big)$ and substituting into \eqref{eq:predictive_bias}, the expected bias is
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_bias_dir_disc}	
\mathrm{Bias}(\xrm; \upthetam,\upthetac) & = & \Erm_{\uppsim' | \upthetam'}\Big[\gammam'\big(T(\xrm); \uppsim'\big)\Big] \Big( \alphac\big(T(\xrm)\big) - \upthetac'\big(T(\xrm)\big) \Big) \nonumber \\
&& \quad + \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \;.
\end{IEEEeqnarray}
While similar in form to \eqref{eq:predictive_bias_dir}, observe that the bias includes an additional term that quantifies a ``discretization bias''. Since the discretized conditional empirical function $\uppsic(t)$ integrates all training samples falling in the subset $\Scal(t) \subset \Xcal$, its expected value integrates the true predictive distributions $\upthetac$ in the same region.
...
\Crm_{\uppsim',\uppsic' | \upthetam,\upthetac} \big[\Prm_{\yrm | \trm,\uppsim',\uppsic'} \big] \nonumber \\
\quad = \Crm_{\uppsim' | \upthetam'}\big[ \gammam'(\trm; \uppsim') \big] \big( \alphac(\trm) - \upthetac'(\trm) \big) \otimes \big( \alphac(\trm) - \upthetac'(\trm) \big) \nonumber \\
\qquad + \Erm_{\uppsim' | \upthetam'}\left[ \frac{\big(1 - \gammam'(\trm; \uppsim')\big)^2}{N \uppsim'(\trm)} \right] \Big( \diag\big(\upthetac'(\trm)\big) - \upthetac'(\trm) \otimes \upthetac'(\trm) \Big) 
\end{IEEEeqnarray}
and thus the covariance \eqref{eq:predictive_cov} can be represented as 
\begin{IEEEeqnarray}{L} \label{eq:predictive_cov_dir_disc}
\mathrm{Cov}(\xrm; \upthetam,\upthetac) = \Crm_{\uppsim',\uppsic' | \upthetam,\upthetac} \big[\Prm_{\yrm | \trm,\uppsim',\uppsic'} \big] \big(T(\xrm); \upthetam,\upthetac\big) \;.
\end{IEEEeqnarray}

Substituting the estimator bias and variance into \eqref{eq:predictive_del_sq}, the conditional second moments of $\Delta(\xrm; \Drm,\upthetac)$ are
\begin{IEEEeqnarray}{L} \label{eq:predictive_del_sq_dir_disc}
\Erm_{\Drm | \upthetam,\upthetac} \big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \big] \nonumber \\
\quad \equiv \Erm_{\uppsim' | \upthetam'}\Big[ \gammam'\big(T(\xrm); \uppsim'\big)^2 \Big] \Big( \alphac\big(T(\xrm)\big) - \upthetac\big(T(\xrm)\big) \Big) \otimes \Big( \alphac\big(T(\xrm)\big) - \upthetac\big(T(\xrm)\big) \Big) \nonumber \\
\qquad + \Erm_{\uppsim' | \upthetam'}\left[ \frac{\Big(1 - \gammam'\big(T(\xrm); \uppsim'\big)\Big)^2}{N \uppsim'\big(T(\xrm)\big)} \right] \bigg( \diag\Big( \upthetac'\big(T(\xrm)\big) \Big) - \upthetac'\big(T(\xrm)\big) \otimes \upthetac'\big(T(\xrm)\big) \bigg) \nonumber \\
\qquad + \Erm_{\uppsim' | \upthetam'}\Big[\gammam'\big(T(\xrm); \uppsim'\big)\Big] \Big( \alphac\big(T(\xrm)\big) - \upthetac'\big(T(\xrm)\big) \Big) \otimes \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \nonumber \\
\qquad + \Erm_{\uppsim' | \upthetam'}\Big[\gammam'\big(T(\xrm); \uppsim'\big)\Big] \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \otimes \Big( \alphac\big(T(\xrm)\big) - \upthetac'\big(T(\xrm)\big) \Big) \nonumber \\
\qquad + \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \otimes \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \;.
\end{IEEEeqnarray}

\todohigh{Consider OUTER PRODUCT TERM FROM BIAS?!?!?!?!?!?}



\subsection{Trends}

The bias/variance trends for the discretized Dirichlet distribution estimate \eqref{eq:P_y_xD_dir_cont_disc} have similarities with the trends of the discrete-domain Dirichlet distribution as detailed in Section \ref{sec:predictive_est_dir}. The bias and covariance formulae contain the same expectation forms as before, with $\upthetam$, $\upthetac$, $\uppsim$, and $\gammam$ being replaced by their new discretization variants; the earlier analysis is directly applicable with the appropriate substitutions. Note that $N \uppsim'(t) | \upthetam'(t) \sim \Bi\big(N, \upthetam'(t)\big)$; the relevant expectations will thus depend on the selection of the discretization function $T$.

There is one critical difference from the previous results: since the distribution estimate \eqref{eq:P_y_xD_dir_cont_disc} tends to the discretized $\upthetac'$ instead of the true model $\upthetac$, the new bias formula \eqref{eq:predictive_bias_dir_disc} contains an additional discretization term. This term depends neither on the training data volume $N$ nor on the Dirichlet parameterization $(\alpha_0, \alpham, \alphac)$. Consider the effect of the training volume $N$ on the bias. For $N=0$, the data-independent distribution is again maximally biased and has zero variance; the bias $\mathrm{Bias}(\xrm; \upthetam,\upthetac) = \alphac\big(T(\xrm)\big) - \upthetac(\xrm)$ now depends on the conditional prior mean only at values $t \in \Tcal \subset \Xcal$. As $N \to \infty$, for values $x$ satisfying $\upthetam(x) > 0$, only the first term in \eqref{eq:predictive_bias_dir_disc} vanishes. The residual $\mathrm{Bias}(\xrm; \upthetam,\upthetac) = \upthetac'\big(T(\xrm)\big) - \upthetac(\xrm)$ can not be reduced any further. It is clear that, unlike its discrete-domain relative \eqref{eq:P_y_xD_dir}, the distribution estimate \eqref{eq:P_y_xD_dir_cont_disc} will \emph{not} necessarily converge to the true predictive distribution $\upthetac$ in the limit of training data volume. 
...
First, consider the finest discretization, such that $|\Tcal| \to \infty$; assume that $\Xcal$ is bounded, such that the volume of the sets $\Scal(t)$ tend to zero. As a result, $T(x) \approx x$ and any smooth function $g$ over the domain $\Xcal$ will satisfy $g(x) \approx g(t), \ \forall x \in \Scal(t)$. Consequently, the discretized predictive model tends to $\upthetac'\big(T(x)\big) \approx \upthetac(x)$ and the discretization bias $\Big\|\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big\| \to 0$. However, the overall bias may still be high. If the marginal model $\upthetam$ is bounded, the discretized model tends to $\upthetam' \to 0$ and thus $\uppsim' | \upthetam' \to 0$. As a result, $\gammam'(\uppsim') \to 1$ and the first term in \eqref{eq:predictive_bias_dir_disc} is maximal. Also, observe that the covariance function \eqref{eq:predictive_cov_dir_disc} will tend to zero. Note that these trends are the same as those exhibited by the continuous-domain Dirichlet predictive distribution, as detailed in Section \ref{sec:predictive_est_dir_cont}; the higher the cardinality of the transformed observation set, the larger the training volume $N$ required to effectively learn the true model.

Next, consider $\Tcal$ to be a singleton, such that $|\Tcal| = 1$ and thus $\Scal(t) = \Xcal$. The discretization bias is at its highest, as the discretized conditional model is $\upthetac'(t) = \int_{\Xcal} \upthetac(x) \upthetam(x) {\drm}x$ and the same predictive model $\upthetac'\big(T(x)\big)$ is used for all $x \in \Xcal$. The benefit to such coarse discretization is that, on average, more data is available for each transformed observation. In this case, $\upthetam' = 1$, such that $\uppsim' | \upthetam' \to 1$ and the weighting function tends to $\gammam'(\uppsim') \to \left(1 + \frac{N}{\alpha_0 \alpham'(t)}\right)^{-1}$. As a result, the first term in the bias formula \eqref{eq:predictive_bias_dir_disc} is minimal for a given value $x$ (assuming the Dirichlet parameters are held constant). Additionally, if the training data volume $N$ is sufficiently high, this will cause the variance to decrease; this is a consequence of the conditional variance of $\uppsic'$ being inversely proportional to $\uppsim'$.

Evidently, the choice of how heavily to discretize the observation effects its own trade-off. Finer discretization is required to avoid irreducible discretization bias, yet may incur higher bias overall if the prior mean $\alphac$ is poorly selected. Coarser discretization provides more data for the estimation of each distribution $\upthetac'(t)$, but the discretization bias may severe if the true predictive distribution $\upthetac$ changes rapidly with $x$.
