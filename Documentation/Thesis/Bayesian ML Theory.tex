...
\todohi{move continuous integral formulas here}
...
The model $\uptheta$ has a continuous domain. Thus, the marginal model is defined as $\upthetam \equiv \int_{\Ycal} \uptheta(y,\cdot) {\drm}y \in \Pcal{\Xcal}$. 


\subsection{Empirical Statistic}

The training data is distributed as 
\begin{IEEEeqnarray}{rCl}
\Prm_{\Drm | \uptheta}\big( D | \theta \big) & = & \prod_{n=1}^N \Prm_{\Drm_n | \uptheta}\big( D_n | \theta \big) \;.
\end{IEEEeqnarray}
The dependency of the likelihood on the training data $\Drm$ can be shown to be only via $\Psi : \Dcal \mapsto \Uppsi \subset \Uptheta$, defined as
\begin{IEEEeqnarray}{rCl}
\Psi(y,x;D) & = & \frac{1}{N} \sum_{n=1}^N \delta \big( (y,x) - D_n \big) \\
& \equiv & \frac{1}{N} \sum_{n=1}^N \delta(y - Y_n) \delta(x - X_n) \nonumber \;.
\end{IEEEeqnarray}

\todohi{CONFIRM??}


As shown in Appendix \ref{app:EP}, given $\uptheta$, the new random process $\uppsi \equiv \Psi(\Drm) \in \Uppsi$ is a continuous-domain empirical process. The first and second joint moments of this Empirical process $\uppsi \sim \EP(N,\theta)$ are
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi | \uptheta} & = & \uptheta
\end{IEEEeqnarray}
and
...
The dependency of $\Prm_{\Xrm | \uptheta}\big( X | \theta \big) \equiv \Prm_{\Xrm | \upthetam}\big( X | \thetam \big) = \prod_{n=1}^N \Prm_{\Xrm_n | \uptheta}\big( X_n | \theta \big)$ on $X$ is expressed via the marginal empirical statistic $\Psim : \Xcal^N \mapsto \Uppsim \subset \Pcal(\Xcal)$, defined as
\begin{IEEEeqnarray}{rCl}
...
Recall that $\Drm_n \Leftrightarrow \big( \Yrm_n,\Xrm_n \big)$, where $\Yrm \in \Ycal^N$ and $\Xrm \in \Xcal^N$. Since $\prm_{\yrm,\xrm | \Drm} = \mu_{\uptheta | \Drm}$, by Bayes rule the conditional PDF of interest is
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \xrm,\Drm} & = & \frac{\alpha_0 \alpha(\cdot,\xrm) + N \Psi(\cdot,\xrm;\Drm)}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Drm)} \\
...
& = & \left(\frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm)+N \Psim(\xrm;\Drm)}\right) \alphac(\xrm) + \left(\frac{N \Psim(\xrm;\Drm)}{\alpha_0 \alpham(\xrm)+N \Psim(\xrm;\Drm)}\right) \Psic(\xrm;\Drm) \nonumber \;,
\end{IEEEeqnarray}
where the marginal and conditional functions are $\Psim(D) = \int_{\Ycal} \Psi(y,\cdot;D) {\drm}y \equiv N^{-1} \sum_{n=1}^N \delta(\cdot - X_n)$ and $\Psic(x;D) = \frac{\Psi(\cdot,x;D)}{\Psim(x;D)} \equiv \frac{\sum_{n=1}^N \delta(\cdot - Y_n) \delta(x - X_n)}{\sum_{n=1}^N \delta(\cdot - X_n)} = \frac{\sum_{n=1}^N \delta(\cdot - Y_n) \delta[x,X_n]}{\sum_{n=1}^N \delta[x,X_n]}$.

The conditional distribution when $\Xcal$ is a Euclidean space has notable differences from its form for a countable set $\Xcal$. Specifically, since $\Psim(x;D)$ is either zero or tends towards infinity, a non-impulsive prior mean $\alpha$ will result in convex coefficients that are zero or one. In this case, the distribution for a given observation $\xrm$ will be either strictly dependent on either the training data or the prior knowledge.



\subsubsection{Via the Conditional Model Process}

Using the properties detailed in Appendix \ref{app:DP_post}, it can be shown that the marginal process satisfies $\upthetam | \Drm \sim \upthetam | \Xrm \sim \DP(\alpha_0, \mu_{\upthetam | \Xrm})$, where
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetam | \Xrm} & = & \left(\frac{\alpha_0}{\alpha_0+N}\right) \alpham + \left(\frac{N}{\alpha_0+N}\right) \Psim(\Xrm) \;.
\end{IEEEeqnarray}

As the marginal empirical statistic is sufficient, observe that $\upthetam | \{\Drm = D\} \sim \upthetam | \big\{\uppsim = \Psim(X)\big\}$ and that $\upthetam | \uppsim \sim \DP(\alpha_0, \mu_{\upthetam | \uppsim})$, where
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetam | \uppsim} & = & \left(\frac{\alpha_0}{\alpha_0+N}\right) \alpham + \left(\frac{N}{\alpha_0+N}\right) \uppsim \;.
\end{IEEEeqnarray}


\todohigh{CONFIRM?}

Also, it can be shown that $\upthetac(x) | \xrm,\Drm \sim \upthetac(x) | \Drm \sim \DP\big(\alpha_0 \alpham(x), \mu_{\upthetac(x) | \Drm} \big)$, where
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetac(x) | \Drm} & = & \left(\frac{\alpha_0 \alpham(x)}{\alpha_0 \alpham(x)+N \Psim(x;\Drm)}\right) \alphac(x) + \left(\frac{N \Psim(x;\Drm)}{\alpha_0 \alpham(x)+N \Psim(x;\Drm)}\right) \Psic(x;\Drm) \nonumber \;.
\end{IEEEeqnarray}

Using the empirical data representation, observe that $\upthetac(x) | \{\xrm,\Drm = x,D\} \sim \upthetac(x) | \left\{ \uppsim = \Psim(D), \uppsic = \Psic(D) \right\}$ and that $\DP\big(\alpha_0 \alpham(x), \mu_{\upthetac(x) | \uppsim,\uppsic} \big)$, where
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetac(x) | \uppsim,\uppsic} & = & \left(\frac{\alpha_0 \alpham(x)}{\alpha_0 \alpham(x)+N \uppsim(x)}\right) \alphac(x) + \left(\frac{N \uppsim(x)}{\alpha_0 \alpham(x)+N \uppsim(x)}\right) \uppsic(x) \nonumber \;.
\end{IEEEeqnarray}

\todohigh{introduce empirical statistic functions??}
...
\begin{IEEEeqnarray}{rCl}
\prm_{\Xrm_n} & = & \mu_{\upthetam} = \alpham \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\prm_{\Yrm | \Xrm}(Y | X) & = & \frac{\alpha\big( Y_1,X_1 \big)}{\alpha'\big( X_1 \big)} \prod_{n=2}^N \frac{\alpha\big( Y_n,X_n \big) + \sum_{i=1}^{n-1} \delta\big( Y_n-Y_i \big) \delta\big( X_n-X_i \big)}{\alpham\big( X_n \big) + \sum_{i=1}^{n-1} \delta\big( X_n-X_i \big)} 
\end{IEEEeqnarray}
...
\todomid{Below: location?}

Additionally, observe that $\uptheta | \{\Drm = D\} \sim \uptheta | \left\{ \uppsi = \Psi(D)\right\}$; that is, the statistic $\Psi(\Drm)$ encapsulates all the relevant information about the training set. This is a natural generalization of the results for the discrete-domain model process. Additionally, note that $\upthetam | \{\Drm = D\} \sim \upthetam | \big\{ \uppsim = \Psim(D)\big\}$. The model aggregation process is only dependent on the aggregation of the empirical distribution. These properties result from the independence of $\upthetam$ from $\upthetac$, a property that Dirichlet processes hold.

Also, when an Empirical process $\uppsi \sim \EP(N,\theta)$ has a mean function which is characterized by a Dirichlet process $\uptheta \sim \DP(\alpha_0,\alpha)$, the posterior $\uptheta | \uppsi \sim \DP(\alpha_0 + N, \mu_{\uptheta | \uppsi})$, with
\begin{IEEEeqnarray}{rCl}
