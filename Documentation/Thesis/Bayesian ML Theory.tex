...
As shown in Appendix \ref{app:EP}, given $\uptheta$, the new random process $\uppsi \equiv \Psi(\Drm) \in \Uppsi$ is a continuous-domain empirical process $\uppsi \sim \EP(N,\theta)$. The first and second joint moments are
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi | \uptheta} & = & \uptheta
\end{IEEEeqnarray}
and
...
The dependency of $\Prm_{\Xrm | \uptheta}\big( X | \theta \big) \equiv \Prm_{\Xrm | \upthetam}\big( X | \thetam \big) = \prod_{n=1}^N \thetam(X_n)$ on $X$ is expressed via the marginal empirical statistic $\Psim : \Xcal^N \mapsto \Uppsim \subset \Pcal(\Xcal)$, defined as
\begin{IEEEeqnarray}{rCl}
...
Additionally, given the model $\uptheta$, the conditional processes $\uppsic \in \Uppsic$ are independent Empirical processes $\uppsic(x) | \uppsim(x),\upthetac(x) \sim \EP\big( N \uppsim(x) / \delta(0),\upthetac(x) \big)$. They are independent of the marginal model $\upthetam$.
...
Using the empirical statistic representation, observe that $\upthetam | \uppsim,\uppsic \sim \upthetam | \uppsim \sim \DP(\alpha_0, \mu_{\upthetam | \uppsim})$, where
\begin{IEEEeqnarray}{rCl}
...
Using the empirical statistic representation, observe that $ \upthetac(x) | \xrm,\uppsim,\uppsic \sim \upthetac(x) | \uppsim(x),\uppsic(x) \sim \DP\big(\alpha_0 \alpham(x), \mu_{\upthetac(x) | \uppsim(x),\uppsic(x)} \big)$, where
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetac(x) | \uppsim(x),\uppsic(x)} & = & \left(\frac{\alpha_0 \alpham(x)}{\alpha_0 \alpham(x)+N \uppsim(x)}\right) \alphac(x) + \left(\frac{N \uppsim(x)}{\alpha_0 \alpham(x)+N \uppsim(x)}\right) \uppsic(x) \nonumber \;.
\end{IEEEeqnarray}
...
To find the distribution $\prm_{\Yrm | \Xrm}$, note that the independent observations $\Yrm_n | \Xrm_n,\upthetac$ are conditionally independent of $\Xrm_i$, $i \neq n$, and are characterized by the joint processes $\upthetac(x) \sim \DP\big(\alpha_0 \alpham(x), \alphac(x)\big)$, such that 
\begin{IEEEeqnarray}{rCl}
...
The first and second order conditional distributions are
\begin{IEEEeqnarray}{rCl}
\Prm_{\Yrm_n | \Xrm_n}(x) & = & \Prm_{\yrm | \xrm}(x) = \mu_{\upthetac(x)} = \alphac(x) 
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{L}
...
Observe that by the aggregation principle, $\uppsim \sim \DEP(N,\alpha_0,\alpham)$ is a DEP over the set $\Xcal$.

Additionally, the 1-dimensional subsets conditioned on the marginalized DEP are characterized as

\begin{equation}
\uppsic(x) \Big| \uppsim(x) \sim \DEP\left( \frac{\uppsim(x)}{\delta(0)}, \alpha_0 \alpham(x) / \delta(0), \alphac(x) \right)
\end{equation}
...
= \frac{\alpha_0 \alpham(\xrm) \int_{\Ycal} \alphac(y;\xrm) \Lcal(h,y) {\drm}y + N \Psim(\xrm;\Drm) \int_{\Ycal} \Psic(y;\xrm;\Drm) \Lcal(h,y) {\drm}y}{\alpha_0 \alpham(\xrm) + N \Psim(\xrm;\Drm)} \nonumber \\
