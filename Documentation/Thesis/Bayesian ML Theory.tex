...
\todolo{CHECK for $\Theta$ vs $\Uptheta$}

\todolo{check notation throughout for expectation/variance operators, proper arguments}
...
\todohi{Cartesian product notation!!}
...
\end{equation}
which can be easily shown to be less than both the asymptotic values for $\alpha'(x) \to 0$ and $\alpha'(x) \to \infty$. Thus the concentation values \eqref{eq:alpha_x_min_Rex} can be used to find the optimal values of $\alpha_0$ and $\alpham(x)$ leading to the minimum excess risk for the given prior conditional distributions $\alphac(x)$.
...
& = & \frac{\alpha\big( D_1 \big)}{\alpha_0} \prod_{n=2}^N \frac{\alpha(D_n) + \sum_{i=1}^{n-1} \delta\big( D_n - D_i \big)}{\alpha_0+n-1} \nonumber \;.
\end{IEEEeqnarray}
...
\todohi{Specify dirac delta domains!}
...
If the Empirical random process $\uppsi$ is conditioned on its aggregation $\uppsim$ over the partition $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, the distinct segements $\uppsi_z$ become independent random processes, such that for $\psi \in \Uppsi'(\psim)$,
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
Thus, when conditioned on the aggregation $\uppsim \in \Uppsim \subset \Pcal(\Zcal)$, the individual segments of the random process $\uppsic \in \Pcal(\Scal_z)^{\Zcal}$ are independent empirical processes of $N \uppsim(z)$ samples, parameterized by the individual segments of $\thetac \in \Pcal(\Scal_z)^{\Zcal}$.
...
\section{Discrete-domain Dirichlet Process conditioned on its aggregation}
\label{app:Dir_agg}
...
\section{Discrete-domain Dirichlet-Empirical process conditioned on its aggregation} 
\label{app:DE}
...
While these individual function segments are independent, they are not Dirichlet-Empirical processes. Defining the conditional distributions $\alphac(z) \equiv \alpha_z / \alpham(z) \in \Pcal(\Scal_z)$ and the normalized segments $\uppsic(z) \equiv \uppsi_z / \uppsim(z)$, it can be shown that
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
Thus, when conditioned on the aggregation $\uppsim \in \Uppsim \subset \Pcal(\Zcal)$, the individual segments of the random process $\uppsic \in \Pcal(\Scal_z)^{\Zcal}$ are independent Dirichlet-Empirical processes of $N \uppsim(z)$ samples and concentration $\alpha_0 \alpham(z)$, parameterized by the conditional distributions $\alphac(z)$.
...
\section{Discrete-domain Dirichlet-Multinomial random process conditioned on its aggregation} 
\label{app:DM_agg}
...
\section{First and Second moments of a continuous-domain Dirichlet Process} \label{app:E_DP}
...
The defining characteristic of Dirichlet processes is that their aggregations are also Dirichlet. Define the partition of $\Ycal$, $\big\{ \Scal(y),S^\text{c}(y) \big\}$ where $\Scal(y) = \{ t \in \Ycal : t \leq y \}$. The transform random variable $\upthetam(y) \equiv \int_{-\infty}^y \uptheta(t) \mathrm{d} t$ is thus a Beta random variable with parameters $\lambda = \alpha_0 \int_{-\infty}^y \alpha(t) \mathrm{d} t$ and $\lambda^\text{c} = \alpha_0 \int_y^\infty \alpha(t) \mathrm{d} t$. Observe that $\mu_{\upthetam}(y) \equiv \int_{-\infty}^y \mu_{\uptheta}(t) \mathrm{d} t$ and that using the formula for the expected value of a beta random variable \cite{papoulis},
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetam} & = & \frac{\lambda}{\lambda+\lambda^\text{c}} \\
& = & \int_{-\infty}^y \alpha(t) \mathrm{d} t = \int_{-\infty}^y \mu_{\uptheta}(t) \mathrm{d} t \nonumber \;.
\end{IEEEeqnarray}
Differentiating with respect to $y$, we have the expected value of the DP.

Next, the correlation function is shown to be 
\begin{equation}
\Erm_{\uptheta}\big[ \uptheta(y_1)\uptheta(y_2) \big] = \frac{\alpha_0 \alpha(y_1)\alpha(y_2) + \alpha(y_1)\delta(y_1-y_2)}{\alpha_0+1} \;.
\end{equation}
First, assume $y_2 \geq y_1$ and define a new partition of $\Ycal$, $\big\{ (-\infty,y_1], (y_1,y_2], (y_2,\infty) \big\}$. By the aggregation property, the random triplet $\left( \int_{-\infty}^{y_1} \uptheta(t) \mathrm{d} t, \int_{y_1}^{y_2} \uptheta(t) \mathrm{d} t, \int_{y_2}^{\infty} \uptheta(t) \mathrm{d} t \right)$ is Dirichlet with concentration $\alpha_0$ and mean $\left( \int_{-\infty}^{y_1} \alpha(t) \mathrm{d} t, \int_{y_1}^{y_2} \alpha(t) \mathrm{d} t, \int_{y_2}^{\infty} \alpha(t) \mathrm{d} t \right)$.
...
\quad = \frac{\left( \int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right) \left( 1 + \alpha_0\int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right) + \alpha_0 \left( \int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right) \left( \int_{y_1}^{y_2} \alpha(t_2) \mathrm{d} t_2 \right)}{\alpha_0+1} \nonumber \\
\quad = \frac{\alpha_0 \left( \int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right) \left( \int_{-\infty}^{y_2} \alpha(t_2) \mathrm{d} t_2 \right) + \left( \int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right)}{\alpha_0+1} \quad \forall y_2 \geq y_1 \nonumber \;.
\end{IEEEeqnarray}
Following the same steps provides the values of $g$ for $t_2 \leq t_1$; the combined formula can be given as
\begin{IEEEeqnarray}{L}
g(t_1,t_2) = \frac{\alpha_0 \left( \int_{-\infty}^{y_1} \alpha(t_1) \mathrm{d} t_1 \right) \left( \int_{-\infty}^{y_2} \alpha(t_2) \mathrm{d} t_2 \right) + \left( \int_{-\infty}^{\min(y_1,y_2)} \alpha(t_1) \mathrm{d} t_1 \right)}{\alpha_0+1} \;. 
\end{IEEEeqnarray}
Finally,
\begin{IEEEeqnarray}{L}
\Erm_{\uptheta}\big[ \uptheta(y_1)\uptheta(y_2) \big] = \frac{\mathrm{d}^2}{\mathrm{d} t_1 \mathrm{d} t_2} g(t_1,t_2) \\
\quad = \frac{\frac{\mathrm{d}}{\mathrm{d} t_2} \left[ \alpha_0 \alpha(y_1) \left( \int_{-\infty}^{y_2} \alpha(t_2) \mathrm{d} t_2 \right) + u(t_2-t_1) \alpha\big( \min(t_1,t_2) \big) \right]}{\alpha_0+1} \nonumber \\
\quad = \frac{\alpha_0 \alpha(y_1)\alpha(y_2) + \alpha(y_1)\delta(y_1-y_2)}{\alpha_0+1} \nonumber \;.
\end{IEEEeqnarray}
...
\section{Proof: Continuous-domain Model Posterior Distribution is Dirichlet} \label{app:DP_post}

In this section, it is shown that if the model $\uptheta \sim \DP(\alpha_0, \alpha)$ is a Dirichlet process, then the model conditioned on the training data $\Drm$ is also a Dirichlet process with concentration $\alpha_0 + N$ and mean function 
\begin{equation}
...
The defining characteristic of Dirichlet processes is that their aggregations are also Dirichlet. Consider a DP over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$ and the corresponding function partitions $\theta_z \in \Rbbgeq^{\Scal_z}$, such that $\uptheta = \left\{ \ldots,\uptheta_z,\ldots \right\}$, and $\alpha_z \in \Rbbgeq^{\Scal_z}$, such that $\alpha = \left\{ \ldots,\alpha_z,\ldots \right\}$. The transformed random process $\upthetam \in \Pcal(\Zcal)$, $\upthetam(z) \equiv \int_{\Scal_z} \uptheta_z(y) \mathrm{d} y$, is neccessarily Dirichlet with concentration $\alpha_0$ and a mean function $\alpham \in {\Rbb^+}^{\Zcal}$, $\alpham(z) \equiv \int_{\Scal_z} \alpha_z(y) \mathrm{d} y$.
...
\end{equation}
and $\Psim(z;\Drm) = \int_{\Scal_z} \Psi(y;D) \mathrm{d} y = N^{-1} \sum_{n=1}^N \chi(\Drm_n;\Scal_z)$. $\chi$ is the indicator function
\begin{equation}
...
To prove the hypothesis, exploit the results of Appendix \ref{app:Dir_agg} to represent the training data distribution conditioned on the aggregation $\upthetam$. Introducing the normalized segments $\upthetac(z) \equiv \uptheta_z / \upthetam(z) \in \Pcal(\Scal_z)$, it can be shown that
...
\end{IEEEeqnarray}
Observe that the dependency of this likelihood function on $\upthetam$ is polynomial. The training data marginal distribution is
\begin{IEEEeqnarray}{rCl}
...
\section{The continuous-domain Empirical Process} \label{app:EP}


\subsection{Definition}

This section introduces a new continuous-domain random process, referred to as the Empirical process (EP). It is the generalization of the Empirical distribution for i.i.d. samples drawn from a PDF. The Empirical process assumes functions from the set $\Uppsi = \left\{ N^{-1} \sum_{n=1}^N \delta(\cdot - D_n) : D \in \Ycal^N \right\}$ and is parameterized by a function $\theta : \Ycal \mapsto \Rbbgeq$.

The continuous-domain Empirical process possesses the same aggregation property as its discrete-domain variant. Define a partition of $\Ycal$, $\left\{ \ldots,\Scal_z,\ldots \right\}$, $z \in \Zcal$, and the corresponding function partitions $\uppsi_z \in \Rbbgeq^{\Scal_z}$, such that $\uppsi = \left\{ \ldots,\uppsi_z,\ldots \right\}$, and $\theta_z \in \Rbbgeq^{\Scal_z}$, such that $\theta = \left\{ \ldots,\theta_z,\ldots \right\}$. Thus, for a Empirical process $\uppsi \in \Uppsi$, the transformed random process $\uppsim$ satisfying $\uppsim(z) \equiv \int_{\Scal_z} \uppsi_z(y) \mathrm{d} y$ is neccessarily Emprical with parameterizing function $\thetam$ satisfying $\thetam(z) \equiv \int_{\Scal_z} \theta_z(y) \mathrm{d} y$.

Consequently, when conditioned on the aggregation $\uppsim$, the normalized segments $\uppsic(z) \equiv \uppsi_z / \uppsim(z)$ are independent continuous-domain Empirical processes, $\uppsic(z) | \uppsim(z), \upthetac(z) \sim \EP\big(N \uppsim(z), \upthetac(z)\big)$, where $\thetac(z) \equiv \theta_z / \thetam(z) \in \Pcal(\Scal_z)$.



\subsection{Proof that $\uppsi \equiv \sum_{n=1}^N \delta(\cdot - \Drm_n)$ given $\uptheta$ is an EP}

Next, it is demonstrated that conditioned on $\uptheta$, the random process $\uppsi \equiv \Psi(\Drm) = \sum_{n=1}^N \delta(\cdot - \Drm_n)$ is an EP, given that $\prm_{\Drm|\uptheta}(D|\theta) = \prod_{n=1}^N \theta(D_n)$.

Observe that $\uppsim(z) \equiv N^{-1} \sum_{n=1}^N \chi(\Drm_n;\Scal_z)$, where $\chi$ is the indicator function
\begin{equation}
\chi(x;S) = \begin{cases} 1 & \mathrm{if} \ x \in S, \\ 0 & \mathrm{if} \ x \notin S.  \end{cases}
\end{equation}
and note that $\Prm\big( \chi(\Drm_n;\Scal_z) = 1 \big| \uptheta \big) = \upthetam(z)$, where $\upthetam(z) \equiv \int_{\Scal_z} \uptheta(y) \mathrm{d} y$. Since the events $\Drm_n \in \Scal_z$ are independent given $\uptheta$, $\uppsim$ conditioned on the model $\uptheta$ is characterized by an Empirical distribution 
\begin{equation}
\Prm_{\uppsim | \uptheta}(\psim | \theta) \equiv \Mcal(N \psim) \prod_{z \in \Zcal} \left( \thetam(z) ^{\psim(z)} \right)^{N} = \Emp\big( \psim(z); N,\thetam(z) \big) \;.
\end{equation}
Since the aggregation property is satisfied, $\uppsi$ is a Empirical process.



\subsection{Mean and Correlation Functions}

In this subsection the mean and correlation functions of a continuous-domain EP are expressed. The mean function is defined as
\begin{IEEEeqnarray}{rCl}
\mu_{\uppsi}(y) & = & \Erm_{\Drm | \uptheta}\left[ \frac{1}{N} \sum_{n=1}^N \delta\big( y-\Drm_n \big) \right] \\
& = & \frac{1}{N} \sum_{n=1}^N \Prm_{\Drm_n | \uptheta(y)} = \frac{1}{N} \sum_{n=1}^N \uptheta(y) \nonumber \\
& = & \uptheta(y) \nonumber \;.
\end{IEEEeqnarray}
...
\section{The continuous-domain Dirichlet-Emprical Process} \label{app:DMP}
...
\chi(x;S) = \begin{cases} 1 & \mathrm{if} \ x \in S, \\ 0 & \mathrm{if} \ x \notin S.  \end{cases}
\end{equation}
and note that $\Prm\Big( \chi(\Drm_n;\Scal_z) = 1 \big| \uptheta \Big) = \int_{\Scal_z} \uptheta(y) \mathrm{d} y$. As such, $\uppsim$ conditioned on the model $\uptheta$ is characterized by a multinomial distribution 
\begin{equation}
\Prm_{\uppsim | \uptheta}(\psim | \theta) = \Mcal(N \psim) \prod_{z \in \Zcal} \left( \int_{\Scal_z} \theta(y) \mathrm{d} y \right)^{N \psim(z)} = \Multi\left( n' ; N,\thetam(z) \right) \;,
\end{equation}
...
\section{The continuous-domain Dirichlet-Multinomial Process} \label{app:DMP}
