...
Also of interest is the distribution of $\uppsic \in \Uppsic$ conditioned on its aggregation $\uppsim$, where $\uppsic(x) \equiv \uppsi(\cdot,x) / \uppsim(x)$. Using the Empirical process properties proven in Appendix \ref{app:emp}, it can be shown that when conditioned on the model $\uptheta$ as well, the PMF of $\uppsic$ is
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \uppsim , \uptheta}(\psi | \psim , \theta) & \equiv & \Prm_{\uppsi | \uppsim , \upthetac}(\psi | \psim , \thetac) \\
...
\end{IEEEeqnarray}
over the domain $\prod_{x \in \Xcal} \left\{ \frac{n}{N \psim(x)}: n \in {\Zbbgeq}^{\Ycal}, \ \sum_{y \in \Ycal} n(y) = N \psim(x) \right\}$.
\todohigh{Set notation above?}

Observe that conditioning on the marginal empirical process renders the conditional processes $\uppsic(x)$ independent of one another and that they are also Empirically distributed, such that $\uppsic(x) | \uppsim(x),\upthetam,\upthetac = \uppsic(x) | \uppsim(x),\upthetac \sim \Emp\big( N \uppsim(x),\upthetac(x) \big)$. Furthermore, the dependency on $\uptheta$ is expressed only through the conditional model $\upthetac$
...
& \equiv & \prod_{z \in \Zcal} \Bigg[ \Mcal(N \psi_z) \left(\frac{\prod_{y \in \Scal_z} \theta_z(y)^{\psi_z(y)}}{\thetam(z)^{\psim(z)}} \right)^N \Bigg] \nonumber \\
& = & \prod_{z \in \Zcal} \Bigg[ \Mcal(N \psi_z) \left( \prod_{y \in \Scal_z} \left(\frac{\theta_z(y)}{\thetam(z)}\right)^{\psi_z(y)} \right)^N \Bigg] \nonumber \;.
\end{IEEEeqnarray}

While these function segments are independent, they are not Empirical processes. Introducing the conditional distributions $\thetac(z) \equiv \theta_z / \thetam(z) \in \Pcal(\Scal_z)$ and the normalized segments $\uppsic(z) \equiv \uppsi_z / \uppsim(z)$, it can be shown that
\begin{IEEEeqnarray}{rCl}
