...
\subsubsection{Example V2} \label{sec:SE_dir_example}
...
\Cref{Discrete/SE/risk_bayes_N_leg_a0,fig:Discrete/SE/risk_bayes_a0_leg_N} display the Bayesian squared-error realized by the Dirichlet-based regressor. The model $\uptheta$ is randomly selected from a Dirichlet distribution with the same $\alpham$ and $\alphac$ used for the regressor design and with the localization fixed at $\alpha_0 = 100$. Observe that with increasing training data volume $N$, all the regressors tend towards the expected irreducible risk. Additionally, the regressor performs best when its localization matches $\alpha_0 = 100$, regardless of the value $N$, achieving the Bayesian squared-error \eqref{eq:Risk_min_SE_dir}.
\begin{figure}
	\centering
