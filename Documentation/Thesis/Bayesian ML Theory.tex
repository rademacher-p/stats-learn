...
\todohi{fix arguments for theta, nbar dists.}
...
\todohigh{change from nbar to emprical RV???}
...
& = & \prod_{y \in \Ycal} \prod_{x \in \Xcal} \theta(y,x)^{\Psi(y,x;D)} \nonumber \;,
\end{IEEEeqnarray}
where the dependency on the training data $\Drm$ is expressed though a transform function $\Psi : \Dcal \mapsto \Theta_\uppsi \subset \Theta$, where the range is 
\begin{IEEEeqnarray}{rCl}
\Theta_\uppsi & = & \left\{ \frac{\psi}{N} : \psi \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal}, \ \sum_{y \in \Ycal} \sum_{x \in \Xcal} \psi(y,x) = N \right\} 
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
This function determines the empirical probability of the pair $(y,x)$. Note that the set $\Theta_\uppsi$ is a finite subset of $\Theta$ and thus that the empirical distribution $\Psi(\Drm)$ assumes one of a finite number of the elements from $\Theta$.


Note that $\Prm_{\Drm | \uptheta}$ depends on the training data $\Drm$ only through the transform $\Psi$; $\Psi(\Drm)$ is thus a sufficient statistic \cite{bernardo} for the model $\uptheta$. Consequently, other distributions of interest $\Prm_{\Drm}$, $\Prm_{\xrm | \Drm}$, and $\Prm_{\yrm | \xrm,\Drm}$ will also depend on $\Drm$ via $\Psi(\Drm)$. As such, it is useful to define a new random process $\uppsi \equiv \Psi(\Drm) \in \Theta_\uppsi$. 
...
The cardinality of the random process' domain is $|\Theta_\uppsi| = \Mcal\big( \{N,|\Ycal||\Xcal|-1\} \big)$; this can be shown using the stars-and-bars method \cite{feller}. The cardinality of original set is $|\Dcal| = \big( |\Ycal| |\Xcal| \big)^N$; thus $|\Theta_\uppsi| \leq |\Dcal|$ and the sufficient statistic compactly represents the valuable information in the training data. 
...
\end{IEEEeqnarray}
This can be easily determined using the first and second moments of the Multinomial distribution.
...
\begin{IEEEeqnarray}{rCl}
\theta_\mathrm{ML}\big( \psi \big) & = & \argmax_{\theta \in \Theta} \Prm_{\uppsi | \uptheta}(\psi | \theta) \\ \nonumber
& = & \psi \;.
\end{IEEEeqnarray}
...
& = & \prod_{x \in \Xcal} \thetam(x)^{N'(x;X)} \nonumber \;,
\end{IEEEeqnarray}
where the dependency on $\uptheta$ is only through the marginal model $\upthetam$. Additionally, note that the dependency on the training observations $\Xrm$ is expressed though a ``marginal'' counting function $N' : \Xcal \mapsto \Ncal'$ with range
\begin{IEEEeqnarray}{rCl}
\Ncal' & = & \left\{ n' \in {\Zbb_{\geq 0}}^{\Xcal}: \sum_{x \in \Xcal} n'(x) = N \right\} \;,
\end{IEEEeqnarray}
defined as
\begin{IEEEeqnarray}{rCl}
N'(X) & = & \sum_{n=1}^N \delta\big[ \cdot,X_n \big] \equiv \sum_{y \in \Ycal} \Psi(y,\cdot;D) \;.
\end{IEEEeqnarray}
...
& = & \prod_{x \in \Xcal} \left[ \prod_{y \in \Ycal} \thetac(y;x)^{\Psi(y,x;Y,X)} \right] \nonumber \;,
\end{IEEEeqnarray}
which is dependent on the model $\uptheta$ only through the conditional models $\upthetac(x)$.


As before, the dependency on the training data can be simplified using a sufficient statistic. Introduce the ``marginalized'' random process $\nrm'$ over the set $\Xcal$, defined as $\nrm' \equiv \sum_{y \in \Ycal} \uppsi(y,\cdot) \equiv N'(\Xrm) \in \Ncal'$. By the aggregation property of Multinomial random processes \cite{johnson}, the aggregation conditioned on the model $\uptheta$ is distributed as $\nrm' | \uptheta \sim \Multi(N,\upthetam)$. 

Also of interest is the distribution of $\uppsi$ conditioned on its aggregation $\nrm'$. Using the multinomial distribution properties proven in Appendix \ref{app:mult}, it can be shown that when conditioned on the model $\uptheta$ as well, the PMF of $\uppsi$ is
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \nrm' , \uptheta}(\psi | n' , \theta) & \equiv & \prod_{x \in \Xcal} \Bigg[ \Mcal\big( \psi(\cdot,x) \big) \prod_{y \in \Ycal} \thetac(y;x)^{\psi(y,x)} \Bigg] \\
& = & \prod_{x \in \Xcal} \Multi\Big( \psi(\cdot,x) ; n'(x) , \thetac(x) \Big) \nonumber \;,
\end{IEEEeqnarray}
over the domain $\left\{ \psi \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \psi(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Multinomial, such that $\uppsi(\cdot,x) | \nrm'(x),\uptheta \sim \Multi\big( \nrm'(x),\upthetac(x) \big)$. Furthermore, the dependency on $\uptheta$ is expressed through the conditional model $\upthetac$
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi}(\thetam,\thetac | \psi) & = & \frac{\Prm_{\uppsi | \nrm',\upthetac}\big( \psi | \sum_y \psi(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \nrm'}\big( \psi | \sum_y \psi(y,\cdot) \big)} 
\frac{\Prm_{\nrm' | \upthetam}\big( \sum_y \psi(y,\cdot) | \thetam \big)}{\Prm_{\nrm'}\big( \sum_y \psi(y,\cdot) \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\nrm'}\Big( \thetac | \psi,\sum_y \psi(y,\cdot) \Big) \prm_{\upthetam | \nrm'}\Big( \thetam | \sum_y \psi(y,\cdot) \Big) 
\frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \nrm'}\big( \thetac | \sum_y \psi(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}

% Note that $\prm_{\upthetac | \Xrm}(X) = \prm_{\upthetac | \nrm'}\big( N'(X) \big) = \Erm_{\upthetam | \Xrm}\big[ \prm_{\upthetac | \upthetam} \big](X)$.

Note that $\prm_{\upthetac | \Xrm} = \Erm_{\upthetam | \Xrm}\big[ \prm_{\upthetac | \upthetam} \big]$ and $\prm_{\upthetac | \nrm'} = \Erm_{\upthetam | \nrm'}\big[ \prm_{\upthetac | \upthetam} \big]$. Also, note that $\Prm_{\Yrm | \Xrm} = \Erm_{\upthetam | \Xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and $\Prm_{\uppsi | \nrm'} = \Erm_{\upthetam | \nrm'} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\uppsi | \nrm',\upthetac} \big] \Big]$.
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \uppsi,\xrm}(\thetam,\thetac | \psi,x) & = & \frac{\Prm_{\uppsi | \nrm',\upthetac}\big( \psi | \sum_y \psi(y,\cdot),\thetac \big)}{\Prm_{\uppsi | \nrm',\xrm}\big( \psi | \sum_y \psi(y,\cdot),x \big)} 
\frac{\Prm_{\nrm',\xrm | \upthetam}\big( \sum_y \psi(y,\cdot),x | \thetam \big)}{\Prm_{\nrm',\xrm}\big( \sum_y \psi(y,\cdot),x \big)} \prm_{\upthetam,\upthetac}(\thetam,\thetac) \\
& = & \prm_{\upthetac | \uppsi,\nrm'}\Big( \thetac | \psi,\sum_y \psi(y,\cdot) \Big) \prm_{\upthetam | \nrm',\xrm}\Big( \thetam | \sum_y \psi(y,\cdot),x \Big) \nonumber \\
&& \quad \frac{\Prm_{\uppsi | \nrm'}\big( \psi | \sum_y \psi(y,\cdot) \big)}{\Prm_{\uppsi | \nrm',\xrm}\big( \psi | \sum_y \psi(y,\cdot),x \big)} \frac{\prm_{\upthetac | \upthetam}(\thetac | \thetam)}{\prm_{\upthetac | \nrm'}\big( \thetac | \sum_y \psi(y,\cdot) \big)} \nonumber
\end{IEEEeqnarray}

Note that $\Prm_{\Yrm | \Xrm,\xrm} = \Erm_{\upthetam | \Xrm,\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and also that $\Prm_{\uppsi | \nrm',\xrm} = \Erm_{\upthetam | \nrm',\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\uppsi | \nrm',\upthetac} \big] \Big]$.
...
\begin{equation}
\theta_\mathrm{max} = \argmax_{\theta \in \Theta} \prm_{\uptheta}(\theta) = \frac{\alpha_0 \alpha - 1}{\alpha_0 - |\Ycal||\Xcal|} \;.
\end{equation}
...
and
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi} & = & |\Theta_\uppsi|^{-1} = \Mcal\big( \{N,|\Ycal||\Xcal|-1\} \big)^{-1} \;.
\end{IEEEeqnarray}
The distribution of $\uppsi$ is uniform over the set $\Theta_\uppsi$. The PMF for $\Drm$ depends on the training data only through the multinomial coefficient; consequently, more ``concentrated''  training sets are more probable.
\begin{figure}
\centering
...
& = & \Erm_{\upthetam} \left[ \prod_{x \in \Xcal} \upthetam(x)^{N'(x;X)} \right] \nonumber \\
& = & \beta(\alpha_0 \alpham)^{-1} \beta\big( \alpha_0 \alpham + N'(X) \big) \nonumber \;.
\end{IEEEeqnarray}
...
The corresponding distributions for the sufficient statistics will be expressed as well. Recall that $\nrm' | \uptheta \sim \Multi(N,\upthetam)$; by the aggregation property of Dirichlet-Multinomial functions \cite{johnson}, the random process is distributed as $\nrm' \sim \DM(N,\alpha')$.

Also of interest is the distribution of $\uppsi$ conditioned on its aggregation $\nrm'$. Using the Dirichlet-Multinomial properties presented in Appendix \ref{app:DM_agg}, it can be shown that
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \nrm'}(\psi | n') & = & \prod_{x \in \Xcal} \left[ \Mcal\big( \psi(\cdot,x) \big) \beta\big( \alpha(\cdot,x) \big)^{-1} \beta\big( \alpha(\cdot,x) + \psi(\cdot,x) \big) \right] \\
& = & \prod_{x \in \Xcal} \DM\Big( \psi(\cdot,x) ; n'(x) , \alpha(\cdot,x) \Big) \nonumber
\end{IEEEeqnarray}
over the domain $\left\{ \psi \in {\Zbb_{\geq 0}}^{\Ycal \times \Xcal} : \sum_{y \in \Ycal} \psi(y,\cdot) = n' \right\}$. Observe that conditioning on the aggregation renders the function segments $\uppsi(\cdot,x)$ independent of one another and that they are also Dirichlet-Multinomial, such that $\uppsi(\cdot,x) | \nrm'(x) \sim \DM\big( \nrm'(x),\alpha(\cdot,x) \big)$.
...
\begin{IEEEeqnarray}{rCl}
\theta_\mathrm{MAP}(D) & = & \argmax_{\theta \in \Theta} \Prm_{\uptheta | \Drm}(\theta | D) = \frac{\Psi(D) + \alpha - 1}{N + \alpha_0 - |\Ycal||\Xcal|} \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\xrm | \uppsi}(\psi) & = & \mu_{\upthetam | \uppsi}(\psi) = \mu_{\upthetam | \nrm'}\left( \sum_y \psi(y,\cdot) \right) \\
& = & \frac{\alpha' + \sum_y \psi(y,\cdot)}{\alpha_0 + N} \nonumber \;,
\end{IEEEeqnarray}
where the dependency on $\uppsi$ is expressed only through the marginal random process $\nrm'$.
...
It is instructive to treat the distribution $\Prm_{\yrm | \xrm,\uppsi}$ as an estimate of the unknown conditional PMF $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and investigate the effects of informative prior knowledge. For a given $\xrm$ and corresponding number of training samples $\nrm'(\xrm)$, the expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
\Erm_{\uppsi | \nrm',\uptheta}\big[ \Prm_{\yrm | \xrm,\uppsi} \big] 
= \Erm_{\uppsi(\cdot,\xrm) | \nrm'(\xrm),\upthetac(\xrm)}\big[ \Prm_{\yrm | \xrm,\uppsi(\cdot,\xrm)} \big] \\ 
\quad = \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)}\right) \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} + \left(\frac{\nrm'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)}\right) \upthetac(\xrm) \nonumber \;,
\end{IEEEeqnarray}
where the properties of a multinomial distribution conditioned on its aggregation have been used. The result is a convex combination of the conditional data-independent distribution $\alpha(\cdot,x) / \alpha'(x)$ and the true conditional distribution $\upthetac(\xrm)$. The convex coefficients are dependent on the ``marginal'' values $\alpha'$ and $\nrm'$; note that as the number of matching training samples $\nrm'(x)$ increases relative to $\alpha'(x)$, the estimate tends towards the true conditional PMF. 

PGR: suppress delta dependency on nbar and theta?

To aid characterization of the estimator, define the random process $\Delta(\xrm,\uppsi,\uptheta) \equiv \Prm_{\yrm | \xrm,\uppsi} - \Prm_{\yrm | \xrm,\uptheta} \in \Rbb^{\Ycal}$. For a given $\xrm$ and corresponding number of training samples $\nrm'(\xrm)$, the bias of the conditional PMF estimate is
\begin{IEEEeqnarray}{rCl} \label{eq:predictive_bias}
\mathrm{Bias}(\xrm,\nrm',\uptheta) & = & \Erm_{\uppsi | \nrm',\uptheta}\big[ \Delta(\xrm,\uppsi,\uptheta) \big] \\
& = & \frac{\alpha'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)} \left( \frac{\alpha(\cdot,\xrm)}{\alpha'(\xrm)} - \upthetac(\xrm) \right) \nonumber 
\end{IEEEeqnarray}
and its covariance function is 
\begin{IEEEeqnarray}{L} \label{eq:predictive_cov}
\mathrm{Cov}(y,y';\xrm,\nrm',\uptheta) = \Crm_{\uppsi | \nrm',\uptheta} \big[\Prm_{\yrm | \xrm,\uppsi}(\cdot | \xrm,\uppsi) \big](y,y') \\
\quad = \frac{\Sigma_{\uppsi(\cdot,\xrm) | \nrm'(\xrm),\upthetac(\xrm)}(y,y')}{\big( \alpha'(\xrm) + \nrm'(\xrm) \big)^2} \nonumber \\
\quad = \frac{\nrm'(\xrm)}{\big( \alpha'(\xrm) + \nrm'(\xrm) \big)^2} \left( \upthetac(y;\xrm) \delta[y,y'] - \upthetac(y;\xrm) \upthetac(y';\xrm) \right) \nonumber \;,
\end{IEEEeqnarray}
...
\mathcal{E}(y,y' ; \xrm,\nrm',\uptheta) & = & \Erm_{\uppsi | \nrm',\uptheta} \Big[ \Delta(y;\xrm,\uppsi,\uptheta) \Delta(y';\xrm,\uppsi,\uptheta) \Big] \\
...
\quad = \Erm_{\xrm | \uptheta}\left[ \Erm_{\nrm'(\xrm) | \uptheta}\left[ \left(\frac{\alpha'(\xrm)}{\alpha'(\xrm) + \nrm'(\xrm)}\right)^2 \right] \left( \frac{\alpha(y,\xrm)}{\alpha'(\xrm)} - \upthetac(y;\xrm) \right) \left( \frac{\alpha(y',\xrm)}{\alpha'(\xrm)} - \upthetac(y';\xrm) \right) \right] \nonumber \\
\qquad + \Erm_{\xrm | \uptheta}\left[ \Erm_{\nrm'(\xrm) | \uptheta}\left[ \frac{\nrm'(\xrm)}{\big( \alpha'(\xrm) + \nrm'(\xrm) \big)^2} \right] \left( \upthetac(y;\xrm) \delta[y,y'] - \upthetac(y;\xrm) \upthetac(y';\xrm) \right) \right] \nonumber \;.
\end{IEEEeqnarray}
...
Figures \ref{fig:P_yx_error_a0_0_1} and \ref{fig:P_yx_error_a0_10} show how the bias and variance of the estimate change for different values of $\nrm'(x)$ and $\alpha'(x)$. The plot markers represent the conditional mean of the estimator, $\Erm_{\uppsi | \nrm',\uptheta}\big[ \Prm_{\yrm | \xrm,\uppsi}(y | \xrm,\uppsi) \big]$; the upper and lower error bars represent the square-root of the expected squared deviation above and below the conditional mean, respectively. Each individual plot heading provides the error $\sqrt{\sum_{y \in \Ycal} \mathcal{E}(y,y ; \xrm,\nrm',\uptheta)}$ to assess the quality of the PMF estimate. 
...
\begin{IEEEeqnarray}{rCl}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] & = & \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \frac{\big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big)^2} \right] \right] \\
& = & \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
& = & \Erm_{\xrm} \left[ \Erm_{\nrm'} \left[ \frac{\alpha_0 \Erm_{\uppsi | \nrm'} \left[ \big( \alpha'(\xrm) \mu_{\yrm|\xrm} + \sum_{y \in \Ycal} y \uppsi(y,\xrm) \big)^2 \right]}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
& = & \ldots \nonumber \\
...
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \Erm_{\uppsi} \left[ \frac{\alpha_0 \big( \alpha(y,\xrm)+\uppsi(y,\xrm) \big) \big(\alpha(y',\xrm) + \uppsi(y',\xrm) \big)}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
%= \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\xrm} \left[ \Erm_{\nrm'} \left[ \frac{\alpha_0 \Erm_{\uppsi | \nrm'} \left[ \big( \alpha(y,\xrm)+\uppsi(y,\xrm) \big) \big(\alpha(y',\xrm)+\uppsi(y',\xrm) \big) \right]}{\alpha'(\xrm) \big(\alpha'(\xrm) + \nrm'(\xrm) \big) (\alpha_0+N)} \right] \right] \nonumber \\
%= \ldots \nonumber \\
...
%\begin{IEEEeqnarray}{L}
%\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] =
%\sum_{\psi \in \Theta_\uppsi} \sum_{x \in \Xcal} \Prm_{\xrm,\uppsi}(x,\psi) \left( \sum_{y \in \Ycal} y \Prm_{\yrm | \xrm,\uppsi}(y | x,\psi) \right)^2 \\
%= \sum_{x \in \Xcal} \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\uppsi} \left[ \frac{\big( \alpha(y,x)+\uppsi(y,x) \big) \big(\alpha(y',x)+\uppsi(y',x) \big)}{(\alpha_0+N) \big(\alpha'(x) + \nrm'(x) \big)} \right] \nonumber \\
%= \sum_{x \in \Xcal} \sum_{y \in \Ycal} y \sum_{y' \in \Ycal} y' \Erm_{\nrm'} \left[ \frac{\Erm_{\uppsi | \nrm'} \left[ \big( \alpha(y,x)+\uppsi(y,x) \big) \big(\alpha(y',x)+\uppsi(y',x) \big) \right]}{(\alpha_0+N) \big(\alpha'(x) + \nrm'(x) \big)} \right] \nonumber \\
...
%\end{IEEEeqnarray}
The above formulation exploits the statistical characterization of the aggregation, $\nrm' \sim \DM(N,\alpha')$; also used is the property that the Dirichlet-Multinomial random process $\uppsi$ conditioned on its aggregation $\nrm'$ yields independent  conditional DM functions $\uppsi(\cdot,x) | \nrm'(x) \sim \DM\big( \nrm'(x),\alpha(\cdot,x) \big)$.
...
The two second-order (in terms of $y$) terms are scaled by factors dependent on the prior concentrations $\alpha'(x)$ and on $\upthetam(x)$ and $N$ via conditional expectations with respect to $\nrm'(x)$. Note that by the aggregation property of multinomial distributions, the random variable $\nrm'(x) | \upthetam(x) \sim \Bi \big(N,\upthetam(x)\big)$. Closed-forms have not been found for the function expectations of binomial random variables above.
...
\end{IEEEeqnarray}
Note that the first term's scaling factor is proportionate to the first inverse moment of a positive binomial random variable \cite{stephan}. The second term's scaling factor tends to $\Prm_{\nrm'(\xrm) | \upthetam(\xrm)}\big( 0 | \thetam(x) \big)$, the probability that no training samples are observed matching the value $\xrm$. As $N$ increases, this term tends to zero, the risk due to the prior estimate bias decreases, and the excess risk becomes a function of $\uptheta$ only.
...
\begin{IEEEeqnarray}{rCl}
\Rcal^* & = & 1 - \Erm_{\xrm,\Drm} \left[ \max_{y \in \Ycal} \Prm_{\yrm | \xrm,\Drm}(y | \xrm,\Drm) \right] \\
& = & 1 - \Erm_{\xrm,\uppsi} \left[ \frac{ \max_{y \in \Ycal} \big( \alpha(y,x) + \uppsi(y,x) \big)}{\alpha'(x) + \nrm'(x)} \right] \nonumber \\
...
To perform the expectation over the Dirichlet-Multinomial process $\uppsi \sim \DMP(\alpha_0 \alpha)$, split the expectation into an expectation over the marginal DMP $\nrm' \sim \DMP(\alpha')$ and a conditional expectation over $\uppsi$ given $\nrm'$. The characterization of the conditional DMP is found in Appendix \ref{app:DM_agg}.

\begin{IEEEeqnarray}{L}
\Erm_{\xrm,\uppsi} \Big[ \mu_{\yrm | \xrm,\uppsi}^2 \Big] \\
\quad = \Erm_{\xrm} \left[ \Erm_{\uppsi | \xrm} \left[ \left( \frac{\alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y}{\alpha'(\xrm) + \nrm'(\xrm)} \right)^2 \right] \right] \nonumber \\
\quad = \Erm_{\xrm} \Erm_{\nrm'} \left[ \frac{\alpha_0 \Erm_{\uppsi | \nrm'} \left[ \left( \alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y \right)^2 \right] }{\alpha'(\xrm)\big(\alpha'(x) + \nrm'(x) \big) (\alpha_0+N)} \right] \nonumber
\end{IEEEeqnarray}


Evaluating the conditional expectation,
\begin{IEEEeqnarray}{L}
\Erm_{\uppsi | \nrm'} \left[ \left( \alpha'(\xrm) \mu_{\yrm | \xrm} + \int_{\Ycal} y \uppsi(y,\xrm) \mathrm{d}y \right)^2 \right] \\
...
%\quad = \int_{\Xcal} \Erm_{\nrm'} \left[ \frac{ \Erm_{\uppsi | \nrm'} \left[ \left( \alpha'(x) \mu_{\yrm | \xrm}(x) + \int_{\Ycal} y \uppsi(y,x) \mathrm{d}y \right)^2 \right] }{(\alpha_0+N) \big(\alpha'(x) + \nrm'(x) \big)} \right] \mathrm{d}x \nonumber
%\end{IEEEeqnarray}
%
%
%Evaluating the conditional expectation,
%\begin{IEEEeqnarray}{L}
%\Erm_{\uppsi | \nrm'} \left[ \left( \alpha'(x) \mu_{\yrm | \xrm}(x) + \int_{\Ycal} y \uppsi(y,x) \mathrm{d}y \right)^2 \right] \\
...
First, define the PDF of a Dirichlet aggregation \cite{ferguson}. Let the random process $\uptheta \in \Theta = \Pcal(\Ycal)$ be Dirichlet over the countable set $\Ycal$ with parameterizing function $\alpha \in {\Rbb^+}^{\Ycal}$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal(z),\ldots \right\}$, $z \in \Zcal$; the aggregation $\upthetam \in \Pcal(\Zcal)$, $\upthetam(z) \equiv \sum_{y \in \Scal(z)} \uptheta(y)$ is thus also Dirichlet and has a parameterizing function $\alpha' \in {\Rbb^+}^{\Zcal}$, $\alpha'(z) \equiv \sum_{y \in \Scal(z)} \alpha(y)$.
...
\end{IEEEeqnarray}
which is defined for $\left\{ \theta \in {\Rbb_{\geq 0}}^{\Ycal} : \sum_{y \in \Scal(z)} \theta(y) = \thetam(z), \quad \forall z \in \Zcal \right\}$.
...
A characteristic of a Multinomial random process is that its aggregations are also Multinomial \cite{johnson}. Consider a random process $\uppsi \sim \Multi(N,\theta)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal(z),\ldots \right\}$, $z \in \Zcal$; the transformed random process $\nrm'(z) \equiv \sum_{y \in \Scal(z)} \uppsi(y)$ is distributed as $\nrm' \sim \Multi(N,\thetam)$ with parameterizing function $\thetam(z) = \sum_{y \in \Scal(z)} \theta(y)$.

To prove this principle, define the subset $\tilde{\Ncal} = \big\{ \psi \in \Ncal : \sum_{y \in \Scal(z)} \psi(y) = n'(z), \quad \forall z \in \Zcal \big\} \subseteq \Theta_\uppsi$, where the original random process $\uppsi \in \Theta_\uppsi$. Next, observe
\begin{IEEEeqnarray}{rCl}
\Prm_{\mathrm{n}'}(n') & = & \sum_{\psi \in \tilde{\Ncal}} \Prm_{\uppsi}(\psi) \\
& = & \Mcal(n') \prod_{z \in \Zcal} \sum_{\substack{n'(z) = \\ \sum_{y \in \Scal(z)} \psi(y)}} \Mcal\big( \{ \psi(y): y \in \Scal(z) \} \big) \prod_{y \in \Scal(z)} \theta(y)^{\psi(y)} \nonumber \\
& = & \Mcal(n') \prod_{z \in \Zcal} \thetam(z)^{n'(z)} = \Multi(n' ; N,\thetam) \nonumber \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \nrm'}(\psi | n') & = & \frac{\Mcal(N \psi) \prod_{y \in \Ycal} \theta(y)^{\psi(y)}}{\Mcal(n') \prod_{z \in \Zcal} \thetam(z)^{n'(z)}} \\
...
& = & \prod_{z \in \Zcal} \Multi\Big( \big\{ \psi(y) : y \in \Scal(z) \big\} ; n'(z) , \big\{ \thetac_z(y) : y \in \Scal(z) \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
on the domain $\left\{ \psi \in {\Zbb_{\geq 0}}^{\Ycal} : \sum_{y \in \Scal(z)} \psi(y) = n'(z), \quad \forall z \in \Zcal \right\}$. Observe that the segment over the set $\Scal(z)$ sums to $\nrm'(z)$ and is parameterized by the normalized segment $\thetac_z \equiv \big\{ \theta(y)/\thetam(z) : y \in \Scal(z) \big\}$.
...
A defining characteristic of a Dirichlet-Multinomial random process is that its aggregations are also Dirichlet-Multinomial \cite{johnson}. Consider a DM random process $\uppsi \sim \DM(N,\alpha)$ over the set $\Ycal$. Define an arbitrary partition of $\Ycal$: $\left\{ \ldots,\Scal(z),\ldots \right\}$, $z \in \Zcal$; the transformed random process $\nrm'(z) \equiv \sum_{y \in \Scal(z)} \uppsi(y)$ is neccessarily Dirichlet-Multinomial with parameterizing function $\alpha'(z) = \sum_{y \in \Scal(z)} \alpha(y)$.

It can be shown that conditioned on the aggregation $\nrm'$, the segments $\big\{ \uppsi(y) : y \in \Scal(z) \big\}$ of the original random process become independent Dirichlet-Multinomial random processes, such that
\begin{IEEEeqnarray}{rCl}
\Prm_{\uppsi | \nrm'}(\psi | n') & = & \frac{\Mcal(N \psi) \beta(\alpha_0 \alpha)^{-1} \beta(\alpha+\psi)}{\Mcal(n') \beta(\alpha')^{-1} \beta(\alpha'+n')} \\
& = & \left( \prod_{z \in \Zcal} \frac{\Gamma\big( \alpha'(z)+n'(z) \big)}{n'(z)! \Gamma\big( \alpha'(z) \big)} \right)^{-1} \left( \prod_{y \in \Ycal} \frac{\Gamma\big( \alpha(y)+\psi(y) \big)}{\psi(y)! \Gamma\big( \alpha(y) \big)} \right) \nonumber \\
& = & \prod_{z \in \Zcal} \left[ \frac{n'(z)! \Gamma\big( \alpha'(z) \big)}{\Gamma\big( \alpha'(z)+n'(z) \big)} \prod_{y \in \Scal(z)} \frac{\Gamma\big( \alpha(y)+\psi(y) \big)}{\psi(y)! \Gamma\big( \alpha(y) \big)} \right] \nonumber \\
& = & \prod_{z \in \Zcal} \DM\Big( \big\{ \psi(y) : y \in \Scal(z) \big\} ; n'(z), \big\{ \alpha(y) : y \in \Scal(z) \big\} \Big) \nonumber \;,
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl}
%\Prm(\uppsi | \nrm') & = & \frac{\Prm(\uppsi)}{\Prm(\nrm')} \Prm(\nrm' | \uppsi) \\ 
%& = & \frac{\Mcal(\uppsi) \beta(\alpha_0 \alpha)^{-1} \beta(\alpha+\uppsi)}{\Mcal(\nrm') \beta(\alpha')^{-1} \beta(\alpha'+\nrm')} \prod_{z \in \Zcal} \delta\left[ \nrm'(z),\sum_{y \in \Scal(z)} \uppsi(y) \right] \nonumber \\
%& = & \left( \prod_{z \in \Zcal} \frac{\Gamma\big( \alpha'(z)+\nrm'(z) \big)}{\nrm'(z)! \Gamma\big( \alpha'(z) \big)} \right)^{-1} \left( \prod_{y \in \Ycal} \frac{\Gamma\big( \alpha(y)+\uppsi(y) \big)}{\uppsi(y)! \Gamma\big( \alpha(y) \big)} \right) \nonumber \\
%&& \quad \prod_{z \in \Zcal} \delta\left[ \nrm'(z),\sum_{y \in \Scal(z)} \uppsi(y) \right] \nonumber \\
%& = & \prod_{z \in \Zcal} \left[ \delta\left[ \nrm'(z),\sum_{y \in \Scal(z)} \uppsi(y) \right] \frac{\nrm'(z)! \Gamma\big( \alpha'(z) \big)}{\Gamma\big( \alpha'(z)+\nrm'(z) \big)} \prod_{y \in \Scal(z)} \frac{\Gamma\big( \alpha(y)+\uppsi(y) \big)}{\uppsi(y)! \Gamma\big( \alpha(y) \big)} \right] \nonumber \\
%& = & \prod_{z \in \Zcal} \DM\Big( \big\{ \uppsi(y) : y \in \Scal(z) \big\} ; \nrm'(z), \big\{ \alpha(y) : y \in \Scal(z) \big\} \Big) \nonumber \;,
%\end{IEEEeqnarray}
on the domain $\left\{ \psi \in {\Zbb_{\geq 0}}^{\Ycal} : \sum_{y \in \Scal(z)} \psi(y) = n'(z), \quad \forall z \in \Zcal \right\}$. 
...
\begin{equation}
\Prm_{\nrm' | \uptheta}(n' | \theta) = \Mcal(n') \prod_{z \in \Zcal} \left( \int_{\Scal(z)} \theta(y) \mathrm{d} y \right)^{n'(z)} = \Multi\left( n' ; N,\thetam(z) \right) \;,
\end{equation}
...
If $\uppsi$ is a Dirichlet-Multinomial process over a Euclidean space $\Ycal$, then conditioning on its discrete aggregation $\nrm'$ produces independent Dirichlet-Multinomial processes $\big\{ \uppsi(y) : y \in \Scal(z) \big\} \sim \DMP\Big( \nrm'(z),\big\{ \alpha(y) : y \in \Scal(z) \big\} \Big)$ over the partition spaces $\Scal(z)$.

The previous result can be extended to conditioning on a continuous aggregation. Define $\uppsi \sim \DMP(N,\alpha)$ over the set $\Ycal \times \Xcal$ and the aggregation DMP $\nrm' = \int_{\Ycal} \uppsi(y,\cdot) \mathrm{d}y$ over set $\Xcal$ with parameterizing function $\alpha' = \int_{\Ycal} \alpha(y,\cdot) \mathrm{d}y$.
