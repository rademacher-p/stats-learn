...
\newcommand{\todohi}[1]{\todo[inline,color=red!50,linecolor=red]{#1}}
...
\todolo{use todonotes package instead of my initials?}

\todolo{equation numbers to final line!}

\todolo{brackets for expectation ops?}

\todolo{line break symbol format, before/after?}

\todomid{learners (full) or decision functions (range)?}

\todomid{likelihood function terminology}

\todomid{Dirichlet localization or concentration?}

\todomid{fix arguments for theta, nbar dists.}

\todomid{suppress arguments where sensible? eg Pd = theta}

\todomid{introduce tilde alpha to match tilde theta?}

\todohi{use x,y subs instead of prime/tilde?}

\todohi{ALL figure notation: theta font + Ycal indexing. use R,f opt?}

\todolo{is $\Theta$ redundant given $\Pcal$?}
...
\todohi{change from nbar to emprical RV???}

\todohi{investigate N lim for nbar given theta, bayes risk vs model support}
...
\todolo{NFLT investigation? try matlab examples}




\todomid{use clairvoyant/irreducible terms and symbols}
...
\todolo{generalize y,x,h from scalars to functions!!!}

\todolo{jeffrey prior, fisher info?}
