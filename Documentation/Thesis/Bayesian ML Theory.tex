...
It is instructive to treat the Bayesian predictive distribution $\Prm_{\yrm | \xrm,\Drm}$ as an estimator of the clairvoyant predictive distribution $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and investigate the effects of prior knowledge. Of specific interest are the mean $\Erm_{\Drm | \uptheta}\big[\mu_{\yrm | \xrm, \Drm}\big]$ and covariance $\Crm_{\Drm | \uptheta}\big[\mu_{\yrm | \xrm, \Drm}\big]$ with respect to the true model $\uptheta$.  To simplify the analysis, the training data $\Drm$ will be represented using the marginal and conditional sufficient statistics $(\uppsim, \uppsic)$. 
...
As seen in Chapter \ref{ch:dir_cont}, the flexibility of using a full-support prior, while beneficial for countable data spaces, has severe drawbacks when operating on observations drawn from continuous spaces. This section develops a new Bayesian predictive distribution using discretization that may be used to realize much of the power of Dirichlet process priors while avoiding the aforementioned limitation. 

\todohi{CITE for discretization work}

Define a discretizing data transformation $T: \Xcal \mapsto \Xcal'$, where the range is a countable subset $\Xcal' \subset \Xcal$. This transform is applied to each element of the training values $\Xrm$, as well as to the new observation $\xrm$, and can be thought of as a dimensionality-reducing feature transformation.

Having effectively changed the domain of the observations, the Dirichlet prior marginal mean is re-defined as $\alpham = \sum_{x' \in \Xcal'} \alpham'(x') \delta(\cdot - x')$, a mixture of delta functions at the discretized values. 
...
\begin{IEEEeqnarray}{rCl}
\prm_{\yrm | \xrm,\Drm} & = & \gammam'\big(T(x); \Xrm \big) \alphac\big(T(x)\big) + \left(1 - \gammam'\big(T(x); \Xrm \big)\right) \Psic\big(T(x); \Drm\big) \;,
\end{IEEEeqnarray}
where the modified weighting function is 
\begin{IEEEeqnarray}{rCl}
\gammam'\big(x'; X \big) & = & \left(1 + \frac{\sum_{n=1}^N \delta\big(x' - T(X_n)\big)}{\alpha_0 \sum_{x'' \in \Xcal'} \alpham'(x'') \delta(x' - x'')}\right)^{-1} \nonumber \\
...
\end{IEEEeqnarray}
and the modified marginal empirical function is 
\begin{IEEEeqnarray}{rCl}
