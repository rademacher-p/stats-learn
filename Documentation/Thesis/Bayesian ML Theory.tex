...
\end{IEEEeqnarray}
where the properties of an Empirical distribution conditioned on its aggregation have been used. The result is a convex combination of the conditional data-independent distribution $\alphac(\xrm)$ and the true conditional distribution $\upthetac(\xrm)$. The convex coefficients are dependent on the ``marginal'' values $\alpham(\xrm)$ and $\upthetam(\xrm)$. Note that as the number of training samples $N$ and the true marginal value $\upthetam(\xrm)$) increase, the estimate tends towards the true conditional PMF; conversely, as the prior concentration $\alpha_0$ and marginal value $\alpha_m(\xrm)$ increase, the estimate tends toward the prior estimate $\alphac(\xrm)$.
...
\quad = \frac{N \uppsim(\xrm)}{\alpha_0 \alpham(\xrm) + N \uppsim(\xrm)}\big(\uppsic(\xrm) - \upthetac(\xrm)\big) \nonumber \\
\qquad + \left( \frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \uppsim(\xrm)} - \Erm_{\uppsim | \upthetam}\left[ \frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \uppsim(\xrm)} \right] \right) \big(\alphac(\xrm) - \upthetac(\xrm)\big) \nonumber \;,
\end{IEEEeqnarray}
...
\mathrm{Cov}(\xrm;\upthetam,\upthetac) = \Crm_{\uppsim,\uppsic | \upthetam,\upthetac} \big[\Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] \\
\quad = \Erm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\delta(0) N \uppsim(\xrm)}{\big( \alpha_0 \alpham(\xrm) + N \uppsim(\xrm) \big)^2} \right] \big( \diag\big(\upthetac(\xrm)\big) - \upthetac(\xrm) \otimes \upthetac(\xrm) \big) \nonumber \\
\qquad + \Crm_{\uppsim(\xrm) | \upthetam(\xrm)}\left[ \frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \uppsim(\xrm)} \right] \big( \alphac(\xrm) - \upthetac(\xrm) \big) \otimes \big( \alphac(\xrm) - \upthetac(\xrm) \big) \nonumber
\end{IEEEeqnarray}
