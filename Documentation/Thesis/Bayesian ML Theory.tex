...
\todomid{Discuss: no longer independent of thetam!? Use original model terms only?}
...
The Bayesian predictive distribution \eqref{eq:P_y_xD_dir_cont_disc} in terms of the discretized sufficient statistics is
\begin{IEEEeqnarray}{rCl}
...
\Erm_{\Drm | \upthetam,\upthetac} \Big[ \Delta(\xrm; \Drm,\upthetac) \otimes \Delta(\xrm; \Drm,\upthetac) \Big] \nonumber \\
\quad \equiv \Erm_{\uppsim' | \upthetam'}\big[ \gammam'(T(\xrm); \uppsim')^2 \big] \big( \alphac(T(\xrm)) - \upthetac(T(\xrm)) \big) \otimes \big( \alphac(T(\xrm)) - \upthetac(T(\xrm)) \big) \nonumber \\
\qquad + \Erm_{\uppsim' | \upthetam'}\left[ \frac{\big(1 - \gammam'(T(\xrm); \uppsim')\big)^2}{N \uppsim'(T(\xrm))} \right] \Big( \diag\big(\upthetac'(T(\xrm))\big) - \upthetac'(T(\xrm)) \otimes \upthetac'(T(\xrm)) \Big) \nonumber \\
\qquad \quad + \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \otimes \Big(\upthetac'\big(T(\xrm)\big) - \upthetac(\xrm) \Big) \;.
\end{IEEEeqnarray}

\todolo{bracket formatting}


\subsection{Trends}

The bias/variance trends for the discretized Dirichlet distribution estimate \eqref{eq:P_y_xD_dir_cont_disc} have similarities and important differences from the trends of the discrete-domain Dirichlet distribution as detailed in Section \ref{sec:predictive_est_dir}. Note that $N \uppsim'(t) | \upthetam'(t) \sim \Bi\big(N, \upthetam'(t)\big)$; the relevant expectations will thus depend on the selection of the discretization function $T$.

First consider the effect of the training volume $N$. For $N=0$, the data-independent distribution is again maximally biased and has zero variance; the bias $\mathrm{Bias}(\xrm; \upthetam,\upthetac) = \alphac\big(T(\xrm)\big) - \upthetac(\xrm)$ now depends on the conditional prior mean only at values $t \in \Tcal \subset \Xcal$. As $N \to \infty$, the discretization has notable effects. For values $x$ outside the support of $\upthetam$, the bias again reaches its maximal value, as no data satisfying $\Xrm_n = x$ will be observed. However, for values satisfying $\upthetam(x) > 0$, only the first term in \eqref{eq:predictive_bias_dir_disc} vanishes, leaving the residual $\mathrm{Bias}(\xrm; \upthetam,\upthetac) = \upthetac'\big(T(\xrm)\big) - \upthetac(\xrm)$. Observe that this term is independent of $N$ and the Dirichlet parameterization -- the bias can not be reduced beyond this minimum value. Regardless of the discretization transform $T$, the covariance function \eqref{eq:predictive_cov_dir_disc} will tend to zero. Combining these effects, it is clear that, unlike its discrete-domain relative \eqref{eq:P_y_xD_dir}, the distribution estimate \eqref{eq:P_y_xD_dir_cont_disc} will \emph{not} necessarily converge to the true predictive distribution $\upthetac$ in the limit of training data volume. 

The dependency of the bias/variance trade-off on the Dirichlet prior parameterization is conceptually identical to that of the discrete-domain scenario in Section \ref{sec:predictive_est_dir}. Again, as $\alpha_0 \to \infty$, the bias is maximal and the covariance is zero. As $\alpha_0 \to 0$ the expectations of $\uppsim'$ take the same forms, substituting $\upthetam'$ for $\upthetam$. 

\todohi{Generalize expectation comment, move to front}

\todohigh{discretization degree trade-off!!}
