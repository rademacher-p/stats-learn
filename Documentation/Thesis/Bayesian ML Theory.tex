...
& = & \prod_{x \in \Xcal} \left[ \prod_{y \in \Ycal} \thetac(y;x)^{\bar{N}(y,x;Y,X)} \right] \nonumber \;,
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam,\upthetac | \nbarrm}(\thetam,\thetac | \bar{n}) & = & \frac{\Prm_{\nbarrm | \nrm',\upthetac}\big( \bar{n} | \sum_y \bar{n}(y,\cdot),\thetac \big)}{\Prm_{\nbarrm | \nrm'}\big( \bar{n} | \sum_y \bar{n}(y,\cdot) \big)} 
...
Note that $\prm_{\upthetac | \Xrm} = \Erm_{\upthetam | \Xrm}\big[ \prm_{\upthetac | \upthetam} \big]$ and $\prm_{\upthetac | \nrm'} = \Erm_{\upthetam | \nrm'}\big[ \prm_{\upthetac | \upthetam} \big]$. Also, note that $\Prm_{\Yrm | \Xrm} = \Erm_{\upthetam | \Xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and $\Prm_{\nbarrm | \nrm'} = \Erm_{\upthetam | \nrm'} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\nbarrm | \nrm',\upthetac} \big] \Big]$.
...
Note that $\Prm_{\Yrm | \Xrm,\xrm} = \Erm_{\upthetam | \Xrm,\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\Yrm | \Xrm,\upthetac} \big] \Big]$ and also that $\Prm_{\nbarrm | \nrm',\xrm} = \Erm_{\upthetam | \nrm',\xrm} \Big[ \Erm_{\upthetac | \upthetam} \big[ \Prm_{\nbarrm | \nrm',\upthetac} \big] \Big]$.
...
By the aggregation property \cite{ferguson}, $\upthetam$ is a Dirichlet random process parameterized by $\alpha_0 \alpham$. Note that the concentration parameter is $\alpha_0$ and that $\Prm_{\xrm} = \mu_{\upthetam} = \alpham$.
...
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \upthetam}\big( \thetac | \thetam \big) & = & \prod_{x \in \Xcal} \Bigg[ \beta\big( \alpha(\cdot,x) \big)^{-1} \prod_{y \in \Ycal} \thetac(y;x)^{\alpha(y,x)-1} \Bigg] \\
& = & \prod_{x \in \Xcal} \Dir\Big( \thetac(x) ; \alpha(\cdot,x) \Big) \nonumber \;,
\end{IEEEeqnarray}
...
& = & \prod_{x \in \Xcal} \Erm_{\upthetac(x)}\left[ \prod_{y \in \Ycal} \upthetac(y;x)^{\bar{N}(y,x;Y,X)} \right] \nonumber \\
...
\end{IEEEeqnarray}
This is a mixture distribution of the prior expectation $\mu_{\uptheta} = \alpha/\alpha_0$ and the empirical distribution $\bar{N}(\Drm)/N$. The more informative the model prior (i.e. larger $\alpha_0$), the more the prior mean is favored; the more data, the more the empirical PMF is favored. The marginal distribution for $\xrm$ given $\Drm$ is
\begin{IEEEeqnarray}{rCl}
...
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha(\cdot,x') + \bar{N}(\cdot,x';D) \big) \nonumber
\end{IEEEeqnarray}
and the distinct model conditional PMF's are independent from one another. A similar treatment demonstrates that
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetac | \nbarrm,\xrm}\big( \thetac | \bar{n},x \big) & = & \prm_{\upthetac | \nbarrm}\big( \thetac | \bar{n} \big) = \prod_{x' \in \Xcal} \prm_{\upthetac(x') | \nbarrm(\cdot,x')}\big(\thetac(x') | \bar{n}(\cdot,x') \big) \\
& = & \prod_{x' \in \Xcal} \Dir\big( \thetac(x') ; \alpha(\cdot,x') + \bar{n}(\cdot,x') \big) \nonumber \;.
\end{IEEEeqnarray}
Observe that when the conditioning is performed using the sufficient statistic, the independent conditional models $\upthetac(x)$ are only dependent on their corresponding subset of the  empirical PMF, $\nbarrm(\cdot,x)$.
...
It is instructive to treat the distribution $\Prm_{\yrm | \xrm,\nbarrm}$ as an estimate of the unknown conditional PMF $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and investigate the effects of informative prior knowledge. For a given $\xrm$ and corresponding number of training samples $\nrm'(\xrm)$, the expected value of the estimate condtioned on the true model $\uptheta$ is
\begin{IEEEeqnarray}{L}
\Erm_{\nbarrm | \nrm',\uptheta}\big[ \Prm_{\yrm | \xrm,\nbarrm} \big] 
= \Erm_{\nbarrm(\cdot,\xrm) | \nrm'(\xrm),\upthetac(\xrm)}\big[ \Prm_{\yrm | \xrm,\nbarrm(\cdot,\xrm)} \big] \\ 
...
To exemplify how the model estimate $\Prm_{\yrm | \xrm,\nbarrm}$ approximates $\Prm_{\yrm | \xrm,\uptheta}$, consider a scenario with $|\Ycal| = 10$. The data-independent PMF $\alpha(\cdot,x) / \alpha'(x)$ and true model $\upthetac(x)$ are shown in Figure \ref{fig:P_yx_error_N_0} - note the significant mismatch. 
...
\begin{IEEEeqnarray}{rCl}
\Rcal_{\Theta}(f ; \uptheta) & = & 1 - \sum_{x \in \Xcal} \upthetam(x) \Erm_{\nbarrm | \uptheta} \bigg[ \upthetac\Big( \argmax_{y \in \Ycal} \big( \nbarrm(y,x) + \alpha(y,x) \big) ;x \Big) \bigg] \;.
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
\upthetam | \Drm & \sim & \Dir\big( \alpha' + \bar{N}(\Drm) \big) \;,
\end{IEEEeqnarray}
...
= \left( \prod_{z \in \Zcal} \upthetam(z)^{\bar{N}(z;D)} \right) \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal(z) \big)} \right] \nonumber \;,
\end{IEEEeqnarray}
Observe that the dependency on $\upthetam$ is polynomial. The training data marginal distribution is
\begin{IEEEeqnarray}{rCl}
\prm_{\Drm}(D) & = & \Erm_{\upthetam} \left[ \prod_{z \in \Zcal} \upthetam(z)^{\bar{N}(z;D)} \right] \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal(z) \big)} \right] \\
& = & \frac{\beta\big( \alpha' + \bar{N}(D) \big)}{\beta(\alpha')} \prod_{z \in \Zcal} \Erm_{\upthetac_z}\left[ \prod_{n=1}^N \upthetac_z\big( D_n \big)^{\chi\big( D_n;\Scal(z) \big)} \right] \nonumber
\end{IEEEeqnarray}
and thus the distribution of interest is
\begin{IEEEeqnarray}{rCl}
\prm_{\upthetam | \Drm}(\thetam | D) & = & \frac{\prod_{z \in \Zcal} \thetam(z)^{\alpha'(z)+\bar{N}(z;D)-1}}{\beta\big( \alpha' + \bar{N}(D) \big)} \\
& = & \Dir\left( \upthetam ; \alpha' + \bar{N}(\cdot;\Drm) \right) \nonumber \;.
\end{IEEEeqnarray}
