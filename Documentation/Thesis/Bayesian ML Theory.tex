...
For continuous data, the expected value of the predictive distribution \eqref{eq:predictive_dist_avg} has unique properties.
%\begin{IEEEeqnarray}{L}
%\Erm_{\uppsim,\uppsic | \upthetam,\upthetac}\big[ \Prm_{\yrm | \xrm,\uppsim,\uppsic} \big] 
%\equiv \Erm_{\uppsim(\xrm),\uppsic(\xrm) | \upthetam(\xrm),\upthetac(\xrm)}\big[ \Prm_{\yrm | \xrm,\uppsim(\xrm),\uppsic(\xrm)} \big] \\ 
%\quad = \Erm_{\uppsim(\xrm) | \upthetam(\xrm)} \big[\gammam(\xrm; \uppsim)\big] \alphac(\xrm) + \Erm_{\uppsim(\xrm) | \upthetam(\xrm)} \big[1 - \gammam(\xrm; \uppsim)\big] \upthetac(\xrm) \nonumber
%\end{IEEEeqnarray}
Note that by the aggregation property of Empirical distributions, the empirical process value $\uppsim(x)$ conditioned on the model $\upthetam(x)$ is distributed as
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
where $\Bi$ is the binomial PMF. As such, if $\upthetam$ is upper-bounded, the expected value of $\gammam(\uppsim)$ is unity for all values $x \in \Xcal$ and the predictive distribution tends to the conditional prior mean $\alphac$. Conversely, if $\uptheta$ is impulsive at $x$, the weight is $\big(1 - \delta(0)^{-1} \upthetam(x)\big)^N$ if $\alpham$ is bounded and $asdf$ if $\alpham$ is impulsive.
