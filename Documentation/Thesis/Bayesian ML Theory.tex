...
To simplify the analysis, the training data $\Drm$ will be represented using the marginal and conditional sufficient statistics $(\uppsim, \uppsic)$. 
...
Inspecting the new predictive distribution $\prm_{\yrm | \xrm,\Drm}$, it is clear that the dependency on the joint observations $(\xrm, \Drm)$ can be represented using the sufficient statistics $\big(T(\xrm), \Psim'(\Xrm), \Psic'(\Drm)\big)$. Again, it is useful to define and characterize new random processes. Introduce a partitioning $\{\ldots, \Scal(t), \ldots\}$ of the observation space $\Xcal$, where $\Scal(t) = \{x \in \Xcal: T(x) = t\}$.
...
\end{IEEEeqnarray}
By the aggregation property of Empirical processes (Appendix \ref{app:EP}), it is shown that $\uppsi' | \uptheta \sim \EP(N, \uptheta')$, where the dependency on the model is expressed through the discretized $\uptheta' \in \Pcal(\Ycal \times \Tcal)$, defined as $\uptheta'(y,t) = \int_{\Scal(t)} \uptheta(y,x) {\drm}x$.
...
The Bayesian predictive distribution in terms of the discretized sufficient statistics is
\begin{IEEEeqnarray}{rCl}
...
\todomid{Check for residual uses of subscript args on gamma expectations}
...
Additionally, redefine the  define the random process $\Delta(\xrm;\uppsim,\uppsic,\upthetac) \equiv \Prm_{\yrm | \xrm,\uppsim,\uppsic} - \Prm_{\yrm | \xrm,\upthetac} \in \Rbb^{\Ycal}$.
...
\end{IEEEeqnarray}
Note that the mixture distribution no longer includes the true predictive distribution $\upthetac$, but rather an approximation $\upthetac'$. Noting that $\prm_{\yrm | \xrm,\uppsim',\uppsic'} = \prm_{\yrm | \trm, \uppsim', \uppsic'}\big(T(\xrm), \uppsim',\uppsic' \big)$ and substituting into \eqref{eq:predictive_bias}, the expected bias is
...
\Crm_{\uppsim',\uppsic' | \upthetam,\upthetac} \big[\Prm_{\yrm | \trm,\uppsim',\uppsic'} \big] = \Crm_{\uppsim' | \upthetam'}\big[ \gammam'(\trm; \uppsim') \big] \big( \alphac(\trm) - \upthetac'(\trm) \big) \otimes \big( \alphac(\trm) - \upthetac'(\trm) \big) \nonumber \\
\quad + \Erm_{\uppsim' | \upthetam'}\left[ \frac{\big(1 - \gammam'(\trm; \uppsim')\big)^2}{N \uppsim'(\trm)} \right] \Big( \diag\big(\upthetac'(\trm)\big) - \upthetac'(\trm) \otimes \upthetac'(\trm) \Big) 
\end{IEEEeqnarray}
