...
%\end{IEEEeqnarray}
Note that as the prior localization increases relative to the training data volume, $\gamma(x; \uppsim) \inprob 1$ and the posterior mean tends to $\alphac(x)$, reflecting confidence in the prior. Conversely, for increasingly large training sets, $\gamma(x; \uppsim) \inprob 0$ and the mean tends to $\uppsic(x)$. Also, as $N \uppsim(x) \inprob \infty$, the PDF $\prm_{\upthetac(x) | \uppsim(x),\uppsic(x)} \to \delta\big( \cdot - \uppsic(x) \big)$ and the model is positively identified. This results from the full support of the Dirichlet distribution and does not hold in general. The adaptation of the posterior is demonstrated in Figure \ref{fig:P_theta_post_tilde} for $\alpha_0 \alpham(x) = 6$ and $N \psim(x) = 3$.
