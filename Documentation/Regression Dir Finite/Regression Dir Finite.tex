...
\begin{figure}
\centering
\includegraphics[width=1\linewidth]{SSP_2021/P_theta_post.pdf}
...
\lambda_{\text{Bias}}(x) & = & \Erm_{\uppsim(x) | \upthetam(x)}\left[ \gamma(x; \uppsim)^2 \right] 
\end{IEEEeqnarray}
and
\begin{IEEEeqnarray}{rCl} \label{eq:var}
\lambda_{\text{Var}}(x) & = & \Erm_{\uppsim(x) | \upthetam(x)}\left[ \frac{\big( 1 - \gamma(x; \uppsim) \big)^2}{N \uppsim(x)} \right] \;.
\end{IEEEeqnarray}
%\begin{IEEEeqnarray}{rCl}
...
\section{Examples}
To demonstrate the efficacy of Dirichlet prior regressors, consider a data model where $\Xcal$ and $\Ycal$ are 128-point discretizations of the unit interval. The PMF $\thetam$ is uniform and $\thetac(y;x) = \Bi(128y; 128, \mu_{\yrm | \xrm,\uptheta}(x))$, where the clairvoyant regressor $\mu_{\yrm | \xrm,\uptheta}(x) = 1 / \big(2 + \sin(2\pi x)\big)$. The Dirichlet estimator has $\alphac(y;x) = \Bi(128y; 128, 0.5)$. For comparison, a Bayesian linear regressor using a Gaussian prior $\Ncal\big([0, 0.5], \sigma^2_{\uptheta} \bm{I}\big)$ and $\sigma^2_{\yrm | \xrm}$ is evaluated, as well \cite{theodoridis-ML}.

Figure \ref{fig:Predict_SE_biased_hard_comp_2} 

Figure \ref{fig:Risk_cond_SE_N_biased_hard} compares the Dirichlet-based estimator.... Note that only the Dirichlet estimator can achieve zero excess risk with sufficient data.

\begin{figure}
\centering
\includegraphics[width=1\linewidth]{SSP_2021/Predict_SE_biased_hard_comp_2.png}
\caption{Estimator statistics}
\label{fig:Predict_SE_biased_hard_comp_2}
\end{figure}
