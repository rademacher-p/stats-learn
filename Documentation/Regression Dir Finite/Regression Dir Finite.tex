...
The squared-error (SE) loss function is arguably the most commonly used loss function for regression, or in fact for any estimation problem. This can be attributed to its quadratic form, which enables closed-form evaluation of the optimal estimate.
...
As shown in Equation \eqref{eq:f_opt_xD}, the decision selected by the optimally designed function depends on the predictive distribution of the unobserved $\yrm$ conditioned on all observable random elements. Observe that $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$ and thus that $\Prm_{\yrm | \xrm,\Drm} = \Erm_{\uptheta | \xrm,\Drm}\big[ \Prm_{\yrm | \xrm,\uptheta} \big] \equiv \mu_{\upthetac(\xrm) | \xrm,\Drm}$; the Bayesian predictive PMF is dependent only on the statistics of $\upthetac(\xrm)$.

PGR: prove? apps?

The training data $\Drm$ is represented using the empirical distribution $\uppsi$. Due to the independence of the conditional models from one another and from the marginal model, it can be shown that $\prm_{\upthetac(\xrm) | \uppsi, \xrm} \equiv \prm_{\upthetac(\xrm) | \uppsim(\xrm),\uppsic(\xrm)} \sim \Prm_{\uppsic(\xrm) | \uppsim(\xrm), \upthetac(\xrm)} \prm_{\upthetac(\xrm)}$. Since the Empirical PMF of $\uppsic(\xrm)$ has exponential form, the Dirichlet PDF $\prm_{\upthetac(\xrm)}$ is a conjugate prior \cite{theodoridis-ML}; thus, the model posterior PDF given the training data is distributed as 
...
\begin{IEEEeqnarray}{rCl}
\mu_{\upthetac(\xrm) | \uppsim(\xrm), \uppsic(\xrm)} & = & \left(\frac{\alpha_0 \alpham(\xrm)}{\alpha_0 \alpham(\xrm) + N \psim(\xrm)}\right) \alphac(\xrm) \nonumber \\
&& \quad + \left(\frac{N \psim(\xrm)}{\alpha_0 \alpham(\xrm) + N \psim(\xrm)}\right) \psic(\xrm) \nonumber \;.
\end{IEEEeqnarray}

The Bayesian predictive distribution is the mean of the posterior \cite{murphy}; to express in terms of the training data, the Empirical processes can be replaced, such that $\uppsim \equiv N^{-1} \sum_{n=1}^N \delta[\cdot,\Xrm_n]$ and $\uppsic(x) \equiv \frac{\sum_{n=1}^N \delta[\cdot,\Yrm_n] \delta[x,\Xrm_n]}{\sum_{n=1}^N \delta[x,\Xrm_n]}$.
