...
Next consider the effects of the Dirichlet prior parameters. The analysis will interpret the Dirichlet parameters as the conditional prior means $\alphac(x)$ and the corresponding localizations $\alpha'(x) \equiv \alpha_0 \alpham(x)$. 

First consider the functions $\alphac(x)$, which only affect the squared estimator bias. Regardless of the prior localization, the best selections for these conditional priors must render $\mu_{\yrm | \xrm}$ unbiased; as a result, the excess risk is equivalent to the first term in \eqref{eq:risk_cond_SE_dir_ex}, measuring additional variance due to model uncertainty.
