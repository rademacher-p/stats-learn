...
The empirical process can be decomposed into marginal and conditional empirical processes via a bijection $\uppsi \Leftrightarrow (\uppsim, \uppsic)$, where $\uppsim \equiv \sum_{y \in \Ycal} \uppsi(y,\cdot)$ and $\uppsic(x) \equiv \uppsi(\cdot,x) / \uppsim(x)$. Like the Multinomial random process \cite{johnson}, the Empirical random process has an aggregation property, such that $\uppsim | \upthetam \sim \Emp(N,\upthetam)$. Also, $\uppsic(x) | \uppsim(x),\upthetac(x) \sim \Emp\big( N \uppsim(x),\upthetac(x) \big)$ are conditionally independent Empirical processes.
...
Observe that $\Prm_{\yrm | \xrm,\uptheta} \equiv \upthetac(\xrm)$; thus, the clairvoyant estimate will depend only on the conditional model.
