...
\end{IEEEeqnarray}
where $\beta$ is the multivariate beta function, and the user-selected localization parameter $\alpha_0 \in \Rbb^+$ and mean $\alpha = \mu_{\uptheta} \in \Pcal(\Ycal \times \Xcal)$ are introduced. Note that as $\alpha_0 \to \infty$, the PDF tends to a maximally informative prior $\prm_{\uptheta} \to \delta(\cdot - \alpha)$, where $\delta$ is the Dirac delta function over the set $\Pcal(\Ycal \times \Xcal)$.
