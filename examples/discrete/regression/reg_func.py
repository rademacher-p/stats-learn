...
parser = argparse.ArgumentParser(
    description="Example: regularization against function complexity on a discrete domain"
)
parser.add_argument(
    "sims", nargs="*", choices=["fit", "predict", "risk_N"], help=f"Simulations to run"
)
...
# Dirichlet
prior_mean = random.models.DataConditional.from_mean_emp(alpha_y_x, n_y, lambda x: 0.5, model_x)
...
weight_decays = [0.0, 1e-3]  # controls L2 regularization

proc_funcs = {
    "pre": [],
    "post": [make_clipper([np.min(model_x.values), np.max(model_x.values)])],
}
