...
def make_normalized(n_t_iter_, dir_params_):
    ...
    for n_t_, params_ in zip(n_t_iter_, dir_params_full_):
        values_t_ = np.linspace(*model_x.lims, n_t_, endpoint=False) + 0.5 / n_t_

        prior_mean_x_ = random.elements.DataEmpirical(
            values_t_, counts=np.ones_like(values_t), space=model_x.space
        )
...
if "risk_T_leg_N" in sim_names:
    ...
    dir_predictors, dir_params_full = make_normalized(
        np.logspace(1, 7, 7, base=2, dtype=int), {"alpha_0": [4.5]}
    )
