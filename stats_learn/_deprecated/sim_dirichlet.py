...
alpha_0_plot = supp_x_s.size * supp_y_s.size * np.array([0.1, 1, 10])
...
for i, (n_train, alpha_0) in enumerate(itertools.product(n_train_plot, alpha_0_plot)):
    ...
    bayes_model = DirichletFiniteYcXModelBayesNew(
        alpha_0, mean_x, mean_y_x, rng_prior=random.default_rng()
    )

    learner = ModelClassifier(bayes_model)
    # learner = BayesEstimator(bayes_model)

    risk_plot[np.unravel_index([i], risk_plot.shape)] = learn_eval_mc_bayes(
        learner, bayes_model, n_train, n_mc=2000, verbose=False
    )
